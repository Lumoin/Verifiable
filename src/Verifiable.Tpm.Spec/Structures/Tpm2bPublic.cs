...
/// </code>
/// <para>
/// Specification reference: TPM 2.0 Library Part 2, clause 12.2.5, Table 236.
/// </para>
/// </remarks>
...
public sealed class Tpm2bPublic: IDisposable, ITpmWireType
{
    ...
    /// <exception cref="InvalidOperationException">The declared size is zero, the public area inside is malformed, or the declared size exceeds the octets the public area consumes — Part 4 <c>TPM2B_PUBLIC_Unmarshal</c>'s <c>TPM_RC_SIZE</c>, so no caller-chosen octets ride inside the sized buffer the object's Name is computed over.</exception>
    ...
    public static Tpm2bPublic Parse(ref TpmReader reader, BaseMemoryPool pool)
    {
        ...
        try
        {
            ...
            //otherwise be retained and hashed into the object's Name.
            if(innerReader.Remaining != 0)
            {
                publicArea.Dispose();
                throw new InvalidOperationException($"TPM2B_PUBLIC size {size} exceeds the {size - innerReader.Remaining} octets its TPMT_PUBLIC consumes.");
            }
    ...
    /// <summary>
    /// Creates a TPM2B_PUBLIC for an unrestricted RSA decryption key template — the RSA counterpart of an ECC
    /// KEM key: <paramref name="scheme"/> selects RSAES, OAEP, or NULL (raw RSAEP/RSADP) over a
    /// <c>decrypt</c>-SET, <c>restricted</c>-CLEAR key, per TPM 2.0 Library Part 2, Table 228's row: "for an
    /// unrestricted decryption key, TPM_ALG_RSAES, TPM_ALG_OAEP, or TPM_ALG_NULL unless the object also has the
    /// sign attribute". That row's own NULL branch is deprecated — "Support for TPM_ALG_NULL except for Storage
    /// Keys, and keys intended for use with the raw RSAEP/RSADP primitive, was deprecated in version 185. See
    /// Part 0." (clause 12.2.3.4, Table 228) — and a NULL <paramref name="scheme"/> template is exactly the
    /// raw-primitive form TPM 2.0 Library Part 0, clause 3.1.4.2's carve-out admits.
    /// </summary>
    /// <param name="nameAlg">Hash algorithm for Name computation.</param>
    ...
    /// <returns>The sized public buffer.</returns>
    public static Tpm2bPublic CreateRsaDecryptKeyTemplate(
        TpmAlgIdConstants nameAlg,
        TpmaObject objectAttributes,
        ushort keyBits,
        TpmtRsaScheme scheme)
    {
        var publicArea = TpmtPublic.CreateRsaDecryptKeyTemplate(nameAlg, objectAttributes, keyBits, scheme);
