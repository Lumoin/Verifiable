...
public static partial class TpmCcConstantsExtensions
{
    ...
    public static TpmaCc GetCommandAttributes(this TpmCcConstants commandCode) => commandCode switch
    {
        //clause 11.1 - TPM2_StartAuthSession.
        ...
        //clause 16.1 - TPM2_GetRandom.
        //Handle area: none (0 handles).
        //Response: no handles.
        TpmCcConstants.TPM_CC_GetRandom
            => TpmaCc.FromCommandCode((uint)commandCode, cHandles: 0),

        //Clause 16.2 - TPM2_StirRandom.
        //Handle area: none (0 handles).
        //Response: no handles.
        TpmCcConstants.TPM_CC_StirRandom
            => TpmaCc.FromCommandCode((uint)commandCode, cHandles: 0, isNv: true),

        //Clause 30.3 - TPM2_TestParms.
        //Handle area: none (0 handles).
        //Response: no handles.
        TpmCcConstants.TPM_CC_TestParms
            => TpmaCc.FromCommandCode((uint)commandCode, cHandles: 0),

        //Part 3, clause 9.4, Table 6 - TPM2_Shutdown.
        //Handle area: none (0 handles).
        //Response: no handles.
        TpmCcConstants.TPM_CC_Shutdown
            => TpmaCc.FromCommandCode((uint)commandCode, cHandles: 0),

        //Part 3, clause 10.2, Table 8 - TPM2_SelfTest.
        //Handle area: none (0 handles).
        //Response: no handles.
        TpmCcConstants.TPM_CC_SelfTest
            => TpmaCc.FromCommandCode((uint)commandCode, cHandles: 0),

        //Part 3, clause 10.4, Table 12 - TPM2_GetTestResult.
        //Handle area: none (0 handles).
        //Response: no handles.
        TpmCcConstants.TPM_CC_GetTestResult
            => TpmaCc.FromCommandCode((uint)commandCode, cHandles: 0),

        //clause 24.1 - TPM2_CreatePrimary.
        ...
        //clause 12.2 - TPM2_Load.
        //Handle area: @parentHandle (1 handle, requires auth).
        //Response: objectHandle (1 handle).
        TpmCcConstants.TPM_CC_Load
            => TpmaCc.FromCommandCode((uint)commandCode, cHandles: 1, hasResponseHandle: true),

        //Clause 12.3 - TPM2_LoadExternal.
        //Handle area: none (0 handles) — inPrivate/inPublic/hierarchy are all parameters.
        //Response: objectHandle (1 handle).
        TpmCcConstants.TPM_CC_LoadExternal
            => TpmaCc.FromCommandCode((uint)commandCode, cHandles: 0, hasResponseHandle: true),

        //Clause 12.8 - TPM2_ObjectChangeAuth.
        //Handle area: @objectHandle (ADMIN-role auth), parentHandle (no auth) - 2 handles, one requires auth.
        //Response: no handles.
        TpmCcConstants.TPM_CC_ObjectChangeAuth
            => TpmaCc.FromCommandCode((uint)commandCode, cHandles: 2),

        //clause 28.4 - TPM2_FlushContext.
        //Handle area: none (0 handles).
        ...
        //Clause 28.2 - TPM2_ContextSave.
        //Handle area: saveHandle (1 handle, Auth Index: None — no authorization ladder runs at all).
        //Response: no handles (the whole parameter area is the TPMS_CONTEXT).
        TpmCcConstants.TPM_CC_ContextSave
            => TpmaCc.FromCommandCode((uint)commandCode, cHandles: 1),

        //Clause 28.3 - TPM2_ContextLoad.
        //Handle area: none (0 handles) — the whole TPMS_CONTEXT is the command's single parameter.
        //Response: loadedHandle (1 handle), no parameters.
        TpmCcConstants.TPM_CC_ContextLoad
            => TpmaCc.FromCommandCode((uint)commandCode, cHandles: 0, hasResponseHandle: true),

        //clause 30.2 - TPM2_GetCapability.
        //Handle area: none (0 handles).
        //Response: no handles.
        TpmCcConstants.TPM_CC_GetCapability
            => TpmaCc.FromCommandCode((uint)commandCode, cHandles: 0),

        //clause 31.3 - TPM2_NV_DefineSpace.
        ...
        //clause 31.13 - TPM2_NV_Read.
        //Handle area: @authHandle, nvIndex (2 handles; authHandle requires auth).
        //Response: no handles.
        TpmCcConstants.TPM_CC_NV_Read
            => TpmaCc.FromCommandCode((uint)commandCode, cHandles: 2),

        //clause 31.7 - TPM2_NV_Write.
        //Handle area: @authHandle, nvIndex (2 handles; authHandle requires auth).
        //Response: no handles.
        TpmCcConstants.TPM_CC_NV_Write
            => TpmaCc.FromCommandCode((uint)commandCode, cHandles: 2, isNv: true),

        //clause 31.4 - TPM2_NV_UndefineSpace.
        //Handle area: @authHandle, nvIndex (2 handles; authHandle requires auth).
        //Response: no handles.
        TpmCcConstants.TPM_CC_NV_UndefineSpace
            => TpmaCc.FromCommandCode((uint)commandCode, cHandles: 2, isNv: true),

        //TPM 2.0 Library Part 3, clause 31.5 - TPM2_NV_UndefineSpaceSpecial.
        //Handle area: @nvIndex (Auth Index 1, ADMIN role), @platform (Auth Index 2, USER role) - 2 handles.
        //Response: no handles.
        TpmCcConstants.TPM_CC_NV_UndefineSpaceSpecial
            => TpmaCc.FromCommandCode((uint)commandCode, cHandles: 2, isNv: true),

        //clause 31.8 - TPM2_NV_Increment.
        //Handle area: @authHandle, nvIndex (2 handles; authHandle requires auth).
        //Response: no handles.
        TpmCcConstants.TPM_CC_NV_Increment
            => TpmaCc.FromCommandCode((uint)commandCode, cHandles: 2, isNv: true),

        //clause 31.9 - TPM2_NV_Extend {NV}.
        //Handle area: @authHandle, nvIndex (2 handles; authHandle requires auth).
        //Response: no handles.
        TpmCcConstants.TPM_CC_NV_Extend
            => TpmaCc.FromCommandCode((uint)commandCode, cHandles: 2, isNv: true),

        //clause 31.10 - TPM2_NV_SetBits {NV}.
        //Handle area: @authHandle, nvIndex (2 handles; authHandle requires auth).
        //Response: no handles.
        TpmCcConstants.TPM_CC_NV_SetBits
            => TpmaCc.FromCommandCode((uint)commandCode, cHandles: 2, isNv: true),

        //clause 31.11 - TPM2_NV_WriteLock {NV}.
        //Handle area: @authHandle, nvIndex (2 handles; authHandle requires auth).
        //Response: no handles.
        TpmCcConstants.TPM_CC_NV_WriteLock
            => TpmaCc.FromCommandCode((uint)commandCode, cHandles: 2, isNv: true),

        //clause 31.12 - TPM2_NV_GlobalWriteLock {NV}.
        //Handle area: @authHandle (1 handle, requires auth; TPMI_RH_PROVISION).
        //Response: no handles.
        TpmCcConstants.TPM_CC_NV_GlobalWriteLock
            => TpmaCc.FromCommandCode((uint)commandCode, cHandles: 1, isNv: true),

        //clause 31.14 - TPM2_NV_ReadLock {NV}.
        //Handle area: @authHandle, nvIndex (2 handles; authHandle requires auth).
        //Response: no handles.
        TpmCcConstants.TPM_CC_NV_ReadLock
            => TpmaCc.FromCommandCode((uint)commandCode, cHandles: 2, isNv: true),

        //clause 31.15 - TPM2_NV_ChangeAuth.
