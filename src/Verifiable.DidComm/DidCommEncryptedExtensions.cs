...
public static class DidCommEncryptedExtensions
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        using(parsed)
        {
            ...
            using(decrypted)
            {
                ...
                //Anoncrypt does not authenticate the sender: no skid, no apu, no sender key id, no proof.
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        using(parsed)
        {
            ...
            //The resolved keyAgreement method's own id, expanded to its absolute DID URL form when the
            //document declares it relatively ("#key-agreement-1") -- the same normalization
            //TryResolveKeyAgreementKey already applied when matching it against senderKeyId, so
            //BoundProvenance.TryBindByKeyAgreement below compares like-for-like against the claimed skid.
            string resolvedSenderMethodId = ExpandVerificationMethodId(senderMethod!.Id, resolution.Document.Id?.Id);
        ...
        CancellationToken cancellationToken)
    {
        ...
        using(decrypted)
        {
            ...
            //authenticated sender key id. Bind it: the decryption above already succeeded under the
            //key resolvedSenderMethodId names, so TryBindByKeyAgreement's evidence is present; it also
            //re-checks resolvedSenderMethodId against the claimed senderKeyId for consistency.
            BoundProvenance? provenance = BoundProvenance.TryBindByKeyAgreement(
                new KeyId(senderKeyId), isDecryptionAuthenticated: true, new KeyId(resolvedSenderMethodId), plaintextMessage);

            if(provenance is null || Verified<DidCommMessage>.TryCreateBound(plaintextMessage, provenance) is not { } verified)
            {
                //Should not occur in the honest flow -- the sender key material used to decrypt was
                //resolved from resolvedSenderMethodId. Fail closed rather than mint an unproven
                //"authenticated" result.
                return DidCommEncryptedUnpackResult.Failed(DidCommEncryptionMode.Authcrypt, DidCommDecryptionError.IdentityBindingFailed);
            }
    ...
    //Expands a possibly-relative verification method id ("#key-agreement-1") to its absolute DID URL form
    //against documentDid, mirroring the normalization IsSameVerificationMethodId already applies when
    //matching -- so BoundProvenance.TryBindByKeyAgreement compares like-for-like against the claimed skid.
        ...
        CancellationToken cancellationToken)
    {
        ...
        //(it already ran the four-gate signed-path recipe over this exact message instance) rather than
        //re-deriving one -- inner.IsVerified true guarantees inner.Verified is present. A verified
        ...
        CancellationToken cancellationToken)
    {
        ...
        {
            return DidCommEncryptedUnpackResult.Unpacked(
                plaintextMessage, verified, mode, senderKeyId, isSenderAuthenticated, isSignedInner, isRecipientAddressedInTo);
        }
