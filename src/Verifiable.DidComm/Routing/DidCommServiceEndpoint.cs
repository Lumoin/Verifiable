...
public sealed record DidCommServiceEndpoint
{
    ...
    /// <summary>
    /// Determines whether this endpoint equals <paramref name="other"/> by value: <see cref="Uri"/> by ordinal
    /// comparison and the <see cref="Accept"/>/<see cref="RoutingKeys"/> lists element-wise in order. The computed
    /// <see cref="AcceptsDidCommV2"/>/<see cref="IsDidUri"/> are derived, so they are not compared independently.
    /// </summary>
    /// <param name="other">The endpoint to compare with, or <see langword="null"/>.</param>
    /// <returns><see langword="true"/> when the two endpoints are value-equal.</returns>
    public bool Equals(DidCommServiceEndpoint? other)
    {
        if(other is null)
        {
            return false;
