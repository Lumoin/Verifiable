...
public static class RoutingForwardExtensions
{
    ...
    /// base64url string. The returned envelope is a freshly-constructed <see cref="DidCommMessage"/>, so an
    /// inner message's <see cref="DidCommMessage.ReturnRoute"/> can never leak onto it — consistent with the
    /// Return-Route and Queue Transport extension's own scope: "This extension is not valid when routing
    /// keys are in use" (<see href="https://github.com/decentralized-identity/didcomm-messaging/blob/main/extensions/return_route/main.md">Return-Route and Queue Transport Extension</see> §Scope).
    /// </summary>
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        return resolution.Targets.Count > 0 ? resolution.Targets[0].RoutingKeys : [];
    }
