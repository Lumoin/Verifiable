...
public sealed record ProblemReport
{
    ...
    /// <summary>
    /// Determines whether this problem report equals <paramref name="other"/> by value: <see cref="Code"/> by its
    /// own equality, the <see cref="ParentThreadId"/>/<see cref="Comment"/>/<see cref="EscalateTo"/> strings by
    /// ordinal comparison, and the <see cref="Args"/>/<see cref="Ack"/> lists element-wise in order.
    /// </summary>
    /// <param name="other">The problem report to compare with, or <see langword="null"/>.</param>
    /// <returns><see langword="true"/> when the two reports are value-equal.</returns>
    public bool Equals(ProblemReport? other)
    {
        if(other is null)
        {
            return false;
        ...
        return Equals(Code, other.Code)
            ...
            && string.Equals(EscalateTo, other.EscalateTo, StringComparison.Ordinal)
            && StructuralEquality.SequenceEqual(Args, other.Args)
    ...
    public override int GetHashCode()
    {
        ...
        hash.Add(EscalateTo, StringComparer.Ordinal);
        hash.Add(StructuralEquality.SequenceHashCode(Args));
