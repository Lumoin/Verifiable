...
/// library's <see cref="Tag"/> mechanism rather than a bespoke provenance type. It lives in the
/// cryptography layer alongside <see cref="Verified{T}"/> and is <see langword="public"/> so every
/// first-party verification library that mints a <see cref="Verified{T}"/> — the credential and
/// presentation paths in <c>Verifiable.Core</c>, DIDComm, and the ACDC message verify — builds the
/// same shape of context, rather than each replicating the recipe.
/// </remarks>
