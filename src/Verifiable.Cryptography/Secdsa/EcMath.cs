...
/// sourced from <see cref="EllipticCurveConstants.P256"/>.
/// </remarks>
public static class EcMath
{
    ...
    /// <summary>
    /// Masks the high bit of the first byte of a random buffer so the resulting
    /// <see cref="BigInteger"/> is guaranteed to be below 2^255. The negligible
    /// bias relative to the P-256 order q is acceptable for key and nonce generation.
    /// </summary>
    private const byte HighBitMask = 0x7F;

    /// <summary>
    /// Generates a cryptographically random non-zero scalar in [1, q-1] using the
    /// operating system's cryptographically secure random number generator.
    /// </summary>
    /// <remarks>
    /// <para>
    /// The returned scalar is suitable for use as a private key scalar or signing nonce.
    /// <see cref="System.Security.Cryptography.RandomNumberGenerator"/> is used as the
    /// entropy source. In a production EUDI Wallet deployment, the Native Cryptographic
    /// Hardware (NCH) — such as a TPM, Secure Enclave, or StrongBox — should supply
    /// the entropy. Callers that need hardware-bound randomness should pass a delegate
    /// rather than calling this method directly.
    /// </para>
    /// <para>
    /// <see cref="BigInteger"/> copies the supplied byte array, so the local buffer
    /// does not need to be pinned or pooled for correctness; however, callers with
    /// strict memory-zeroing requirements should clear the array after the call.
    /// </para>
    /// </remarks>
    /// <returns>A random scalar in [1, q-1].</returns>
