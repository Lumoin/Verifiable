...
public sealed class CBAdESReferences: IDisposable
{
...
/// reuses <see cref="AdESCertificateThumbprint"/> verbatim as <see cref="Thumbprint"/> rather than
...
public sealed class CBAdESCertificateReference: IDisposable
{
    ...
    public CBAdESCertificateReference(
        AdESCertificateThumbprint thumbprint,
    ...
    /// </summary>
    public AdESCertificateThumbprint Thumbprint { get; }
...
/// directly, matching the pattern already used for <see cref="AdESDigestAlgorithmIdentifier"/>'s <c>int /
/// tstr</c> union.
/// </remarks>
public abstract class CBAdESCertificateReferenceKeyIdentifier
{
...
/// <remarks>
/// The arm is nothing but the integer it wraps, so equality is <see cref="Value"/>'s: two arms naming the same
...
public sealed class CBAdESCertificateReferenceKeyIdentifierInteger : CBAdESCertificateReferenceKeyIdentifier, IEquatable<CBAdESCertificateReferenceKeyIdentifierInteger>
{
    ...
    public CBAdESCertificateReferenceKeyIdentifierInteger(int value)
    {
        Value = value;
    }

    /// <summary>The integer key identifier.</summary>
    public int Value { get; }

    /// <inheritdoc/>
    public bool Equals(CBAdESCertificateReferenceKeyIdentifierInteger? other)
    {
    ...
    {
        return Equals(obj as CBAdESCertificateReferenceKeyIdentifierInteger);
    }
    ...
    /// <summary>Reports whether two integer key identifiers hold the same value.</summary>
    public static bool operator ==(CBAdESCertificateReferenceKeyIdentifierInteger? left, CBAdESCertificateReferenceKeyIdentifierInteger? right)
    {
        return left is null ? right is null : left.Equals(right);
    }

    /// <summary>Reports whether two integer key identifiers hold different values.</summary>
    public static bool operator !=(CBAdESCertificateReferenceKeyIdentifierInteger? left, CBAdESCertificateReferenceKeyIdentifierInteger? right)
    {
        return !(left == right);
    }
...
/// <remarks>
/// A <c>kid</c> text arm is identified by the text it carries, so equality is an ordinal comparison of
/// <see cref="Value"/>. Ordinal, not culture-aware: the CBOR text string is an opaque identifier the wire
...
public sealed class CBAdESCertificateReferenceKeyIdentifierText : CBAdESCertificateReferenceKeyIdentifier, IEquatable<CBAdESCertificateReferenceKeyIdentifierText>
{
    ...
    public CBAdESCertificateReferenceKeyIdentifierText(string value)
    {
        Value = value;
    }

    /// <summary>The textual key identifier.</summary>
    public string Value { get; }

    /// <inheritdoc/>
    public bool Equals(CBAdESCertificateReferenceKeyIdentifierText? other)
    {
    ...
    public override bool Equals(object? obj)
    {
        return Equals(obj as CBAdESCertificateReferenceKeyIdentifierText);
    }
    ...
    /// <summary>Reports whether two text key identifiers hold the same value.</summary>
    public static bool operator ==(CBAdESCertificateReferenceKeyIdentifierText? left, CBAdESCertificateReferenceKeyIdentifierText? right)
    {
        return left is null ? right is null : left.Equals(right);
    }

    /// <summary>Reports whether two text key identifiers hold different values.</summary>
    public static bool operator !=(CBAdESCertificateReferenceKeyIdentifierText? left, CBAdESCertificateReferenceKeyIdentifierText? right)
    {
        return !(left == right);
    }
...
/// </summary>
/// <remarks>
/// Equality compares <see cref="Value"/>'s bytes, not the identity of the buffer backing them, so two
/// instances wrapping byte-identical <c>kid</c> payloads are equal regardless of which buffer holds them —
/// that is what "the same value" means for an opaque carrier. <see cref="Value"/> is a borrowed view (see the
...
public sealed class CBAdESCertificateReferenceKeyIdentifierBytes : CBAdESCertificateReferenceKeyIdentifier, IEquatable<CBAdESCertificateReferenceKeyIdentifierBytes>
{
    ...
    /// </param>
    public CBAdESCertificateReferenceKeyIdentifierBytes(ReadOnlyMemory<byte> value)
    {
        Value = value;
    }
    ...
    /// <inheritdoc/>
    public bool Equals(CBAdESCertificateReferenceKeyIdentifierBytes? other)
    {
    ...
    public override bool Equals(object? obj)
    {
        return Equals(obj as CBAdESCertificateReferenceKeyIdentifierBytes);
    }
    ...
    /// <summary>Reports whether two byte key identifiers hold the same octets.</summary>
    public static bool operator ==(CBAdESCertificateReferenceKeyIdentifierBytes? left, CBAdESCertificateReferenceKeyIdentifierBytes? right)
    {
        return left is null ? right is null : left.Equals(right);
    }

    /// <summary>Reports whether two byte key identifiers hold different octets.</summary>
    public static bool operator !=(CBAdESCertificateReferenceKeyIdentifierBytes? left, CBAdESCertificateReferenceKeyIdentifierBytes? right)
    {
        return !(left == right);
    }
...
/// </para>
/// <para>
/// <strong><c>otherRefs</c>'s CDDL comment is a spec-original copy-paste defect.</strong> The
...
public sealed class CBAdESRevocationReferences: IDisposable
{
    ...
    /// (CB-A.1.1-29; see the remarks on this type for why this is not OCSP-only), or
...
/// </code>
/// <para>
/// <c>digAlgVal</c>'s two-element array is flattened onto this instance as <see cref="HashAlgorithm"/> and
/// <see cref="Digest"/>, mirroring <see cref="AdESSignaturePolicyIdentifier"/>'s flattening of its own
/// <c>digAlgVal</c> member and <see cref="AdESCertificateThumbprint"/>'s flattening of <c>COSE_CertHash</c> —
...
public sealed class CBAdESCrlReference: IDisposable
{
    ...
    public CBAdESCrlReference(
        AdESDigestAlgorithmIdentifier hashAlgorithm,
        DigestValue digest,
...
/// therefore implements no <see cref="IDisposable"/>, matching <see cref="AdESPkiObject"/>'s convention.
/// </para>
/// </remarks>
[DebuggerDisplay("CBAdESCrlIdentifier: {Issuer.Length} bytes issuer, IssueTime={IssueTime}")]
public sealed class CBAdESCrlIdentifier
{
...
/// <see cref="AdESSignaturePolicyIdentifier"/>'s flattening of the same <c>[hashAlg, hashValue]</c> shape.
/// </para>
/// <para>
...
public sealed class CBAdESOcspReference: IDisposable
{
...
/// </para>
/// <para>
/// <strong>Ownership.</strong> <see cref="Responder"/>'s sibling types carry only borrowed byte views (see
...
/// </para>
/// <para>
/// <strong>Equality.</strong> An <c>ocspId</c> is the triple it states — which responder, produced when, found
/// where — so equality is that triple's, member by member, with <see cref="Responder"/> deferring to the arm's
/// own byte comparison. Two <c>ocspId</c> maps decoded from byte-identical wire therefore compare equal, which
/// is what a reference to one and the same OCSP response has to mean.
/// </para>
/// </remarks>
[DebuggerDisplay("CBAdESOcspIdentifier: {Responder}, ProducedAt={ProducedAt}")]
public sealed class CBAdESOcspIdentifier: IEquatable<CBAdESOcspIdentifier>
{
    ...
    /// <inheritdoc/>
    public bool Equals(CBAdESOcspIdentifier? other)
    {
        return other is not null
            && Responder.Equals(other.Responder)
            && ProducedAt == other.ProducedAt
    ...
    public override bool Equals(object? obj)
    {
        return Equals(obj as CBAdESOcspIdentifier);
    }

    /// <inheritdoc/>
    public override int GetHashCode()
    {
        return HashCode.Combine(Responder, ProducedAt, LocationHint);
    }

    /// <summary>Reports whether two OCSP identifiers hold the same responder, production time, and location hint.</summary>
    public static bool operator ==(CBAdESOcspIdentifier? left, CBAdESOcspIdentifier? right)
    {
        return left is null ? right is null : left.Equals(right);
    }

    /// <summary>Reports whether two OCSP identifiers hold a different responder, production time, or location hint.</summary>
    public static bool operator !=(CBAdESOcspIdentifier? left, CBAdESOcspIdentifier? right)
    {
        return !(left == right);
    }
...
/// </para>
/// </remarks>
public abstract class CBAdESOcspResponderIdentifier
{
...
/// </summary>
/// <remarks>
/// Equality compares <see cref="Name"/>'s bytes rather than buffer identity, so two instances carrying the
/// same DER-encoded responder name are equal even when read from different buffers. <see cref="Name"/> is a
...
public sealed class CBAdESOcspResponderIdentifierByName : CBAdESOcspResponderIdentifier, IEquatable<CBAdESOcspResponderIdentifierByName>
{
    ...
    /// </param>
    public CBAdESOcspResponderIdentifierByName(ReadOnlyMemory<byte> name)
    {
        Name = name;
    }
    ...
    /// <inheritdoc/>
    public bool Equals(CBAdESOcspResponderIdentifierByName? other)
    {
        return other is not null && Name.Span.SequenceEqual(other.Name.Span);
    }

    /// <inheritdoc/>
    public override bool Equals(object? obj)
    {
        return Equals(obj as CBAdESOcspResponderIdentifierByName);
    }
    ...
    {
        var hash = new HashCode();
        hash.AddBytes(Name.Span);

        return hash.ToHashCode();
    }

    /// <summary>Reports whether two by-name responder identifiers hold the same DER-encoded name.</summary>
    public static bool operator ==(CBAdESOcspResponderIdentifierByName? left, CBAdESOcspResponderIdentifierByName? right)
    {
        return left is null ? right is null : left.Equals(right);
    }

    /// <summary>Reports whether two by-name responder identifiers hold a different DER-encoded name.</summary>
    public static bool operator !=(CBAdESOcspResponderIdentifierByName? left, CBAdESOcspResponderIdentifierByName? right)
    {
        return !(left == right);
    }
...
/// <remarks>
/// <para>
/// <strong>Ruled reading.</strong> The prose additionally says: "If the responder is identified by
...
/// CDDL type of this arm is a plain <c>bstr</c> — a raw CBOR byte string, not CBOR text. The ruled reading:
...
/// </para>
/// <para>
/// Equality compares <see cref="KeyDigest"/>'s bytes rather than buffer identity, so two responder-by-key
/// identifiers carrying the same digest are equal regardless of which buffer backs them; <see cref="KeyDigest"/>
/// is a borrowed view, never disposed, making the comparison always safe.
/// </para>
/// </remarks>
...
public sealed class CBAdESOcspResponderIdentifierByKey : CBAdESOcspResponderIdentifier, IEquatable<CBAdESOcspResponderIdentifierByKey>
{
    ...
    /// caller (creation path) or the wire-bytes source (parse path) owns the underlying memory.
    /// </param>
    public CBAdESOcspResponderIdentifierByKey(ReadOnlyMemory<byte> keyDigest)
    {
        KeyDigest = keyDigest;
    }
    ...
    /// caller (creation path) or the wire-bytes source (parse path) owns the underlying memory.
    /// </summary>
    public ReadOnlyMemory<byte> KeyDigest { get; }

    /// <inheritdoc/>
    public bool Equals(CBAdESOcspResponderIdentifierByKey? other)
    {
        return other is not null && KeyDigest.Span.SequenceEqual(other.KeyDigest.Span);
    }

    /// <inheritdoc/>
    public override bool Equals(object? obj)
    {
        return Equals(obj as CBAdESOcspResponderIdentifierByKey);
    }
    ...
    {
        var hash = new HashCode();
        hash.AddBytes(KeyDigest.Span);

        return hash.ToHashCode();
    }

    /// <summary>Reports whether two by-key responder identifiers hold the same key digest.</summary>
    public static bool operator ==(CBAdESOcspResponderIdentifierByKey? left, CBAdESOcspResponderIdentifierByKey? right)
    {
        return left is null ? right is null : left.Equals(right);
    }

    /// <summary>Reports whether two by-key responder identifiers hold a different key digest.</summary>
    public static bool operator !=(CBAdESOcspResponderIdentifierByKey? left, CBAdESOcspResponderIdentifierByKey? right)
    {
        return !(left == right);
    }
