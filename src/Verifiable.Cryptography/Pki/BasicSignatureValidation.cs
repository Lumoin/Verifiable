...
public sealed record BasicSignatureValidationResult
{
    ...
    /// <summary>
    /// The cryptographic verification building block's own outcome (step 5), carried so a pipeline-gated format
    /// mint (PAdES) can read the certificate crypto ran under; <see langword="null"/> when the process did not
    /// reach step 5, or reached it with a non-<see cref="SignatureCryptographicOutcome.Verified"/> outcome
    /// carrying no certificate. Non-owning — the referenced certificate belongs to the run's
    /// <see cref="SignatureValidationResources"/>.
    /// </summary>
    public SignatureCryptographicVerification? CryptographicVerification { get; init; }
...
public static class BasicSignatureValidation
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        outcome = outcome with { CryptographicVerification = cryptographic.Verification };
