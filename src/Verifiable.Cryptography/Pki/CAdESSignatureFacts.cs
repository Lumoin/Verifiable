...
public static class CAdESSignatureFacts
{
    ...
    public static SignatureFormatSeam Seam { get; } = new()
    {
        ...
        //A method-group conversion to ExtractSignatureFactsAsyncDelegate requires an exact 3-parameter match --
        //ExtractAsync's own trailing optional `format` parameter (added for PAdESSignatureFacts's own reuse, RP-3)
        //makes the bare method group incompatible with the delegate's own Invoke signature, so the 3-argument call
        //is spelled out explicitly here instead.
        ExtractFacts = (context, pool, cancellationToken) => ExtractAsync(context, pool, cancellationToken),
    ...
    /// <param name="cancellationToken">A cancellation token.</param>
    /// <param name="format">
    /// The <see cref="SignatureFacts.Format"/> the returned facts state; defaults to <see cref="SignatureFormatIdentifier.CAdES"/>.
    /// A base format whose own Signature Dictionary/header structure states outright that its embedded object
    /// "forms a CAdES signature" (PAdES's own PA-6.3-h is exactly this join point) passes its own identity here
    /// rather than re-parsing the identical CMS <c>SignedData</c> a second time — <see cref="PAdESSignatureFacts"/>
    /// is this method's second caller, added later without touching a byte of the parse below.
    /// </param>
    /// <returns>The extracted facts, stamped with <paramref name="format"/>, or a <see cref="SignatureFactsStatus.FormatFailure"/> when the bytes are not a processable CMS SignedData. The caller disposes them.</returns>
    [SuppressMessage("Reliability", "CA2000:Dispose objects before losing scope",
    ...
    public static ValueTask<SignatureFacts> ExtractAsync(
        ...
        BaseMemoryPool pool,
        CancellationToken cancellationToken,
        SignatureFormatIdentifier? format = null)
    {
        ArgumentNullException.ThrowIfNull(context);
        ArgumentNullException.ThrowIfNull(pool);
        cancellationToken.ThrowIfCancellationRequested();

        return ValueTask.FromResult(Extract(context, pool, format ?? SignatureFormatIdentifier.CAdES));
    }
        ...
        CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            return new SignatureCryptographicVerification { Outcome = SignatureCryptographicOutcome.Verified, SigningCertificate = context.SigningCertificate };
        }
    ...
    /// <param name="format">The <see cref="SignatureFacts.Format"/> the result states.</param>
    /// <returns>The facts, or a format failure.</returns>
    private static SignatureFacts Extract(SignatureFactsExtractionContext context, BaseMemoryPool pool, SignatureFormatIdentifier format)
    {
        ...
        catch(AsnContentException exception)
        {
            ...
            return SignatureFacts.FormatFailure(format, exception.Message);
        }
    ...
    /// <summary>Copies DER bytes into a freshly rented, pooled <see cref="PkiCertificateMemory"/>. The caller owns and disposes the returned carrier.</summary>
    ...
    internal static PkiCertificateMemory Copy(ReadOnlyMemory<byte> der, Tag tag, BaseMemoryPool pool)
    {
