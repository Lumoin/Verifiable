...
public static class OcspResponseVerification
{
    ...
    /// <summary>
    /// Reads only the <c>BasicOCSPResponse.tbsResponseData.producedAt</c> field
    /// (<see href="https://www.rfc-editor.org/rfc/rfc6960#section-4.2.1">RFC 6960 §4.2.1</see>) — a minimal,
    /// targeted read for a cross-document consistency check (a XAdES <c>OCSPIdentifier/ProducedAt</c> "shall
    /// indicate the same time as the referenced OCSP response's own <c>ProducedAt</c> field," ETSI EN 319 132-1
    /// V1.3.1 Annex A.1.2), never a substitute for <see cref="VerifyAsync"/>'s own full RFC 6960 §3.2 checks:
    /// this method performs no signature verification, no responder-authorisation check, and no freshness check.
    /// </summary>
    /// <param name="response">The DER-encoded <c>OCSPResponse</c> octets.</param>
    /// <param name="producedAt">The decoded <c>producedAt</c> instant on success.</param>
    /// <returns><see langword="true"/> when the envelope parsed as a successful <c>id-pkix-ocsp-basic</c> response far enough to read <c>producedAt</c>; <see langword="false"/> on any malformed or non-matching content — fails closed, never throws, since the response bytes are attacker-reachable input.</returns>
    public static bool TryReadProducedAt(ReadOnlyMemory<byte> response, out DateTimeOffset producedAt)
    {
        producedAt = default;
        try
        {
            (OcspResponseStatus? status, ReadOnlyMemory<byte>? basicResponseBytes) = ReadResponseEnvelope(response);
            if(status != OcspResponseStatus.Successful || basicResponseBytes is not { } basic)
            {
                return false;
            }

            var basicReader = new AsnReader(basic, AsnEncodingRules.DER);
            AsnReader basicSequence = basicReader.ReadSequence();
            ReadOnlyMemory<byte> tbsResponseDataDer = basicSequence.ReadEncodedValue();
            ...
            _ = ReadResponderId(tbsResponseData);          //responderID -- skipped, not needed for this read.
            producedAt = tbsResponseData.ReadGeneralizedTime();
    ...
    {
        (WellKnownOids.EcdsaWithSha256, EllipticCurveTypes.P256) => CryptoAlgorithm.P256,
        (WellKnownOids.EcdsaWithSha384, EllipticCurveTypes.P384) => CryptoAlgorithm.P384,
        (WellKnownOids.EcdsaWithSha512, EllipticCurveTypes.P521) => CryptoAlgorithm.P521,
        _ => null
    };
