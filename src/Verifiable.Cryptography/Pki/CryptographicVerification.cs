...
public sealed record CryptographicVerificationResult
{
    ...
    /// <summary>
    /// The binding's own cryptographic-verification outcome (the certificate it ran under included), which the
    /// conclusion maps onto Table 15's rows and a later binding gate
    /// (<see cref="Verifiable.Cryptography.BoundProvenance.TryBindByCertificateDigestAsync"/>) reads the signing
    /// certificate off.
    /// </summary>
    public required SignatureCryptographicVerification Verification { get; init; }

    /// <summary>The format binding's own outcome, which the conclusion maps onto Table 15's rows.</summary>
    public SignatureCryptographicOutcome Outcome => Verification.Outcome;
}
...
public static class CryptographicVerification
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        return new CryptographicVerificationResult { Conclusion = conclusion, Verification = verification };
    }
}
