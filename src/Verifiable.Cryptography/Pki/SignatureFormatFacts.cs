...
public readonly record struct SignatureFormatIdentifier(string Value)
{
    ...
    /// <summary>
    /// CBOR-Binary Advanced Electronic Signatures over <c>COSE_Sign1</c>/<c>COSE_Sign</c>
    /// (<see href="https://www.etsi.org/deliver/etsi_ts/119100_119199/11915201/01.01.01_60/ts_11915201v010101p.pdf">
    /// ETSI TS 119 152-1 V1.1.1</see>).
    /// </summary>
    public static SignatureFormatIdentifier CBAdES { get; } = new("CBAdES");
}
...
public sealed record SignatureCryptographicVerification
{
    ...
    /// <summary>
    /// The certificate the binding's verify delegate ran under — the SAME instance
    /// <see cref="SignatureCryptographicVerificationContext.SigningCertificate"/> supplied as input — carried
    /// forward rather than discarded so a later binding gate (<see cref="Verifiable.Cryptography.BoundProvenance.TryBindByCertificateDigestAsync"/>)
    /// has exactly one certificate in scope: the one crypto verified under. <see langword="null"/> when
    /// <see cref="Outcome"/> is not <see cref="SignatureCryptographicOutcome.Verified"/>, or when a binding
    /// produced no cryptographic outcome to attribute a certificate to.
    /// </summary>
    /// <remarks>
    /// <strong>Ownership.</strong> A non-owning reference, exactly like <see cref="SignatureCryptographicVerificationContext.SigningCertificate"/>
    /// it is copied from — this record does not own it and must not dispose it.
    /// </remarks>
    public PkiCertificateMemory? SigningCertificate { get; init; }
}
