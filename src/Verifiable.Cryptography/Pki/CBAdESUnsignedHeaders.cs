...
/// load-bearing, not incidental. <see cref="AdESTimestampContainer"/>'s
...
/// bstr-wrapped bytes; the codec owns both directions of that wrapping. The
...
/// </para>
/// <para>
/// <strong>Label/tag (CB-5.3.1-06).</strong> "The <c>uHeaders</c> CBOR array
...
/// this model carries no tag/label field of its own for the same reason
...
public sealed class CBAdESUnsignedHeaders: IReadOnlyList<CBAdESUnsignedHeaderElement>, IDisposable
{
    /// <summary>
...
/// </para>
/// </remarks>
public abstract class CBAdESUnsignedHeaderElement
{
...
/// </summary>
/// <remarks>
/// An independent identity from <see cref="AdESSignaturePolicyQualifierLabel"/>'s occurrence of the same
...
public sealed class CBAdESUnsignedHeaderElementSignatureTimestamp    : CBAdESUnsignedHeaderElement, IDisposable
{
    ...
    public CBAdESUnsignedHeaderElementSignatureTimestamp(CBAdESSignatureTimestamp signatureTimestamp)
    {
        SignatureTimestamp = signatureTimestamp;
    }

    /// <summary>The wrapped signature time-stamp component.</summary>
    public CBAdESSignatureTimestamp SignatureTimestamp { get; }
...
public sealed class CBAdESUnsignedHeaderElementValidationData    : CBAdESUnsignedHeaderElement
{
    ...
    public CBAdESUnsignedHeaderElementValidationData(CBAdESValidationData validationData)
    {
        ValidationData = validationData;
    }

    /// <summary>The wrapped certificate/revocation validation-data component.</summary>
    public CBAdESValidationData ValidationData { get; }
...
public sealed class CBAdESUnsignedHeaderElementArchiveTimestamp    : CBAdESUnsignedHeaderElement, IDisposable
{
    ...
    public CBAdESUnsignedHeaderElementArchiveTimestamp(CBAdESArchiveTimestamp archiveTimestamp)
    {
        ArchiveTimestamp = archiveTimestamp;
    }

    /// <summary>The wrapped archive time-stamp component.</summary>
    public CBAdESArchiveTimestamp ArchiveTimestamp { get; }
...
public sealed class CBAdESUnsignedHeaderElementReferences    : CBAdESUnsignedHeaderElement, IDisposable
{
    ...
    public CBAdESUnsignedHeaderElementReferences(CBAdESReferences references)
    {
        References = references;
    }

    /// <summary>The wrapped certificate/revocation references component.</summary>
    public CBAdESReferences References { get; }
...
public sealed class CBAdESUnsignedHeaderElementSignatureAndReferencesTimestamp    : CBAdESUnsignedHeaderElement, IDisposable
{
    ...
    public CBAdESUnsignedHeaderElementSignatureAndReferencesTimestamp(CBAdESSignatureAndReferencesTimestamp signatureAndReferencesTimestamp)
    {
        SignatureAndReferencesTimestamp = signatureAndReferencesTimestamp;
    }

    /// <summary>The wrapped signature-and-references time-stamp component.</summary>
    public CBAdESSignatureAndReferencesTimestamp SignatureAndReferencesTimestamp { get; }
...
public sealed class CBAdESUnsignedHeaderElementReferencesTimestamp    : CBAdESUnsignedHeaderElement, IDisposable
{
    ...
    public CBAdESUnsignedHeaderElementReferencesTimestamp(CBAdESReferencesTimestamp referencesTimestamp)
    {
        ReferencesTimestamp = referencesTimestamp;
    }

    /// <summary>The wrapped references-only time-stamp component.</summary>
    public CBAdESReferencesTimestamp ReferencesTimestamp { get; }
...
/// </summary>
/// <remarks>
/// The arm adds a wire label to the component it wraps and no state of its own, so equality is the wrapped
/// <see cref="SignaturePolicyStore"/>'s — itself a by-value comparison. Two <c>sigPSt</c> entries stating the
/// same policy store are the same entry, which is what a caller scanning <see cref="CBAdESUnsignedHeaders"/>
...
public sealed class CBAdESUnsignedHeaderElementSignaturePolicyStore    : CBAdESUnsignedHeaderElement, IEquatable<CBAdESUnsignedHeaderElementSignaturePolicyStore>
{
    ...
    /// <exception cref="ArgumentNullException"><paramref name="signaturePolicyStore"/> is <see langword="null"/>.</exception>
    public CBAdESUnsignedHeaderElementSignaturePolicyStore(CBAdESSignaturePolicyStore signaturePolicyStore)
    {
    ...
    /// <summary>The wrapped signature-policy-store component.</summary>
    public CBAdESSignaturePolicyStore SignaturePolicyStore { get; }
    ...
    /// <inheritdoc/>
    public bool Equals(CBAdESUnsignedHeaderElementSignaturePolicyStore? other)
    {
        return other is not null && SignaturePolicyStore.Equals(other.SignaturePolicyStore);
    }

    /// <inheritdoc/>
    public override bool Equals(object? obj)
    {
        return Equals(obj as CBAdESUnsignedHeaderElementSignaturePolicyStore);
    }

    /// <inheritdoc/>
    public override int GetHashCode()
    {
        return HashCode.Combine(SignaturePolicyStore);
    }

    /// <summary>Reports whether two <c>sigPSt</c> elements wrap the same signature policy store.</summary>
    public static bool operator ==(CBAdESUnsignedHeaderElementSignaturePolicyStore? left, CBAdESUnsignedHeaderElementSignaturePolicyStore? right)
    {
        return left is null ? right is null : left.Equals(right);
    }

    /// <summary>Reports whether two <c>sigPSt</c> elements wrap a different signature policy store.</summary>
    public static bool operator !=(CBAdESUnsignedHeaderElementSignaturePolicyStore? left, CBAdESUnsignedHeaderElementSignaturePolicyStore? right)
    {
        return !(left == right);
    }
...
/// single <c>COSE_CounterSignature</c> or the encoded array form, whichever the wire uses — carried verbatim,
/// never reinterpreted by this type. Decoding into the RFC 9338 substrate model
/// (<c>Verifiable.JCose.CounterSignatureV2</c>) is a separate, opt-in step
/// (<c>Verifiable.Cbor.CoseSerialization.ParseCounterSignatureHeaderValue</c>) a caller performs over these
/// same bytes.
/// </para>
/// <para>
/// <strong>Append-only.</strong> Neither this type nor
/// <see cref="CBAdESUnsignedHeaders"/> exposes a verb that mutates an already-incorporated element's own
/// bytes — <see cref="CBAdESUnsignedHeaders.Append"/> is the sole growth operation, adding a new element
/// without touching any existing one. CB-5.3.5.1-03 ("the contents of the counter signature element should
/// not be changed, once it has been time-stamped by the <c>arcTst</c>") is satisfied structurally by this
/// absence, the same ruling authority the CAdES precedent at
/// <see cref="CAdESSignatureAugmentation.AddSignatureTimestampAsync"/>'s own countersignature interaction draws
/// ("mutating a countersignature does not [leave an existing archive time-stamp valid]") — no mutation verb
/// exists here to reach that outcome, rather than one existing and being merely discouraged.
/// </para>
/// <para>
/// Equality compares <see cref="Value"/>'s encoded bytes rather than buffer identity — the encoded bytes ARE
/// this arm's content — so two elements carrying an identical counter-signature encoding are equal no matter
/// which buffer backs them. <see cref="Value"/> is a borrowed view, never disposed, so the comparison is
/// always safe.
/// </para>
/// </remarks>
...
public sealed class CBAdESUnsignedHeaderElementFullCounterSignature    : CBAdESUnsignedHeaderElement, IEquatable<CBAdESUnsignedHeaderElementFullCounterSignature>
{
    ...
    /// </param>
    public CBAdESUnsignedHeaderElementFullCounterSignature(ReadOnlyMemory<byte> value)
    {
        Value = value;
    }
    ...
    /// <inheritdoc/>
    public bool Equals(CBAdESUnsignedHeaderElementFullCounterSignature? other)
    {
    ...
    public override bool Equals(object? obj)
    {
        return Equals(obj as CBAdESUnsignedHeaderElementFullCounterSignature);
    }
    ...
    /// <summary>Reports whether two full-counter-signature elements hold the same encoded bytes.</summary>
    public static bool operator ==(CBAdESUnsignedHeaderElementFullCounterSignature? left, CBAdESUnsignedHeaderElementFullCounterSignature? right)
    {
        return left is null ? right is null : left.Equals(right);
    }

    /// <summary>Reports whether two full-counter-signature elements hold different encoded bytes.</summary>
    public static bool operator !=(CBAdESUnsignedHeaderElementFullCounterSignature? left, CBAdESUnsignedHeaderElementFullCounterSignature? right)
    {
        return !(left == right);
    }
...
/// <para>
/// <see cref="Value"/> is the raw, already-CBOR-encoded value bytes for this <c>UHeaderInstance</c> arm — a
/// bare <c>bstr</c> per RFC 9338 §3.2 — carried verbatim, never reinterpreted by this type. Decoding into the
/// RFC 9338 substrate model (<c>Verifiable.JCose.CounterSignature0V2</c>) is a separate, opt-in step
/// (<c>Verifiable.Cbor.CoseSerialization.ParseCounterSignatureHeaderValue</c>) a caller performs over these
/// same bytes. This abbreviated form carries no protected headers of its own (RFC 9338 §3.2: "no provision
/// for any protected attributes") — structurally less material for a caller-side completeness resolver
/// (CB-5.3.5.1-02) to decode than the full <see cref="CBAdESUnsignedHeaderElementFullCounterSignature"/> arm.
/// </para>
/// <para>
/// <strong>Append-only.</strong> See
/// <see cref="CBAdESUnsignedHeaderElementFullCounterSignature"/>'s own remarks — the identical reasoning
/// applies verbatim: no mutation verb exists here, only <see cref="CBAdESUnsignedHeaders.Append"/>, so
/// CB-5.3.5.1-03 is satisfied structurally.
/// </para>
/// <para>
/// Equality compares <see cref="Value"/>'s bytes, not buffer identity, so two abbreviated counter-signature
/// elements carrying the same encoded <c>COSE_CounterSignature0</c> bytes are equal regardless of backing
/// buffer. <see cref="Value"/> is a borrowed view, never disposed, so the comparison is always safe.
/// </para>
/// </remarks>
...
public sealed class CBAdESUnsignedHeaderElementAbbreviatedCounterSignature    : CBAdESUnsignedHeaderElement, IEquatable<CBAdESUnsignedHeaderElementAbbreviatedCounterSignature>
{
    ...
    /// the wire-bytes source (parse path) owns the underlying memory.
    /// </param>
    public CBAdESUnsignedHeaderElementAbbreviatedCounterSignature(ReadOnlyMemory<byte> value)
    {
        Value = value;
    }
    ...
    /// <inheritdoc/>
    public bool Equals(CBAdESUnsignedHeaderElementAbbreviatedCounterSignature? other)
    {
        return other is not null && Value.Span.SequenceEqual(other.Value.Span);
    }
    ...
    public override bool Equals(object? obj)
    {
        return Equals(obj as CBAdESUnsignedHeaderElementAbbreviatedCounterSignature);
    }
    ...
    /// <summary>Reports whether two abbreviated-counter-signature elements hold the same encoded bytes.</summary>
    public static bool operator ==(CBAdESUnsignedHeaderElementAbbreviatedCounterSignature? left, CBAdESUnsignedHeaderElementAbbreviatedCounterSignature? right)
    {
        return left is null ? right is null : left.Equals(right);
    }

    /// <summary>Reports whether two abbreviated-counter-signature elements hold different encoded bytes.</summary>
    public static bool operator !=(CBAdESUnsignedHeaderElementAbbreviatedCounterSignature? left, CBAdESUnsignedHeaderElementAbbreviatedCounterSignature? right)
    {
        return !(left == right);
    }
...
/// </summary>
/// <remarks>
/// Carried OPAQUE: <see cref="Value"/> is the raw, already-CBOR-encoded <c>x5chain</c> value
...
/// <para>
/// Equality compares <see cref="Value"/>'s bytes rather than buffer identity, so two elements carrying the
/// identical opaque <c>x5chain</c> encoding are equal regardless of which buffer backs them. <see cref="Value"/>
/// is a borrowed view, never disposed, so the comparison is always safe to perform.
/// </para>
/// </remarks>
...
public sealed class CBAdESUnsignedHeaderElementCertificateChain    : CBAdESUnsignedHeaderElement, IEquatable<CBAdESUnsignedHeaderElementCertificateChain>
{
    ...
    /// the wire-bytes source (parse path) owns the underlying memory.
    /// </param>
    public CBAdESUnsignedHeaderElementCertificateChain(ReadOnlyMemory<byte> value)
    {
        Value = value;
    }
    ...
    /// <inheritdoc/>
    public bool Equals(CBAdESUnsignedHeaderElementCertificateChain? other)
    {
        return other is not null && Value.Span.SequenceEqual(other.Value.Span);
    }
    ...
    public override bool Equals(object? obj)
    {
        return Equals(obj as CBAdESUnsignedHeaderElementCertificateChain);
    }
    ...
    /// <summary>Reports whether two certificate-chain elements hold the same encoded bytes.</summary>
    public static bool operator ==(CBAdESUnsignedHeaderElementCertificateChain? left, CBAdESUnsignedHeaderElementCertificateChain? right)
    {
        return left is null ? right is null : left.Equals(right);
    }

    /// <summary>Reports whether two certificate-chain elements hold different encoded bytes.</summary>
    public static bool operator !=(CBAdESUnsignedHeaderElementCertificateChain? left, CBAdESUnsignedHeaderElementCertificateChain? right)
    {
        return !(left == right);
    }
...
/// <see cref="AdESSignaturePolicyQualifierLabel"/>'s own occurrence of the same CDDL rule for
...
/// <see cref="CBAdESArchiveTimestamp"/>, both straight aliases of <see cref="AdESTimestampContainer"/>).
