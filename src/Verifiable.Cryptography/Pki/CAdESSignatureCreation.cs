...
/// </para>
/// <para>
/// <strong>Algorithm gating.</strong> MD5 is refused unconditionally and SHA-1 is refused as a
...
/// positional call written against this type's public surface ends at <c>cancellationToken</c>, and a new trailing optional
...
/// mirror the three phases above and reuse the same attribute builders, the same algorithm gate, the same
...
public static class CAdESSignatureCreation
{
    ...
    /// <param name="shouldIncludeSigningTimeAttribute">
    /// Whether the mandatory <c>signing-time</c> attribute (RFC 5652 §11.3) is added, default <see langword="true"/>
    /// — CAdES-B-B's own baseline shape. A profile that states its own, different carrier for the claimed signing
    /// time and forbids this attribute's duplication of it (PAdES's PA-6.3-T13: "SPO: signing-time attribute in CMS
    /// signature: shall not be present", cardinality 0, because ISO 32000-1's Signature Dictionary <c>M</c> entry
    /// already carries it, PA-6.3-T12/g)) passes <see langword="false"/>; <paramref name="signingTime"/> is then
    /// unused by this call, since nothing here states it.
    /// </param>
    ...
    [SuppressMessage("Design", "CA1068:CancellationToken parameters must come last",
        Justification = "optionalAttributes and shouldIncludeSigningTimeAttribute are deliberately the LAST parameters, after cancellationToken (see the class remarks): every positional call written against this type's public surface ends at cancellationToken, and a new trailing optional parameter never breaks a shorter positional call, while inserting it before cancellationToken would silently retarget those calls' final positional argument.")]
    public static async ValueTask<CAdESSignaturePreparation> PrepareAsync(
        ...
        bool shouldIncludeSigningTimeAttribute = true)
    {
        ...
        try
        {
            ...
            if(shouldIncludeSigningTimeAttribute)
            {
    ...
    /// <param name="shouldIncludeSigningTimeAttribute">Whether the mandatory <c>signing-time</c> attribute is added; see <see cref="PrepareAsync"/>.</param>
    /// <returns>The completed CAdES-B-B <c>SignedData</c> wire bytes. The caller owns and disposes it.</returns>
    [SuppressMessage("Design", "CA1068:CancellationToken parameters must come last",
        Justification = "optionalAttributes and shouldIncludeSigningTimeAttribute are deliberately the LAST parameters, after cancellationToken; see PrepareAsync's remarks and its matching suppression.")]
        ...
        bool shouldIncludeSigningTimeAttribute = true)
    {
        ...
        return SignAsync(
            ...
            shouldIncludeSigningTimeAttribute: shouldIncludeSigningTimeAttribute);
    }
        ...
        bool shouldIncludeSigningTimeAttribute = true)
    {
        ...
        using CAdESSignaturePreparation preparation = await PrepareAsync(
            ...
            algorithmConstraints, cmsAlgorithmProtectionOid, pool, cancellationToken, optionalAttributes,
            shouldIncludeSigningTimeAttribute).ConfigureAwait(false);
    ...
    /// <summary>
    /// Refuses a digest algorithm this surface must not create a signature under: MD5 unconditionally
    ...
    {
        var a when a == CryptoAlgorithm.P256 => new CAdESSigningProfile(PkiDigestAlgorithm.Sha256, WellKnownOids.EcdsaWithSha256, IsEllipticCurve: true, EllipticCurveFieldWidthBytes: 32),
        var a when a == CryptoAlgorithm.P384 => new CAdESSigningProfile(PkiDigestAlgorithm.Sha384, WellKnownOids.EcdsaWithSha384, IsEllipticCurve: true, EllipticCurveFieldWidthBytes: 48),
        var a when a == CryptoAlgorithm.P521 => new CAdESSigningProfile(PkiDigestAlgorithm.Sha512, WellKnownOids.EcdsaWithSha512, IsEllipticCurve: true, EllipticCurveFieldWidthBytes: 66),
