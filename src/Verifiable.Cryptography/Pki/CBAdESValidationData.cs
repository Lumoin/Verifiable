...
/// imprecise; the real, mechanically-enforced constraint is that a present
...
/// </para>
/// <para>
/// <strong>Ownership.</strong> Every <see cref="AdESPkiObject"/> reachable through
/// <see cref="CertificateValues"/> or <see cref="RevocationValues"/> carries a borrowed
/// <see cref="ReadOnlyMemory{T}"/> view (see <see cref="AdESPkiObject.Val"/>'s ownership remarks); this type
...
/// second — the prose ("An <c>x509Cert</c> item shall contain ..."; "An
...
/// The encapsulating <see cref="AdESPkiObject"/> instance. "An <c>x509Cert</c> item shall contain one
/// DER-encoded X.509 certificate encapsulated within an instance of <c>pkiOb</c> type" — since
/// <see cref="Certificate"/> carries no explicit <see cref="AdESPkiObject.Encoding"/>, DER is the default
...
/// <see cref="AdESPkiObject.SpecRef"/> identify the format. "An <c>otherCert</c> item is a placeholder for
...
/// independently-optional non-empty lists of <see cref="AdESPkiObject"/> instances (CRLs, OCSP responses,
...
public sealed record CBAdESRevocationValues
{
    ...
    /// encapsulated in a <see cref="AdESPkiObject"/> (CB-5.3.4-06/07), or <see langword="null"/> when absent.
    /// </summary>
    /// <remarks>
    ...
    /// each encapsulated in a <see cref="AdESPkiObject"/> (CB-5.3.4-09/10, RFC 6960 clause 4.2.1), or
    /// <see langword="null"/> when absent.
    /// </summary>
