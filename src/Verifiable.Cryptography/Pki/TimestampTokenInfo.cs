...
/// and, when produced by <see cref="ReadFromTokenAsync"/>, every carrier in <see cref="EmbeddedCertificates"/>
/// and <see cref="EmbeddedCrls"/>; the caller disposes the instance, which disposes them.
/// </para>
/// </remarks>
...
public sealed class TimestampTokenInfo: IDisposable
{
    ...
    /// <summary>The token's own embedded certificates/CRLs and signer identity, owned by this instance; <see langword="null"/> when this instance never saw the wrapping CMS layer.</summary>
    private CmsEmbeddedMaterial? EmbeddedMaterial { get; }
    ...
    /// <param name="embeddedMaterial">The token's own embedded certificates/CRLs and signer identity, owned by this instance; <see langword="null"/> when this instance never saw the wrapping CMS layer.</param>
    private TimestampTokenInfo(
        ...
        CmsEmbeddedMaterial? embeddedMaterial)
    {
        Status = status;
        Version = version;
        PolicyOid = policyOid;
    ...
    /// <summary>
    /// Gets whether the token's own CMS <c>SignedData</c>'s embedded certificates and CRLs could be read, and
    /// if not, why. <see cref="CmsEmbeddedMaterialStatus.NotRead"/> when <see cref="ReadFromTokenAsync"/> did
    /// not produce this instance — the same "no visibility" case <see cref="HasEmbeddedCertificates"/>'s
    /// remarks describe — so a genuinely <see cref="CmsEmbeddedMaterialStatus.Malformed"/> embedded structure
    /// is never silently indistinguishable from a token that simply embeds nothing.
    /// </summary>
    public CmsEmbeddedMaterialStatus EmbeddedMaterialStatus => EmbeddedMaterial?.Status ?? CmsEmbeddedMaterialStatus.NotRead;
    ...
    /// </summary>
    public bool HasEmbeddedCertificates => EmbeddedCertificates.Count > 0;

    /// <summary>
    /// Gets the certificates the token's own CMS <c>SignedData</c> carries in its optional <c>certificates</c>
    /// set (RFC 5652 §5.1), owned by this instance; empty when <see cref="ReadFromTokenAsync"/> did not produce
    /// this instance, since only that entry point sees the wrapping CMS layer. Read through
    /// <see cref="CmsEmbeddedMaterial"/> — a parse of the token's own bytes, independent of whichever
    /// <see cref="VerifyCmsSignedDataDelegate"/> backend verified the token's signature.
    /// </summary>
    public IReadOnlyList<PkiCertificateMemory> EmbeddedCertificates => EmbeddedMaterial?.Certificates ?? [];

    /// <summary>
    /// Gets the Certificate Revocation Lists the token's own CMS <c>SignedData</c> carries in its optional
    /// <c>crls</c> set (RFC 5652 §5.1), owned by this instance; empty when <see cref="ReadFromTokenAsync"/> did
    /// not produce this instance, or when the token embeds none — this library's closure of CB-6.3-28's
    /// revocation half (the "certificate and revocation values embedded in the electronic time-stamp itself"
    /// service-provision option, ETSI TS 119 152-1 clause 6.3 additional requirements (h)/(i)).
    /// </summary>
    public IReadOnlyList<PkiCertificateMemory> EmbeddedCrls => EmbeddedMaterial?.Crls ?? [];

    /// <summary>
    /// Gets the token's own signer certificate among <see cref="EmbeddedCertificates"/>, identified by matching
    /// the token's <c>SignerInfo</c> identity (RFC 5652 §5.3); <see langword="null"/> when it is not among the
    /// embedded set, or when <see cref="ReadFromTokenAsync"/> did not produce this instance.
    /// </summary>
    /// <remarks>
    /// This is an IDENTITY match against the embedded set, never a cryptographic binding to the signature the
    /// token's own <c>SignerInfo</c> actually produced — see <see cref="CmsEmbeddedMaterial.SignerCertificate"/>'s
    /// remarks, including the duplicate-<c>sid</c> decoy residue this identity match inherits unresolved. The
    /// returned instance is a non-owned alias into <see cref="EmbeddedCertificates"/>, never a separate rental:
    /// never dispose it directly — disposing this <see cref="TimestampTokenInfo"/> is what disposes the carrier
    /// it aliases, and disposing both is the double-dispose trap.
    /// </remarks>
    public PkiCertificateMemory? SignerCertificate => EmbeddedMaterial?.SignerCertificate;

    /// <summary>
    /// Determines whether <paramref name="certificateDer"/> identifies this token's own signer — the SAME
    /// issuer-and-serial-number/subject-key-identifier identity comparison <see cref="SignerCertificate"/> runs
    /// against <see cref="EmbeddedCertificates"/>, run instead against a caller-supplied candidate this token's
    /// own bytes never carried (a signature's own <c>valData</c> certificate values, for instance). Never a
    /// cryptographic binding to the signature this token's own <c>SignerInfo</c> actually produced — the same
    /// reading <see cref="SignerCertificate"/>'s own remarks state.
    /// </summary>
    /// <param name="certificateDer">The candidate certificate's DER encoding.</param>
    /// <returns>
    /// <see langword="false"/> when <see cref="ReadFromTokenAsync"/> did not produce this instance, when this
    /// token's own embedded-material parse did not reach a <c>SignerInfo</c>, or when <paramref name="certificateDer"/>
    /// does not itself parse as a certificate; otherwise whether its identity matches this token's own signer
    /// identifier.
    /// </returns>
    public bool IsSignerCertificate(ReadOnlyMemory<byte> certificateDer) => EmbeddedMaterial?.IsSignerCertificate(certificateDer) ?? false;
    ...
    /// other members carry data. <see cref="EmbeddedCertificates"/> and <see cref="EmbeddedCrls"/> are empty
    /// and <see cref="SignerCertificate"/> is <see langword="null"/> on the instance this overload returns —
    /// see those members' remarks. The caller disposes the returned instance in every case.
    /// </returns>
    public static TimestampTokenInfo Read(ReadOnlyMemory<byte> tstInfo, BaseMemoryPool pool) =>
        Read(tstInfo, pool, embeddedMaterial: null);


    /// <summary>
    /// The core of <see cref="Read(ReadOnlyMemory{byte}, BaseMemoryPool)"/>, additionally threading through the
    /// <see cref="CmsEmbeddedMaterial"/> <see cref="ReadFromTokenAsync"/> alone can observe (it opens the
    /// wrapping CMS layer this overload never sees) — the single choke point both entry points share, so the
    /// fact is set exactly once, at the point of construction, never patched onto an already-minted instance.
    /// </summary>
    /// <param name="tstInfo">The DER-encoded <c>TSTInfo</c>.</param>
    /// <param name="pool">The memory pool the message-imprint carrier is rented from.</param>
    /// <param name="embeddedMaterial">The token's own embedded certificates/CRLs and signer identity; ownership transfers to the returned instance.</param>
    /// <returns>The read facts; see <see cref="Read(ReadOnlyMemory{byte}, BaseMemoryPool)"/>.</returns>
    private static TimestampTokenInfo Read(ReadOnlyMemory<byte> tstInfo, BaseMemoryPool pool, CmsEmbeddedMaterial? embeddedMaterial)
    {
        ...
        try
        {
            ...
            {
                return Failed(TimestampTokenInfoStatus.Malformed, embeddedMaterial);
            }
        ...
        {
            return Failed(TimestampTokenInfoStatus.UnsupportedMessageImprintAlgorithm, embeddedMaterial);
        }
        ...
        //embeddedMaterial explicitly (no closure capture) even though every call site in THIS overload passes
        //through the same enclosing parameter, since a static local function cannot close over it.
        static TimestampTokenInfo Failed(TimestampTokenInfoStatus status, CmsEmbeddedMaterial? embeddedMaterial) =>
            new(status, version: 0, policyOid: string.Empty, messageImprintAlgorithm: default,
                ...
                serialNumber: string.Empty, nonce: null, timestampAuthorityName: null, embeddedMaterial);
    }
    ...
    /// instance's <see cref="EmbeddedCertificates"/>, <see cref="EmbeddedCrls"/> and <see cref="SignerCertificate"/>
    /// reflect a genuine parse of the token's own bytes. The caller disposes the returned instance in every case.
    /// </returns>
    ...
    /// </para>
    /// <para>
    /// The embedded material is read through <see cref="CmsEmbeddedMaterial.Parse"/> over the token's own
    /// bytes — a separate managed parse from the signature check the registry-resolved
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        CmsEmbeddedMaterial? embeddedMaterial = null;
        try
        {
            ...
            using(content)
            {
                ...
                //The single choke point where the token's embedded material is actually observable: parsed from
                //the token's own bytes, independently of whichever backend content came from, before the CMS
                //layer this call opened goes out of scope — see the private Read(bytes, pool, embeddedMaterial)
                //overload's remarks.
                embeddedMaterial = CmsEmbeddedMaterial.Parse(token.AsReadOnlyMemory(), pool);

                //Read takes ownership of embeddedMaterial on every path it returns from — each of its own
                //return statements embeds it into the TimestampTokenInfo it constructs before returning. Once
                //Read has returned without throwing, that transfer is complete, so the local is cleared: the
                //finally below then disposes embeddedMaterial only for the window before this point, where a
                //failure (Read throwing before it constructs anything, nothing having reached Read at all, or
                //cancellation) leaves this the sole reference.
                TimestampTokenInfo result = Read(content.Content, pool, embeddedMaterial);
                embeddedMaterial = null;
