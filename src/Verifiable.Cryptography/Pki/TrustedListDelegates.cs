...
/// </para>
/// <para>
/// <strong>Example implementation shape:</strong> the reader walks the document bytes directly with a
/// span-based reader — the discipline <c>JwkJsonReader</c> follows for fixed-shape wire JSON — never a
/// reflection-driven document object model. The platform's XML types are banned in this library
/// (BannedSymbols.Serialization.txt) because they are neither trim- nor ahead-of-time-safe and because a
/// signature's canonical byte form must be produced by code this library owns.
/// </para>
/// <code>
/// ParseTrustedListDelegate parseTrustedList = (document, pool, cancellationToken) =>
/// {
///     //... walk document.AsReadOnlySpan() with an explicit Stack&lt;T&gt; (never recursion) into a TrustedList.
...
/// project stays serialization-agnostic. An implementation composes this library's own signature
/// primitives over a canonical byte form it produces itself, plus a small, targeted read of the
/// <c>xades:SigningCertificate</c> binding — structurally the XML analogue of the ESS
/// <c>signing-certificate-v2</c> check <see cref="CAdESVerification"/> already performs for CMS. The
/// platform's XML and XML-signature types are banned here (BannedSymbols.Serialization.txt): they are
/// neither trim- nor ahead-of-time-safe, and a signature's canonical bytes must come from code this
/// library owns.
/// </para>
/// <para>
