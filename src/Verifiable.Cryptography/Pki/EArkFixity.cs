...
/// </para>
/// </remarks>
public abstract class EArkFixity: IDisposable
{
    /// <summary>
...
public sealed class EArkRecomputableFixity: EArkFixity
{
    /// <summary>
...
/// </para>
/// <para>
/// Everything the case holds is text and a reason, so equality is the stated algorithm name, the stated value
/// and <see cref="EArkFixity.Status"/> together — the strings compared ordinally, since they are carried
/// exactly as written and this case is precisely the one that declines to normalise them. Two documents
/// stating the same unrecomputable fixity therefore yield equal instances.
/// </para>
/// </remarks>
[DebuggerDisplay("{DebuggerDisplay,nq}")]
public sealed class EArkStatedFixity: EArkFixity, IEquatable<EArkStatedFixity>
{
    /// <summary>
    ...
    /// <inheritdoc/>
    public bool Equals(EArkStatedFixity? other)
    {
        return other is not null
            && Status == other.Status
            && string.Equals(ChecksumType, other.ChecksumType, StringComparison.Ordinal)
    ...
    {
        return Equals(obj as EArkStatedFixity);
    }
    ...
    {
        return HashCode.Combine(Status, ChecksumType, Checksum);
    }


    /// <summary>Reports whether two stated fixities claim the same algorithm, value, and status.</summary>
    public static bool operator ==(EArkStatedFixity? left, EArkStatedFixity? right)
    {
        return left is null ? right is null : left.Equals(right);
    }


    /// <summary>Reports whether two stated fixities claim a different algorithm, value, or status.</summary>
    public static bool operator !=(EArkStatedFixity? left, EArkStatedFixity? right)
    {
        return !(left == right);
    }
