...
/// ETSI EN 319 412-5 V2.6.1</see>, and the CertificatePolicies, KeyUsage and ExtendedKeyUsage extension contents
...
/// determination tables select rows only by the known types); of duplicate extensions — an RFC 5280
/// §4.2 profile violation — the first occurrence is read; Annex B SIZE and value-range constraints —
/// including clause 4.2.3's <c>SEQUENCE SIZE (1)</c> "one and only one" bound on <c>QcType</c> — are
/// recorded as read, not enforced (they are issuance-profile requirements, and the determination tables
/// handle multi-type certificates explicitly); and a clause 4.2.4, 4.2.5 or 4.3 statement whose
/// <c>statementInfo</c> is malformed or carries trailing data declares nothing this library can read — the
/// affected fact stays absent and extraction of the rest continues, per the clause 4.1 NOTE ("this extension
/// is not processed as part of IETF RFC 5280 path validation and there are no security implications with
/// accepting a certificate in a system that cannot parse this extension") and because no determination
/// selects rows by these statements. The statements the determination does read —
/// <c>QcCompliance</c>, <c>QcSSCD</c> and <c>QcType</c> — keep the strict posture, where malformed content
/// throws, because silently degrading a consumed fact would alter determination row selection.
/// </para>
/// <para>
...
public static class QualifiedCertificateFactsExtractor
{
    ...
    public static QualifiedCertificateFacts Extract(PkiCertificateMemory certificate)
    {
        ...
        return new QualifiedCertificateFacts
        {
            ...
            QcLimitValue = extensions.QcLimitValue,
            QcRetentionPeriodYears = extensions.QcRetentionPeriodYears,
            QcPdsLocations = extensions.QcPdsLocations,
            QcCcLegislationCountryCodes = extensions.QcCcLegislationCountryCodes,
            QcIdentityVerificationMethods = extensions.QcIdentityVerificationMethods,
            QcQscdLegislationCountryCodes = extensions.QcQscdLegislationCountryCodes,
    ...
    private static ExtensionFacts ReadExtensions(AsnReader tbsCertificate)
    {
        ...
        return new ExtensionFacts(
            ...
            qcStatements?.QcLimitValue,
            qcStatements?.QcRetentionPeriodYears,
            qcStatements?.QcPdsLocations ?? [],
            qcStatements?.QcCcLegislationCountryCodes ?? [],
            qcStatements?.QcIdentityVerificationMethods ?? [],
            qcStatements?.QcQscdLegislationCountryCodes ?? [],
    ...
    /// <see href="https://www.etsi.org/deliver/etsi_en/319400_319499/31941205/02.06.01_60/en_31941205v020601p.pdf">
    /// ETSI EN 319 412-5 V2.6.1</see> statements the determination reads: <c>id-etsi-qcs-QcCompliance</c>,
    /// <c>id-etsi-qcs-QcSSCD</c>, the <c>id-etsi-qcs-QcType</c> statement whose info is a
    /// <c>SEQUENCE OF OBJECT IDENTIFIER</c> naming the declared certificate types, the clause 4.2.4/4.2.5
    /// <c>QcCClegislation</c>/<c>QcQSCDlegislation</c> country-name statements, and the clause 4.3 generic
    /// statements — <c>QcLimitValue</c> (clause 4.3.2), <c>QcRetentionPeriod</c> (clause 4.3.3),
    /// <c>QcPDS</c> (clause 4.3.4) and <c>QcIdentMethod</c> (clause 4.3.5.1) — per the Annex B ASN.1 module,
    /// which QCS-4.1-04 gives precedence over the body syntax on discrepancy. A clause 4.2.4, 4.2.5 or 4.3
    /// statement whose <c>statementInfo</c> is malformed degrades — the affected fact stays absent and the
    /// walk continues — while <c>QcCompliance</c>, <c>QcSSCD</c> and <c>QcType</c> keep a strict posture and
    /// throw <see cref="AsnContentException"/> on malformed content, since they are the statements the
    /// determination reads.
    /// </summary>
    /// <param name="qcStatements">The reader positioned at the extension's inner value.</param>
    /// <returns>
    /// The statement facts. Unknown statements and unknown type/method identifiers are ignored. A duplicate
    /// instance of one of the clause 4.2.4, 4.2.5 and 4.3 statements (QCS-4.1-02A forbids more than one) is
    /// skipped without parsing, while a duplicate <c>QcType</c> statement's declared identifiers accumulate
    /// across instances.
    /// </returns>
    private static QcStatementFacts ReadQcStatements(AsnReader qcStatements)
    {
        ...
        bool qcLimitValueSeen = false;
        QcMonetaryValue? qcLimitValue = null;
        bool qcRetentionPeriodSeen = false;
        BigInteger? qcRetentionPeriodYears = null;
        bool qcPdsSeen = false;
        List<PdsLocation> qcPdsLocations = [];
        bool qcCcLegislationSeen = false;
        List<string> qcCcLegislationCountryCodes = [];
        bool qcIdentMethodSeen = false;
        List<EuIdentityVerificationMethod> qcIdentityVerificationMethods = [];
        bool qcQscdLegislationSeen = false;
        List<string> qcQscdLegislationCountryCodes = [];
            ...
            switch(statementId)
            {
                ...
                case WellKnownOids.QcType:
                    ...
                    if(statement.HasData)
                    {
                        ...
                        statement.ThrowIfNotEmpty();
                    }

                    break;

                //QCS-4.1-02A forbids more than one instance of a statement; a second instance is a profile
                //violation skipped without parsing, mirroring the duplicate-extension tolerance above.
                case WellKnownOids.QcLimitValue:
                    if(qcLimitValueSeen)
                    {
                        break;
                    }

                    qcLimitValueSeen = true;
                    if(statement.HasData)
                    {
                        //A malformed QcLimitValue statementInfo declares nothing this library can read; the
                        //fact stays absent and the walk continues (EN 319 412-5 clause 4.1 NOTE).
                        try
                        {
                            //MonetaryValue ::= SEQUENCE { currency Iso4217CurrencyCode, amount INTEGER, exponent INTEGER } (Annex B clause 4.3.2).
                            AsnReader monetaryValue = statement.ReadSequence();
                            Asn1Tag currencyTag = monetaryValue.PeekTag();
                            string? alphabeticCurrencyCode = null;
                            BigInteger? numericCurrencyCode = null;
                            if(currencyTag.TagClass == TagClass.Universal && (UniversalTagNumber)currencyTag.TagValue == UniversalTagNumber.PrintableString)
                            {
                                alphabeticCurrencyCode = monetaryValue.ReadCharacterString(UniversalTagNumber.PrintableString);
                            }
                            else if(currencyTag.TagClass == TagClass.Universal && (UniversalTagNumber)currencyTag.TagValue == UniversalTagNumber.Integer)
                            {
                                numericCurrencyCode = monetaryValue.ReadInteger();
                            }
                            else
                            {
                                throw new AsnContentException("A MonetaryValue currency must be the Iso4217CurrencyCode CHOICE of PrintableString or INTEGER (EN 319 412-5 clause 4.3.2).");
                            }

                            BigInteger amount = monetaryValue.ReadInteger();
                            BigInteger exponent = monetaryValue.ReadInteger();
                            monetaryValue.ThrowIfNotEmpty();
                            statement.ThrowIfNotEmpty();
                            qcLimitValue = new QcMonetaryValue(alphabeticCurrencyCode, numericCurrencyCode, amount, exponent);
                        }
                ...
                case WellKnownOids.QcRetentionPeriod:
                    if(qcRetentionPeriodSeen)
                    {
                        break;
                    }

                    qcRetentionPeriodSeen = true;
                    if(statement.HasData)
                    {
                        //A malformed QcRetentionPeriod statementInfo declares nothing this library can
                        //read; the fact stays absent and the walk continues (EN 319 412-5 clause 4.1 NOTE).
                        try
                        {
                            //QcEuRetentionPeriod ::= INTEGER (Annex B clause 4.3.3); a BigInteger carrier
                            //records the value as read, unbounded by the CLR's 64-bit integer types.
                            BigInteger retentionPeriodYears = statement.ReadInteger();
                            statement.ThrowIfNotEmpty();
                            qcRetentionPeriodYears = retentionPeriodYears;
                        }
                        catch(AsnContentException)
                        {
                            //A malformed QcRetentionPeriod statementInfo declares nothing this library can
                ...
                case WellKnownOids.QcPds:
                    ...
                    if(statement.HasData)
                    {
                        //A malformed QcPDS statementInfo declares nothing this library can read; the fact
                        //stays absent and the walk continues (EN 319 412-5 clause 4.1 NOTE).
                        try
                        {
                            //PdsLocations ::= SEQUENCE SIZE (1..MAX) OF PdsLocation { url IA5String, language PrintableString } (Annex B clause 4.3.4).
                            List<PdsLocation> declaredLocations = [];
                            AsnReader pdsLocations = statement.ReadSequence();
                            while(pdsLocations.HasData)
                            {
                                AsnReader pdsLocation = pdsLocations.ReadSequence();
                                string url = pdsLocation.ReadCharacterString(UniversalTagNumber.IA5String);
                                string language = pdsLocation.ReadCharacterString(UniversalTagNumber.PrintableString);
                                pdsLocation.ThrowIfNotEmpty();
                                declaredLocations.Add(new PdsLocation(url, language));
                            }

                            statement.ThrowIfNotEmpty();
                            qcPdsLocations.AddRange(declaredLocations);
                        }
                        catch(AsnContentException)
                        {
                ...
                case WellKnownOids.QcCcLegislation:
                    if(qcCcLegislationSeen)
                    {
                        break;
                    }

                    qcCcLegislationSeen = true;
                    if(statement.HasData)
                    {
                        ...
                        try
                        {
                            //QcCClegislation ::= SEQUENCE OF CountryName (Annex B clause 4.2.4).
                            List<string> declaredCountryCodes = [];
                            AsnReader ccLegislationCountries = statement.ReadSequence();
                            while(ccLegislationCountries.HasData)
                            {
                                declaredCountryCodes.Add(ccLegislationCountries.ReadCharacterString(UniversalTagNumber.PrintableString));
                            }

                            statement.ThrowIfNotEmpty();
                            qcCcLegislationCountryCodes.AddRange(declaredCountryCodes);
                        }
                        catch(AsnContentException)
                        {
                            //A malformed QcCClegislation statementInfo declares nothing this library can
                ...
                case WellKnownOids.QcIdentMethod:
                    if(qcIdentMethodSeen)
                    {
                        break;
                    }

                    qcIdentMethodSeen = true;
                    if(statement.HasData)
                    {
                        //A malformed QcIdentMethod statementInfo declares nothing this library can read;
                        //the fact stays absent and the walk continues (EN 319 412-5 clause 4.1 NOTE).
                        try
                        {
                            //QcIdentMethod ::= SEQUENCE SIZE (1) OF OBJECT IDENTIFIER (clause 4.3.5.1 /
                            //Annex B); an identifier outside the four known methods is omitted, mirroring
                            //QcType above.
                            List<EuIdentityVerificationMethod> declaredMethods = [];
                            AsnReader identMethods = statement.ReadSequence();
                            while(identMethods.HasData)
                            {
                                EuIdentityVerificationMethod? method = EuIdentityVerificationMethodMapping.FromOid(identMethods.ReadObjectIdentifier());
                                if(method is EuIdentityVerificationMethod knownMethod)
                                {
                                    declaredMethods.Add(knownMethod);
                                }
                            }

                            statement.ThrowIfNotEmpty();
                            qcIdentityVerificationMethods.AddRange(declaredMethods);
                        }
                        catch(AsnContentException)
                        {
                            //A malformed QcIdentMethod statementInfo declares nothing this library can read;
                ...
                case WellKnownOids.QcQscdLegislation:
                    if(qcQscdLegislationSeen)
                    {
                        break;
                    }

                    qcQscdLegislationSeen = true;
                    if(statement.HasData)
                    {
                        ...
                        try
                        {
                            //QcQSCDlegislation ::= SEQUENCE SIZE (1..MAX) OF CountryName (Annex B clause 4.2.5;
                            //QCS-4.1-04 gives this Annex B identifier precedence over the body's typo'd spelling).
                            List<string> declaredCountryCodes = [];
                            AsnReader qscdLegislationCountries = statement.ReadSequence();
                            while(qscdLegislationCountries.HasData)
                            {
                                declaredCountryCodes.Add(qscdLegislationCountries.ReadCharacterString(UniversalTagNumber.PrintableString));
                            }

                            statement.ThrowIfNotEmpty();
                            qcQscdLegislationCountryCodes.AddRange(declaredCountryCodes);
                        }
                        catch(AsnContentException)
                        {
                            //A malformed QcQSCDlegislation statementInfo declares nothing this library can
        ...
        return new QcStatementFacts(
            hasQcCompliance,
            hasQcSscdStatement,
            qcTypes,
            qcLimitValue,
            qcRetentionPeriodYears,
            qcPdsLocations,
    ...
    /// <param name="QcLimitValue">The <c>QcLimitValue</c> statement's transaction-value limit, or <see langword="null"/> when absent.</param>
    /// <param name="QcRetentionPeriodYears">The <c>QcRetentionPeriod</c> statement's retention period in years, or <see langword="null"/> when absent.</param>
    /// <param name="QcPdsLocations">The <c>QcPDS</c> statement's PKI Disclosure Statement locations, in certificate order.</param>
    /// <param name="QcCcLegislationCountryCodes">The <c>QcCClegislation</c> statement's country codes, in certificate order.</param>
    /// <param name="QcIdentityVerificationMethods">The known <c>QcIdentMethod</c> identification methods, in certificate order.</param>
    /// <param name="QcQscdLegislationCountryCodes">The <c>QcQSCDlegislation</c> statement's country codes, in certificate order.</param>
    private sealed record QcStatementFacts(
        ...
        QcMonetaryValue? QcLimitValue,
        BigInteger? QcRetentionPeriodYears,
        IReadOnlyList<PdsLocation> QcPdsLocations,
        IReadOnlyList<string> QcCcLegislationCountryCodes,
        IReadOnlyList<EuIdentityVerificationMethod> QcIdentityVerificationMethods,
        IReadOnlyList<string> QcQscdLegislationCountryCodes);
