...
public sealed class Nonce: SensitiveMemory, IEquatable<Nonce>
{
    ...
    /// The entropy source delegate. Must fill the entire span with cryptographically random bytes.
    /// Providers pass the platform CSPRNG (e.g. <c>RandomNumberGenerator.Fill</c>) or BouncyCastle's
    /// <c>SecureRandom</c>; a TPM/HSM delegate supplies hardware entropy.
    /// </param>
    ...
    /// <inheritdoc/>
    [EditorBrowsable(EditorBrowsableState.Never)]
    public override bool Equals([NotNullWhen(true)] object? obj) => obj switch { Nonce n => Equals(n), SensitiveMemory sm => base.Equals(sm), _ => false };
    ...
    public static bool operator ==(Nonce? left, Nonce? right) => left is null ? right is null : left.Equals(right);
