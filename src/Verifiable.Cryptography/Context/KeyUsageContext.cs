...
public readonly struct KeyUsageContext: IEquatable<KeyUsageContext>
{
    ...
    /// <summary>
    /// Signing of OpenID Federation 1.0 Entity Statements (both Entity
    /// Configurations and Subordinate Statements) per
    /// <see href="https://openid.net/specs/openid-federation-1_0.html#section-3">Federation §3</see>.
    /// </summary>
    /// <remarks>
    /// Distinct from <see cref="AccessTokenIssuance"/> and
    /// <see cref="IdTokenIssuance"/> because Federation signing keys are
    /// published in the entity's own <c>jwks</c> claim, have a different
    /// rotation cadence (typically aligned with chain expiry), and the
    /// algorithm set is constrained by Federation §3 (no symmetric, no
    /// <c>none</c>; same constraints as DPoP, but different signing
    /// surface).
    /// </remarks>
