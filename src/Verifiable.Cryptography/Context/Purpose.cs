...
public readonly struct Purpose: IEquatable<Purpose>
{
    ...
    /// <summary>
    /// Salt value mixed with input to a hash, key derivation, or
    /// digest-of-disclosure construction.
    /// </summary>
    /// <remarks>
    /// Used for password-based key derivation (PBKDF2, Argon2), SD-JWT and SD-CWT
    /// disclosure salts (RFC 9901 §4.2.2; draft-ietf-spice-sd-cwt §4.2), and similar
    /// constructions where a non-secret random value is bound to a digest output.
    /// Salts are not entropic primitives in the secrecy sense; their forensic value
    /// is in the binding between the salt and the hash output, and in distinguishing
    /// the construction's purpose from generic random <see cref="Data"/> or
    /// session-freshness <see cref="Nonce"/> values.
    /// </remarks>
...
public static class PurposeNames
{
    ...
    public static string GetName(int code) => code switch
    {
        ...
        var c when c == Purpose.Salt.Code => nameof(Purpose.Salt),
        _ => $"Custom ({code})"
    };
}