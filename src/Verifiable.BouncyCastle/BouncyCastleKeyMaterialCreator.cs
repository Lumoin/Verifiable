...
public static class BouncyCastleKeyMaterialCreator
{
    ...
    /// <summary>Creates a NIST P-384 ECDH exchange key pair (uncompressed public point).</summary>
    /// <remarks>
    /// Curve <c>secp384r1</c> per <see href="https://www.rfc-editor.org/rfc/rfc7518#section-6.2.1.2">RFC 7518 §6.2.1.2</see>;
    /// coordinate/field size is 48 bytes, so the uncompressed point is 97 bytes (<c>0x04 || X || Y</c>).
    /// </remarks>
    /// <param name="memoryPool">The memory pool for key data allocation.</param>
    /// <returns>The public and private key material.</returns>
    ...
    /// <summary>Creates a NIST P-521 ECDH exchange key pair (uncompressed public point).</summary>
    /// <remarks>
    /// Curve <c>secp521r1</c> per <see href="https://www.rfc-editor.org/rfc/rfc7518#section-6.2.1.3">RFC 7518 §6.2.1.3</see>;
    /// coordinate/field size is 66 bytes (521 bits round up to 66), so the uncompressed point is 133 bytes (<c>0x04 || X || Y</c>).
    /// </remarks>
    /// <param name="memoryPool">The memory pool for key data allocation.</param>
    /// <returns>The public and private key material.</returns>
    ...
    /// <summary>Creates a Brainpool P-256r1 key pair for signing and verification.</summary>
    /// <remarks>
    /// Curve parameters per <see href="https://www.rfc-editor.org/rfc/rfc5639">RFC 5639</see>.
    /// Brainpool curves resolve through the unified <c>ECNamedCurveTable</c> lookup, not
    /// <c>SecNamedCurves</c>, so this path uses an explicit Brainpool-aware factory.
    /// </remarks>
    /// <param name="memoryPool">The memory pool for key data allocation.</param>
    /// <returns>The public and private key material.</returns>
    ...
    private static PublicPrivateKeyMaterial<PublicKeyMemory, PrivateKeyMemory> CreateBrainpoolEcKeys(
        ...
        MemoryPool<byte> memoryPool)
    {
        ...
        //big-endian padding) is identical to the SEC ECDSA path.
    ...
    //ECDH exchange counterpart of CreateBrainpoolEcKeys: identical key generation,
    //but the public point is encoded UNCOMPRESSED (0x04 || X || Y) because the ECDH
    //agreement and the wallet_metadata JWK both consume the uncompressed form.
    private static PublicPrivateKeyMaterial<PublicKeyMemory, PrivateKeyMemory> CreateBrainpoolExchangeKeys(
        string brainpoolCurveName,
        Tag publicKeyTag,
        Tag privateKeyTag,
        MemoryPool<byte> memoryPool)
    {
        ...
        byte[] uncompressedPublicKey = publicKeyParam.Q.GetEncoded(compressed: false);
        byte[] privateKeyBytes = privateKeyParam.D.ToByteArrayUnsigned();
    ...
    //ECDH exchange counterpart of CreateEcKeys for the NIST SEC curves: identical key
    //generation, but the public point is encoded UNCOMPRESSED (0x04 || X || Y) because the
    //ECDH agreement and the wallet_metadata JWK both consume the uncompressed form.
    private static PublicPrivateKeyMaterial<PublicKeyMemory, PrivateKeyMemory> CreateNistExchangeKeys(
        string secCurveName,
        Tag publicKeyTag,
        Tag privateKeyTag,
        MemoryPool<byte> memoryPool)
