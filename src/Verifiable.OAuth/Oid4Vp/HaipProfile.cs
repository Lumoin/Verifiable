...
public static class HaipProfile
{
    ...
    /// <param name="state">
    /// The opaque CSRF state value per
    /// <see href="https://www.rfc-editor.org/rfc/rfc6749#section-4.1.1">RFC 6749 §4.1.1</see>
    /// and <see href="https://www.rfc-editor.org/rfc/rfc9700#section-4.7">RFC 9700 §4.7</see>.
    /// In the OID4VP server-side verifier flow this equals the per-flow
    /// <c>request_uri</c> token.
    /// </param>
        ...
        string? responseMode = null)
    {
        ...
        return new AuthorizationRequestObject
        {
            ...
            //HAIP 1.0 §5.1 mandates direct_post.jwt (encrypted); deployments
            //may override to direct_post for plaintext responses per
            //OID4VP 1.0 §8.2.
            ResponseMode = responseMode ?? WellKnownResponseModes.DirectPostJwt,
    ...
    /// stamps timing claims from <paramref name="now"/> and
    /// <paramref name="requestObjectLifetime"/>, signs with
    /// <paramref name="signingKey"/>, and returns both the
    /// <see cref="JarSigned"/> input for the PDA transition and the compact JWS
    /// string for serving over HTTP. Usable from both the client-side verifier
    /// flow and the server-side verifier flow without coupling to a specific
    /// state type.
    /// </para>
    /// <para>
    ...
    /// </para>
    /// </remarks>
    /// <param name="now">The current UTC instant from the active TimeProvider.</param>
    /// <param name="requestObjectLifetime">
    /// The window <c>exp - nbf</c> applied to the JAR.
    /// </param>
    /// <param name="state">
    /// The opaque CSRF state value, equal to the per-flow <c>request_uri</c>
    /// token in the OID4VP server-side verifier flow.
    /// </param>
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        if(requestObjectLifetime <= TimeSpan.Zero)
        {
            throw new ArgumentOutOfRangeException(
                ...
                "Request-object lifetime must be positive. " +
                "FAPI 2.0 §5.2.2 Clause 13 requires the JAR to carry a positive exp window.");
        }
    ...
    /// <summary>
    /// Verifier-side: composes an Authorization Request Object and emits it
    /// as an unsigned compact JAR (<c>alg=none</c> per RFC 7515 §6.1). Used
    /// for OID4VP 1.0 §5.9.3 <c>redirect_uri</c> client-identifier prefix
    /// flows, where the spec mandates the request MUST NOT be signed; trust
    /// rests on the wallet POSTing the response back to the asserted
    /// redirect URI rather than on a cryptographic signature.
    /// </summary>
    /// <remarks>
    /// <para>
    /// Mirrors the parameter set of <see cref="BuildJarAsync"/> except no
    /// signing key is required (the <c>alg=none</c> JWS has an empty
    /// signature segment). Wallets that accept the result MUST gate
    /// acceptance on the JAR's <c>client_id</c> using the
    /// <c>redirect_uri:</c> prefix and the prefix value equalling the
    /// <c>response_uri</c> — see
    /// <see cref="JarExtensions.ParseUnsignedJarAsync"/> which performs no
    /// such check on its own.
    /// </para>
    /// </remarks>
    public static ValueTask<string> BuildUnsignedJarAsync(
        DateTimeOffset now,
        TimeSpan requestObjectLifetime,
        string state,
        TransactionNonce nonce,
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        return ValueTask.FromResult(compactJar);
    }
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        return await EncryptResponseAsync(
            encryptionPublicKey,
            selectedEnc,
            vpTokenPayloadBytes,
            headerSerializer,
    ...
    /// encryption key comes from the resolved trust chain's effective
    /// metadata. Caller is responsible for resolving the key.
    /// </summary>
    /// <param name="encryptionPublicKey">
    /// The Verifier's pre-resolved P-256 exchange key. Caller owns the
    /// key; this method does not dispose it.
    /// </param>
    /// <param name="selectedEnc">
    /// The JWE encryption algorithm (<c>A128GCM</c> / <c>A256GCM</c>).
