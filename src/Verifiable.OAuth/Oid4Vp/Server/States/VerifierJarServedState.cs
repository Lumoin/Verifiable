...
public sealed record VerifierJarServedState: FlowState
{
    ...
    /// <summary>
    /// The DCQL credential query identifiers the Verifier set when
    /// constructing the JAR — one per credential the verifier requested.
    /// Threaded forward so
    /// <see cref="VerifierResponseReceivedState.NextAction"/> can pass them
    /// into the <see cref="DecryptResponseAction"/> for response-side lookup
    /// against the JSON-encoded <c>vp_token</c> (which is keyed by these
    /// identifiers per OID4VP 1.0 §8.1).
    /// </summary>
    ...
    /// per OID4VP 1.0 §8.4. Threaded forward so the
    /// <see cref="DecryptResponseAction"/> handler can recompute the expected
    /// hashes for validation against the Wallet's KB-JWT
