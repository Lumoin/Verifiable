...
{
    /// <summary>
    /// The standard OIDC Core contributor set — subject identifier,
    /// §5.4 scope-driven claims (profile, email, address, phone), plus
    /// authentication context (acr/amr/auth_time) and cnf binding
    /// emission. Returns a fresh mutable list per call so the
    ...
    /// resolver-emitted <c>sub</c> lands in the payload before any
    /// later rule could be ordered around it.
    /// </remarks>
    public static List<ClaimDelegate<ClaimContributionTarget>> StandardRules() =>
    [
        new ClaimDelegate<ClaimContributionTarget>(
            SubjectIdentifierContributor.GenerateSubjectClaim,
