...
/// <see cref="AuthorizationServer.ClaimContributors"/> for each applicable
...
/// <remarks>
/// <para>
/// The returned contribution is merged into the producer's payload by the
/// endpoint handler in list order; later entries with the same claim name
/// overwrite earlier ones. Contributors that read from external sources
/// (databases, downstream services, hardware attestations) do their I/O
/// here. Contributors that compute claims from in-context state alone
/// return the claims directly without I/O.
/// </para>
/// <para>
/// Return <see cref="ClaimContribution.Empty"/> when there is nothing to
/// contribute for a given producer in a given request. A contributor may
/// add claims at any path supported by <see cref="JwtPayload"/>; nested
/// structures (e.g., the <c>verified_claims</c> object) are supplied as
/// the <see cref="ClaimEntry.Value"/> of a single entry — the JWT payload
/// serializer the application wires into
/// <see cref="AuthorizationServerCodecs.JwtPayloadSerializer"/> handles
/// the JSON nesting transparently.
/// </para>
/// <para>
/// The <paramref name="producer"/> argument allows contributors that target
/// more than one token type to branch their output without storing per-call
/// state.
/// </para>
/// </remarks>
/// <param name="context">The per-request issuance context.</param>
...
public delegate ValueTask<ClaimContribution> ClaimContributorBuildDelegate(
    IssuanceContext context,
    TokenProducer producer,
    CancellationToken cancellationToken);
