...
/// fetches it on demand via <see cref="AuthorizationServerIntegration.ResolveClientMetadataAsync"/>.
...
public sealed record ClientRecord
{
    ...
    /// </summary>
    public required ImmutableHashSet<CapabilityIdentifier> AllowedCapabilities { get; init; }
    ...
    /// <see cref="AuthorizationServerCryptography.SelectSigningKey"/> (or its default)
    /// when signing, and publishes them via
    /// <see cref="AuthorizationServerCryptography.BuildJwksDocumentAsync"/> when
    /// serving JWKS.
    /// </para>
    /// </remarks>
    ...
    /// </summary>
    public bool IsCapabilityAllowed(CapabilityIdentifier capability) =>
