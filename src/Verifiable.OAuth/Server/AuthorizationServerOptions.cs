...
public sealed class AuthorizationServerOptions
{
    ...
    /// <summary>
    /// The composable configuration: endpoint builders, token producers, and
    /// claim contributors. Mirrors the snapshot-swap pattern on
    /// <see cref="AuthorizationServer.Configuration"/> so that mutating these
    /// surfaces across configuration steps and reading them at validation
    /// time both go through the same immutable type.
    /// </summary>
    /// <remarks>
    /// <para>
    /// The three flat list properties that previously lived directly on
    /// <see cref="AuthorizationServerOptions"/>
    /// (<c>EndpointBuilders</c>, <c>TokenProducers</c>, <c>ClaimContributors</c>)
    /// have been folded into this single property. Construction is unchanged
    /// in shape but more uniform: build a <see cref="ServerConfiguration"/>,
    ...
    /// </para>
    /// <code>
    /// options.Configuration = new ServerConfiguration
    /// {
    ...
    /// </code>
    /// <para>
    /// Or use the non-destructive update helpers on
    /// <see cref="ServerConfiguration"/>:
    /// </para>
    /// <code>
    /// options.Configuration = ServerConfiguration.Empty
    ///     .WithEndpointBuilders(new EndpointBuilderSet([AuthCodeEndpoints.Builder]));
    /// </code>
    /// <para>
    /// Library-shipped builders: <c>AuthCodeEndpoints.Builder</c>,
    /// <c>Oid4VpEndpoints.Builder</c>, <c>MetadataEndpoints.Builder</c>.
    /// Library-shipped token producers:
    /// <see cref="TokenProducer.Rfc9068AccessToken"/>,
    /// <see cref="TokenProducer.Oidc10IdToken"/>.
    /// </para>
    /// </remarks>
    public ServerConfiguration Configuration { get; set; } = ServerConfiguration.Empty;
    ...
    /// matchers do not consume tokens run without this delegate set.
    /// </para>
    /// <para>
    /// Wire <see cref="Verifiable.JCose.JoseTokenClassifier.ClassifyAsync"/>
    /// as the implementation, supplying the application's Base64Url decoder,
    /// JOSE header deserializer, and memory pool. Deployments that issue
    /// non-JOSE tokens supply their own classifier.
    /// </para>
    /// </remarks>
    ...
    /// <remarks>
    /// <para>
    /// Wire from a provider-side implementation registered on
    /// <see cref="CryptographicKeyFactory"/>:
    /// </para>
    /// <code>
    /// options.ComputeDigest = MicrosoftEntropyFunctions.ComputeDigest;
    /// </code>
    /// <para>
    ...
    /// <see cref="ClassifyTokenAsync"/> is optional — required only when
    /// token-aware matchers (introspection, revocation, userinfo, OID4VCI proof)
    /// are registered.
    /// </remarks>
    ...
    public void Validate()
    {
        ...
        {
            missing.Add($"{nameof(Configuration)}.{nameof(ServerConfiguration.EndpointBuilders)}");
        }
