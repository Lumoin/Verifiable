...
/// and a <see cref="ExchangeContext"/>. The library does the rest: tenant
...
public sealed class AuthorizationServer: IDisposable
{
    ...
    ///   <see cref="ExchangeContextIncomingRequestExtensions.SetIncomingRequest"/>
    ...
    /// <see cref="ExchangeContextServerExtensions.TransactionNonce"/> and
    /// <see cref="ExchangeContextServerExtensions.PreparedQuery"/>, JAR-fetch's
    /// <see cref="ExchangeContextServerExtensions.CorrelationKey"/>) and is the
        ...
        CancellationToken cancellationToken)
    {
        ...
        else
        {
            ...
            else
            {
                ...
                //typed extensions in PolicyExchangeContextExtensions.
        ...
        CancellationToken cancellationToken)
    {
        ...
        if(endpoint.StartsNewFlow)
        {
            ...
            //indexes time-locality for free. OI-003 chunk B routes identifier
            //generation through Integration.GenerateIdentifierAsync so audit
            //and replay infrastructure can intercept; the library default
            //preserves the v7 GUID shape and threads TimeProvider through.
            flowId = await Integration.GenerateIdentifierAsync!(
                WellKnownIdentifierPurposes.OAuthFlowId, context, cancellationToken)
                .ConfigureAwait(false);
