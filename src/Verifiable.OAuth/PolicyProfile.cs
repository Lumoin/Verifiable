...
/// <remarks>
/// <para>
/// Follows the dynamic-value-type pattern shared with
...
public readonly struct PolicyProfile: IEquatable<PolicyProfile>
{
    ...
    /// <summary>
    /// OID4VP verifier (presentation) profile —
    /// <see href="https://openid.net/specs/openid-4-verifiable-presentations-1_0.html">OpenID4VP 1.0</see>.
    /// For registrations that act as an OID4VP <em>Verifier</em>: they issue the
    /// signed Authorization Request (JAR) and verify the returned
    /// <c>vp_token</c>, but expose no token/authorize endpoint. Distinct from
    /// <see cref="Fapi20"/>/<see cref="Haip10"/>, which carry the
    /// token-endpoint security axes (PKCE, token lifetimes, scope, JTI replay)
    /// that have no meaning for a presentation flow. This profile sets only the
    /// axes the verification pipeline actually consumes — KB-JWT <c>iat</c>
    /// clock-skew and freshness — leaving credential replay/freshness to the
    /// mandatory <c>nonce</c>/<c>aud</c> binding per
    /// <see href="https://www.rfc-editor.org/rfc/rfc9901">RFC 9901</see>.
    /// </summary>
