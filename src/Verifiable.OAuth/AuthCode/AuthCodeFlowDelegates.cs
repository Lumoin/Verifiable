...
/// </para>
/// <list type="bullet">
///   <item><description>After PAR completes — state is <see cref="States.ParCompletedState"/>, keyed by <c>FlowId</c> and <c>Par.RequestUri</c>.</description></item>
///   <item><description>After the authorization code arrives — state is <see cref="States.AuthorizationCodeReceivedState"/>.</description></item>
///   <item><description>After tokens are received — state is <see cref="States.TokenReceivedState"/>.</description></item>
/// </list>
/// <para>
...
/// <param name="context">
/// The threaded per-operation <see cref="ExchangeContext"/>. Lets the
/// implementation scope persistence by tenant (read tenant identity off the
/// context) so flow state from one tenant cannot collide with another's.
/// </param>
...
/// <param name="context">
/// The threaded per-operation <see cref="ExchangeContext"/>. Lets the
/// implementation select a per-tenant <c>HttpClient</c> or attach tenant-scoped
/// transport policy without capturing it.
/// </param>
/// <param name="cancellationToken">Cancellation token.</param>
/// <returns>
