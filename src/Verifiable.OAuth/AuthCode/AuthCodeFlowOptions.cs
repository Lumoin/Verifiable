...
public sealed class AuthCodeFlowOptions
{
    ...
    /// <summary>
    /// The redirect URI registered for this client. Used in the PAR request body and
    ...
    /// Supply <see cref="OAuthCallbackValidators.Haip10"/> for HAIP 1.0 / FAPI 2.0,
    /// or <see cref="OAuthCallbackValidators.Rfc6749WithPkce"/> for plain RFC 6749.
    /// </summary>
    public ValidateCallbackDelegate ValidateCallback { get; private init; } = null!;
    ...
    /// <summary>
    /// Provides the current UTC time. Defaults to <see cref="TimeProvider.System"/>.
    /// Inject a <see cref="Microsoft.Extensions.Time.Testing.FakeTimeProvider"/> in tests.
    /// </summary>
    public TimeProvider TimeProvider { get; private init; } = TimeProvider.System;


    /// <summary>
    /// Creates a validated <see cref="AuthCodeFlowOptions"/> instance.
    /// </summary>
    ...
    /// <param name="saveStateAsync">Persists flow state to durable storage.</param>
    /// <param name="loadStateAsync">Loads flow state by flow identifier.</param>
    /// <param name="loadStateByRequestUriAsync">Loads flow state by PAR request_uri.</param>
    /// <param name="sendFormPostAsync">Sends form-encoded HTTP POST requests.</param>
    /// <param name="parseParResponseAsync">
    /// Parses PAR endpoint responses. Pass <see cref="OAuthResponseParsers.ParseParResponse"/>
    /// for the default span-based implementation.
    /// </param>
    /// <param name="parseTokenResponseAsync">
    /// Parses token endpoint responses. Pass <see cref="OAuthResponseParsers.ParseTokenResponse"/>
    /// for the default span-based implementation.
    /// </param>
    /// <param name="validateCallback">
    /// Validates callback fields. Pass <see cref="OAuthCallbackValidators.Haip10"/> for
    /// HAIP 1.0 / FAPI 2.0 compliance.
    /// </param>
    /// <param name="base64UrlEncoder">Base64url encoder without padding.</param>
    /// <param name="timeProvider">
    /// UTC time source. Pass <see cref="TimeProvider.System"/> for production.
    /// </param>
        ...
        TimeProvider? timeProvider = null)
    {
        ...
        ArgumentNullException.ThrowIfNull(validateCallback);
