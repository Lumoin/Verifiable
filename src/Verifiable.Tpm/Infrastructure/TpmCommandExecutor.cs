...
public static class TpmCommandExecutor
{
    ...
    /// - TPM 2.0 Library Part 2, Section 10.12.3, Table 157.
    /// </summary>
    ...
    /// explicit Name. Per TPM 2.0 Part 1, clause 15.7, equation 15, cpHash is computed over entity Names, not handle values.
        ...
        where TResponse : ITpmWireType
    {
        ...
        //(TPM 2.0 Part 1, Section 18.1: the encrypt/decrypt attribute may be set in at most one session each).
        ...
        try
        {
            if(hasSessions)
            {
                ...
                //Encrypt the data portion of the first command parameter (Part 1 §18) before cpHash is computed
            ...
            //itself authorizes a command handle (TPM 2.0 Library Part 1, clause 16.6.3.4): the fold's own
            ...
            try
            {
                ...
                try
                {
                    if(hasResponseSessions)
                    {
                        ...
                        //Part 1 §18.1), so verification does not need the typed parse; deferring both the parse
                        ...
                        //Decrypt the data portion of the first response parameter (Part 1 §18), now that the
    ...
    /// attribute. Per TPM 2.0 Part 1, Section 18.1 each attribute may be set in at most one session per command.
    /// </exception>
    ...
    /// order (TPM 2.0 Part 1, clause 15.7, equation 15). A permanent, PCR, or session handle's Name is its 4-byte handle
        ...
        int outHandleCount)
    {
        ...
        //the already-validated responseSize before anything is read from them (Part 1, §15.10 response framing).
        ...
        if(responseHasSessions)
        {
            ...
            //out-of-range exception past the fail-closed TpmResult contract (Part 1, §15.10 parameter/auth split).
    ...
    /// <summary>
    /// Asynchronously computes cpHash per TPM 2.0 Part 1, Section 15.7.
    /// </summary>
    ...
    /// <summary>
    /// Asynchronously computes rpHash per TPM 2.0 Part 1, Section 15.8.
    /// </summary>
