...
/// </list>
/// <para>
/// <b>HMAC computation (spec Part 1, clause 16.6.5):</b>
/// </para>
/// <code>
...
/// session declared <see cref="MarkBoundToAuthorizedEntity"/> keys on sessionKey alone (clause 16.6.10,
/// equation 22). Parameter encryption keys on sessionKey || authValue whenever the session authorizes an
/// entity, whatever the session is bound to (clause 18.1: "The binding of the session is ignored").
/// </para>
/// <para>
...
/// </para>
/// <para>
/// See TPM 2.0 Library Part 1, clause 16.5 - Sessions.
/// </para>
/// </remarks>
public sealed class TpmSession: TpmSessionBase, IDisposable
{
    ...
    /// <summary>
    /// The session key: empty for an unbound/unsalted session, the KDFa-derived key for a bound or salted
    /// one (TPM 2.0 Library Part 1, clause 16.6.5). Assigned once, by the constructor.
    /// </summary>
    private Tpm2bAuth SessionKey { get; }
    ...
    /// clause 16.6.10, equation 22). It never affects the parameter-encryption key, which folds the authValue
    /// regardless (clause 18.1: "The binding of the session is ignored").
    /// </summary>
    ...
    /// <param name="rng">The entropy this session mints its rolled caller nonces from.</param>
    ...
    public TpmSession(
        ...
        : this(sessionHandle, nonceTPM, sessionAlg, Tpm2bAuth.CreateEmpty(pool), rng, pool, symmetric)
    {
    }
    ...
    /// clause 16.6.4), folded into the session key by the bound-session KDFa (clause 16.6.10, equation 20).
    ...
    /// parameter-encryption key: clause 18.1 keys the cipher on <c>sessionKey ∥ authValue</c> whenever the
    ...
    /// session-key KDFa (TPM 2.0 Library Part 1, clause 16.6.12, equation 25) — never reversed.
    /// </param>
    ...
    /// Library Part 1, clause 16.6.10, equation 22). It is equivalent to calling
    /// <see cref="MarkBoundToAuthorizedEntity"/> on the returned session, and it leaves the
    /// parameter-encryption key alone (clause 18.1). Leave it <see langword="false"/> for a session bound to
    ...
    /// <remarks>
    /// <para>
    /// Per TPM 2.0 Library Part 1, clause 16.6.10 (equation 20) and clause 16.6.12 (equation 25) the session
    ...
    /// <see cref="TpmSession(TpmHandle, Tpm2bNonce, TpmAlgIdConstants, FillEntropyDelegate, BaseMemoryPool, TpmtSymDef?)"/>
    /// constructor instead of calling this factory with an empty <paramref name="bindAuthValue"/> and no
    /// <paramref name="salt"/>.
    /// </para>
    /// <para>
    /// <strong>Manual disposal, not <see langword="using"/> declarations.</strong> <c>keyOwner</c> is
    /// declared <see langword="null"/> and assigned only in the concatenated-key branch (a
    /// <see langword="using"/> declaration accepts only a single assignment at its own declaration), and
    /// both <c>keyOwner</c> and <c>derived</c> are zeroed before disposal since they carry key-derivation
    /// secret material — a <see langword="using"/> declaration only ever calls <c>Dispose()</c>, never a
    /// zeroing step first.
    /// </para>
    /// </remarks>
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            //KDFa key = bindAuthValue || salt (Part 1, clause 16.6.12 equation 25), concatenated into a pooled
        ...
        var session = new TpmSession(sessionHandle, nonceTPM, sessionAlg, sessionKey, rng, pool, symmetric);
    ...
    /// clause 16.6.10 has the TPM omit the authValue of the bound entity from the HMAC key when the
    ...
    /// </para>
    /// <para>
    /// Parameter encryption is unaffected: clause 18.1 keys the cipher on <c>sessionKey || authValue</c>
    ...
    /// (clause 18.1's malleability property), so a command decrypts to garbage the TPM then acts on and signs.
    /// </para>
    /// </remarks>
    ...
    /// authValue term of an authorization computation: TPM 2.0 Library Part 1, clause 16.6.4.3 ("Trailing octets
    /// of zero are to be removed from any string before it is used as an authValue") and clause 16.6.5's
    ...
    /// non-empty, immediately after nonceOlder and before <see cref="TpmSessionBase.SessionAttributes"/> — the caller is
    ...
    /// </para>
    /// <para>
    /// This adoption is what fulfils the per-use nonce roll TPM 2.0 Library Part 1, clause 16.6.3.1 requires:
    ...
    /// Part 3, clause 11.1.1: sessionKey derivation, and with it this nonce protocol, does not vary by
    /// sessionType).
    /// </para>
    /// <para>
    ...
    public override void RollNonceCaller(BaseMemoryPool pool)
    {
        ...
        Tpm2bNonce freshNonceCaller = Tpm2bNonce.CreateRandom(DigestSize, Rng, pool);
        nonceCaller.Dispose();
        ...
        CancellationToken cancellationToken)
    {
        ...
        //Command direction (Part 1, clause 18.2): nonceNewer = nonceCaller, nonceOlder = nonceTPM.
        ...
        CancellationToken cancellationToken)
    {
        ...
        //Response direction (Part 1, clause 18.2): nonceNewer = nonceTPM (the value adopted in VerifyAndUpdateAsync),
    ...
    /// </para>
    /// <para>
    /// <strong>Manual disposal, not a <see langword="using"/> declaration.</strong> <c>sessionValueOwner</c>
    /// is bound by a tuple deconstruction from <see cref="BuildSessionValue"/> (a <see langword="using"/>
    /// declaration accepts only a single simple declaration, never a deconstruction target), and it is
    /// zeroed before disposal since it carries key material — a <see langword="using"/> declaration only
    /// ever calls <c>Dispose()</c>.
    /// </para>
    /// </remarks>
    ...
    /// <summary>
    /// Builds <c>sessionValue = sessionKey || authValue</c> (TPM 2.0 Library Part 1, clause 16.6.5 equation 21
    /// for the authorization HMAC key, clause 18.1 for the parameter-encryption key), or <c>sessionKey</c>
    ...
    /// session bound to the entity it authorizes (clause 16.6.10, equation 22), while parameter encryption
    /// folds it regardless of the binding (clause 18.1).
    /// </param>
    /// <returns>
    ...
    /// <remarks>
    /// <strong>Manual disposal, not a <see langword="using"/> declaration.</strong> <c>keyOwner</c> is bound
    /// by a tuple deconstruction from <see cref="BuildSessionValue"/> and zeroed before disposal since it
    /// carries key material, so it is disposed in its own <see langword="finally"/> rather than through a
        ...
        CancellationToken cancellationToken)
    {
        ...
        //16.6.5 equation 21). A session bound to the entity it authorizes drops the authValue term, because the
