...
/// <summary>
/// Session-based parameter encryption primitives (TPM 2.0 Library Part 1, Section 18).
/// </summary>
/// <remarks>
/// <para>
...
/// of the parameter unprotected and do not change the parameter length (Part 1, Section 18.1).
/// </para>
/// <para>
/// The caller assembles <c>sessionValue</c> (Part 1, Section 18.1: <c>sessionKey</c>, or
...
public static class TpmParameterEncryption
{
    ...
    /// </param>
    /// <param name="contextU">
    /// The first context field: <c>nonceNewer</c> (Part 1, Section 18.2). For a command this is the caller
    ...
    /// TPM 2.0 Library Part 1, Section 18.3, equation (32).
    /// </summary>
    /// <param name="hashAlgorithm">The hash algorithm associated with the session (drives the KDF).</param>
    /// <param name="keyBits">The AES key size in bits (128, 192, or 256), from the session's TPMT_SYM_DEF.</param>
    /// <param name="key">The <c>sessionValue</c> used as the KDF key (Part 1, Section 18.1).</param>
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            //Most-significant octets are the key; the remaining block-size octets are the IV (Part 1 §18.3).
    ...
    /// register so it handles arbitrary data lengths with no padding (Part 1, Section 18.1: the encrypted and
