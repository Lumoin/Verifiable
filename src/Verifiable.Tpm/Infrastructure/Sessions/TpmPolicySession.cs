...
/// (no session key, and the entity authValue is not folded in), so per TPM 2.0 Library Part 1, Section 16.6.16 the
...
public sealed class TpmPolicySession: TpmSessionBase, IDisposable
{
    ...
    private TpmPolicySession(TpmHandle sessionHandle, TpmAlgIdConstants sessionAlg, BaseMemoryPool pool, Tpm2bAuth? password)
    {
    ...
    public static TpmPolicySession ForSession(uint sessionHandle, TpmAlgIdConstants sessionAlg, BaseMemoryPool pool)
    {
        ...
        return new TpmPolicySession(new TpmHandle(sessionHandle), sessionAlg, pool, password: null);
    }
    ...
    /// </summary>
    /// <remarks>
    /// TPM 2.0 Library Part 3, Section 23.18 and Part 1, Section 16.6.16 ("the password takes precedence and
    ...
    public static TpmPolicySession ForSessionWithPassword(uint sessionHandle, TpmAlgIdConstants sessionAlg, ReadOnlySpan<byte> password, BaseMemoryPool pool)
    {
        ArgumentNullException.ThrowIfNull(pool);

        return new TpmPolicySession(new TpmHandle(sessionHandle), sessionAlg, pool, Tpm2bAuth.Create(password, pool));
    }
    ...
    /// <see cref="VerifyAndUpdateAsync"/> has nothing to check (TPM 2.0 Library Part 1, Section 16.6.16: "if hmac
