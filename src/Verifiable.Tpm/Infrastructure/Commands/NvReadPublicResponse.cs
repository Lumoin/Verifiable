...
/// <remarks>
/// <para>
/// Response structure (TPM 2.0 Library Part 3, clause 31.6, Table 252):
/// </para>
/// <list type="bullet">
///   <item><description>nvPublic (TPM2B_NV_PUBLIC) - the public area of the NV Index: a UINT16
///   size prefix wrapping a <see cref="TpmsNvPublic"/> (TPM 2.0 Library Part 2, clause 13.6, Table
///   251). There is no dedicated wrapper type for TPM2B_NV_PUBLIC; the size prefix is read here
...
///   H_nameAlg(TPMS_NV_PUBLIC)</c> (TPM 2.0 Library Part 1, clause 13, Table 9). The digest covers the
...
/// (TPM 2.0 Library Part 3, clause 5.4's read/write-lock gates apply only to commands that access the
...
public sealed class NvReadPublicResponse: IDisposable, ITpmWireType
{
    ...
    public static NvReadPublicResponse Parse(ref TpmReader reader, BaseMemoryPool pool)
    {
        ...
        using(IMemoryOwner<byte> rawNvPublic = pool.Rent(nvPublicSize))
        {
