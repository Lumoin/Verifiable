...
public sealed class NvDefineSpaceInput: ITpmCommandInput, IDisposable
{
    ...
    /// 2.0 Library Part 3, clause 31.3, Table 245), so a decrypt-attributed session encrypts the new Index's
    /// authorization value on the bus before it is ever committed (Part 3, clause 5.7's first-sized-parameter
    /// rule; Part 1, clause 20). The (non-sensitive) public-area template is never the encryption target.
    /// </remarks>
