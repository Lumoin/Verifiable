...
/// parameter records that eligibility); the in-house simulator admits the same TPM_ST_SESSIONS frame through
/// the shared no-authorization session admission (TPM 2.0 Library Part 3, clause 5.5), an audit-only companion
/// succeeding and a decrypt companion protecting <c>context</c>, this digest-only form's first command
/// parameter.
/// </para>
/// <para>
...
public sealed class VerifyDigestSignatureInput: ITpmCommandInput, IDisposable
{
    ...
    /// <inheritdoc/>
    /// <remarks>
    /// <c>keyHandle</c> carries Auth Index None (TPM 2.0 Library Part 3, clause 20.4, Table 120) — the first
