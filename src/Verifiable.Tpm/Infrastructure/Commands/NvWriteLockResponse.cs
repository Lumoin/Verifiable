...
/// <summary>
/// Response for TPM2_NV_WriteLock. This command has no response handles and no response parameters, so the
...
/// Specification</see>, Part 3, Section 31.11 (Table 262).
/// </remarks>
public sealed class NvWriteLockResponse: ITpmWireType
{
    /// <summary>
    /// The shared instance returned for a successful, parameterless response.
    /// </summary>
    public static NvWriteLockResponse Instance { get; } = new();

    /// <summary>
    /// Prevents external construction: the parameterless response resolves to <see cref="Instance"/>.
    /// </summary>
    private NvWriteLockResponse()
    {
    }
}
