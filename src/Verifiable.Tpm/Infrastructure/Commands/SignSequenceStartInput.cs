...
/// input; the in-house simulator admits a TPM_ST_SESSIONS frame carrying an audit-only or a decrypt companion
/// over the same no-authorization session admission every other companion-bearing command shares (TPM 2.0
/// Library Part 3, clause 5.5), the decrypt claim protecting <c>auth</c>, matching
/// <see cref="VerifyDigestSignatureInput"/>'s posture for its own no-authorization handle.
/// </para>
/// <para>
...
public sealed class SignSequenceStartInput: ITpmCommandInput, IDisposable
{
    ...
    /// <inheritdoc/>
    /// <remarks>
    /// <c>keyHandle</c> carries Auth Index None (TPM 2.0 Library Part 3, clause 17.5, Table 87) — the first
