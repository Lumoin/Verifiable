...
/// Specification</see>, Part 3, clause 31.6.1): the public area of an NV Index is not privacy-sensitive, so the
...
/// WRITELOCKED - none of those gates apply, because the command never touches the data area (Part 3, clause
...
/// 2.0 Library Part 1, clause 13, Table 9), the digest covering the whole marshaled public area - whose own
...
/// clause 15.7, equation 15) - a caller cannot safely recompute it blind from locally cached attributes, since a
...
public static class TpmDeviceExtensions
{
    extension(TpmDevice device)
    {
        ...
        /// <remarks>
        /// <para>
        /// No authorization is presented or checked (<c>Auth Index: None</c>, TPM 2.0 Library Part 3, clause
        ...
        /// are the generic handle-validation ladder (Part 3, clause 5.4): <c>TPM_RC_VALUE</c> when
        ...
        /// </para>
        /// <para>
        /// The returned Name (<c>nameAlg ‖ H_nameAlg(TPMS_NV_PUBLIC)</c>, TPM 2.0 Library Part 1, clause
        /// 13, Table 9; Part 2, clause 13.6, Table 251) hashes the whole public area including
        ...
        TpmDevice device, uint nvIndex, CancellationToken cancellationToken)
    {
        ...
        //Part 3, clause 31.6), so the executor is given no sessions and frames TPM_ST_NO_SESSIONS, exactly as
