...
///   HMAC session (TPM 2.0 Library Part 1, Section 16.6.9's Empty Buffer session key, equation 19) and set the
///   Index's own authorization value as that session's authValue, so it becomes the per-command HMAC key's
///   authValue term (Part 1, Section 16.6.5, equation 17: <c>sessionValue = sessionKey || authValue</c>) rather
///   than plaintext on the bus. Part 1, Section 34.2.8.3's outright prohibition on binding a session to the
...
///   session BOUND to <c>TPM_RH_OWNER</c> (Part 1, Section 16.6.10, equation 20): the owner authorization value
...
///   folds the Counter Index's own real, current Name (Part 1, Section 13, Table 9) into the cpHash of every
...
///   Part 1, Section 16.8.1 enumerates the password use, the equation-17 HMAC authValue term, and the authValue
...
///   <c>TPM_RC_BAD_AUTH</c> with the counter untouched (Part 2, Section 13.4, Table 249, bit 25); in Lockout mode
///   either arm is refused with <c>TPM_RC_LOCKOUT</c> (Part 1, Section 16.8.3). Flipping the channel changes what
...
/// deleted (the "phantom counter" mechanism, TPM 2.0 Library Part 1, Section 34.2.6.3 NOTE 2/NOTE 6), so a
...
public static class TpmDeviceExtensions
{
    ...
    extension(TpmDevice device)
    {
        ...
        /// </para>
        /// <para>
        /// The session is BOUND to <c>TPM_RH_OWNER</c> (Part 1, Section 16.6.10, equation 20), so
        ...
        /// the command's first sized parameter, which Part 1, Section 18.1 makes eligible for session encryption
        ...
        /// so <c>sessionKey = KDFa(ownerAuth ‖ salt, …)</c> (Part 1, Section 16.6.12, equation 25) and the
        ...
        /// rule) and the read-back through <c>TPMA_NV_AUTHREAD</c> (Part 1, Section 34.2.5), both of which this
        /// group's define sets.
        /// </para>
        /// <para>
        /// <b>Honest channel accounting.</b> The session is UNBOUND and unsalted (Part 1, Section 16.6.9's Empty
        /// Buffer session key) with <paramref name="counterAuth"/> set as its authValue, so the per-command HMAC
        /// key is <c>sessionValue = sessionKey || authValue = Empty || counterAuth</c> (Part 1, Section 16.6.5,
        ...
        /// Index Name hashes the whole <c>TPMS_NV_PUBLIC</c> including <c>TPMA_NV_WRITTEN</c> (Part 1, Section 13,
        /// Table 9), and the first increment SETs that attribute (Part 1, Section 34.2.6.3), so the cpHash Name the
        ...
        /// recovered salt (Part 1, Section 16.6.11, equation 23) alongside <paramref name="counterAuth"/> as the
        ...
        /// Section 34.2.5), which this group's define sets.
        /// </para>
        /// <para>
        ...
        /// unsalted session key is the Empty Buffer (Part 1, Section 16.6.9), <paramref name="counterAuth"/> is the
        /// session's authValue and therefore the whole per-command HMAC key (Part 1, Section 16.6.5, equation 17
        ...
        /// <paramref name="tpmKey"/> (Part 1, Section 16.6.11, equation 23) with
        ...
        /// Part 1, Section 34.2.6.3 NOTE 2/NOTE 6): a subsequent
        /// <see cref="DefineCounterAsync(ReadOnlyMemory{byte}, uint, ReadOnlyMemory{byte}, bool, CancellationToken)"/>
        ...
        /// (<see cref="Infrastructure.Commands.StartAuthSessionInputExtensions.CreateBoundAndSaltedHmacSession(uint, uint, ReadOnlyMemory{byte}, uint, TpmAlgIdConstants, TpmAlgIdConstants, TpmRsaOaepEncryptDelegate, BaseMemoryPool, CancellationToken, TpmtSymDef?)"/>),
        /// so <c>sessionKey = KDFa(ownerAuth ‖ salt, …)</c> (Part 1, Section 16.6.12, equation 25) with the
    ...
    /// </summary>
    /// <remarks>
    /// The session key becomes <c>KDFa(ownerAuth ‖ salt, …)</c> (TPM 2.0 Library Part 1, Section 16.6.12, equation
    ...
    /// attribute (Part 1, Section 18.1's first-sized-parameter rule). The attribute is set here because this
        ...
        CancellationToken cancellationToken)
    {
        ...
        //Unbound, unsalted (Part 1, Section 16.6.9): TPM_RH_NULL bind, Empty Buffer sessionKey. No symmetric is
        ...
        //parameter Part 1, Section 18.1 makes encryption-eligible - see ReadCounterCoreAsync for why this group
        ...
        try
        {
            ...
            //Index's authorization value becomes the per-command HMAC key (Part 1, Section 16.6.5, equation 17,
        ...
        CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            try
            {
                //Unbound (bindAuthValue empty) but salted: sessionKey = KDFa(salt, ...) (Part 1, Section 16.6.11,
    ...
    /// so Name1 and Name2 of each cpHash are the same value (TPM 2.0 Library Part 1, Section 15.7, equation 15).
    ...
    /// 14, Table 9) and the first increment of a fresh Counter Index SETs that attribute (Part 1, Section
        ...
        CancellationToken cancellationToken)
    {
        ...
        //data TPM2_NV_Read returns is the first sized response parameter, which Part 1, Section 18.1 makes
    ...
    /// <c>nvIndex</c>), so Name1 and Name2 of the cpHash are the same value (TPM 2.0 Library Part 1, Section 15.7,
    ...
    /// real, current, hash-based Name (TPM 2.0 Library Part 1, Section 15.7, equation 15's Name2 term).
    /// </remarks>
    /// <param name="device">The TPM device.</param>
    /// <param name="pool">The memory pool.</param>
    ...
    /// Section 16.6.10, equation 20): binding folds <paramref name="ownerAuth"/> into the session key via KDFa, so
        ...
        TpmtSymDef? symmetric = null)
    {
        ...
        try
        {
            ...
            //equation 20 (Part 1, Section 16.6.10) on the stripped form, so an owner authValue ending in zero
        ...
        TpmtSymDef? symmetric = null)
    {
        ...
        try
        {
            ...
            try
            {
                //Both terms of equation 25 (Part 1, Section 16.6.12) are present here: the stripped bind authValue
    ...
    /// (TPM 2.0 Library Part 1, Section 16.6.4.3: "Trailing octets of zero are to be removed from any string before
    /// it is used as an authValue"; Section 16.6.5 states the same for the authValue term of the HMAC key).
    /// </summary>
    /// <remarks>
