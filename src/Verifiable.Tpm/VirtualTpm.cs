...
public sealed class VirtualTpm
{
    ...
    /// <param name="pool">Optional memory pool for serialization.</param>
    public void RecordError<TInput>(TInput input, TpmRc errorCode, MemoryPool<byte>? pool = null)
        where TInput : ITpmCommandInput<TInput>
    {
        pool ??= MemoryPool<byte>.Shared;
    ...
    public int Submit(ReadOnlySpan<byte> command, Span<byte> response)
    {
        ...
        return WriteErrorResponse(response, TpmRc.Failure);
    }
    ...
    private static int WriteErrorResponse(Span<byte> response, TpmRc errorCode)
    {
