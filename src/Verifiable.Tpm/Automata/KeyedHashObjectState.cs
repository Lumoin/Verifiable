...
/// </param>
/// <param name="IsPublicOnly">
/// Whether this object carries no sensitive area — loaded by <c>TPM2_LoadExternal()</c> from <c>inPublic</c>
/// alone (TPM 2.0 Library Part 3, clause 12.3.1). Unlike <see cref="TransientKeyState.IsPublicOnly"/>, this is
/// an explicit stored field rather than a predicate over <see cref="Data"/>: a legitimately loaded sealed data
/// object may itself carry an empty <see cref="Data"/> (a zero-length seal), so emptiness cannot double as the
/// public-only signal here. Every <c>@</c>-decorated authorizing use of such an object is refused
/// <c>TPM_RC_AUTH_UNAVAILABLE</c> (Part 3, clause 5.6, check 1). <c>TPM2_Load()</c> always populates a full
/// sensitive area, so its one construction site passes <see langword="false"/>.
/// </param>
public sealed record KeyedHashObjectState(
    ...
    Tpm2bName QualifiedName,
    bool IsPublicOnly): IDisposable
{
    /// <summary>
    ...
    public bool Equals(KeyedHashObjectState? other) =>
        ...
        && IsPublicOnly == other.IsPublicOnly;
    ...
    public override int GetHashCode() => HashCode.Combine(Handle, Hierarchy, NoDa, UserWithAuth, IsDuplicable, IsPublicOnly);
}
