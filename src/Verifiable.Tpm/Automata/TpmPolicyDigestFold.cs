...
public enum TpmPolicyDigestFold
{
    ...
    /// <summary><c>TPM2_PolicySecret()</c>'s trial and no-ticket arms: <c>H(policyDigest ‖ TPM_CC_PolicySecret ‖ authName)</c> then the policyRef hash (Part 3, clause 23.4).</summary>
    Secret,

    /// <summary><c>TPM2_PolicySigned()</c>'s trial arm: <c>H(policyDigest ‖ TPM_CC_PolicySigned ‖ authObjectName)</c> then the policyRef hash (Part 3, clause 23.3).</summary>
    Signed,

    /// <summary><c>TPM2_PolicyAuthorize()</c>'s trial arm: <c>H(0…0 ‖ TPM_CC_PolicyAuthorize ‖ keySign)</c> then the policyRef hash (Part 3, clause 23.16).</summary>
    Authorize,
