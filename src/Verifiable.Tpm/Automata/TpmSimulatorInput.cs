...
/// NV Index with the given attributes and authorization value, authorized by <see cref="AuthHandle"/> —
/// the owner or the platform hierarchy (<c>TPMI_RH_PROVISION</c>, Part 2, clause 9.21, Table 67). The
...
/// </summary>
/// <param name="AuthHandle">The provisioning hierarchy authorizing the definition — <c>TPM_RH_OWNER</c> or <c>TPM_RH_PLATFORM</c>.</param>
/// <param name="AuthSupplied">The authorization value the caller supplied — the password session's plaintext authValue, which is the same <c>TPM2B_AUTH</c> wire field a real session carries an HMAC in (TPM 2.0 Library Part 2, clause 10.12.2, Table 156: "either an HMAC, a password, or an EmptyAuth") — in a pooled carrier this record OWNS, rented as the parse's last act. It authorizes <see cref="AuthHandle"/>'s hierarchy. The authorizing transition is its terminal owner; every refusing path releases it through <see cref="IDisposable.Dispose"/>.</param>
...
/// <param name="Attributes">The Index attributes (<c>TPMA_NV</c>), whose <c>TPMA_NV_NO_DA</c> bit decides dictionary-attack protection, whose <c>TPMA_NV_PLATFORMCREATE</c> bit must agree with <see cref="AuthHandle"/>'s hierarchy, and whose <c>TPMA_NV_POLICY_DELETE</c> bit admits only the platform arm.</param>
    ...
    TpmNvIndexData IndexData): TpmSimulatorInput, IDisposable
{
    ...
    /// calls this. <see cref="AuthSupplied"/> is released on every refusing path; the authorizing
...
/// after authorizing against it. Index authorization (the authorization handle equals
/// the Index itself) and owner authorization (<c>TPM_RH_OWNER</c>, gated on <c>TPMA_NV_OWNERREAD</c>) are
/// modelled; a policy-authorized read is not.
/// </summary>
...
/// an offset after authorizing against it, then sets <c>TPMA_NV_WRITTEN</c>. Index
/// authorization (the authorization handle is the Index itself) is modelled; an owner- or policy-authorized
/// write is not, mirroring <see cref="TpmNvReadRequested"/>.
/// </summary>
...
/// definition and frees its handle, authorized by <see cref="AuthHandle"/> — the owner or the platform
/// hierarchy. The policy-delete variant (<c>TPM2_NV_UndefineSpaceSpecial()</c>, clause 31.5) is the separate
/// command <see cref="TpmNvUndefineSpaceSpecialRequested"/> models: an Index carrying
/// <see cref="NvIndexState.IsPolicyDeleteRequired"/> refuses this command instead.
/// </summary>
/// <param name="AuthHandle">The provisioning hierarchy authorizing the removal — <c>TPM_RH_OWNER</c> or <c>TPM_RH_PLATFORM</c>.</param>
...
/// <param name="AuthSupplied">The authorization value the caller supplied — the password session's plaintext authValue, which is the same <c>TPM2B_AUTH</c> wire field a real session carries an HMAC in (TPM 2.0 Library Part 2, clause 10.12.2, Table 156: "either an HMAC, a password, or an EmptyAuth") — in a pooled carrier this record OWNS, rented as the parse's last act. It is compared against <see cref="AuthHandle"/>'s hierarchy authValue. The authorizing transition is its terminal owner; every refusing path releases it through <see cref="IDisposable.Dispose"/>.</param>
...
/// Index's 8-octet value by one after authorizing against it. Index authorization (the
/// authorization handle is the Index itself) and owner authorization are modelled, mirroring
/// <see cref="TpmNvWriteRequested"/>; a policy- or platform-authorized increment is not.
/// </summary>
/// <param name="AuthHandle">The authorization handle (<c>TPMI_RH_NV_AUTH</c>); for Index authorization this equals <paramref name="NvIndex"/>, or <c>TPM_RH_OWNER</c> for the owner arm.</param>
...
/// <summary>
/// A <c>TPM2_NV_UndefineSpace()</c> command whose <c>authHandle</c> is authorized by an HMAC session
...
/// <summary>
/// A <c>TPM2_NV_DefineSpace()</c> command whose <c>authHandle</c> is authorized by an HMAC session rather
...
/// <param name="ResolvedAuthValue">
/// The bind-omission-resolved hierarchy authValue term to fold into the command and response HMAC keys — a
...
/// Whether <see cref="Hmac"/> was the Empty Buffer, read and recorded at the continuation that releases
/// <see cref="Hmac"/> — since the digest-effect hop between authorization and response framing does not itself
/// retain the carrier, this flag is what TPM 2.0 Library Part 1, clause 16.6.16's response-mirroring rule
/// survives on into the response-framing step that follows the digest effect.
/// </param>
public sealed record TpmNvExtendOverSessionRequested(
    ...
    bool IsSuppliedHmacEmpty = false): TpmSimulatorInput, IDisposable
{
    /// <summary>
    /// Releases the owned <see cref="RawParameterArea"/>, <see cref="Data"/>,
    /// <see cref="ResolvedIndexName"/>, <see cref="NonceCaller"/> and <see cref="Hmac"/> carriers.
...
public sealed record TpmNvSessionResponseFramed(
    ...
    Tpm2bAuth Hmac,
    Tpm2bDigest? NewAuditDigest = null): TpmSimulatorInput;
...
/// <summary>
/// One session's material for framing an authValue rotation's, or an ADMIN-policy deletion's, response entry —
/// <c>TPM2_NV_ChangeAuth()</c>, <c>TPM2_HierarchyChangeAuth()</c>, or <c>TPM2_NV_UndefineSpaceSpecial()</c>: the
/// effect rolls a fresh nonceTPM and computes a real response HMAC for it, or emits the empty-nonce,
/// empty-HMAC placeholder a <c>TPM_RS_PW</c> slot is owed. The NV-family counterpart of
/// <see cref="TpmCreateResponseSession"/>, needed because these are the commands whose authorization area can
/// carry two sessions each owed its own entry (Part 1, clause 15.6.1).
/// </summary>
...
/// meaningless when it is set. <c>TPM2_HierarchyChangeAuth()</c>'s authorizing slot and
/// <c>TPM2_NV_UndefineSpaceSpecial()</c>'s <c>@platform</c> slot can both set it; <c>TPM2_NV_ChangeAuth()</c>'s
/// and <c>TPM2_NV_UndefineSpaceSpecial()</c>'s ADMIN policy slot never does (Part 3, clause 5.6, check 5.2
/// admits only a policy session there).
/// </param>
...
/// <param name="NewAuditDigest">The extended audit digest (TPM 2.0 Library Part 1, clause 17.1, equation 30), OWNED, when this session claims <c>audit</c>; <see langword="null"/> otherwise (never set when <see cref="IsPolicySession"/>, since a policy session may never claim audit). Released by <c>RollHmacSessionNonceAndAudit</c>'s adoption onto the durable session record, or by the entry's own release on a fault.</param>
...
/// <summary>
/// The result of executing a <see cref="Automata.TpmFrameTwoSlotResponseAction"/>: every session's framed
...
/// </summary>
/// <remarks>
/// There is no parameter area: none of <c>TPM2_NV_ChangeAuth()</c> (TPM 2.0 Library Part 3, clause 31.15, Table
/// 270), <c>TPM2_HierarchyChangeAuth()</c> (clause 24.8) or <c>TPM2_NV_UndefineSpaceSpecial()</c> (clause 31.5,
/// Table 250) return parameters, so rpHash covers the empty parameter area and the entries are the whole of
/// what the response adds beyond its header.
/// </remarks>
...
/// <summary>
/// A <c>TPM2_NV_UndefineSpaceSpecial()</c> command (TPM 2.0 Library Part 3, clause 31.5). Deletes a
/// platform-created NV Index carrying <see cref="NvIndexState.IsPolicyDeleteRequired"/>, authorized by TWO
/// sessions in the one authorization area: <c>@nvIndex</c> (Auth Index 1, ADMIN role — a policy session whose
/// commandCode is <c>TPM_CC_NV_UndefineSpaceSpecial</c>, Table 249) and <c>@platform</c> (Auth Index 2, USER
/// role — password or HMAC, Table 250). There is no password form of this command: the ADMIN role over an NV
/// Index admits only a policy session (Part 4 <c>SessionProcess.c</c>'s <c>IsPolicySessionRequired()</c>), so
/// this is the only request record the parser ever builds.
/// </summary>
/// <remarks>
/// No parameter is encryptable in either direction (clause 5.7), so there is no bind-omission-flag field the
/// way <see cref="TpmNvChangeAuthOverSessionRequested.HasDecryptSlot"/> carries one: a third (companion) block
/// the authorization area may still carry claims decrypt or encrypt is refused, generically, by
/// <c>ValidateSessionArea</c>'s per-attribute gate in the entry transition (<c>OnNvUndefineSpaceSpecial</c>)
/// rather than ever being queued; an audit-only claim, at the companion or at the platform slot alongside its
/// own authorization, is admitted instead (TPM 2.0 Library Part 1, clause 17.1).
/// </remarks>
/// <param name="NvIndex">The Index to delete, typed <c>TPMI_RH_NV_DEFINED_INDEX</c> by the command's own handle area (Part 2, clause 9.26, Table 72), the same range <see cref="TpmNvUndefineSpaceOverSessionRequested.NvIndex"/> carries.</param>
/// <param name="PolicySessionHandle">The ADMIN-role policy session authorizing <paramref name="NvIndex"/>'s deletion.</param>
/// <param name="PolicyNonceCaller">The policy slot's caller nonce for this command (<c>TPM2B_NONCE</c>, TPM 2.0 Library Part 2, clause 10.3.4, Table 92), in a pooled carrier this record OWNS, rented as the parse's last act. The accepting continuation transfers it into the response-framing step, whose effect releases it once the response HMAC has keyed its nonceOlder term on it; every refusing path releases it through <see cref="IDisposable.Dispose"/>.</param>
/// <param name="PolicySessionAttributes">The policy slot's command session-attributes octet.</param>
/// <param name="PolicyHmac">The policy slot's supplied command <c>hmac</c> field (<c>TPM2B_AUTH</c>, TPM 2.0 Library Part 2, clause 10.12.2, Table 156), in a pooled carrier this record OWNS, rented as the parse's last act. Everything downstream BORROWS it; the accepting continuation is its terminal owner, disposed once the deletion's completion has consumed it.</param>
/// <param name="PlatformSessionHandle">The <c>@platform</c> session (<c>TPM_RS_PW</c> or a real HMAC session) authorizing with the platform hierarchy's own authValue.</param>
/// <param name="PlatformNonceCaller">The platform slot's caller nonce, owned the same way <paramref name="PolicyNonceCaller"/> is; the dispose-immune empty sentinel for a <c>TPM_RS_PW</c> slot.</param>
/// <param name="PlatformSessionAttributes">The platform slot's command session-attributes octet.</param>
/// <param name="PlatformHmac">The platform slot's supplied credential — a real session's command <c>hmac</c>, or a <c>TPM_RS_PW</c> slot's plaintext platformAuth — owned the same way <paramref name="PolicyHmac"/> is. A <c>TPM_RS_PW</c> slot's compare is judged in WIRE ORDER as a queued verification (<see cref="TpmPendingSessionVerification.ForPasswordCompare"/>) rather than inline, so this credential is never read before the Name hop.</param>
/// <param name="HasCompanionSlot">Whether a third (non-authorizing) block accompanied the two required slots.</param>
/// <param name="CompanionSessionHandle">The companion slot's session handle, meaningful only when <see cref="HasCompanionSlot"/> is set; owns no carrier, since a companion here is refused before any of its credential material is ever read.</param>
/// <param name="CompanionSessionAttributes">The companion slot's command session-attributes octet, meaningful only when <see cref="HasCompanionSlot"/> is set.</param>
/// <param name="CompanionNonceLength">The companion slot's caller-nonce octet count as it arrived on the wire, meaningful only when <see cref="HasCompanionSlot"/> is set — <c>ValidateSessionArea</c>'s password-slot nonce-width rule needs the length alone, not the octets.</param>
/// <param name="RawParameterArea">The (empty) parameter-area wire bytes — this command carries none — held in a pooled carrier this record OWNS, rented as the parse's last act; captured for cpHash uniformity with every other NV command's session-form parameter term, the same posture <see cref="TpmNvUndefineSpaceOverSessionRequested.RawParameterArea"/> takes.</param>
/// <param name="ResolvedIndexName">The Index's computed Name; see <see cref="TpmNvReadOverSessionRequested.ResolvedIndexName"/>.</param>
/// <param name="ResolvedAuthValue">The policy slot's bind-omission-resolved authValue term folded into its command HMAC key — the Index's current authValue when the policy asserted <c>TPM2_PolicyAuthValue()</c>, the shared empty carrier otherwise (Part 1, clause 16.6.5's policy Note); a borrow of durable state, resolved once the Index's Name is known.</param>
/// <param name="ResolvedPlatformAuthValue">The platform slot's bind-omission-resolved authValue term — the live <see cref="TpmSimulatorState.PlatformAuth"/>, or the shared empty carrier when the slot's session is bound to <c>TPM_RH_PLATFORM</c> itself (equation 22); a borrow of durable state, reused unchanged by the response HMAC (the policy slot's own response term is the Empty Buffer regardless, TPM 2.0 Library Part 3, clause 31.5.1's own sentence).</param>
public sealed record TpmNvUndefineSpaceSpecialRequested(
    TpmiRhNvDefinedIndex NvIndex,
    TpmiShAuthSession PolicySessionHandle,
    Tpm2bNonce PolicyNonceCaller,
    TpmaSession PolicySessionAttributes,
    Tpm2bAuth PolicyHmac,
    TpmiShAuthSession PlatformSessionHandle,
    Tpm2bNonce PlatformNonceCaller,
    TpmaSession PlatformSessionAttributes,
    Tpm2bAuth PlatformHmac,
    bool HasCompanionSlot,
    TpmiShAuthSession CompanionSessionHandle,
    TpmaSession CompanionSessionAttributes,
    int CompanionNonceLength,
    TpmParameterArea RawParameterArea,
    Tpm2bName ResolvedIndexName,
    Tpm2bAuth? ResolvedAuthValue = null,
    Tpm2bAuth? ResolvedPlatformAuthValue = null): TpmSimulatorInput, IDisposable
{
    /// <summary>
    /// Releases the owned <see cref="RawParameterArea"/>, <see cref="ResolvedIndexName"/>, and both slots'
    /// nonce and credential carriers. <see cref="ResolvedAuthValue"/> and <see cref="ResolvedPlatformAuthValue"/>
    /// are borrows of durable state and are never touched here.
    /// </summary>
    /// <remarks>
    /// Both slots' credentials are owned outright from the parse until a terminal arm takes them, and only the
    /// accepting path takes them: it disposes each hmac per carrier once the verification queue is done with
    /// it, and transfers each slot's caller nonce into that slot's response entry — the same discipline
    /// <see cref="TpmNvChangeAuthOverSessionRequested.Dispose"/> applies to its own two slots.
    /// </remarks>
    public void Dispose()
    {
        RawParameterArea.Dispose();
        ResolvedIndexName.Dispose();
        PolicyNonceCaller.Dispose();
        PolicyHmac.Dispose();
        PlatformNonceCaller.Dispose();
...
/// </summary>
/// <param name="Hierarchy">The hierarchy authorizing the creation (<c>TPM_RH_OWNER</c>; the platform and endorsement hierarchies are not modelled).</param>
...
/// 3, clause 24.1, Table 191), included verbatim in the creation data. An owned pooled carrier rented at
...
/// 128; Part 3, clause 24.1, Table 191), the PCR selection the creation data's <c>pcrDigest</c> is computed
...
/// carrying a caller-supplied <c>TPMT_TK_HASHCHECK</c> validation ticket alongside an <c>inScheme</c> override —
/// unlike <c>TPM2_SignDigest()</c>, the caller may retarget the key's own scheme. Signs the digest with a loaded
/// signing key — an asymmetric transient object, or a KEYEDHASH HMAC key taking Table 115's HMAC row
/// ("Signs/verifies the digest", clause 20.1).
/// </summary>
...
/// is the NULL Ticket (clause 10.6.2) an unrestricted key's caller may supply, exactly as
/// <see cref="TpmSignDigestRequested.ValidationHierarchy"/> is.
/// </param>
...
/// at parse; the asymmetric arm's consuming transition transfers it into the ticket-validating sign action when
/// the ticket is non-NULL (or the key is restricted), whose effect is its terminal owner. A NULL Ticket (an
...
{
    /// <summary>
    /// Releases the owned <see cref="SuppliedKeyPassword"/>, <see cref="Digest"/> and
    /// <see cref="ValidationDigest"/> carriers on a refusing path; the consuming transition instead releases the
    /// password itself once the key-slot compare has consumed it and threads the digest (and, on the asymmetric
    /// arm, a non-NULL ticket) into the signing action, and never calls this.
    /// </summary>
    public void Dispose()
    {
...
/// <param name="HashAlg">The requested <c>hashAlg</c> (<c>TPMI_ALG_HASH+</c>) selected against the key's scheme through Part 3's Table 79.</param>
...
/// <param name="NewAuditDigest">The extended audit digest (TPM 2.0 Library Part 1, clause 17.1, equation 30), OWNED, when this session claims <c>audit</c>; <see langword="null"/> otherwise. Released by <c>RollHmacSessionNonceAndAudit</c>'s adoption onto the durable session record, or by <see cref="Release"/> on a path that frames no response.</param>
...
{
    /// <summary>
    /// Releases the entry's owned carriers — both rolled nonces, the response HMAC and the audit digest — on a
    /// path that frames no response; the completing transition and the serializer otherwise take them as their
    /// terminal owners.
    /// </summary>
    public void Release()
    {
...
/// <param name="ValidationHierarchy">The caller-supplied <c>TPMT_TK_HASHCHECK</c> validation ticket's own <c>hierarchy</c> field, decoded at parse — it sits behind <c>digest</c> and <c>inScheme</c> and so is never encrypted, exactly as <see cref="TpmSignDigestOverSessionRequested.ValidationHierarchy"/> is.</param>
/// <param name="ValidationDigest">The caller-supplied validation ticket's HMAC octets (<c>TPM2B_DIGEST</c>), an owned pooled carrier rented at parse, or the dispose-immune <see cref="Tpm2bDigest.Empty"/> singleton for a NULL Ticket. The asymmetric arm's completing transition transfers it into the ticket-validating action on a non-NULL ticket; the NULL-Ticket arm owns nothing to release. The KEYEDHASH arm releases it unconsulted.</param>
...
{
    /// <summary>
    /// Releases the owned area, the recovered digest, the validation ticket digest, and the raw parameter area
    /// on a refusing path.
    /// </summary>
    public void Dispose()
    {
...
/// 10.3.2, Table 90; empty for the generic storage parent; a standard endorsement key's "PolicyA" for
...
/// <param name="AuthPolicy">
/// The authorization policy digest carried in the template (<c>TPM2B_DIGEST</c>, TPM 2.0 Library Part 2, clause
/// 10.3.2, Table 90; empty for a generic RSA storage parent; a standard RSA endorsement key's "PolicyA" for
...
/// <param name="CreationPcr">
/// The <c>creationPCR</c> parameter (<c>TPML_PCR_SELECTION</c>, TPM 2.0 Library Part 2, clause 10.8.7, Table
/// 128; Part 3, clause 12.1, Table 18), the PCR selection the creation data's <c>pcrDigest</c> is computed
...
/// empty password.
/// </param>
/// <param name="ObjectType">The public area's object type; only a sealed <c>TPM_ALG_KEYEDHASH</c> object is modelled, an asymmetric sealed object is not.</param>
...
/// <summary>
/// A <c>TPM2_LoadExternal()</c> command (TPM 2.0 Library Part 3, clause 12.3.1): loads a public area alone, or a
/// public area with an unencrypted sensitive area, as a Temporary Object carrying no wire-protected private
/// blob — the object arrives already in the clear, so this record's <see cref="InPrivate"/> is the parsed
/// sensitive area itself (or <see langword="null"/> for a public-only load), never a wrapped carrier the load
/// effect must decrypt. A decrypt claim over <see cref="InPrivate"/> or an encrypt claim over the response
/// <c>name</c> reaches the same record through the generic no-authorization wrapper
/// (<see cref="TpmNoAuthOverSessionsRequested"/>, TPM 2.0 Library Part 3, clause 12.3, Table 22's session
/// table) — its parameter core re-parses the (possibly decrypted) area through the same helper this record's
/// own parser uses, so this is the ONE request shape both the <c>TPM_ST_NO_SESSIONS</c> and
/// <c>TPM_ST_SESSIONS</c> forms resolve to.
/// </summary>
...
/// </param>
/// <param name="InPublic">
/// The public area to load (<c>TPM2B_PUBLIC+</c>, TPM 2.0 Library Part 2, clause 12.2.5, Table 236 — the
/// <c>+</c> admitting <c>nameAlg = TPM_ALG_NULL</c>, TPM 2.0 Library Part 4's <c>flag</c> parameter on the
/// unmarshal). OWNED: rented at parse; TRANSFERRED into <see cref="Automata.TpmLoadExternalAction"/> on
/// acceptance, whose effect hands it on whole as the loaded object's own public area; released through this
/// record's <see cref="IDisposable.Dispose"/> on every refusing path.
/// </param>
/// <param name="Hierarchy">
/// The raw <c>hierarchy</c> handle value (<c>TPMI_RH_HIERARCHY</c>, TPM 2.0 Library Part 2, clause 9.13, Table
/// 59) exactly as received, unvalidated: Table 59 membership and hierarchy-enablement are judged at the
/// transition, not the parse (mirroring every other <c>TPMI_RH_HIERARCHY</c> parameter this simulator reads
/// raw and validates downstream).
/// </param>
public sealed record TpmLoadExternalRequested(
    TpmtSensitive? InPrivate,
    Tpm2bPublic InPublic,
    uint Hierarchy): TpmSimulatorInput, IDisposable
{
    /// <summary>
    /// Releases the owned <see cref="InPrivate"/> (when present) and <see cref="InPublic"/> carriers on a
    /// refusing path; the accepting transition instead transfers both into
    /// <see cref="Automata.TpmLoadExternalAction"/> and never calls this.
    /// </summary>
    public void Dispose()
    {
        InPrivate?.Dispose();
...
/// <summary>
/// The result of executing a <see cref="Automata.TpmLoadExternalAction"/>: the object Name the effectful loop
/// computed and the built object-model record, fed back so the transition can install the loaded object and
/// frame the <c>TPM2_LoadExternal()</c> response — the plain form's <c>TpmLoadResponse</c> shape, or, over a
/// session, the dedicated encrypt-eligible framing (TPM 2.0 Library Part 3, clause 12.3.1). Internal to the
/// effect loop; never arrives from the command transport.
/// </summary>
/// <param name="ResponseCode">
/// <c>TPM_RC_SUCCESS</c> with <see cref="KeyState"/> or <see cref="SealedState"/> populated (never both), or the
/// key-validation refusal the effect found (<c>TPM_RC_KEY</c>/<c>KEY_SIZE</c>/<c>BINDING</c>/<c>ECC_POINT</c>/
/// <c>TYPE</c>/<c>SIZE</c>/<c>SCHEME</c>) — then both are <see langword="null"/> and <see cref="Name"/> is the
/// dispose-immune <see cref="Tpm2bName.Empty"/> sentinel, the action's carriers already released by the effect.
/// </param>
/// <param name="Handle">The transient handle the transition allocated for the loaded object.</param>
/// <param name="Name">The object Name (<c>nameAlg ‖ H_nameAlg(TPMT_PUBLIC)</c>, or the Empty Buffer for <c>nameAlg = TPM_ALG_NULL</c>) in an owned <c>TPM2B_NAME</c> carrier; ownership flows to the framed response and is released once framed.</param>
/// <param name="KeyState">The built RSA/ECC object-model record on a success loading an asymmetric key; <see langword="null"/> otherwise. Ownership transfers to the installing transition's dictionary entry.</param>
/// <param name="SealedState">The built KEYEDHASH object-model record on a success loading a keyed-hash object; <see langword="null"/> otherwise. Ownership transfers to the installing transition's dictionary entry.</param>
/// <param name="Request">The request the response is framed for (<see cref="TpmLoadExternalRequested"/>, whether the command arrived plain or over the generic no-authorization wrapper).</param>
public sealed record TpmExternalObjectLoaded(
    TpmRcConstants ResponseCode,
    TpmiDhObject Handle,
    Tpm2bName Name,
    TransientKeyState? KeyState,
    KeyedHashObjectState? SealedState,
    TpmSimulatorInput Request): TpmSimulatorInput, IDisposable
{
    /// <summary>
    /// Releases the <see cref="Name"/> carrier on a refusing path (the dispose-immune empty sentinel, a no-op);
    /// the installing transition never calls this on a success, since <see cref="Name"/> transfers into the
    /// framed response and <see cref="KeyState"/>/<see cref="SealedState"/> are installed, not disposed.
    /// </summary>
    public void Dispose()
    {
        Name.Dispose();
...
/// <param name="PolicySessionAttributes">The DUP-role session's command session-attributes octet, judged by <see cref="TpmLifecycleTransitions.ValidateSessionArea"/> before the export authorizes — a policy session may not claim <c>audit</c> (TPM 2.0 Library Part 1, Table 15).</param>
...
/// <summary>
/// A <c>TPM2_ObjectChangeAuth()</c> command (TPM 2.0 Library Part 3, clause 12.8): a new authorization value
/// for a TPM-resident object, returned as a freshly wrapped private area under its parent — the resident
/// object itself is never altered ("This command does not change the authorization of the TPM-resident object
/// on which it operates. Therefore, the old authValue... is used when generating the response HMAC key if
/// required."). The object's slot carries ADMIN-role authorization (Part 1, clause 16.2): a password, an HMAC
/// session, or a policy session, admissible structurally here alike — which of the three the object's own
/// <c>adminWithPolicy</c> attribute actually authorizes (Part 3, clause 5.6, check 5.1) is a transition-time
/// judgment, not a parse-time one. The parent's slot carries no authorization at all (Table 32, Auth Index
/// None). One record covers every session kind at this slot, mirroring <see cref="TpmDuplicateRequested"/>'s
/// single-slot shape rather than splitting into an all-password and an over-session form the way
/// <see cref="TpmCertifyRequested"/>/<see cref="TpmCertifyOverSessionRequested"/> do.
/// </summary>
/// <param name="ObjectHandle">The TPM-resident object whose authorization value is replaced (the ADMIN-role handle, Auth Index 1).</param>
/// <param name="ParentHandle">The object's parent — judged by recomputing its Qualified Name against the object's retained one (Part 3, clause 12.8.1), never trusted as a handle alone; carries no authorization.</param>
/// <param name="SessionHandle">The ADMIN-role slot's session handle: a <c>TPM_RS_PW</c> password, an HMAC session, or a policy session.</param>
/// <param name="NonceCaller">The slot's caller nonce for this command (<c>TPM2B_NONCE</c>, TPM 2.0 Library Part 2, clause 10.3.4, Table 92), in a pooled carrier this record OWNS, rented as the parse's last act. Every refusing path releases it through <see cref="IDisposable.Dispose"/>. On acceptance an HMAC slot's nonce TRANSFERS into the response-session entry, whose framing effect releases it; a policy slot's continuation is its terminal owner and disposes it directly, since no response entry is framed for that slot; a <c>TPM_RS_PW</c> slot never rents one, because a non-empty password-slot nonce is refused <c>TPM_RC_NONCE</c> before the parse's rental step and the empty nonce is the shared dispose-immune sentinel.</param>
/// <param name="SessionAttributes">The slot's command session-attributes octet.</param>
/// <param name="Hmac">The supplied command <c>hmac</c> field (<c>TPM2B_AUTH</c>, TPM 2.0 Library Part 2, clause 10.12.2, Table 156) — a password session's plaintext password, exactly as <see cref="TpmDuplicateRequested"/>'s sibling records read a <c>TPM_RS_PW</c> slot's field, or a real session's command HMAC. In a pooled carrier this record OWNS, rented as the parse's last act. The authorizing transition (a password's inline compare) or the queued verification effect (an HMAC/policy session) is its terminal reader; every refusing path releases it through <see cref="IDisposable.Dispose"/>.</param>
/// <param name="RawParameterArea">The raw <c>newAuth</c> wire bytes exactly as received (TPM 2.0 Library Part 1, clause 15.7 equation 15's <c>parameters</c> term), captured before the field is decoded. Held in a pooled carrier this record OWNS, rented as the parse's last act; released through <see cref="IDisposable.Dispose"/> on every refusing path and by the completing continuation once used.</param>
/// <param name="NewAuth">The parsed <c>newAuth</c> value (<c>TPM2B_AUTH</c>) in an owned carrier rented at parse time holding the wire-exact octets — plaintext only when no decrypt claim accompanied the command. TRANSFERRED into <see cref="Automata.TpmObjectChangeAuthAction"/> on acceptance, whose effect wraps it into the new private area; released through <see cref="IDisposable.Dispose"/> on every refusing path.</param>
/// <param name="ResolvedAuthValue">The authValue term the HMAC-session or policy-session slot's command HMAC folded — the object's value, or the shared empty carrier under equation 22's bind omission or where the policy asserted no <c>TPM2_PolicyAuthValue()</c> — threaded from the entry transition so the response HMAC keys on the identical term (TPM 2.0 Library Part 1, clause 16.6.5; Part 3, clause 12.8.1's "the old authValue"). <see langword="null"/> until the entry transition resolves it, and on the password form throughout.</param>
public sealed record TpmObjectChangeAuthRequested(
    TpmiDhObject ObjectHandle,
    TpmiDhObject ParentHandle,
    TpmiShAuthSession SessionHandle,
    Tpm2bNonce NonceCaller,
...
{
    /// <summary>
    /// Releases every owned carrier this record still holds — the slot's caller nonce and supplied credential,
    /// the raw parameter area, and the parsed <c>newAuth</c> — on a refusing path. The accepting continuation
    /// instead transfers <see cref="NewAuth"/> into <see cref="Automata.TpmObjectChangeAuthAction"/>, transfers
    /// an HMAC slot's <see cref="NonceCaller"/> into its response entry, and disposes the rest itself once their
    /// one use (the credential compare or HMAC verification, and the cpHash capture) has run, swapping each
    /// released slot to its dispose-immune empty sentinel so a later refusal's cascade through this method
...
/// <summary>
/// The result of executing a <see cref="Automata.TpmObjectChangeAuthAction"/>: the freshly wrapped private
/// area carrying the object's new authorization value (TPM 2.0 Library Part 3, clause 12.8.1), fed back so the
/// transition can frame the <c>TPM2_ObjectChangeAuth()</c> response. Internal to the effect loop; never
/// arrives from the command transport.
/// </summary>
/// <param name="ResponseCode"><c>TPM_RC_SUCCESS</c> with <see cref="OutPrivate"/> populated, or the effect's own refusal (<c>TPM_RC_TYPE</c> on a Qualified-Name mismatch, <c>TPM_RC_SIZE</c> on <paramref name="OutPrivate"/>'s width rule) — then <see cref="OutPrivate"/> is the dispose-immune <see cref="Tpm2bPrivate.Empty"/> sentinel.</param>
/// <param name="OutPrivate">The wrapped private area (<c>TPM2B_PRIVATE</c>, TPM 2.0 Library Part 3, clause 12.8.2, Table 33) in an owned pooled carrier; ownership flows to the framed response and is released once framed.</param>
/// <param name="Request">The original parsed request, threaded through so the completing transition knows which session kind (password, HMAC, or policy) the response must be framed for.</param>
public sealed record TpmObjectAuthChanged(
    TpmRcConstants ResponseCode,
    Tpm2bPrivate OutPrivate,
    TpmSimulatorInput Request): TpmSimulatorInput, IDisposable
{
    /// <summary>
    /// Releases the owned <see cref="OutPrivate"/> carrier on a refusing path (the dispose-immune empty
    /// sentinel, a no-op); the framing path transfers it to the response intent instead and never calls this.
    /// </summary>
    public void Dispose() => OutPrivate.Dispose();
}

/// <summary>
/// A <c>TPM2_RSA_Encrypt()</c> command (TPM 2.0 Library Part 3, clause 14.2, Table 44): RSA-encrypts
/// <c>message</c> to the public portion of an RSA key under a padding scheme selected between the key's own
/// retained scheme and <c>inScheme</c> (Table 42). <c>keyHandle</c> carries no authorization (Auth Index:
/// None) — "Because only the public portion of the key needs to be loaded for this command, the caller can
/// manipulate the attributes of the key in any way desired" (clause 14.2.1's Note). This is also the request
/// the <c>TPM_ST_SESSIONS</c> form parses to, once the no-authorization wrapper's authorization area has
/// verified (<see cref="TpmNoAuthOverSessionsRequested"/>): the wrapper's <c>Inner</c> is built from its own
/// already-parsed handle rather than reading <c>keyHandle</c> a second time.
/// </summary>
/// <param name="KeyHandle">The RSA key to encrypt under.</param>
/// <param name="Message">
/// The message to encrypt (<c>TPM2B_PUBLIC_KEY_RSA</c>, TPM 2.0 Library Part 2, clause 11.2.4.6, Table 194), an
/// owned pooled carrier rented at parse. TRANSFERRED into <see cref="Automata.TpmRsaEncryptAction"/> on
/// acceptance, whose effect is its terminal owner; released through this record's
/// <see cref="IDisposable.Dispose"/> on every refusing path.
/// </param>
/// <param name="InScheme">The padding scheme to apply if the key's own scheme is <c>TPM_ALG_NULL</c> (<c>TPMT_RSA_DECRYPT+</c>, Table 192/193).</param>
/// <param name="Label">
/// The optional label associated with the message (<c>TPM2B_DATA</c>, TPM 2.0 Library Part 2, clause 10.3.3,
/// Table 91), an owned pooled carrier rented at parse. TRANSFERRED into <see cref="Automata.TpmRsaEncryptAction"/>
/// on acceptance; released through this record's <see cref="IDisposable.Dispose"/> on every refusing path.
/// </param>
public sealed record TpmRsaEncryptRequested(
    TpmiDhObject KeyHandle,
    Tpm2bPublicKeyRsa Message,
    TpmtRsaDecrypt InScheme,
    Tpm2bData Label): TpmSimulatorInput, IDisposable
{
    /// <summary>
    /// Releases the owned <see cref="Message"/> and <see cref="Label"/> carriers on a refusing path, the
    /// message zeroed first — a plaintext being wrapped to the key, never left in a freed pooled buffer; the
    /// accepting transition instead transfers both into <see cref="Automata.TpmRsaEncryptAction"/> and never
    /// calls this.
    /// </summary>
    public void Dispose()
    {
...
/// <summary>
/// The result of executing a <see cref="Automata.TpmRsaEncryptAction"/>: the encrypted <c>outData</c> the
/// effect produced (TPM 2.0 Library Part 3, clause 14.2, Table 45), fed back so the transition can frame the
/// response. Internal to the effect loop; never arrives from the command transport.
/// </summary>
/// <remarks>
/// This is the sole feedback for both <c>TPM_ST_NO_SESSIONS</c> and <c>TPM_ST_SESSIONS</c> forms:
/// <see cref="Request"/> is always <see cref="TpmRsaEncryptRequested"/>, since the no-authorization wrapper
/// dispatches its verified <c>Inner</c> to the SAME transition either form reaches. A success on the routed
/// form is reframed with sessions afterward by the wrapper's own completion hook, over the plain response this
/// feedback declares; a refusal is answered bare here on either form, and the wrapper reframes nothing when the
/// terminal response was never a success (TPM 2.0 Library Part 3, clause 5.9).
/// </remarks>
/// <param name="ResponseCode"><c>TPM_RC_SUCCESS</c> with <see cref="OutData"/> populated, or the effect's own refusal (<c>TPM_RC_VALUE</c> on a message-size, message-value, or padding rule) — then <see cref="OutData"/> is the dispose-immune <see cref="Tpm2bPublicKeyRsa.Empty"/> sentinel.</param>
/// <param name="OutData">The encrypted output (<c>TPM2B_PUBLIC_KEY_RSA</c>), exactly <c>k</c> octets wide (the modulus width), in an owned pooled carrier; ownership flows to the framed response and is released once framed.</param>
/// <param name="Request">The original parsed request, threaded through so a refusal can be blamed on the right form's session slot.</param>
/// <param name="Nature">Whether <see cref="ResponseCode"/> is attributable to <c>message</c> (designated) or to the operation itself (left at N = 0) — see <see cref="TpmRsaEncryptRefusalNature"/>. Irrelevant on <c>TPM_RC_SUCCESS</c>.</param>
public sealed record TpmRsaEncrypted(
    TpmRcConstants ResponseCode,
    Tpm2bPublicKeyRsa OutData,
    TpmSimulatorInput Request,
    TpmRsaEncryptRefusalNature Nature = TpmRsaEncryptRefusalNature.Operation): TpmSimulatorInput, IDisposable
{
    /// <summary>
    /// Releases the owned <see cref="OutData"/> carrier on a refusing path (the dispose-immune empty sentinel,
    /// a no-op); the framing path transfers it to the response intent instead and never calls this.
    /// </summary>
    public void Dispose() => OutData.Dispose();
}

/// <summary>
/// Whether a <see cref="TpmRsaEncrypted"/> refusal can be attributed to a specific <c>TPM2_RSA_Encrypt()</c>
/// command field (TPM 2.0 Library Part 2, clause 6.6.2, Table 15's last sentence: "If an implementation is not
/// able to designate the handle, session, or parameter in error, then P and N will be zero"). The effect
/// carries this alongside <see cref="TpmRsaEncrypted.ResponseCode"/> so the continuation designates the
/// response without re-deriving it from the code value or any message text.
/// </summary>
public enum TpmRsaEncryptRefusalNature
{
    /// <summary>
    /// The refusal is a property of the operation itself, not of any field the caller supplied — the signing
    /// backend faulted, or produced a result of the wrong width. Table 15's last sentence: N stays zero.
    /// </summary>
    Operation,

    /// <summary>
    /// The refusal is a property of <c>message</c> alone — its width or numeric value judged against the key's
    /// modulus and selected scheme (TPM2_RSA_Encrypt()'s first parameter, Table 44).
    /// </summary>
    Message
}

/// <summary>
/// A <c>TPM2_RSA_Decrypt()</c> command (TPM 2.0 Library Part 3, clause 14.3, Table 46): RSA-decrypts
/// <c>cipherText</c> with the private portion of an RSA key under a padding scheme selected between the key's
/// own retained scheme and <c>inScheme</c> (Table 42). <c>@keyHandle</c> carries Auth Index 1, Auth Role USER —
/// "This command uses the private key of keyHandle for this operation and authorization is required"
/// (clause 14.3.1) — so <c>TPM_ST_NO_SESSIONS</c> is never admitted for this command at all
/// (<see cref="TpmiDhObject"/>'s host mirrors <see cref="TpmSignRequested.KeyHandle"/>'s own authorized shape).
/// </summary>
/// <param name="KeyHandle">The handle of the decrypting key — a transient or persistent RSA key, a loaded KEYEDHASH object, or an open hash sequence context, each running its own authorization ladder before the command's own "required to be an RSA key" gate (<c>TPM_RC_KEY</c>) is ever reached.</param>
/// <param name="SuppliedKeyPassword">
/// The plaintext authorization value the caller supplied for the <c>keyHandle</c> slot, compared against the
/// resolved entity's own retained credential — <see cref="TransientKeyState.AuthValue"/>,
/// <see cref="KeyedHashObjectState.UserAuth"/>, or <see cref="SequenceObjectState.AuthValue"/> — both sides
/// trailing-zero-stripped (TPM 2.0 Library Part 1, clause 16.6.4.3), exactly as <see cref="TpmSignRequested.SuppliedKeyPassword"/>
/// is compared. An owned pooled <see cref="Tpm2bAuth"/> carrier rented at parse; the consuming transition is its
/// terminal owner. The dispose-immune empty sentinel for an empty password.
/// </param>
/// <param name="CipherText">
/// The ciphertext to decrypt (<c>TPM2B_PUBLIC_KEY_RSA</c>, TPM 2.0 Library Part 2, clause 11.2.4.6, Table 194),
/// an owned pooled carrier rented as the parse's last act. TRANSFERRED into <see cref="Automata.TpmRsaDecryptAction"/>
/// on acceptance, whose effect is its terminal owner; released through this record's
/// <see cref="IDisposable.Dispose"/> on every refusing path. Not secret — the wire ciphertext, never the
/// recovered plaintext the response carries.
/// </param>
/// <param name="InScheme">The padding scheme to apply if the key's own scheme is <c>TPM_ALG_NULL</c> (<c>TPMT_RSA_DECRYPT+</c>, Table 192/193).</param>
/// <param name="Label">
/// The label whose association with the message is to be verified (<c>TPM2B_DATA</c>, TPM 2.0 Library Part 2,
/// clause 10.3.3, Table 91), an owned pooled carrier rented at parse. TRANSFERRED into
/// <see cref="Automata.TpmRsaDecryptAction"/> on acceptance; released through this record's
/// <see cref="IDisposable.Dispose"/> on every refusing path.
/// </param>
public sealed record TpmRsaDecryptRequested(
    TpmiDhObject KeyHandle,
    Tpm2bAuth SuppliedKeyPassword,
...
{
    /// <summary>
    /// Releases the owned <see cref="SuppliedKeyPassword"/>, <see cref="CipherText"/> and <see cref="Label"/>
    /// carriers on a refusing path; the accepting transition instead disposes <see cref="SuppliedKeyPassword"/>
    /// itself once the slot's compare has consumed it and transfers <see cref="CipherText"/>/<see cref="Label"/>
    /// into <see cref="Automata.TpmRsaDecryptAction"/>, and never calls this.
    /// </summary>
    public void Dispose()
    {
        SuppliedKeyPassword.Dispose();
...
/// <summary>
/// The session-authorized form of <c>TPM2_RSA_Decrypt()</c> (TPM 2.0 Library Part 3, clause 14.3, Table 46):
/// <c>TPM2_Sign()</c>'s own authorized-slot shape — the wire shape <see cref="TpmRsaDecryptRequested"/> takes
/// when the <c>@keyHandle</c> slot is a real session, or a <c>TPM_RS_PW</c> slot rides alongside a companion.
/// The authorizing slot is <see cref="Area"/>'s first block; up to two companions may follow it (Part 1, clause
/// 15.6.1, Table 12).
/// </summary>
/// <remarks>
/// <see cref="CipherText"/> is the dispose-immune empty sentinel until the first-parameter decryption step
/// supplies the recovered value: the parse only steps over the field's framing on this form, because a decrypt
/// session may have left it ciphertext and cpHash covers the octets as received (Part 1, clause 18.1). The
/// completing transition is the terminal owner of every slot's supplied hmac and of
/// <see cref="RawParameterArea"/>, transfers each slot's caller nonce into its response entry and
/// <see cref="CipherText"/>/<see cref="Label"/> into <see cref="Automata.TpmRsaDecryptAction"/>, and every
/// refusing arm releases everything through <see cref="IDisposable.Dispose"/>.
/// </remarks>
/// <param name="KeyHandle">The handle of the decrypting key — a transient or persistent RSA key, a loaded KEYEDHASH object, or an open hash sequence context.</param>
/// <param name="Area">The authorization area: the key slot first, then any companions; owned.</param>
/// <param name="CipherText">The ciphertext to decrypt (<c>TPM2B_PUBLIC_KEY_RSA</c>), recovered by the decryption step into an owned pooled carrier; the empty sentinel until then.</param>
/// <param name="InScheme">The padding scheme selector <c>inScheme</c> carried, decoded eagerly at parse (it sits behind <see cref="CipherText"/> and is never encrypted).</param>
/// <param name="Label">The label associated with the message (<c>TPM2B_DATA</c>), an owned pooled carrier rented eagerly at parse; TRANSFERRED into <see cref="Automata.TpmRsaDecryptAction"/> on acceptance; released through this record's <see cref="IDisposable.Dispose"/> on every refusing path.</param>
/// <param name="RawParameterArea">The parameter octets exactly as received — cpHash's parameters term (Part 1, clause 15.7 equation 15), and the buffer the decryption step transforms in place; owned.</param>
/// <param name="ResolvedAuthValues">The bind-omission-resolved authValue term each authorizing slot's command HMAC used (Part 1, clause 16.6.10 equation 22), in slot order, so the response HMAC keys on the identical term; borrowed references. Empty until the entry transition resolves them.</param>
public sealed record TpmRsaDecryptOverSessionRequested(
    TpmiDhObject KeyHandle,
    TpmAuthorizationArea Area,
    Tpm2bPublicKeyRsa CipherText,
    TpmtRsaDecrypt InScheme,
...
{
    /// <summary>
    /// Releases the owned area, the recovered ciphertext, the label, and the raw parameter area on a refusing
    /// path.
    /// </summary>
    public void Dispose()
    {
...
/// <summary>
/// The result of executing a <see cref="Automata.TpmRsaDecryptAction"/>: the recovered plaintext
/// <c>message</c> the effect produced (TPM 2.0 Library Part 3, clause 14.3, Table 47), fed back so the
/// transition can frame the plain response. Internal to the effect loop; never arrives from the command
/// transport.
/// </summary>
/// <remarks>
/// The session form never arrives here on success: the effect frames it directly through the shared
/// over-sessions machinery (<see cref="TpmResponseFramedOverSessions"/>/<see cref="TpmOverSessionsResponse"/>),
/// mirroring <c>TPM2_ObjectChangeAuth()</c>'s HMAC-authorized completion — the same precedent
/// <see cref="TpmRsaEncrypted"/> follows. A refusal on either form — reached before that machinery runs — still
/// arrives here, so <see cref="Request"/> may be either <see cref="TpmRsaDecryptRequested"/> or
/// <see cref="TpmRsaDecryptOverSessionRequested"/>.
/// </remarks>
/// <param name="ResponseCode"><c>TPM_RC_SUCCESS</c> with <see cref="Message"/> populated, or the effect's own refusal (<c>TPM_RC_VALUE</c> on a value-range or padding failure) — then <see cref="Message"/> is the dispose-immune <see cref="Tpm2bPublicKeyRsa.Empty"/> sentinel.</param>
/// <param name="Message">The recovered plaintext (<c>TPM2B_PUBLIC_KEY_RSA</c>) in an owned pooled carrier; ownership flows to the framed response and is released once framed.</param>
/// <param name="Request">The original parsed request, threaded through so a refusal can be blamed on the right form's session slot.</param>
public sealed record TpmRsaDecrypted(
    TpmRcConstants ResponseCode,
    Tpm2bPublicKeyRsa Message,
    TpmSimulatorInput Request): TpmSimulatorInput, IDisposable
{
    /// <summary>
    /// Releases the owned <see cref="Message"/> carrier on a refusing path, zeroing it first — the recovered
    /// plaintext is never left in a freed pooled buffer, matching every other terminal owner of this response's
    /// <c>message</c> (the clear-before-dispose discipline). A no-op for the dispose-immune empty sentinel
    /// every refusal carries; the framing path transfers <see cref="Message"/> to the response intent instead
...
/// clause 10.3.2, Table 90), compared constant-time against the recomputed ticket — an owned pooled carrier
...
/// <summary>
/// A <c>TPM2_ClockRateAdjust()</c> command (TPM 2.0 Library Part 3, clause 29.3): adjusts the rate at which
/// <c>Clock</c> and <c>Time</c> advance, relative to the current rate rather than the nominal one (clause
/// 29.3.1). It carries a single provisioning-hierarchy handle and one parameter, so the authorization handle
/// and the requested adjustment are the whole of what a caller supplies beyond the authorization area.
/// </summary>
/// <remarks>
/// Only <c>TPM_RH_OWNER</c> and <c>TPM_RH_PLATFORM</c> are admissible (<c>TPMI_RH_PROVISION</c>, TPM 2.0 Library
/// Part 2, clause 9.21, Table 67), judged against <see cref="TpmiRhProvision.IsProvision(uint)"/> at the
/// transition.
/// </remarks>
/// <param name="AuthHandle">The authorizing hierarchy handle, <c>TPM_RH_OWNER</c> or <c>TPM_RH_PLATFORM</c>.</param>
/// <param name="AuthSupplied">The authorization value the caller supplied — the password session's plaintext authValue, which is the same <c>TPM2B_AUTH</c> wire field a real session carries an HMAC in (TPM 2.0 Library Part 2, clause 10.12.2, Table 156: "either an HMAC, a password, or an EmptyAuth") — in a pooled carrier this record OWNS, rented as the parse's last act. It authorizes the owner or the platform hierarchy. The authorizing transition is its terminal owner; every refusing path releases it through <see cref="IDisposable.Dispose"/>.</param>
/// <param name="RateAdjust">The requested rate adjustment (<c>TPM_CLOCK_ADJUST</c>, TPM 2.0 Library Part 2, clause 6.7, Table 19), read from the wire unchecked; whether it names one of the seven defined steps is judged at the transition (<see cref="TpmClockAdjustConstantsExtensions.IsDefined(TpmClockAdjustConstants)"/>), not here.</param>
public sealed record TpmClockRateAdjustRequested(
    TpmiRhProvision AuthHandle,
    Tpm2bAuth AuthSupplied,
    TpmClockAdjustConstants RateAdjust): TpmSimulatorInput, IDisposable
{
    /// <summary>
    /// Releases the owned <see cref="AuthSupplied"/> carrier on a refusing path; the authorizing transition
    /// disposes it per carrier once the compare that is its only use has run.
    /// </summary>
    public void Dispose()
    {
...
/// <summary>
/// A <c>TPM2_ClockRateAdjust()</c> command authorized over an HMAC session (TPM 2.0 Library Part 3, clause
/// 29.3).
/// </summary>
/// <param name="AuthHandle">The authorizing hierarchy handle, <c>TPM_RH_OWNER</c> or <c>TPM_RH_PLATFORM</c>.</param>
...
/// <param name="Hmac">The supplied command <c>hmac</c> field (<c>TPM2B_AUTH</c>, TPM 2.0 Library Part 2, clause 10.12.2, Table 156), in a pooled carrier this record OWNS, rented as the parse's last act. Everything downstream BORROWS it — the verification queue reads it at the HMAC primitive and disposes nothing — so the accepting continuation is its terminal owner; every refusing path releases it through <see cref="IDisposable.Dispose"/>.</param>
/// <param name="RawParameterArea">The raw parameter-area bytes exactly as received — the single <c>rateAdjust</c> octet — captured whole so cpHash's parameters term is built the same way for every session-authorized command (Part 1, clause 15.7 equation 15). Held in a pooled carrier this record OWNS, rented as the parse's last act; released through <see cref="IDisposable.Dispose"/> on every refusing path and by the accepting continuation once the command has been framed.</param>
/// <param name="RateAdjust">The requested rate adjustment, read from the wire unchecked; membership is judged at the transition, after the command HMAC verifies.</param>
...
public sealed record TpmClockRateAdjustOverSessionRequested(
    TpmiRhProvision AuthHandle,
    TpmiShAuthSession AuthorizingSessionHandle,
    Tpm2bNonce NonceCaller,
    TpmaSession SessionAttributes,
...
/// <see cref="NewTime"/>, authorized by the owner or the platform hierarchy — the same
/// <c>TPMI_RH_PROVISION</c> handle shape <c>TPM2_ClockRateAdjust()</c> carries (Part 3, Table 234; Part 2,
/// clause 9.21, Table 67). It carries a single provisioning-hierarchy handle and one parameter, so the
/// authorization handle and the requested time are the whole of what a caller supplies beyond the
/// authorization area.
/// </summary>
/// <remarks>
/// Only <c>TPM_RH_OWNER</c> and <c>TPM_RH_PLATFORM</c> are admissible, judged against
/// <see cref="TpmiRhProvision.IsProvision(uint)"/> at the transition.
/// </remarks>
/// <param name="AuthHandle">The authorizing hierarchy handle, <c>TPM_RH_OWNER</c> or <c>TPM_RH_PLATFORM</c>.</param>
/// <param name="AuthSupplied">The authorization value the caller supplied — the password session's plaintext authValue, which is the same <c>TPM2B_AUTH</c> wire field a real session carries an HMAC in (TPM 2.0 Library Part 2, clause 10.12.2, Table 156: "either an HMAC, a password, or an EmptyAuth") — in a pooled carrier this record OWNS, rented as the parse's last act. It authorizes the owner or the platform hierarchy. The authorizing transition is its terminal owner; every refusing path releases it through <see cref="IDisposable.Dispose"/>.</param>
...
/// <summary>
/// A <c>TPM2_ClockSet()</c> command authorized over an HMAC session (TPM 2.0 Library Part 3, clause 29.2).
/// </summary>
/// <param name="AuthHandle">The authorizing hierarchy handle, <c>TPM_RH_OWNER</c> or <c>TPM_RH_PLATFORM</c>.</param>
/// <param name="AuthorizingSessionHandle">The HMAC session presented to authorize <paramref name="AuthHandle"/>.</param>
/// <param name="NonceCaller">The session's caller nonce for this command (<c>TPM2B_NONCE</c>, TPM 2.0 Library Part 2, clause 10.3.4, Table 92), in a pooled carrier this record OWNS, rented as the parse's last act. Every refusing path releases it through <see cref="IDisposable.Dispose"/>; the accepting continuation TRANSFERS it into the response-framing step, whose effect releases it once the response HMAC has keyed its nonceOlder term on it.</param>
/// <param name="SessionAttributes">The session's command session-attributes octet.</param>
/// <param name="Hmac">The supplied command <c>hmac</c> field (<c>TPM2B_AUTH</c>, TPM 2.0 Library Part 2, clause 10.12.2, Table 156), in a pooled carrier this record OWNS, rented as the parse's last act. Everything downstream BORROWS it — the verification queue reads it at the HMAC primitive and disposes nothing — so the accepting continuation is its terminal owner; every refusing path releases it through <see cref="IDisposable.Dispose"/>.</param>
/// <param name="RawParameterArea">The raw parameter-area bytes exactly as received — the eight-octet <c>newTime</c> — captured whole so cpHash's parameters term is built the same way for every session-authorized command (Part 1, clause 15.7 equation 15). Held in a pooled carrier this record OWNS, rented as the parse's last act; released through <see cref="IDisposable.Dispose"/> on every refusing path and by the accepting continuation once the command has been framed.</param>
/// <param name="NewTime">The requested new <c>Clock</c> value, in milliseconds, read from the wire directly (a bare <c>UINT64</c> carries no membership to judge).</param>
/// <param name="ResolvedAuthValue">The authValue term folded into the command HMAC key, resolved at the entry transition and reused for the response key (the command changes no authValue, so both directions agree).</param>
public sealed record TpmClockSetOverSessionRequested(
    TpmiRhProvision AuthHandle,
    TpmiShAuthSession AuthorizingSessionHandle,
    Tpm2bNonce NonceCaller,
    TpmaSession SessionAttributes,
...
/// gates it.
/// </summary>
/// <param name="LockHandle">The authorization handle (<c>TPMI_RH_LOCKOUT</c>); only <c>TPM_RH_LOCKOUT</c> is accepted.</param>
...
/// 10.3.4, Table 92), in an owned pooled carrier rented as part of the parse's last act; transferred into the
/// response-session entry the continuation builds, whose attest effect is its terminal owner; every refusing arm
/// releases it through this record's <see cref="IDisposable.Dispose"/>.
/// </param>
...
/// 15.6.1, Table 12 — "authorization sessions come before sessions used only for encryption, decryption, or
...
/// <summary>
/// A <c>TPM2_GetSessionAuditDigest()</c> command (TPM 2.0 Library Part 3, clause 18.5): a signing key attests an
/// audit session's digest and exclusive status, over a caller nonce — the same shape as
/// <see cref="TpmGetTimeRequested"/> plus a third, unauthorized handle naming the audited session
/// (<see cref="SessionHandle"/>). Requires Endorsement authorization on <see cref="PrivacyAdminHandle"/> in
/// addition to <see cref="SignHandle"/>'s own authorization, so the parser consumes two password sessions in
/// handle order (TPM 2.0 Library Part 1, clause 17.6: "Both TPM2_GetSessionAuditDigest() and
/// TPM2_GetCommandAuditDigest() require Endorsement Authorization").
/// </summary>
/// <param name="PrivacyAdminHandle">The privacy administrator handle (TPMI_RH_ENDORSEMENT); only <c>TPM_RH_ENDORSEMENT</c> is a legal value.</param>
/// <param name="SuppliedPrivacyAdminPassword">
/// The plaintext authorization value the caller supplied for the privacy-administrator slot (session 0's
...
/// it once the hierarchy compare has consumed it. The dispose-immune empty sentinel for an empty password.
/// </param>
/// <param name="SignHandle">
/// The loaded signing key that attests, whose retained private key signs the marshaled attestation — or
/// <c>TPM_RH_NULL</c>, in which case every action of the command still runs and the attestation is "signed"
/// with the NULL Signature (TPM 2.0 Library Part 3, clause 18.1).
/// </param>
/// <param name="SuppliedSignPassword">
/// The plaintext authorization value the caller supplied for the signing-key slot (session 1's <c>hmac</c>
/// field), compared against the key's retained <see cref="TransientKeyState.AuthValue"/> — or, for
/// <c>TPM_RH_NULL</c>, against the Empty Buffer (TPM 2.0 Library Part 4, <c>Entity.c</c>: a <c>TPM_RH_NULL</c>
/// entity's authValue is empty) — both sides trailing-zero-stripped, rather than discarded. An owned pooled
/// <see cref="Tpm2bAuth"/> carrier rented at parse; the consuming transition is its terminal owner. The
/// dispose-immune empty sentinel for an empty password.
/// </param>
/// <param name="SessionHandle">
/// The audit session whose digest and exclusive status are attested (<c>TPMI_SH_HMAC</c>, Table 55) — a third
/// handle in the area carrying no authorization of its own (Auth Index: None); its Name in cpHash is its own
/// 4-octet handle value (TPM 2.0 Library Part 1, Table 9).
/// </param>
/// <param name="QualifyingData">
/// The caller nonce echoed into the attestation's <c>extraData</c> (<c>TPM2B_DATA</c>, TPM 2.0 Library Part 2,
/// clause 10.3.3, Table 91), in an owned pooled carrier rented as the parse's last act. The consuming transition
/// transfers it into the session-audit-attestation action, whose effect is its terminal owner; every refusing
/// arm releases it through this record's <see cref="IDisposable.Dispose"/>.
/// </param>
/// <param name="SignatureScheme">The signing scheme algorithm (<c>TPM_ALG_ECDSA</c>, <c>TPM_ALG_RSASSA</c>, <c>TPM_ALG_RSAPSS</c>, or, for the NULL signer, <c>TPM_ALG_NULL</c> — TPM 2.0 Library Part 3, clause 18.1's own note for <c>signHandle == TPM_RH_NULL</c>).</param>
/// <param name="SchemeHashAlg">The signing scheme's hash algorithm — likewise NULL-admitted for the NULL signer.</param>
public sealed record TpmGetSessionAuditDigestRequested(
    TpmiRhEndorsement PrivacyAdminHandle,
    Tpm2bAuth SuppliedPrivacyAdminPassword,
    TpmiDhObject SignHandle,
    Tpm2bAuth SuppliedSignPassword,
    ...
    TpmiAlgHash SchemeHashAlg): TpmSimulatorInput, IDisposable
{
    ...
    /// Releases the request's owned carriers — both slots' supplied passwords and the qualifying data — on a
    /// refusing path. The consuming transition instead releases the two passwords itself once the per-slot
    /// compares have consumed them and transfers the qualifying data into the session-audit-attestation action,
...
/// <summary>
/// The result of executing a <see cref="TpmSessionAuditAttestAction"/>, <see cref="TpmRsaSessionAuditAttestAction"/>,
/// or <see cref="TpmNullSignedSessionAuditAttestAction"/>: the marshaled <c>TPMS_ATTEST</c> the effectful loop
/// built and the signature over its digest — the NULL Signature for the NULL signer — fed back so the transition
/// can frame the <c>TPM2_GetSessionAuditDigest()</c> response. Internal to the effect loop; never arrives from
/// the command transport.
/// </summary>
/// <param name="AuditInfo">The <c>TPM2B_ATTEST</c> over the marshaled <c>TPMS_ATTEST</c> (the exact bytes the signature is over, or that a NULL Signature carries nothing over); ownership flows to the <c>TpmGetSessionAuditDigestResponse</c> and is released by <see cref="TpmSimulator"/> once framed.</param>
/// <param name="Signature">The <c>TPMT_SIGNATURE</c> over <c>H_hashAlg(auditInfo)</c>, or the dispose-immune <see cref="TpmtSignature.Null"/> for the NULL signer; ownership flows to the <c>TpmGetSessionAuditDigestResponse</c> and is released once framed.</param>
public sealed record TpmSessionAuditAttested(
    Tpm2bAttest AuditInfo,
    TpmtSignature Signature): TpmSimulatorInput;

/// <summary>
/// A session-authorized <c>TPM2_GetSessionAuditDigest()</c> command (TPM 2.0 Library Part 3, clause 18.5): the
/// attest-family counterpart of <see cref="TpmGetSessionAuditDigestRequested"/> for an area at least one of
/// whose two authorizing slots names a real HMAC session — <c>@privacyAdminHandle</c> (Auth Index 1, USER role;
/// fixed to <c>TPM_RH_ENDORSEMENT</c>) then <c>@signHandle</c> (Auth Index 2, USER role) — plus the third,
/// unauthorized <see cref="SessionHandle"/> handle Table 103 carries beside them. The privacy-administrator slot
/// authorizes a HIERARCHY, so its bind Name is the 4-octet handle value; the parser forks here whenever the area
/// is not all-password.
/// </summary>
/// <param name="PrivacyAdminHandle">The privacy administrator handle (<c>TPMI_RH_ENDORSEMENT</c>); only <c>TPM_RH_ENDORSEMENT</c> is legal. Its 4-octet handle value is cpHash's <c>Name1</c> term (Part 1, clause 13, Table 9).</param>
/// <param name="PrivacyAdminSessionHandle">The session authorizing <paramref name="PrivacyAdminHandle"/>: <c>TPM_RS_PW</c> for a password slot, otherwise a real HMAC session handle.</param>
...
/// <param name="PrivacyAdminSessionAttributes">The privacy-administrator slot's command session-attributes octet.</param>
/// <param name="SuppliedPrivacyAdminHmac">The privacy-administrator slot's supplied <c>hmac</c> — the plaintext authValue for a password slot, the command HMAC for a real session — an owned pooled <see cref="Tpm2bAuth"/> carrier rented at parse; the consuming continuation is its terminal owner, refusing arms release it through <see cref="IDisposable.Dispose"/>.</param>
/// <param name="SignHandle">The loaded signing key that attests — cpHash's <c>Name2</c> term — or <c>TPM_RH_NULL</c>, whose Name term is its own 4-octet handle value; authorized in the USER role by the sign slot either way.</param>
/// <param name="SignSessionHandle">The session authorizing <paramref name="SignHandle"/>: <c>TPM_RS_PW</c> for a password slot, otherwise a real HMAC session handle.</param>
/// <param name="SignNonceCaller">
/// The sign slot's caller nonce for this command (<c>TPM2B_NONCE</c>, TPM 2.0 Library Part 2, clause 10.3.4,
/// Table 92), in an owned pooled carrier rented as part of the parse's last act; transferred into the
...
/// <param name="SessionHandle">
/// The audit session whose digest and exclusive status are attested (<c>TPMI_SH_HMAC</c>, Table 55) — a third
/// handle in the area carrying no authorization of its own (Auth Index: None); cpHash's <c>Name3</c> term is its
/// own 4-octet handle value (Part 1, Table 9).
/// </param>
/// <param name="QualifyingData">
/// The caller nonce echoed into the attestation's <c>extraData</c> (<c>TPM2B_DATA</c>, TPM 2.0 Library Part 2,
/// clause 10.3.3, Table 91), in an owned pooled carrier rented as the parse's last act. The consuming
/// continuation transfers it into the session-audit-attestation action, whose effect is its terminal owner;
/// every refusing arm releases it through this record's <see cref="IDisposable.Dispose"/>.
/// </param>
/// <param name="SignatureScheme">The signing scheme algorithm (dispatched on the signing key's type, or, for the NULL signer, admitted verbatim including <c>TPM_ALG_NULL</c>).</param>
/// <param name="SchemeHashAlg">The signing scheme's hash algorithm — likewise NULL-admitted for the NULL signer.</param>
/// <param name="RawParameterArea">The <c>qualifyingData ‖ inScheme</c> wire bytes captured verbatim at parse time — cpHash's <c>parameters</c> term. Held in a pooled carrier this record OWNS, rented as the parse's last act; released through <see cref="IDisposable.Dispose"/> on every refusing path and by the accepting continuation once the command has been framed.</param>
/// <param name="HasCompanionSlot">
/// Whether the authorization area actually carried the slot at the index after the last authorizing one. The
...
/// </param>
/// <param name="ResolvedPrivacyAdminAuthValue">The bind-omission-resolved authValue term the privacy-administrator slot's command HMAC used (the endorsement hierarchy's authValue, or empty when bound to it) — a borrowed reference the response framing reuses; <see langword="null"/> until resolved and for a password slot, read as empty.</param>
/// <param name="ResolvedSignAuthValue">The bind-omission-resolved authValue term the sign slot's command HMAC used (the signing key's authValue, or the Empty Buffer for the NULL signer) — a borrowed reference the response framing reuses; <see langword="null"/> until resolved and for a password sign slot, read as empty.</param>
public sealed record TpmGetSessionAuditDigestOverSessionRequested(
    TpmiRhEndorsement PrivacyAdminHandle,
    TpmiShAuthSession PrivacyAdminSessionHandle,
    Tpm2bNonce PrivacyAdminNonceCaller,
    TpmaSession PrivacyAdminSessionAttributes,
    ...
    Tpm2bAuth? ResolvedSignAuthValue = null): TpmSimulatorInput, IDisposable
{
    ...
    /// qualifying data — on a refusing path. The consuming continuation instead releases the supplied HMACs
    /// itself as their terminal owner and transfers each slot's nonce into that slot's response-session entry and
    /// the qualifying data into the session-audit-attestation action, so it never calls this. The
    /// resolved-authValue references are borrowed and never released here.
    /// </summary>
    public void Dispose()
    {
        SuppliedPrivacyAdminHmac.Dispose();
...
/// <param name="NewAuditDigest">The extended audit digest (TPM 2.0 Library Part 1, clause 17.1, equation 30), OWNED, when this is the ONE entry whose session claims <c>audit</c>; <see langword="null"/> for every other entry. Released by <c>RollHmacSessionNonceAndAudit</c>'s adoption onto the durable session record, or by the entry's own release on a fault ahead of that adoption.</param>
...
/// <param name="ResponseHandle">The response handle a no-authorization command's <c>TPMA_CC.R_HANDLE</c> row carries (TPM 2.0 Library Part 3, clause 4.3), written ahead of <c>parameterSize</c> and excluded from rpHash (TPM 2.0 Library Part 1, clause 15.8), or <see langword="null"/> for every command that returns no handle.</param>
public sealed record TpmResponseFramedOverSessions(
    ...
    TpmHandle? ResponseHandle = null): TpmSimulatorInput;
...
/// (Table 120); this simulator admits a <c>TPM_ST_SESSIONS</c> frame through the shared no-authorization
/// session admission (TPM 2.0 Library Part 3, clause 5.5), an audit-only companion succeeding and a decrypt
/// companion protecting <see cref="Context"/>, this digest-only form's first command parameter.
/// </summary>
/// <param name="KeyHandle">The loaded key whose public part verifies the signature.</param>
...
/// </summary>
/// <param name="ResponseCode"><c>TPM_RC_SUCCESS</c> when the signature verified; otherwise <c>TPM_RC_SIGNATURE</c>.</param>
/// <param name="Validation">The minted <c>TPMT_TK_VERIFIED</c> — the whole single response parameter of <c>TPM2_VerifyDigestSignature()</c> (TPM 2.0 Library Part 3, clause 20.4, Table 121), tagged <c>TPM_ST_DIGEST_VERIFIED</c> with the verified scheme's hash algorithm as its metadata (Part 2, clause 10.6.4, Table 111; clause 10.6.5, Table 113); an owned carrier whose ownership flows to the <c>TpmVerifyDigestSignatureResponse</c> and is released by <see cref="TpmSimulator"/> once framed. <see langword="null"/> when the signature did not verify, where no ticket is framed at all.</param>
...
/// <c>authObject</c>'s signature (TPM 2.0 Library Part 3, clause 23.3), and, on success, the real
...
/// (clause 11.1.1: "For all session types, this command will cause initialization of the sessionKey") — only the
...
/// <summary>
/// A no-authorization command carried over an authorization area of 1-3 companion slots (TPM 2.0 Library Part 3,
/// clause 4.3; Part 1, clause 15.6.1) — the ONE generic wrapper every no-authorization session-admitting command
/// routes through: the three zero-handle commands (<c>TPM2_GetRandom()</c>, <c>TPM2_StirRandom()</c>,
/// <c>TPM2_TestParms()</c>), the six commands whose plain parsers once dropped or misread a session area
/// silently (<c>TPM2_ReadClock()</c>, <c>TPM2_Shutdown()</c>, <c>TPM2_SelfTest()</c>, <c>TPM2_GetTestResult()</c>,
/// <c>TPM2_GetCapability()</c>, <c>TPM2_PCR_Read()</c>), <c>TPM2_ReadPublic()</c> and
/// <c>TPM2_NV_ReadPublic()</c>, the eight key commands (<c>TPM2_Hash()</c>, <c>TPM2_HashSequenceStart()</c>,
/// <c>TPM2_SignSequenceStart()</c>, <c>TPM2_VerifySequenceStart()</c>, <c>TPM2_VerifySignature()</c>,
/// <c>TPM2_VerifyDigestSignature()</c>, <c>TPM2_Encapsulate()</c>, <c>TPM2_MakeCredential()</c>), and
/// <c>TPM2_LoadExternal()</c> and <c>TPM2_RSA_Encrypt()</c> — since none of them has an <c>@</c>-marked handle
/// for a slot to authorize: every slot is a companion, riding the
/// area for its own <c>decrypt</c>, <c>encrypt</c>, or <c>audit</c> attribute alone (TPM 2.0 Library Part 1,
/// clause 15.6.4).
/// </summary>
/// <remarks>
/// The parse reads only the authorization area and captures the raw parameter octets: clause 5.5's session-area
/// checks precede clause 5.8's parameter unmarshaling (TPM 2.0 Library Part 3), so the command's own parameters
/// are neither decoded nor judged before every slot has verified. <see cref="Inner"/> is therefore
/// <see langword="null"/> from the parse until the parameter step installs it — the shared first-parameter
/// decryption step where a slot claims <c>decrypt</c> (its effect transforms <see cref="RawParameterArea"/> in
/// place and then runs the command's own parameter core over the plaintext), the plain parameter step
/// otherwise — and the completing transition dispatches the installed request as the command's own.
/// </remarks>
/// <param name="CommandCode">The wrapped command.</param>
/// <param name="Inner">
/// The command's own plain request once the parameter step has parsed it from <see cref="RawParameterArea"/>;
/// <see langword="null"/> until then. OWNED until the wrapper's completing transition
/// (<c>CompleteNoAuthOverSessions</c>) hands it onward to the command's own dispatch, which becomes its sole
/// owner from there; every refusing path releases it through <see cref="IDisposable.Dispose"/> when it
/// implements the interface.
/// </param>
/// <param name="Area">
/// The parsed authorization area, <see cref="TpmAuthorizationArea.AuthorizingCount"/> always zero since none of
/// this mechanism's handles carries the <c>@</c> decoration (<c>Auth Index: None</c>) — a slot may still be a
/// <c>decrypt</c>, <c>encrypt</c>, or <c>audit</c> companion. OWNED: the completing transition releases each
/// slot's supplied hmac through <see cref="TpmAuthorizationArea.ReleaseCredentials"/> and TRANSFERS each slot's
/// caller nonce into its response entry; every refusing path releases the whole area, nonces included, through
/// <see cref="IDisposable.Dispose"/>.
/// </param>
/// <param name="RawParameterArea">
/// The raw parameter-area octets as they arrived (still ciphertext, if a decrypt slot claims the attribute) —
/// cpHash's parameter term (Part 1, clause 15.7 equation 15), the buffer the decrypt step transforms in place,
/// and the octets the parameter step parses. OWNED: released through <see cref="IDisposable.Dispose"/> on every
/// refusing path, and by the completing transition once the command-HMAC verification and the parameter step
/// have both read it.
/// </param>
/// <param name="Handles">
/// The command's handle area, read in wire order ahead of the authorization area (TPM 2.0 Library Part 3,
/// clause 4.3: authorizationSize "shall immediately follow the handle area"), empty for a handle-less command
/// (<c>TPM2_GetRandom()</c>, <c>TPM2_StirRandom()</c>, <c>TPM2_TestParms()</c>, and every one of the six silent
/// parsers). Each handle is already validated against the command's own interface type at parse (TPM 2.0
/// Library Part 3, clause 5.4: <c>TPM_RC_VALUE</c> through the type's own range check). Values only: a handle
/// carries no owned carrier, so no disposal is owed here.
/// </param>
/// <param name="ComputedHandleName">
/// The handle's Name when it had to be COMPUTED for cpHash rather than read from a loaded entity's own record —
/// an NV Index's Name (<c>TPM2_NV_ReadPublic()</c>), produced by the digest seam and TRANSFERRED here by the
/// resuming transition; <see langword="null"/> for every command whose Name term borrows a loaded object's
/// retained Name and for a handle-less command. OWNED: the Name term the command-HMAC verification reads
/// borrows it; the completing transition releases it with <see cref="RawParameterArea"/> once verification has
/// read it for the last time, and every refusing path releases it through <see cref="IDisposable.Dispose"/>.
/// </param>
public sealed record TpmNoAuthOverSessionsRequested(
    TpmCcConstants CommandCode,
    TpmSimulatorInput? Inner,
    TpmAuthorizationArea Area,
    TpmParameterArea RawParameterArea,
    ImmutableArray<TpmHandle> Handles,
    Tpm2bName? ComputedHandleName = null): TpmSimulatorInput, IDisposable
{
    /// <summary>
    /// Releases the owned <see cref="Area"/> and <see cref="RawParameterArea"/> carriers, the
    /// <see cref="ComputedHandleName"/> when one was transferred in, and <see cref="Inner"/> when it is installed
    /// and owns anything of its own — the terminal release on every refusing path (a session-area gate, a
    /// mismatched command HMAC, or a parameter-step failure). The completing path never calls this: it releases
    /// <see cref="Area"/>'s credentials, <see cref="RawParameterArea"/> and <see cref="ComputedHandleName"/>
    /// individually instead, transferring each slot's nonce into the response framing and <see cref="Inner"/>
    /// onward into the command's own dispatch.
    /// </summary>
    public void Dispose()
    {
        Area.Dispose();
        RawParameterArea.Dispose();
        ComputedHandleName?.Dispose();
        (Inner as IDisposable)?.Dispose();
    }
}

/// <summary>
/// A <c>TPM2_StirRandom()</c> command (TPM 2.0 Library Part 3, clause 16.2): adds <see cref="InData"/> to the
/// RNG's reseed state. It carries no handle, and this <c>TPM_ST_NO_SESSIONS</c> form carries no authorization
/// area either.
/// </summary>
/// <param name="InData">
/// The additional input to fold (<c>TPM2B_SENSITIVE_DATA</c>, TPM 2.0 Library Part 2, clause 11.1.14, Table
/// 170), bounded at <see cref="Tpm2bSensitiveData.MaxSize"/> octets, in an owned pooled carrier rented as the
/// parse's last act (<see cref="Tpm2bSensitiveData"/>'s own parse bound). The
/// accepting transition TRANSFERS it into the <see cref="Automata.TpmStirRandomAction"/>, whose effect is its
/// terminal owner; every refusing path releases it through this record's <see cref="IDisposable.Dispose"/>.
/// </param>
public sealed record TpmStirRandomRequested(Tpm2bSensitiveData InData): TpmSimulatorInput, IDisposable
{
    /// <summary>
    /// Releases the owned <see cref="InData"/> carrier on a refusing path; the accepting transition disposes
    /// it through the stir action once the fold that is its only use has run.
    /// </summary>
    public void Dispose()
    {
        InData.Dispose();
...
/// <summary>
/// The result of executing a <see cref="Automata.TpmStirRandomAction"/>: <c>TPM2_StirRandom()</c>'s reseed has
/// already been folded into the simulator instance's RNG state by the effect, so the resuming transition need
/// only frame the plain, header-only response (TPM 2.0 Library Part 3, clause 16.2, Table 78) — the
/// no-authorization wrapper's own <c>ApplyPendingSessionFrame</c> hook frames the session-authorized form around
/// this SAME plain intent when the command arrived over an authorization area.
/// </summary>
/// <param name="CommandCode">The command being resumed — always <c>TPM_CC_StirRandom</c>.</param>
/// <param name="Request">The <see cref="TpmStirRandomRequested"/> the response is framed for, threaded through unread here.</param>
public sealed record TpmRandomStirred(TpmCcConstants CommandCode, TpmSimulatorInput Request): TpmSimulatorInput;

/// <summary>
/// A <c>TPM2_TestParms()</c> command (TPM 2.0 Library Part 3, clause 30.3): asks whether <see cref="Parameters"/>
/// is a combination of algorithm parameters this TPM supports. It carries no handle, and this
/// <c>TPM_ST_NO_SESSIONS</c> form carries no authorization area either.
/// </summary>
...
/// a trial session (Part 3, clause 23.4.1: "The authorization is checked even for a trial policy session")
/// — carried by <see cref="AuthValueSupplied"/>. A non-trial session additionally runs
/// <see cref="NonceTpm"/>/<see cref="Expiration"/>/<see cref="CpHashA"/> checks (clause 23.2.2) and, on
...
/// (clause 23.2.4); every other arm — an already-latched session, a trial session, and every refusal —
/// releases it, the refusals through this record's <see cref="IDisposable.Dispose"/>.
/// </param>
/// <param name="PolicyRef">
/// The opaque policy qualifier, folded into the policyDigest unconditionally (Part 3, clause 23.2.3), as the
...
/// rides a real session (TPM 2.0 Library Part 3, clause 23.4.1). Carried from
...
/// rather than a password (TPM 2.0 Library Part 3, clause 23.4.1: "A password session, an HMAC session, or a
...
/// <summary>
/// A <c>TPM2_PolicySigned()</c> command (TPM 2.0 Library Part 3, clause 23.3): binds a policy session to a
...
/// <summary>
/// A <c>TPM2_PolicyAuthorize()</c> command (TPM 2.0 Library Part 3, clause 23.16): authorizes a policy session
...
/// <param name="FoldedDigest">The replacement policyDigest the effect computed from the approving key's Name and the policy qualifier alone (<c>ExtendForAuthorize</c>, Part 3, clause 23.16) in an owned pooled carrier rented at the session's own digest width; the continuation installs it on the session, which becomes its owner. The dispose-immune empty sentinel on a failed re-verification, where nothing is folded.</param>
...
/// <param name="FoldedDigest">The policyDigest the effect folded from the authorizing entity's Name and the policy qualifier (<c>PolicyUpdate</c>, Part 3, clause 23.4) in an owned pooled carrier rented at the session's own digest width; the continuation installs it on the session, which becomes its owner.</param>
...
/// <summary>
/// A <c>TPM2_PolicyTicket()</c> command (TPM 2.0 Library Part 3, clause 23.5): authorizes a policy session by
...
/// after the trial-session rejection, the order Part 4's <c>TPM2_PolicyTicket()</c> gives it. The accepting
/// arm consumes the value and releases the
...
/// 10.3.2, Table 90) in an owned pooled carrier rented as the parse's last act, or the dispose-immune empty
...
/// 10.3.4, Table 92) in an owned pooled carrier rented as the parse's last act. The accepting arm transfers it
...
/// matched the caller-supplied one (TPM 2.0 Library Part 3, clause 23.5). A mismatch carries
...
/// <c>TPM_CC_PolicyTicket</c>-keyed one (Part 3, clause 23.5.1). Internal to the effect loop; never arrives
/// from the command transport.
/// </summary>
...
/// <param name="CpHashA">The cpHashA the request supplied in an owned pooled carrier, or the dispose-immune empty sentinel if unbound. On success, and only when non-empty and the session is unlatched, the continuation TRANSFERS it onto the session's first-writer-wins cpHash (Part 3, clause 23.2.4); on every other arm — an already-latched session and a failed re-verification alike — the continuation releases it, so a rejected <c>TPM2_PolicyTicket()</c> leaves the session exactly as it found it.</param>
/// <param name="Timeout">The de-flagged <c>authTimeout</c> magnitude the caller's wire timeout carried, recorded on the session on success per clause 23.2.4's min-with-existing rule.</param>
...
/// <summary>
/// A <c>TPM2_PolicyCounterTimer()</c> command (TPM 2.0 Library Part 3, clause 23.10): binds a policy to a
...
/// 10.3.5, Table 93) in an owned pooled carrier rented at parse; the consuming transition is its terminal owner,
/// releasing it once the authorization arm has consumed it. The dispose-immune empty sentinel for an empty
/// password.
/// </param>
...
/// </param>
/// <param name="TimeoutMagnitude">The deadline magnitude the session's own tracked timeout ranks under Part 3, clause 23.2.4's min-with-existing rule, threaded through unchanged.</param>
...
/// <summary>
/// A <c>TPM2_ContextSave()</c> command (TPM 2.0 Library Part 3, clause 28.2.1): "This command saves a
/// session context, object context, or sequence object context outside the TPM." The handle to save arrives
/// in the handle area (Table 224, <c>Auth Index: None</c>) and is the command's only content — the resource
/// it names is resolved against the five loaded-resource tables at the transition, never here.
/// </summary>
/// <param name="SaveHandle">
/// The session, transient-object, or sequence handle to save (<c>TPMI_DH_CONTEXT</c>, TPM 2.0 Library Part
/// 2, clause 9.11, Table 57). Owns nothing — a bare handle value, mirroring <see cref="TpmFlushContextRequested"/>'s
/// shape.
/// </param>
public sealed record TpmContextSaveRequested(TpmiDhContext SaveHandle): TpmSimulatorInput;

/// <summary>
/// The result of executing a <see cref="Automata.TpmContextSaveAction"/>: the framed, encrypted,
/// integrity-protected <c>TPMS_CONTEXT</c> the save effect produced (TPM 2.0 Library Part 1, clauses 27.3.1
/// and 27.3.2), fed back so the transition can move a saved session out of its live table and frame the
/// <c>TPM2_ContextSave()</c> response. Internal to the effect loop; never arrives from the command transport.
/// </summary>
/// <param name="ResponseCode">
...
/// </param>
/// <param name="Context">
/// The framed context (<c>TPMS_CONTEXT</c>), or <see langword="null"/> on any refusal. OWNED by this record
/// until <c>OnContextSaved</c> hands it, unwrapped, to the <c>TpmContextSaveResponse</c> intent it frames on
/// success — at which point this record's own <see cref="Dispose"/> is never called; every refusing path
/// calls it instead, releasing a populated <see cref="Context"/> (a no-op when it is <see langword="null"/>).
/// </param>
/// <param name="SaveHandle">The original request's handle (TPM 2.0 Library Part 2, clause 9.11, Table 57), read back by the transition to remove a saved session from its live table and key <see cref="TpmSimulatorState.SavedSessionContexts"/>.</param>
/// <param name="IsSession">Whether the saved resource is an HMAC or policy session (TPM 2.0 Library Part 1, clause 27.4 vs 27.5) — the transition removes and tracks a session, and changes no dictionary for an object or sequence.</param>
/// <param name="IsInternalFailure">
/// Whether the save effect's own cryptographic primitives (the KDFa, the cipher, the HMAC, or the hierarchy
/// proof derivation) threw rather than the caller's input provoking a refusal — Part 4's <c>FAIL</c>: a
/// broken TPM, not a bad parameter (distinct from a caller-provoked refusal, which never sets this).
/// </param>
/// <param name="Request">The original <see cref="TpmContextSaveRequested"/>, threaded through for symmetry with every other effect-result record; not read by <c>OnContextSaved</c>, which already carries everything it needs in this record's own fields.</param>
public sealed record TpmContextSaved(
    TpmRcConstants ResponseCode,
    TpmsContext? Context,
    TpmiDhContext SaveHandle,
    bool IsSession,
    bool IsInternalFailure,
    TpmSimulatorInput Request): TpmSimulatorInput, IDisposable
{
    /// <summary>
    /// Releases an un-installed <see cref="Context"/> on a refusing path; the completing transition instead
    /// unwraps it into the framed <c>TpmContextSaveResponse</c> and never calls this.
    /// </summary>
    public void Dispose()
    {
        Context?.Dispose();
...
/// <summary>
/// A <c>TPM2_ContextLoad()</c> command (TPM 2.0 Library Part 3, clause 28.3.1): "This command is used to
/// reload a context that has been saved by TPM2_ContextSave()." The whole <c>TPMS_CONTEXT</c> arrives as the
/// command's single parameter (Table 226) — no handle area, unlike <see cref="TpmContextSaveRequested"/>.
/// </summary>
/// <param name="Context">
/// The parsed context (<c>TPMS_CONTEXT</c>: sequence, savedHandle, hierarchy, and the encrypted,
/// integrity-protected blob) this record OWNS — rented as the parser's last act. TRANSFERRED into
/// <see cref="Automata.TpmContextLoadAction"/> on acceptance, whose effect is its terminal owner; released
/// through this record's <see cref="IDisposable.Dispose"/> on every refusing path.
/// </param>
...
/// <summary>
/// The result of executing a <see cref="Automata.TpmContextLoadAction"/>: the rebuilt resource the load effect
/// decrypted, integrity-verified, and deserialized (TPM 2.0 Library Part 3, clause 28.3.1; Part 1, clauses
/// 27.3.1, 27.4 and 27.5), fed back so the transition can install it — a session at its saved handle, an
/// object or sequence at a freshly drawn one — and frame the <c>TPM2_ContextLoad()</c> response. Internal to
/// the effect loop; never arrives from the command transport.
/// </summary>
/// <param name="ResponseCode">
/// <c>TPM_RC_SUCCESS</c> with <see cref="Resource"/> populated, <c>TPM_RC_SIZE</c> when the blob is shorter
/// than its own fixed framing or declares the wrong integrity width, <c>TPM_RC_INTEGRITY</c> when the
/// recomputed HMAC does not match the leading digest, or <c>TPM_RC_FAILURE</c> when <see cref="IsInternalFailure"/>
/// is set.
/// </param>
/// <param name="Resource">
/// The rebuilt resource envelope, or <see langword="null"/> on any refusal. OWNED by this record until
/// <c>OnContextLoaded</c> installs it into its own dictionary and never calls <see cref="Dispose"/>; every
/// refusing path (a range gate the transition itself judges, a load-once tracking mismatch, a disabled
/// hierarchy, or no free object slot) calls it instead, releasing a populated <see cref="Resource"/> (a no-op
/// when it is <see langword="null"/>).
/// </param>
/// <param name="SavedHandle">The context's <c>savedHandle</c> metadata (<c>TPMI_DH_SAVED</c>, TPM 2.0 Library Part 2, clause 9.12, Table 58), read back by the transition to resolve the load-once tracking entry or to skip it for an object/sequence.</param>
/// <param name="Hierarchy">The context's hierarchy metadata (<c>TPMI_RH_HIERARCHY+</c>), read back by the transition to judge <c>TPM_RC_HIERARCHY</c> for an object or sequence — never for a session or sequence context, which are always in the NULL hierarchy.</param>
/// <param name="Sequence">The context's own sequence number, read back to compare against the tracked value for a session's load-once check.</param>
/// <param name="IsInternalFailure">
/// Whether the load effect's own cryptographic primitives (the KDFa, the cipher, the HMAC, or the hierarchy
/// proof derivation) threw, or the decrypted fingerprint disagreed with <see cref="Sequence"/>, or the
/// deserialized resource's session/object kind disagreed with <see cref="SavedHandle"/>'s own kind — Part 4's
/// <c>FAIL</c>: a broken TPM or a tampered-but-somehow-integrity-passing blob, never something the caller's
/// input alone could provoke past the integrity gate.
/// </param>
/// <param name="Request">The original <see cref="TpmContextLoadRequested"/>, threaded through for symmetry with every other effect-result record; not read by <c>OnContextLoaded</c>, which already carries everything it needs in this record's own fields.</param>
public sealed record TpmContextLoaded(
    TpmRcConstants ResponseCode,
...
{
    /// <summary>
    /// Releases an un-installed <see cref="Resource"/> on a refusing path; the completing transition instead
    /// installs it into its own dictionary — a session at its saved handle, an object or sequence at a freshly
    /// drawn one — and never calls this.
    /// </summary>
    public void Dispose()
    {
        Resource?.Dispose();
...
/// (Part 3, clause 5.6); a policy-authorized activate object session is not modelled. A policy session on <c>keyHandle</c> instead parses as
...
/// 5.6). <c>activateHandle</c> stays password-authorized, exactly as in
/// <see cref="TpmActivateCredentialRequested"/>; a policy-authorized <c>activateHandle</c> is not modelled.
/// </summary>
...
/// The policy session's command session-attributes octet, judged by <see cref="TpmLifecycleTransitions.ValidateSessionArea"/>
/// before the credential is recovered — a policy session may not claim <c>audit</c> (TPM 2.0 Library Part 1,
/// Table 15). Unused by the response: <c>TPM2_ActivateCredential()</c>'s response is framed
/// <c>TPM_ST_NO_SESSIONS</c> regardless (see <see cref="TpmActivateCredentialResponse"/>), so there is no
/// response session entry to echo it into — the same simplification
...
/// <param name="CertInfo">The recovered credential secret in an owned <c>TPM2B_DIGEST</c> carrier, the type Table 27 gives <c>certInfo</c> (TPM 2.0 Library Part 3, clause 12.5.2, Table 27; TPM2B_DIGEST itself is Part 2, clause 10.3.2, Table 90); <see langword="null"/> on rejection; disposed after framing.</param>
...
/// a failure code rather than crash the simulator. <see cref="SizeFailureBlamesDecryptSession"/> distinguishes the
/// DECRYPT STEP'S OWN failure — <c>inSensitive</c>'s outer size field unreadable, or too wide for the parameter
/// area, session-index-encoded to the decrypt session when one was present (TPM 2.0 Library Part 2, clause
/// 6.6.2) — from every failure of the DECRYPTED content, already parameter-designated by the parser to
/// inSensitive (Table 18, index 0) and passed through unchanged. Internal to the effect loop; never arrives from
/// the command transport.
/// </summary>
...
/// Whether <paramref name="ResponseCode"/> is the decrypt step's OWN <c>TPM_RC_INSUFFICIENT</c>/<c>TPM_RC_SIZE</c>
/// over <c>inSensitive</c>'s own declared size while a decrypt session was present — the ONE failure
/// session-index-encoded to that session; every other rejection (the decrypted content's own, already
/// parameter-designated failure, or the SAME decrypt-step size problem with no decrypt session to blame) passes
/// through as <paramref name="ResponseCode"/> already answers it.
/// </param>
...
/// 91; Part 3, clause 12.1, Table 18), included verbatim in the creation data, in an owned pooled carrier the
...
/// 10.8.7, Table 128; Part 3, clause 12.1, Table 18), in an owned pooled carrier the decrypt effect rents as
...
/// <c>auth</c> first parameter has been decrypted (TPM 2.0 Library Part 3, clause 31.3; Part 1, clause 20) and
/// its plaintext value read back, so the completing transition can install it as the new Index's authValue.
/// Reached strictly after the authorizing session's command HMAC verified (Part 3, clause 5.6 precedes clause
...
/// <param name="IsSuppliedHmacEmpty">
/// Whether <see cref="Hmac"/> was the Empty Buffer, read and recorded at the continuation that releases
/// <see cref="Hmac"/> — since the seed-draw hop between authorization and response framing does not itself
/// retain the carrier, this flag is what clause 16.6.16's response-mirroring rule survives on into
/// <c>OnStorageProofSeedGenerated</c>.
/// </param>
...
/// the legacy <c>disableOwnerClear</c> flag (Part 3, clause 24.7.2, Table 203) while the attribute it writes is
...
/// <summary>
/// A password-authorized <c>TPM2_NV_GlobalWriteLock()</c> command (TPM 2.0 Library Part 3, clause 31.12): SETs
/// <c>TPMA_NV_WRITELOCKED</c> on every defined NV Index whose <c>TPMA_NV_GLOBALLOCK</c> attribute is SET. It
/// carries a single hierarchy handle and no parameters at all, so the authorization handle is the whole of what a
/// caller supplies beyond the authorization area.
/// </summary>
/// <remarks>
/// Only <c>TPM_RH_OWNER</c> and <c>TPM_RH_PLATFORM</c> are admissible (<c>TPMI_RH_PROVISION</c>, TPM 2.0 Library
/// Part 2, clause 9.21, Table 67). The handle plays no part in which Indexes lock — Part 4's
/// <c>NV_GlobalWriteLock.c</c>'s <c>TPM2_NV_GlobalWriteLock()</c> marks it <c>NOT_REFERENCED</c> — so both arms run the identical effect over every
/// defined Index once authorized.
/// </remarks>
/// <param name="AuthHandle">The authorizing hierarchy handle, <c>TPM_RH_OWNER</c> or <c>TPM_RH_PLATFORM</c>, judged against <see cref="TpmiRhProvision.IsProvision(uint)"/> at the transition.</param>
/// <param name="AuthSupplied">The authorization value the caller supplied — the password session's plaintext authValue, which is the same <c>TPM2B_AUTH</c> wire field a real session carries an HMAC in (TPM 2.0 Library Part 2, clause 10.12.2, Table 156: "either an HMAC, a password, or an EmptyAuth") — in a pooled carrier this record OWNS, rented as the parse's last act. It authorizes the owner or the platform hierarchy. The authorizing transition is its terminal owner; every refusing path releases it through <see cref="IDisposable.Dispose"/>.</param>
public sealed record TpmNvGlobalWriteLockRequested(
    TpmiRhProvision AuthHandle,
    Tpm2bAuth AuthSupplied): TpmSimulatorInput, IDisposable
{
    /// <summary>
...
/// <summary>
/// A <c>TPM2_NV_GlobalWriteLock()</c> command authorized over an HMAC session (TPM 2.0 Library Part 3, clause
/// 31.12).
/// </summary>
/// <param name="AuthHandle">The authorizing hierarchy handle, <c>TPM_RH_OWNER</c> or <c>TPM_RH_PLATFORM</c>.</param>
/// <param name="AuthorizingSessionHandle">The HMAC session presented to authorize <paramref name="AuthHandle"/>.</param>
...
/// <param name="RawParameterArea">The raw parameter-area bytes exactly as received — always empty, since the command has no parameters, and captured only so cpHash is built the same way for every command. Held in a pooled carrier this record OWNS, rented as the parse's last act; released through <see cref="IDisposable.Dispose"/> on every refusing path and by the accepting continuation once the command has been framed.</param>
/// <param name="ResolvedAuthValue">The authValue term folded into the command HMAC key, resolved at the entry transition and reused for the response key (the command changes no authValue, so both directions agree).</param>
public sealed record TpmNvGlobalWriteLockOverSessionRequested(
    TpmiRhProvision AuthHandle,
    TpmiShAuthSession AuthorizingSessionHandle,
    Tpm2bNonce NonceCaller,
    TpmaSession SessionAttributes,
