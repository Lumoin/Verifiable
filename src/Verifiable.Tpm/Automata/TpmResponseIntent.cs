...
/// the new <c>sequenceHandle</c>, whose parameter area is empty (Table 81 returns no parameter), followed by
...
/// clause 18.3 Table 100, clause 17.4 Table 102, clause 18.7 Table 108, clause 31.16.2 Table 272, clause 20.5
...
public sealed record TpmOverSessionsResponse(
    TpmRcConstants ResponseCode,
    TpmParameterArea ParameterArea,
    ImmutableArray<TpmFramedSessionEntry> Entries): TpmResponseIntent(ResponseCode);
...
/// (tag <c>TPM_ST_AUTH_SECRET</c>, hierarchy <c>TPM_RH_NULL</c>, empty digest — Part 2, Section 10.6.2's
...
/// sessionAttributes, HMAC) (TPM 2.0 Library Part 3, Section 23.4; Part 1, clause 15.6.1) — the
...
/// <c>TPMT_TK_AUTH</c> (tag <c>TPM_ST_AUTH_SIGNED</c> — set even on a NULL ticket, Part 2, Section 10.6.2's
