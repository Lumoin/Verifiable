...
/// (<c>TPMT_KDF_SCHEME</c>, TPM 2.0 Library Part 2, clause 12.2.3.6, Table 229 — v185's KEM admission gate:
...
/// exchange of <c>TPM2_MakeCredential</c> / <c>TPM2_ActivateCredential</c> (Part 1, clause 21; Part 3, clauses 12.6
...
/// separate <c>TPM2B_ECC_PARAMETER</c> buffers (Part 2, clause 11.2.5.1, Table 197; the point itself is <c>TPMS_ECC_POINT</c>, clause 11.2.5.2, Table 198), whereas this slot holds the
...
/// (TPM 2.0 Library Part 2, clause 11.2.4.5, Table 194) — the same buffer type an RSA <c>TPMT_PUBLIC</c>'s
    ...
    Tpm2bName QualifiedName): IDisposable
{
    ...
    /// value, exported public modulus, protection seed, public area, and Qualified Name. Called when the object
    /// leaves its dictionary for good (<c>TPM2_FlushContext()</c>, <c>TPM2_EvictControl()</c>'s evict
    /// arm, <c>TPM2_Clear()</c>, hierarchy disable, simulator teardown). A persisted copy owns separate
    /// deep-copied carriers, so disposing one entry never touches the other; the empty-authValue,
    /// empty-modulus, and empty-Name sentinels are dispose-immune.
    /// </summary>
    public void Dispose()
    {
        PrivateKey.Dispose();
    ...
    /// <see cref="AuthValue"/>, <see cref="PublicModulus"/>, <see cref="SeedValue"/>, <see cref="PublicArea"/>,
    /// and <see cref="QualifiedName"/> instances — reference comparison
