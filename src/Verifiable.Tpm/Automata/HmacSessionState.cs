...
/// 10.4.4, Table 94 names — <c>TPM2B_NONCE</c>, the declared type of <c>TPMS_AUTH_COMMAND.nonce</c> and
/// <c>TPMS_AUTH_RESPONSE.nonce</c> (clauses 10.13.2 and 10.13.3, Tables 153 and 154) — so it rides in a
...
/// <param name="SessionKey">The session key, used as the HMAC key and the parameter-encryption key seed: <c>KDFa</c>-derived (Part 1, clause 17.6.10 equation 20) when bound and/or salted, or the shared Empty-Buffer carrier <see cref="TpmSimulatorState.EmptySessionKey"/> when neither (clause 17.6.9). This record owns the carrier.</param>
/// <param name="NonceTpm">The current nonceTPM (<c>TPM2B_NONCE</c>, TPM 2.0 Library Part 2, clause 10.4.4, Table 94) in an owned pooled carrier: seeded by the initial value returned at start and rolled to a fresh value on each command response (Part 1, clause 17.6.5) through <see cref="WithNonceTpm(Tpm2bNonce)"/>, which releases the superseded carrier as the replacement lands. This record owns the carrier; the pending-verification queue, the response-framing records, and the response intents read it as a borrow and never dispose it.</param>
...
/// <c>SessionComputeBoundEntity()</c>; Part 1, clause 17.6.10). A command-HMAC verification omits the bound
...
/// (clause 17.8.7: "If a session is bound to a permanent entity other than TPM_RH_LOCKOUT, then the session is
...
/// session authorizes, because clause 17.8.7 makes the charge an OR: "the authorization failure counter
...
/// dictionary-attack protected (TPM 2.0 Library Part 1, clause 17.8.1: "lockoutAuth is DA protected even though
...
/// takes the one-strike lockoutAuth discipline (clause 17.8.5: an authorization failure associated with
