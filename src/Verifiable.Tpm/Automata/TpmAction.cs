...
/// <param name="IsSafeToSign">Whether the hashed octets are safe to sign with a restricted key — the settled first-block verdict (clauses 17.7 and 17.8) for a sequence, the Part 4 <c>Hash.c</c> rule for the one-shot; <see langword="false"/> yields the NULL ticket regardless of <paramref name="TicketHierarchy"/>.</param>
...
/// against a retained RSA key's private key under the sequence's own retained scheme, and on success mint the
/// <c>TPM_ST_MESSAGE_VERIFIED</c> ticket the same way.
/// </summary>
/// <param name="SequenceHandle">The handle of the sequence being completed, so the continuation can flush the correct entry on success.</param>
/// <param name="Segments">The sequence's accumulated message, BORROWED from the live <see cref="SequenceObjectState"/> — the state remains their owner on every arm; the effect only reads their octets to build the hash input and, on success, the raw ticket message.</param>
/// <param name="KeyName">The verifying key's Name, BORROWED from the live <see cref="TransientKeyState"/>, folded into the minted ticket's HMAC.</param>
/// <param name="KeyHierarchy">The permanent hierarchy the verifying key was created under, from which its ticket proof re-derives — or, when <c>TPM_RH_NULL</c>, the short-circuit to the NULL ticket tuple.</param>
/// <param name="PrivateKey">The verifying key's retained private key, in the backend's own encoding — a borrowed reference to the carrier the durable object state owns; the effect reads it at the verify primitive and never disposes it.</param>
/// <param name="Scheme">The sequence's own retained RSA signing scheme (<c>TPM_ALG_RSASSA</c> or <c>TPM_ALG_RSAPSS</c>).</param>
/// <param name="HashAlg">The sequence's own accumulator and signature hash algorithm, resolved at <c>TPM2_VerifySequenceStart()</c>.</param>
/// <param name="Signature">The caller-supplied <c>TPMT_SIGNATURE</c> (TPM 2.0 Library Part 2, clause 11.3.6, Table 219), the owned pooled carrier the request parsed; ownership rides this action into the effect, which is its terminal owner.</param>
/// <param name="OverSessions">The session-authorized form's response framing — the command code rpHash folds, every slot's response-entry material, and the completed sequence to flush with the response (<c>{F}</c>, TPM 2.0 Library Part 1, clause 29.4.6) — or <see langword="null"/> on the password form, whose effect feeds the plain <see cref="TpmSequenceSignatureVerified"/> back instead. A <c>TPM_RC_SIGNATURE</c> outcome keeps its header-only feedback on both forms, the sequence retained.</param>
...
/// clause 33.7 — the envelope and nested copies agree), signs <c>H_hashAlg(attest)</c> with the signing key's
...
/// nothing; Part 1, clause 18.1). Response-direction nonces are <c>nonceNewer = </c> the fresh nonceTPM and
/// <c>nonceOlder = NonceCaller</c> (Part 1, clause 18.2), so the host recovers the same keystream after adopting
/// the framed nonceTPM.
/// </remarks>
...
/// <param name="NonceCaller">This command's caller nonce (<c>TPM2B_NONCE</c>, Part 2, clause 10.3.4, Table 92): the response HMAC's nonceOlder and the encryption's nonceOlder. Owned; transferred out of the request record by the continuation that built this action, and released by the effect as its terminal owner.</param>
...
/// <param name="Credential">The secret to wrap (<c>TPM2B_DIGEST</c>, TPM 2.0 Library Part 2, clause 10.3.2, Table 90), the owned pooled carrier the request parsed; ownership rides this action into the effect, which is its terminal owner.</param>
/// <param name="ObjectName">The attestation key's Name the credential is bound to (folded into the <c>KDFa</c> derivations and the outer HMAC) — an owned <c>TPM2B_NAME</c> carrier the transition transferred out of the request; the effect is its terminal owner and releases it in the <c>finally</c>.</param>
/// <param name="CredentialKeyModulus">The credential key's exported public modulus (<c>TPM2B_PUBLIC_KEY_RSA</c>, TPM 2.0 Library Part 2, clause 11.2.4.5, Table 194), unsigned big-endian, the OAEP public-key input — a BORROW of the loaded object's own durable carrier, which outlives this command; the effect reads it and never releases it.</param>
...
/// <c>TpmNvReadLockOverSessionRequested</c>, <c>TpmNvCertifyOverSessionRequested</c>, or
...
/// <c>TPM2_NV_Certify()</c> because its cpHash has THREE (Part 1, clause 15.7 equation 15's
...
/// <c>TPM2_NV_Extend()</c>/<c>TPM2_NV_SetBits()</c>/<c>TPM2_NV_WriteLock()</c>/<c>TPM2_NV_ReadLock()</c> and the
/// hierarchy commands all frame an empty parameter area. For <c>TPM2_NV_Read()</c> it is the requested
...
/// <summary>
/// The RSA counterpart of <see cref="TpmVerifyDigestSignatureAction"/>: verify a digest/signature pair against a
/// loaded RSA key's retained private key under the key's own retained scheme, and on success produce a
/// <c>TPM_ST_DIGEST_VERIFIED</c> <c>TPMT_TK_VERIFIED</c> the same way <see cref="TpmVerifyDigestSignatureAction"/>
/// does.
/// </summary>
/// <param name="KeyName">The verifying key's Name, folded into the ticket HMAC.</param>
/// <param name="KeyHierarchy">The permanent hierarchy the verifying key was created under, from which its ticket proof re-derives — or, when <c>TPM_RH_NULL</c>, the short-circuit to the NULL ticket tuple (TPM 2.0 Library Part 3, clause 17).</param>
...
/// (Part 2, Table 114) before the next transition (TPM 2.0 Library Part 3, Section 23.4). Emitted by the
...
/// <see cref="TpmEncryptRandomAction"/>/<see cref="TpmCreateKeyedHashOverSessionsAction"/> for a command whose response
/// is a timeout/ticket pair rather than an encryptable parameter (TPM 2.0 Library Part 1, clause 15.6.1).
...
/// <param name="SessionAttributes">This session's command session-attributes octet, echoed into its response entry.</param>
/// <param name="Timeout">The already-decided deadline as a <c>TPM2B_TIMEOUT</c> carrier (Section 23.2.5; TPM 2.0 Library Part 2, clause 10.3.10, Table 98) — owned; the effect is its terminal owner, consuming and disposing it while framing the response parameter bytes. The shared empty carrier when <see cref="TicketDigest"/> is <see langword="null"/>, where a NULL timeout is framed.</param>
...
/// <see cref="TpmEncryptRandomAction"/> (TPM 2.0 Library Part 1, clauses 18 and 20; Part 3, clause 5.7). Emitted
...
/// named by <see cref="CommandCode"/> — before the next transition (TPM 2.0 Library Part 1, clause 15.6.1).
...
/// (Part 1, clause 16.6). It carries no parameter area because the response has no parameters at all (Part 3,
/// clause 31.15, Table 270). The authorizing session's <c>AuthValue</c> entry is already the POST-rotation value
...
/// (Part 1, clause 15.8). Because this action serves BOTH authValue rotations, the code must be carried rather
...
/// the next transition — <c>TPM2_Sign()</c>'s <c>digest</c>, <c>TPM2_SignDigest()</c>'s <c>context</c>, or
/// <c>TPM2_SequenceUpdate()</c>'s and <c>TPM2_SignSequenceComplete()</c>'s <c>buffer</c>, named by
/// <see cref="CommandCode"/>: the one shape the four share, since each is the first sized parameter of its
/// command (TPM 2.0 Library Part 1, clause 18.1: "Any first parameter can be encrypted as long as the parameter
/// has a size field"). Declared ALWAYS on the session-authorized arm and emitted only once every session in the
