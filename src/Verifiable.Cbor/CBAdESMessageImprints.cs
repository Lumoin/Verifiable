...
/// (<c>Verifiable.Cryptography.Pki</c>).
/// </para>
/// <para>
...
/// version 2); this file does not itself model the countersignature structures, so
/// <see cref="CBAdESCountersignatureStructureContext"/> simply carries
...
/// </summary>
/// <remarks>
/// This arm additionally identifies the structure for which clause
...
/// </summary>
public abstract class CBAdESPayloadImprintSource
{
...
public sealed class CBAdESAttachedPayloadImprintSource : CBAdESPayloadImprintSource
{
    ...
    /// memory.
    /// </param>
    public CBAdESAttachedPayloadImprintSource(ReadOnlyMemory<byte> payloadBytes)
    {
...
public sealed class CBAdESDetachedPayloadImprintSource : CBAdESPayloadImprintSource
{
    ...
    /// </param>
    public CBAdESDetachedPayloadImprintSource(ReadOnlyMemory<byte> payloadBytes)
    {
        PayloadBytes = payloadBytes;
    }
...
/// this type receives that dereferenced output; it does not itself dereference a <c>pars</c> URI-reference.
/// </summary>
/// <remarks>
...
public readonly record struct CBAdESArchiveTimestampImprintContext
{
    ...
    /// </summary>
    /// <remarks>
    /// <strong>Recorded reading.</strong> Step 4 begins "If the CB-AdES signature is
    /// built on the <c>COSE_Sign</c> structure, then: ..." with no paired "else if <c>COSE_Sign1</c>" clause
    /// anywhere in the spec text — the only sound reading is that step 4 is skipped in its
    ...
    /// combination inconsistent with <see cref="StructureContext"/> for the two structures this builder knows
    /// (<c>COSE_Sign</c>/<c>COSE_Sign1</c>); a <see cref="CBAdESCountersignatureStructureContext"/> is not
    /// checked, since this file does not model countersignature layering.
    /// </remarks>
    ...
    /// </summary>
    /// <remarks>
    /// Step 8's text — "add the <c>other_fields</c>
    /// CBOR array, as defined in clause 3.3 of IETF RFC 9338" — never says "encapsulated in a CBOR byte
    /// string" the way steps 3, 4, 5, 6, 7, and 9 each explicitly do. Read literally, this value is
    /// re-embedded into the accumulator array as the array value it already is (<c>WriteEncodedValue</c>),
    ...
    /// away. Any future countersignature substrate support must agree with this reading.
    /// </remarks>
    ...
    /// </summary>
    /// <remarks>
    /// <strong>The absent-vs-empty-slice asymmetry.</strong> Steps
    /// 10/11 add a zero-length CBOR byte string only "if [the layer] does not have the <c>uHeaders</c> header
    /// parameter" — i.e. when this field is <see langword="null"/>. When this field is non-null but the
    /// applicable slice (all elements for generation; elements strictly before the <c>arcTst</c> under
    ...
    /// parameter IS present; the applicable slice simply yields nothing). This is a real, non-obvious asymmetry
    /// with steps 3-6 (which always contribute exactly one item, empty or not) and is implemented exactly as
    /// written rather than normalized: the sentinel fires only on an ABSENT <c>uHeaders</c>, never on a
    /// present-but-empty prefix.
    /// </remarks>
...
/// at TSA-request time, via the registered digest delegates) and they never perform I/O
/// (dereferencing a <c>sigD</c> reference, or an unreferenced detached
...
/// </para>
/// <para>
/// <strong>"Step 12," not "step 11."</strong> Clause 5.3.5.3's closing sentence names
...
/// </para>
/// <para>
/// <strong>Step-12 reading: no outer <c>bstr</c> item header.</strong>
/// "Encode the generated CBOR array in a CBOR byte string" is read as "the message-imprint input IS the
/// encoded array's bytes" — not those bytes prepended with an additional CBOR byte-string item header.
/// Grounds: the bytes are immediately the hash input (no consumer ever unwraps a further <c>bstr</c> layer
/// before hashing), and
...
/// </para>
/// <para>
/// <strong>Recorded readings.</strong> See the
...
/// use the <c>TryBuild*</c> shape and never throw for malformed input — the walk is an
...
public static class CBAdESMessageImprints
{
    ...
    /// this library's strict conformance rule (parsing never throws for malformed input) — mirroring
    ...
    /// is present but malformed or non-conformant (never throws for that case).
    /// </returns>
    ...
    /// method checks (<c>COSE_Sign</c>/<c>COSE_Sign1</c> — see the field's remarks).
    /// </exception>
    /// <remarks>
    /// See
    /// <see href="https://www.etsi.org/deliver/etsi_ts/119100_119199/11915201/01.01.01_60/ts_11915201v010101p.pdf">
    /// ETSI TS 119 152-1 V1.1.1, clause 5.3.5.3</see>, and this class's remarks for the step-numbering and
    /// step-12 readings. This generation-mode variant's own zero-length-<c>bstr</c> sentinel
    /// (steps 10/11, fired only when <see cref="CBAdESArchiveTimestampImprintContext.UHeadersEncodedArray"/> is
    /// <see langword="null"/>) diverges from <see cref="TryBuildArchiveTimestampValidationMessageImprintInput"/>'s
    /// own reading of that SAME token once it becomes the FIRST <c>uHeaders</c> element (present array, empty
    /// prefix, no sentinel) — a genuine generation/validation asymmetry the spec's own text produces. RULED
    /// unreachable on the conforming B-LTA ladder: Table 14 requires a <c>sigTst</c> instance before the first
    /// <c>arcTst</c> (CB-6.3-21), so <c>uHeaders</c> is never absent by the time it is minted; the producer
    /// additionally refuses pre-Time-Stamping-Authority when that prerequisite is missing
    /// (<see cref="Verifiable.JCose.CBAdESSignatureAugmentation.AddArchiveTimestampAsync"/>).
    /// </remarks>
    ...
    /// <paramref name="arcTstElementIndex"/> is <c>0</c>. That
    /// present-but-empty-prefix reading, applied to a token that was minted under
    /// <see cref="TryBuildArchiveTimestampGenerationMessageImprintInput"/>'s own absent-<c>uHeaders</c> sentinel
    /// arm, is the generation/validation asymmetry that method's remarks describe in full — RULED unreachable
    /// on the conforming B-LTA ladder (CB-6.3-21; the producer's own pre-Time-Stamping-Authority prerequisite
    /// gate).
    /// </remarks>
        ...
        out PooledMemory? result)
    {
        ...
        if(context.StructureContext is CBAdESCoseSign1StructureContext && context.SignerProtectedHeader.HasValue)
        {
            throw new ArgumentException(
                ...
                "so SignerProtectedHeader shall be null.",
                nameof(context));
        }
        ...
        try
        {
            ...
            {
                var uHeadersReader = new CborReader(context.UHeadersEncodedArray.Value, CborOptions.RfcCanonical);
            ...
            result = PooledMemory.FromBytes(buffer.WrittenSpan, pool, EncodedMessageImprintInputTag);
            return true;
        }
    ...
    public static PooledMemory BuildPayloadTimestampMessageImprintInput(CBAdESPayloadImprintSource source, BaseMemoryPool pool)
    {
        ...
        static PooledMemory EncodeWrappedByteString(ReadOnlyMemory<byte> bytes, BaseMemoryPool pool)
        {
            ...
            return PooledMemory.FromBytes(buffer.WrittenSpan, pool, EncodedMessageImprintInputTag);
        }
    }
    ...
    /// recorded reading), or <see langword="null"/> when that layer does not have the <c>uHeaders</c>
    ...
    /// precedent exactly. See this method's own remarks below for why this parameter exists.
    /// </param>
    /// <param name="pool">The memory pool the returned carrier's buffer is rented from.</param>
    /// <param name="result">The message-imprint input on success; <see langword="null"/> on failure.</param>
    /// <returns>
    ...
    /// </para>
    /// <para>
    /// <strong>Recorded reading.</strong> Annex A.1.2.1.2 lacks the
    ...
    /// </para>
    /// <para>
    /// <strong>Recorded reading.</strong> Step 4's text says "take those elements in the
    ...
    /// only the prefix <paramref name="uHeadersSliceBound"/> selects. When the
    ...
    /// <see cref="TryBuildSignatureAndReferencesTimestampMessageImprintInput"/>'s remarks for the recorded
    /// and filter readings, both identical here.
    /// </param>
    /// <param name="uHeadersSliceBound">
    ...
    /// <see cref="TryBuildSignatureAndReferencesTimestampMessageImprintInput"/>'s remarks, identical here
    /// for Annex A.1.2.2.2.
    /// </param>
    /// <param name="pool">The memory pool the returned carrier's buffer is rented from.</param>
    /// <param name="result">The message-imprint input on success; <see langword="null"/> on failure.</param>
    ...
    /// <see cref="TryBuildSignatureAndReferencesTimestampMessageImprintInput"/>'s CB-A.1.2.1-03 remark, and
    /// the same validation-time prefix-replacement reading, identical rationale.
    /// </remarks>
    ...
    /// identical except for the leading signature-value element, so this shared core takes an
    ...
    /// <c>uHeadersSliceBound</c> precedent exactly.
    /// </param>
    /// <param name="pool">The memory pool the returned carrier's buffer is rented from.</param>
    /// <param name="result">The message-imprint input on success; <see langword="null"/> on failure.</param>
        ...
        out PooledMemory? result)
    {
        ...
        try
        {
            ...
            {
                var uHeadersReader = new CborReader(uHeadersEncodedArray.Value, CborOptions.RfcCanonical);
                int elementCount = uHeadersReader.ReadStartArrayExpectLengthRange(1, int.MaxValue);
                ...
                for(int i = 0; i < elementCount; ++i)
                {
                    ...
                    //Only elements strictly before the specific sigRTst/rfsTst instance's own position
        ...
        {
            var innerReader = new CborReader(elementContent, CborOptions.RfcCanonical);
