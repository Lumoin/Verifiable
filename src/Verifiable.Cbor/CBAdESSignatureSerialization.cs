...
/// <summary>
/// CBOR bindings for the CB-AdES JCose seam delegates (<see cref="EncodeCBAdESProtectedHeaderDelegate"/>,
...
/// satisfying this library's own strict-conformance posture with less hand-written validation, exactly as
/// <see cref="CBAdESSerialization"/>'s own class remarks document for the component codecs.
/// </para>
/// <para>
/// <strong>Accept both tagged and untagged, recorded here (CB-4.3).</strong> Clause 4.3 states CB-AdES signatures MAY be
...
/// emits tag 18; this class has no separate encoder for the envelope (<c>Cose.SignAsync</c> composes
...
/// </para>
/// <para>
/// <strong><c>x5u</c> is a plain <c>tstr</c>, NOT a tag-32 URI, recorded here.</strong> Clause 5.1.5 gives no
...
/// </para>
/// <para>
/// <strong>CWT Claims (label 15) tolerates sibling claims; only <c>iat</c> is modeled.</strong>
...
/// </para>
/// <para>
/// <strong><c>alg</c> stays parse-mandatory, deliberately.</strong> Unlike CWT Claims'
...
/// </para>
/// <para>
/// <strong>The protected header's own label space is the general <c>int / tstr</c> union, not integer-only.</strong>
/// CB-4.6-01's integer-only requirement ("The keys of the new CBOR map pairs shall
...
/// which stays exactly as it was for every component's internal map, per CB-4.6-01. Every profiled label
...
/// </para>
/// <para>
/// <strong>Cross-file edit, recorded here.</strong> <see cref="CBAdESSerialization.WriteHashAlgorithmDigestPair"/>
...
/// <c>int/tstr</c>-union digest-algorithm-identifier writer/reader — the shared component codec has no
...
/// </para>
/// <para>
/// <strong>Fourth member, recorded here.</strong>
...
/// </para>
/// <para>
/// <strong>Raw <c>uHeaders</c> capture.</strong>
...
/// exactly, the same precedent that motivated <see cref="CBAdESSign1ParseResult.RawProtectedHeader"/>.
...
/// throws for malformed input — every failure path inside its single try block, including a
...
/// — guarding against a misplaced early-return leaking a partially-parsed carrier, applied at the
/// message-envelope level; the raw <c>uHeaders</c> carrier joins that same cleanup set.
/// </para>
/// </remarks>
public static class CBAdESSignatureSerialization
{
    ...
    /// </summary>
    /// <remarks>
    /// Reuses <see cref="CBAdESSerialization.EncodeUnsignedHeaders"/> (the existing, tested <c>uHeaders</c>
    ...
    /// decompose round trip that reuses the existing encoder rather than duplicating it,
    ...
    /// <see cref="EncodeCBAdESUnprotectedHeader"/>.
    /// </summary>
    /// <remarks>
    /// <para>
    ...
    /// retained element's CONTENT is ever at risk of drifting (re-encoding the DECODED model,
        ...
        try
        {
            ...
            if(rawUnsignedHeaders is not null)
            {
                ...
                if(rawCount != decodedElementCount)
                {
                    ...
                    //meaningless. Fail closed; nothing is emitted. See this property's own remarks
                    //for why this branch is unreachable through ParseCBAdESSign1 + any shipped
    ...
    public static ParseCBAdESSign1Delegate ParseCBAdESSign1 { get; } = static (wireBytes, pool) =>
    {
        ...
        AdESCertificateThumbprint? x5t = null;
        AdESCertificateThumbprints? certificateDigests = null;
        ...
        try
        {
            ...
            //(RFC 9052 section 4.4) -- never a re-encoding of the decoded headers.
            ...
            //Element 1: the unprotected headers map -- CB-4.4-01: at most the one
            ...
            if(unprotectedMapLength.Value == 1)
            {
                ...
                //The raw-bytes precedent extended to
                //uHeaders: capture the exact wire bytes BEFORE handing them to the decoder, so the
            ...
            //CB-AdES components' OWN internal maps (still ReadAscendingMapKey/ReadInt32Array-only), not
            ...
            for(int i = 0; i < headerMapLength.Value; i++)
            {
                ...
                switch(label)
                {
                    ...
                    case CoseHeaderIntegerLabel { Value: CoseHeaderParameters.Kid }:
                        keyId = new CoseKeyIdentifier(headerReader.ReadByteString());
                        break;
                    ...
                    case CoseHeaderIntegerLabel { Value: CoseHeaderParameters.X5T }:
                        CBAdESSerialization.ReadHashAlgorithmDigestPair(
                            headerReader, pool, out AdESDigestAlgorithmIdentifier x5tAlgorithm, out DigestValue x5tDigest);
                        x5t = new AdESCertificateThumbprint(x5tAlgorithm, x5tDigest);
                        break;
            ...
            //non-conformant parsed state: CBAdESHeaderRules.Check reports it as
    ...
    /// <summary>
    /// Gets a delegate that decodes one raw protected-header <c>bstr</c>'s content bytes into a
    /// <see cref="CBAdESProtectedHeaders"/> aggregate, standalone: the content
    /// decode <see cref="ParseCBAdESSign"/>'s own remarks name as out of that seam's scope, exposed here so a
    /// <c>COSE_Sign</c> signer's own captured protected-header bytes (<c>CBAdESSignParseResult.Signers</c>) can
    /// be decoded the SAME way <see cref="ParseCBAdESSign1"/>'s body-layer protected header already is —
    /// reusing the identical map-decode rule body (<see cref="ReadContentType"/>, <see cref="ReadCwtClaims"/>,
    /// <see cref="ReadSignedX5Chain"/>, <see cref="ReadX5Url"/>, the <c>alg</c>-mandatory/RFC 8949 §4.2.3
    /// canonical-key-ordering checks), never a forked reading. Fail-closed: never throws for malformed input;
    /// returns <see langword="null"/> instead.
    /// </summary>
    public static DecodeCBAdESProtectedHeaderDelegate DecodeCBAdESProtectedHeader { get; } = static (protectedHeaderBytes, pool) =>
    {
        ArgumentNullException.ThrowIfNull(pool);
        ...
        try
        {
            ...
            CBAdESProtectedHeaders headers = new(
                algorithm.Value,
                cwtClaims,
                contentType,
                keyId,
    ...
    /// <summary>
    /// Gets a delegate that performs the fail-closed full parse of CB-AdES <c>COSE_Sign</c> (multi-signer) wire
    /// bytes — the COSE-structural envelope generalization
    /// <see cref="ParseCBAdESSign1"/> could not express, since its element-3 read is an unconditional
    /// <c>ReadByteString</c> that fails closed (correctly, but indiscriminately) against a <c>COSE_Sign</c>'s
    /// <c>signatures</c> array.
    /// </summary>
    /// <remarks>
    /// <para>
    /// Accepts both the tagged (<c>COSE_Sign_Tagged</c>, CBOR tag 98) and untagged wire forms, mirroring
    /// <see cref="ParseCBAdESSign1"/>'s own CB-4.3 tolerance for tag 18. <see cref="CBAdESSignParseResult"/>'s
    /// own remarks record why this parse decodes only the COSE-structural envelope (raw protected-header bytes
    /// per layer, decoded <c>uHeaders</c> per signer layer) rather than a full <c>CBAdESProtectedHeaders</c>
    /// aggregate per layer — that content decode is <see cref="DecodeCBAdESProtectedHeader"/>'s job, not this seam's.
    /// </para>
    /// <para>
    /// Enforces clause 4.4's body-layer restriction fail-closed: "CB-AdES signatures supported by a
    /// <c>COSE_Sign</c> structure... shall not contain the <c>uHeaders</c> unprotected header parameter in the
    /// body layer" — read together with clause 4.4's opening sentence (the unprotected map may carry ONLY
    /// <c>uHeaders</c>), the body layer's own unprotected map must be empty. Each signer layer's own unprotected
    /// map may carry at most the one <c>uHeaders</c> member, exactly like <see cref="ParseCBAdESSign1"/>'s body
    /// layer (CB-4.4-01).
    /// </para>
    /// </remarks>
    [SuppressMessage("Reliability", "CA2000:Dispose objects before losing scope",
        Justification = "Ownership of bodyProtectedHeaderCarrier and every entry of signers transfers to the " +
            "returned CBAdESSignParseResult on success; the catch block disposes them on any failure path.")]
    public static ParseCBAdESSignDelegate ParseCBAdESSign { get; } = static (wireBytes, pool) =>
    {
        ArgumentNullException.ThrowIfNull(pool);
        ...
        try
        {
            ...
            //Element 0: body_protected, bstr -- captured verbatim (the same raw-bytes discipline extended to the
            //body layer of COSE_Sign), never decoded into a CBAdESProtectedHeaders aggregate at this seam (see
            //this delegate's own remarks and CBAdESSignParseResult's remarks for why).
            byte[] bodyProtectedHeaderBytes = reader.ReadByteString();
            bodyProtectedHeaderCarrier = EncodedCoseProtectedHeader.FromBytes(bodyProtectedHeaderBytes, pool);

            //Element 1: the body-layer unprotected headers map -- clause 4.4 forbids uHeaders at the body layer
            //under COSE_Sign, and permits no other member either, so a well-formed body-layer map is empty.
            int? bodyUnprotectedMapLength = reader.ReadStartMap();
            if(bodyUnprotectedMapLength is null)
            {
                throw new CborContentException(
                    "The unprotected headers map shall be definite-length (IETF RFC 9052 section 9).");
            }

            if(bodyUnprotectedMapLength.Value > 0)
            {
                throw new CborContentException(
                    "The body layer's unprotected headers map shall be empty under COSE_Sign -- uHeaders shall " +
                    "be placed at the signer layer, never the body layer (ETSI TS 119 152-1 V1.1.1, clause 4.4).");
            }
            ...
            //Element 3: signatures, [+ COSE_Signature] -- the array RFC 9052 section 4.1 puts where COSE_Sign1
            //carries a bare signature bstr; this is the exact shape ParseCBAdESSign1's own element-3 read
            //cannot express.
            int? signerCount = reader.ReadStartArray();
            if(signerCount is null)
            {
                throw new CborContentException("The signatures array shall be definite-length (IETF RFC 9052 section 9).");
            }

            if(signerCount.Value == 0)
            {
                throw new CborContentException(
                    "COSE_Sign requires at least one COSE_Signature entry (IETF RFC 9052 section 4.1, [+ COSE_Signature]).");
            }

            for(int i = 0; i < signerCount.Value; ++i)
            {
                reader.ReadStartArrayExpectLength(3);

                byte[] signerProtectedHeaderBytes = reader.ReadByteString();
                signerProtectedHeaderCarrier = EncodedCoseProtectedHeader.FromBytes(signerProtectedHeaderBytes, pool);

                //The signer layer's own unprotected headers map -- CB-4.4-01's rule, applied per signer: at
                //most the one uHeaders (268) member.
                int? signerUnprotectedMapLength = reader.ReadStartMap();
                if(signerUnprotectedMapLength is null)
                {
                    throw new CborContentException(
                        "An unprotected headers map shall be definite-length (IETF RFC 9052 section 9).");
                }

                if(signerUnprotectedMapLength.Value > 1)
                {
                    throw new CborContentException(
                        $"The unprotected headers map shall contain only the uHeaders member (ETSI TS 119 152-1 " +
                        $"V1.1.1, clause 4.4, CB-4.4-01); got {signerUnprotectedMapLength.Value} members.");
                }
                ...
                if(signerUnprotectedMapLength.Value == 1)
                {
                    ...
                    ReadOnlyMemory<byte> uHeadersBytes = reader.ReadEncodedValue();
                    signerRawUnsignedHeaders = EncodedCBAdESUnsignedHeaders.FromBytes(uHeadersBytes.Span, pool);

                    if(!CBAdESSerialization.TryParseUnsignedHeaders(uHeadersBytes, pool, out signerUnsignedHeaders) || signerUnsignedHeaders is null)
                    {
                        throw new CborContentException("Malformed uHeaders (label 268) value.");
                    }
                }
                ...
                //Ownership of every per-signer carrier transfers into the entry; null the standalone locals so
                //the catch block's cleanup (defense-in-depth from here on) never double-disposes them through
                //two reference paths.
                signers.Add(new CBAdESSignerParseResult(signerProtectedHeaderCarrier, signatureCarrier, signerUnsignedHeaders, signerRawUnsignedHeaders));
                signerProtectedHeaderCarrier = null;
                signatureCarrier = null;
                signerUnsignedHeaders = null;
                signerRawUnsignedHeaders = null;
            }
    ...
    /// union RFC 9052 §3.1's <c>crit</c> syntax reuses unnarrowed.
    /// </summary>
    ...
    /// the tolerant-sibling-claims decision and how <c>NumericDate</c> is read.
    /// </summary>
    /// <param name="reader">The CBOR reader.</param>
    /// <returns>
    ...
    /// existing, tested <see cref="CBAdESSerialization.EncodeUnsignedHeaders"/> encoder, then unwraps that
    ...
    /// raw-copied retained ones. Reuse over reinvention: this never duplicates
    ...
    [SuppressMessage("Reliability", "CA2000:Dispose objects before losing scope",
        Justification = "The one-element CBAdESUnsignedHeaders wrapper built here is a throwaway view over the " +
            "caller-owned `element` argument, solely to reach CBAdESSerialization.EncodeUnsignedHeaders's " +
            "existing, tested encoder (see this method's own remarks). `element` itself is BORROWED, not owned: " +
            "by the time this method runs, the JCose augmentation verb that passed it in has already folded the " +
            "same element into finalUnsignedHeaders and set its own transferred flag -- the fold strictly " +
            "PRECEDES this encode call, never the reverse -- so finalUnsignedHeaders is already `element`'s real " +
            "owner, disposed exactly once by the verb's own finally block. Disposing the wrapper here would " +
            "cascade-dispose `element` (CBAdESUnsignedHeaders.Dispose disposes every element it holds) while " +
            "finalUnsignedHeaders still needs it live, and would double-dispose it once finalUnsignedHeaders is " +
            "later disposed -- so the wrapper is deliberately left undisposed; it owns no resources of its own " +
            "beyond the reference to `element` it never had.")]
    ...
    /// input that <see cref="ParseCBAdESSign1"/> catches to fail closed, as strict conformance requires — mirrors
