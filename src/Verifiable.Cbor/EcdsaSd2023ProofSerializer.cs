...
public static class EcdsaSd2023CborSerializer
{
        ...
        EncodeDelegate base64UrlEncoder)
    {
        ...
        return "u" + base64UrlEncoder(proofBytes);
    }
    ...
    /// <returns>Tuple of (baseSignature, publicKey, hmacKey, signatures, mandatoryPointers).</returns>
    /// <exception cref="FormatException">Thrown when the proof format is invalid.</exception>
    public static (byte[] BaseSignature, byte[] PublicKey, byte[] HmacKey, List<byte[]> Signatures, List<string> MandatoryPointers)
        ParseBaseProof(string proofValue, DecodeDelegate base64UrlDecoder, MemoryPool<byte> memoryPool)
    {
        ...
        if(!proofValue.StartsWith('u'))
        {
            throw new FormatException(
                "Base proof value must start with 'u' indicating base64url-no-pad multibase encoding.");
        }
    ...
    /// <exception cref="FormatException">Thrown when the proof format is invalid.</exception>
    public static (byte[] BaseSignature, byte[] PublicKey, byte[] HmacKey, List<byte[]> Signatures, List<string> MandatoryPointers)
        ParseBaseProofBytes(ReadOnlySpan<byte> proofBytes)
    {
        ...
        {
            throw new FormatException(
                "Invalid base proof header. Expected 0xd9 0x5d 0x00.");
        }
        ...
        {
            throw new FormatException(
                $"Base proof must contain exactly 5 elements, found {arrayLength}.");
        }
        ...
        var signatures = new List<byte[]>(signaturesLength ?? 0);
        while(reader.PeekState() != CborReaderState.EndArray)
        {
            signatures.Add(reader.ReadByteString());
        }
        reader.ReadEndArray();

        //Read mandatory pointers array.
        int? pointersLength = reader.ReadStartArray();
        var mandatoryPointers = new List<string>(pointersLength ?? 0);
        while(reader.PeekState() != CborReaderState.EndArray)
        {
            mandatoryPointers.Add(reader.ReadTextString());
        }
        reader.ReadEndArray();
    ...
    /// <param name="baseSignature">The 64-byte base signature.</param>
    /// <param name="publicKey">The multikey-encoded public key.</param>
    /// <param name="signatures">Signatures for disclosed statements only.</param>
    /// <param name="labelMap">Canonical to HMAC blank node ID mapping (e.g., "c14n0" → "uABC...").</param>
    /// <param name="mandatoryIndexes">Indexes of mandatory statements in the reveal document.</param>
    /// <param name="base64UrlEncoder">Delegate for base64url encoding.</param>
    /// <param name="base64UrlDecoder">Delegate for base64url decoding (for label map compression).</param>
    /// <param name="memoryPool">Memory pool for allocations.</param>
    /// <returns>The multibase-encoded proof value string starting with 'u'.</returns>
        ...
        MemoryPool<byte> memoryPool)
    {
        ...
        byte[] proofBytes = SerializeDerivedProofBytes(
            baseSignature, publicKey, signatures, labelMap, mandatoryIndexes, base64UrlDecoder, memoryPool);
        return "u" + base64UrlEncoder(proofBytes);
    }
    ...
    /// <param name="publicKey">The multikey-encoded public key.</param>
    /// <param name="signatures">Signatures for disclosed statements only.</param>
    /// <param name="labelMap">Canonical to HMAC blank node ID mapping.</param>
    ...
    /// <param name="memoryPool">Memory pool for allocations.</param>
    /// <returns>Tuple of (baseSignature, publicKey, signatures, labelMap, mandatoryIndexes).</returns>
    /// <exception cref="FormatException">Thrown when the proof format is invalid.</exception>
    public static (byte[] BaseSignature, byte[] PublicKey, List<byte[]> Signatures, Dictionary<string, string> LabelMap, List<int> MandatoryIndexes)
        ParseDerivedProof(
            string proofValue,
            DecodeDelegate base64UrlDecoder,
            EncodeDelegate base64UrlEncoder,
            MemoryPool<byte> memoryPool)
    {
        ...
        {
            throw new FormatException(
                "Derived proof value must start with 'u' indicating base64url-no-pad multibase encoding.");
        }

        using IMemoryOwner<byte> decodedOwner = base64UrlDecoder(proofValue.AsSpan()[1..], memoryPool);
        return ParseDerivedProofBytes(decodedOwner.Memory.Span, base64UrlEncoder);
    }
    ...
    /// <exception cref="FormatException">Thrown when the proof format is invalid.</exception>
    public static (byte[] BaseSignature, byte[] PublicKey, List<byte[]> Signatures, Dictionary<string, string> LabelMap, List<int> MandatoryIndexes)
        ParseDerivedProofBytes(ReadOnlySpan<byte> proofBytes, EncodeDelegate base64UrlEncoder)
    {
        ArgumentNullException.ThrowIfNull(base64UrlEncoder);
        ...
        {
            throw new FormatException(
                "Invalid derived proof header. Expected 0xd9 0x5d 0x01.");
        }
        ...
        {
            throw new FormatException(
                $"Derived proof must contain exactly 5 elements, found {arrayLength}.");
        }
        ...
        return (baseSignature, publicKey, signatures, labelMap, mandatoryIndexes);
    }
        ...
        MemoryPool<byte> memoryPool)
    {
        ...
        foreach(var entry in labelMap)
        {
            ...
            {
                throw new FormatException(
                    $"Label map key must start with '{CanonicalBlankNodePrefix}', got: {entry.Key}");
            }
            ...
            {
                throw new FormatException(
                    $"Label map key suffix must be an integer, got: {indexStr}");
            }
            ...
            {
                throw new FormatException(
                    $"Label map value must start with '{HmacBlankNodePrefix}', got: {entry.Value}");
            }
