...
/// per-call context parameter this delegate takes instead of closing over any caller state (house rule:
/// no closure capture).
/// </param>
...
/// <see langword="null"/> when no snapshot has ever been persisted for <paramref name="runId"/> —
/// "No snapshot present ⇒ Initial (first boot)."
/// </returns>
/// <remarks>
...
/// </remarks>
public delegate ValueTask<PooledMemory?> TryLoadSnapshotAsyncDelegate(string runId, BaseMemoryPool pool, CancellationToken cancellationToken);
