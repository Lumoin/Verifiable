...
public sealed record ServerHttpResponse
{
    ...
    /// <summary>
    /// Returns a 413 Content Too Large (RFC 9110 §15.5.14) with a JSON body — for example a VCALM
    /// 1.0 §3.3.2 <c>/presentations/verify</c> request whose body exceeds the configured §2.4
    /// payload-size cap.
    /// </summary>
    public static ServerHttpResponse PayloadTooLarge(string body, string contentType) =>
        new()
        {
    ...
    /// <summary>
    /// Returns a response carrying an arbitrary JSON <paramref name="body"/> under
    /// <paramref name="statusCode"/> — for endpoints whose wire body is not the OAuth
    /// <c>error</c> / <c>error_description</c> shape (for example the VCALM 1.0 §3.3 verifier, whose
    /// 200 VerificationResponse and §3.8 ProblemDetails bodies the caller composes).
    /// </summary>
    public static ServerHttpResponse Json(int statusCode, string body, string contentType) =>
        new()
        {
