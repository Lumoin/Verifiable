...
/// and stepping. The dispatch host calls these at request time — new flows via
/// <see cref="CreateAsync"/>, continuing flows via <see cref="StepAsync"/> after
...
public abstract class StatefulFlowKind: FlowKind
{
    ...
    public abstract ValueTask<(FlowState State, int StepCount)> CreateAsync(
        string runId,
    ...
    /// </summary>
    public abstract ValueTask<(FlowState State, int StepCount)> StepAsync(
        FlowState state,
        int stepCount,
        FlowInput input,
        TimeProvider timeProvider,
    ...
    /// <summary>
    /// Returns <see langword="true"/> when flows of this kind emit effectful action
    /// values during transitions, requiring the application to configure the
    /// family's action executor on the dispatch host. The dispatcher validates this
    /// at startup.
    /// </summary>
