...
public static class ExchangeContextServerExtensions
{
    extension(ExchangeContext context)
    {
        ...
        /// <summary>
        /// Gets the step count the current flow state was loaded at (<c>0</c> for a
        /// freshly-created flow), stamped by the dispatcher before <c>BuildInputAsync</c> runs.
        /// A handler passes this to <see cref="ServerIntegration.ClaimFlowStateAsync"/> as the
        /// <c>expectedStepCount</c> so the claim targets the exact step this request observed.
        /// </summary>
        public int? FlowStepCount =>
            context.TryGetValue(ServerContextKeys.FlowStepCountKey, out object? v)
                && v is int count ? count : null;

        /// <summary>Sets the step count the current flow state was loaded at. Called by the dispatcher.</summary>
        public void SetFlowStepCount(int stepCount)
        {
            context[ServerContextKeys.FlowStepCountKey] = stepCount;
        }
