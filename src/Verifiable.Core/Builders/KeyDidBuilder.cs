...
namespace Verifiable.Core.Builders
{
    ...
    /// var didDocument = builder.Build(publicKey, JsonWebKey2020VerificationMethod.Instance);
    ///
    /// // Multiple key usage
    /// var keyInputs = new[]
    /// {
    ...
    /// };
    /// var didDocument = builder.Build(keyInputs);
    /// </code>
    /// </example>
    public sealed class KeyDidBuilder: Builder<DidDocument, KeyDidBuildState, KeyDidBuilder>
    {
        ...
        {
            if(state is KeyDidBuildState keyDidState)
            {
        ...
        /// </list>
        /// </remarks>
        public DidDocument Build(IEnumerable<KeyMaterialInput> keyInputs, bool includeDefaultContext = false)
        {
        ...
        /// <returns>A fully constructed <c>did:key</c> DID document.</returns>
        public DidDocument Build(PublicKeyMemory publicKey, VerificationMethodTypeInfo verificationMethodType, bool includeDefaultContext = false)
        {
            var keyInputs = new[]
            {
            ...
            return Build(keyInputs, includeDefaultContext);
        }
