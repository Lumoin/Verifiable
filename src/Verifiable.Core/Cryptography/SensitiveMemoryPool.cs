...
namespace Verifiable.Core.Cryptography
{
    ...
    /// - Exact buffer sizes are required (no over-allocation)
    /// - Memory is automatically cleared on disposal for security
    /// - Size-specific pooling optimizes for common crypto buffer sizes
    /// - Comprehensive metrics and tracing support operational monitoring
    /// - Thread-safe operations support concurrent cryptographic operations
    ///
    ...
    public class SensitiveMemoryPool<T>: MemoryPool<T>
    {
        ...
        /// </summary>
        public SensitiveMemoryPool() : this(new Meter(VerifiableMetrics.CoreMeterName, "1.0.0"))
        {
        }
        ...
        public SensitiveMemoryPool(Meter meter)
        {
            ...
            meter.CreateObservableUpDownCounter<int>(
                VerifiableMetrics.SensitiveMemoryPoolTotalSlabs,
                () => totalSlabs,
                "slabs",
                "Total number of memory slabs created across all buffer sizes");

            meter.CreateObservableUpDownCounter<long>(
                VerifiableMetrics.SensitiveMemoryPoolTotalMemoryAllocated,
                () => totalMemoryAllocated,
                "bytes",
                "Total memory allocated across all slabs including available segments");

            meter.CreateObservableUpDownCounter<int>(
                VerifiableMetrics.SensitiveMemoryPoolActiveRentals,
                () => activeRentals,
                "segments",
                "Number of currently rented memory segments");

            meter.CreateObservableUpDownCounter<double>(
                VerifiableMetrics.SensitiveMemoryPoolAllocationEfficiency,
            ...
            BufferSizeHistogram = meter.CreateHistogram<int>(
                VerifiableMetrics.SensitiveMemoryPoolBufferSizeDistribution,
                "bytes",
                "Distribution of requested buffer sizes");

            RentSuccessCounter = meter.CreateCounter<long>(
                VerifiableMetrics.SensitiveMemoryPoolRentOperationsTotal,
                "operations",
                "Total number of successful rent operations");

            ReturnCounter = meter.CreateCounter<long>(
                VerifiableMetrics.SensitiveMemoryPoolReturnOperationsTotal,
                "operations",
        ...
        public override IMemoryOwner<T> Rent(int bufferSize)
        {
            ...
            using var activity = ActivitySource.StartActivity("Rent", ActivityKind.Internal,
            ...
            using(LockObject.EnterScope())
            {
                ...
                //Create the memory owner wrapper.
                result = new ExactSizeMemoryOwner<T>(rentedSegment, availableSlab, this);
            }
        ...
        internal class Slab<TElement>: IDisposable
        {
            ...
            public void Return(ArraySegment<TElement> segment)
            {
                if(IsDisposed)
                {
                    throw new ObjectDisposedException(nameof(Slab<TElement>));
                }
        ...
        {
            /// <summary>
            /// The activity that was current when this memory was rented.
            /// Used for tracing the lifecycle of memory usage.
            /// </summary>
            ...
            /// </exception>
            public ExactSizeMemoryOwner(ArraySegment<TOwner> segment, SensitiveMemoryPool<TOwner>.Slab<TOwner> slab, SensitiveMemoryPool<TOwner> pool)
            {
            ...
            public Memory<TOwner> Memory
            {
                get
                {
                    if(Disposed)
                    {
                        throw new ObjectDisposedException(nameof(ExactSizeMemoryOwner<TOwner>));
                    }
            ...
            public void Dispose()
            {
                if(!Disposed)
                {
                    //Create a disposal activity linked to the original rent activity.
                    using var activity = ActivitySource.StartActivity("Dispose", ActivityKind.Internal,
                        RentActivity?.Context ?? default);
                    activity?.AddTag("segmentSize", Segment.Count.ToString());
                    activity?.AddTag("segmentOffset", Segment.Offset.ToString());
