    ...
    public static class VerifiableCryptoFormatConversions
    {
        ...
        public static JwkToAlgorithmDelegate DefaultJwkToAlgorithmConverter => (jwk, memoryPool) =>
        {
            ...
            static (CryptoAlgorithm, Purpose) MapToAlgorithmAndPurpose(string keyType, string algorithm)
            {
                ...
                {
                    var (key, alg) when WellKnownKeyTypeValues.IsEc(key) && WellKnownJwaValues.IsEs256(alg) => (CryptoAlgorithm.P256, Purpose.Public),
                    var (key, alg) when WellKnownKeyTypeValues.IsEc(key) && WellKnownJwaValues.IsEs384(alg) => (CryptoAlgorithm.P384, Purpose.Public),
                    var (key, alg) when WellKnownKeyTypeValues.IsEc(key) && WellKnownJwaValues.IsEs512(alg) => (CryptoAlgorithm.P521, Purpose.Public),

                    var (key, alg) when WellKnownKeyTypeValues.IsOkp(key) && WellKnownCurveValues.IsEd25519(alg) => (CryptoAlgorithm.Ed25519, Purpose.Public),
                    ...
                    var (key, _) when WellKnownKeyTypeValues.IsRsa(key) => (DetermineRsaAlgorithm(), Purpose.Public),
        ...
        public static Base58ToAlgorithmDelegate DefaultBase58ToAlgorithmConverter => (base58Key, memoryPool, decoder) =>
        {
            ...
            {
                var h when Base58BtcEncodedMulticodecHeaders.IsSecp256k1PublicKey(h) => (CryptoAlgorithm.Secp256k1, Purpose.Public, EncodingScheme.Raw, decodedKeyMaterialWithoutHeader),
                var h when Base58BtcEncodedMulticodecHeaders.IsEd25519PublicKey(h) => (CryptoAlgorithm.Ed25519, Purpose.Public, EncodingScheme.Raw, decodedKeyMaterialWithoutHeader),
                ...
                var h when Base58BtcEncodedMulticodecHeaders.IsP256PublicKey(h) => (CryptoAlgorithm.P256, Purpose.Public, EncodingScheme.Raw, decodedKeyMaterialWithoutHeader),
                var h when Base58BtcEncodedMulticodecHeaders.IsP384PublicKey(h) => (CryptoAlgorithm.P384, Purpose.Public, EncodingScheme.Raw, decodedKeyMaterialWithoutHeader),
                var h when Base58BtcEncodedMulticodecHeaders.IsP521PublicKey(h) => (CryptoAlgorithm.P521, Purpose.Public, EncodingScheme.Raw, decodedKeyMaterialWithoutHeader),
                var h when Base58BtcEncodedMulticodecHeaders.IsRsaPublicKey2048(h) => (CryptoAlgorithm.Rsa2048, Purpose.Public, EncodingScheme.Raw, decodedKeyMaterialWithoutHeader),
                var h when Base58BtcEncodedMulticodecHeaders.IsRsaPublicKey4096(h) => (CryptoAlgorithm.Rsa4096, Purpose.Public, EncodingScheme.Raw, decodedKeyMaterialWithoutHeader),
                _ => throw new ArgumentException("Unsupported header or algorithm.", nameof(base58Key))
            };
        };
