...
namespace Verifiable.Core.Cryptography
{
    ...
    public static class VerifiableCryptoFormatConversions
    {
        ...
        public static JwkToAlgorithmDelegate DefaultJwkToAlgorithmConverter => static (jwk, memoryPool, base64UrlDecoder) =>
        {
            ...
            static byte[] DecodeKeyMaterial(Dictionary<string, object> jwk, string keyType, BufferAllocationDecodeDelegate decoder)
            {
            ...
            static byte[] DecodeEcKey(Dictionary<string, object> jwk, BufferAllocationDecodeDelegate decoder)
            {
            ...
            static byte[] DecodeOkpKey(Dictionary<string, object> jwk, BufferAllocationDecodeDelegate decoder)
            {
                //Fields already validated by ValidateRequiredFields.
            ...
            static byte[] DecodeRsaKey(Dictionary<string, object> jwk, BufferAllocationDecodeDelegate decoder)
            {
                //Fields already validated by ValidateRequiredFields.
            ...
            static IMemoryOwner<byte> DecodeForJwk(string encodedData, BufferAllocationDecodeDelegate decoder)
            {
                //For JWK, we don't have codec headers, so pass 0 as header length.
                return decoder(encodedData, 0, SensitiveMemoryPool<byte>.Shared);
            }
        ...
        public static Base58ToAlgorithmDelegate DefaultBase58ToAlgorithmConverter => (base58Key, memoryPool, base58Decoder) =>
        {
            ...
            var decodedKeyMaterialWithoutHeader = MultibaseSerializer.Decode(base58Key, memoryPool, base58Decoder);
