...
namespace Verifiable.Core.Model.Did
{
    ...
    /// var didDocument = await builder.BuildAsync(publicKey, verificationMethodType, "example.com");
    ///
    /// // Multiple key usage.
    /// var keyInputs = new[]
    /// {
    ...
    /// };
    /// var didDocument = await builder.BuildAsync(keyInputs, "example.com");
    /// </code>
    /// </example>
    public sealed class WebDidBuilder: Builder<DidDocument, WebDidBuildState, WebDidBuilder>
    {
            ...
            params string[] additionalContexts)
        {
            ...
            return BuildAsync(keyInputs, webDomain, representationType, didCoreVersion, additionalContexts);
        }
