...
namespace Verifiable.Core.Model.Did
{
    ...
    /// var didDocument = await builder.BuildAsync(publicKey, JsonWebKey2020VerificationMethod.Instance);
    ///
    /// // Multiple key usage.
    /// var keyInputs = new[]
    /// {
    ...
    /// };
    /// var didDocument = await builder.BuildAsync(keyInputs);
    /// </code>
    /// </example>
    public sealed class KeyDidBuilder: Builder<DidDocument, KeyDidBuildState, KeyDidBuilder>
    {
        ...
        /// </list>
        /// </remarks>
        public ValueTask<DidDocument> BuildAsync(IEnumerable<KeyMaterialInput> keyInputs, bool includeDefaultContext = false)
        {
        ...
        /// <returns>A <see cref="ValueTask{DidDocument}"/> containing the fully constructed DID document.</returns>
        public ValueTask<DidDocument> BuildAsync(PublicKeyMemory publicKey, VerificationMethodTypeInfo verificationMethodType, bool includeDefaultContext = false)
        {
            var keyInputs = new[]
            {
            ...
            return BuildAsync(keyInputs, includeDefaultContext);
        }
