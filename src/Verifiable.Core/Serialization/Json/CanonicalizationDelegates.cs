...
namespace Verifiable.Core.Serialization.Json;
...
/// This delegate abstracts the encoding to allow different encoding strategies.
/// </para>
/// <para>
/// <strong>Example implementation:</strong>
/// </para>
/// <code>
/// ProofValueEncoderDelegate encoder = (signatureBytes) =>
/// {
///     var base58Encoder = DefaultCoderSelector.SelectEncoder(typeof(PublicKeyMultibase));
///     return $"{MultibaseAlgorithms.Base58Btc}{base58Encoder(signatureBytes)}";
/// };
/// </code>
/// </remarks>
/// <param name="signatureBytes">The raw signature bytes to encode.</param>
/// <returns>The encoded proof value string (e.g., multibase-encoded).</returns>
public delegate string ProofValueEncoderDelegate(ReadOnlySpan<byte> signatureBytes);
...
/// <param name="memoryPool">Memory pool for allocating the decoded bytes.</param>
/// <returns>The decoded signature bytes. The caller must dispose the returned memory.</returns>
public delegate IMemoryOwner<byte> ProofValueDecoderDelegate(string proofValue, MemoryPool<byte> memoryPool);