...
public static class ApduConstants
{
    ...
    /// <summary>
    /// Maximum command data length (Lc) for short-length encoding.
    /// </summary>
    /// <remarks>
    /// A short Lc is a single byte (<c>0x01</c>–<c>0xFF</c>), so it carries at most 255 data bytes. Beyond
    /// that the command uses extended-length encoding: a <c>0x00</c> marker then a two-byte Lc.
    /// </remarks>
    public const int MaxShortCommandData = 255;
