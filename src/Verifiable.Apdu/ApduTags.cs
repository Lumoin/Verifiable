...
public static class ApduTags
{
    ...
    /// </summary>
    public static Tag Response { get; } = Tag.Create(ApduTagKind.Response);

    /// <summary>
    /// Tag for command APDU data field bytes.
    /// </summary>
    public static Tag CommandData { get; } = Tag.Create(ApduTagKind.CommandData);

    /// <summary>
    /// Tag for secure messaging session key material.
    /// </summary>
    public static Tag SecureMessagingKey { get; } = Tag.Create(ApduTagKind.SecureMessagingKey);

    /// <summary>
    /// Tag for secure messaging MAC value.
    /// </summary>
    public static Tag SecureMessagingMac { get; } = Tag.Create(ApduTagKind.SecureMessagingMac);

    /// <summary>
    /// Tag for PIN or password bytes submitted via VERIFY.
    /// </summary>
    public static Tag Pin { get; } = Tag.Create(ApduTagKind.Pin);

    /// <summary>
    /// Tag for the assembled content of a transparent elementary file (EF.COM, EF.SOD, a data group).
    /// </summary>
    public static Tag ElementaryFile { get; } = Tag.Create(ApduTagKind.ElementaryFile);

    /// <summary>
    /// Tag for the encoded wire bytes of a command APDU.
    /// </summary>
    public static Tag CommandApdu { get; } = Tag.Create(ApduTagKind.CommandApdu);

    /// <summary>
    /// Tag for the encoded wire bytes of a Secure Messaging protected command APDU.
    /// </summary>
    public static Tag ProtectedCommandApdu { get; } = Tag.Create(ApduTagKind.ProtectedCommandApdu);

    /// <summary>
    /// Tag for the encoded wire bytes of a Secure Messaging protected response APDU (the card side).
    /// </summary>
    public static Tag ProtectedResponseApdu { get; } = Tag.Create(ApduTagKind.ProtectedResponseApdu);

    /// <summary>
    /// Tag for the value of a GENERAL AUTHENTICATE dynamic authentication data object (DO'7C').
    /// </summary>
    public static Tag DynamicAuthenticationData { get; } = Tag.Create(ApduTagKind.DynamicAuthenticationData);

    /// <summary>
    /// Tag for an encoded biometric face image extracted from EF.DG2.
    /// </summary>
    public static Tag FaceImage { get; } = Tag.Create(ApduTagKind.FaceImage);

    /// <summary>
    /// Tag for a CBEFF-wrapped biometric data record extracted from EF.DG3 (finger) or EF.DG4 (iris).
    /// </summary>
    public static Tag BiometricRecord { get; } = Tag.Create(ApduTagKind.BiometricRecord);

    /// <summary>
    /// Tag for a chip's static Chip Authentication private key (the personalisation secret matching an EF.DG14 public key).
    /// </summary>
    public static Tag ChipAuthenticationPrivateKey { get; } = Tag.Create(ApduTagKind.ChipAuthenticationPrivateKey);

    /// <summary>
    /// Tag for a chip's Active Authentication private key (the personalisation secret matching the EF.DG15 public key).
    /// </summary>
    public static Tag ActiveAuthenticationPrivateKey { get; } = Tag.Create(ApduTagKind.ActiveAuthenticationPrivateKey);
    ...
    /// </summary>
    public static Tag RsaPublicKey { get; } = Tag.Create(ApduTagKind.RsaPublicKey);
    ...
    /// </summary>
    public static Tag CardVerifiableCertificate { get; } = Tag.Create(ApduTagKind.CardVerifiableCertificate);
    ...
    /// </summary>
    public static Tag CertificateHolderAuthorization { get; } = Tag.Create(ApduTagKind.CertificateHolderAuthorization);

    /// <summary>
    /// Tag for the displayed signature or usual mark image extracted from EF.DG7.
    /// </summary>
    public static Tag DisplayedSignature { get; } = Tag.Create(ApduTagKind.DisplayedSignature);

    /// <summary>
    /// Tag for the free-format optional-details content of EF.DG13.
    /// </summary>
    public static Tag OptionalDetails { get; } = Tag.Create(ApduTagKind.OptionalDetails);
}
