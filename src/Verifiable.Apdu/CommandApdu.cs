...
/// <summary>
/// Builds ISO/IEC 7816-4 command APDUs.
/// </summary>
/// <remarks>
/// <para>
/// ISO 7816-4 defines four command APDU cases based on the presence of
/// command data and expected response data:
/// </para>
/// <list type="bullet">
...
/// </list>
/// <para>
/// Both short-length (Lc/Le as single byte) and extended-length (Lc/Le as 3 bytes)
/// encodings are supported. Short-length is used by default; extended-length is used
/// when the data exceeds 255 bytes or when Le exceeds 256.
/// </para>
/// </remarks>
public static class CommandApdu
{
    ...
    /// <returns>Owned memory containing the command APDU. The caller must dispose it.</returns>
    public static IMemoryOwner<byte> BuildCase1(
    ...
    /// <param name="pool">Memory pool for the command buffer.</param>
    /// <returns>Owned memory containing the command APDU. The caller must dispose it.</returns>
    public static IMemoryOwner<byte> BuildCase2(
        byte cla, byte ins, byte p1, byte p2,
    ...
    /// <param name="pool">Memory pool for the command buffer.</param>
    /// <returns>Owned memory containing the command APDU. The caller must dispose it.</returns>
    public static IMemoryOwner<byte> BuildCase3(
        byte cla, byte ins, byte p1, byte p2,
    ...
    /// <param name="pool">Memory pool for the command buffer.</param>
    /// <returns>Owned memory containing the command APDU. The caller must dispose it.</returns>
    public static IMemoryOwner<byte> BuildCase4(
        byte cla, byte ins, byte p1, byte p2,
