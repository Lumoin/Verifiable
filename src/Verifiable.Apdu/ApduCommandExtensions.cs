...
public static class ApduCommandExtensions
{
    extension(ApduDevice device)
    {
        ...
        /// <summary>
        /// Issues an EXTERNAL AUTHENTICATE to prove the terminal's knowledge of a shared key to the card.
        /// </summary>
        /// <param name="data">The authentication token. For eMRTD Basic Access Control this is the 40-byte <c>EIFD || MIFD</c>.</param>
        /// <param name="expectedResponseLength">The expected response length (Le). BAC expects 40 bytes (<c>EIC || MIC</c>).</param>
        /// <param name="pool">The memory pool for command and response buffers.</param>
        /// <param name="cancellationToken">A cancellation token.</param>
        /// <returns>The parsed EXTERNAL AUTHENTICATE response, a card error, or a transport error.</returns>
