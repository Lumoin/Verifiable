...
public readonly struct StatusWord : IEquatable<StatusWord>
{
    ...
    /// <summary>Command not allowed — no current elementary file (0x6986).</summary>
    public static StatusWord NoCurrentElementaryFile { get; } = new(0x6986);

    /// <summary>Wrong parameters P1-P2 (0x6B00).</summary>
    public static StatusWord WrongP1P2 { get; } = new(0x6B00);

    /// <summary>Wrong length (0x6700).</summary>
    public static StatusWord WrongLength { get; } = new(0x6700);

    /// <summary>No precise diagnosis (0x6F00).</summary>
    public static StatusWord NoPreciseDiagnosis { get; } = new(0x6F00);

    /// <summary>Authentication failed (0x6300) — a warning the card returns for a rejected EXTERNAL AUTHENTICATE, including a failed eMRTD BAC mutual authentication.</summary>
    ...
    [
        ...
        NoCurrentElementaryFile, WrongP1P2, WrongLength, NoPreciseDiagnosis,
...
public static class StatusWordNames
{
    ...
    public static string GetName(ushort value)
    {
        ...
        string? isoName = value switch
        {
            ...
            var v when v == StatusWord.NoCurrentElementaryFile.Value => "Command not allowed: no current EF",
            var v when v == StatusWord.WrongP1P2.Value => "Wrong parameters P1-P2",
            var v when v == StatusWord.WrongLength.Value => "Wrong length",
            var v when v == StatusWord.NoPreciseDiagnosis.Value => "No precise diagnosis",
            var v when v == StatusWord.AuthenticationFailed.Value => "Authentication failed",
            _ => null
        };
