...
/// <summary>
/// Decodes one raw, already-captured protected-header <c>bstr</c>'s bytes into a <see cref="CBAdESProtectedHeaders"/>
/// aggregate — the inverse of <see cref="EncodeCBAdESProtectedHeaderDelegate"/>, exposed standalone (rather
/// than only as part of a full-message parse) so a caller holding one layer's raw protected-header bytes in
/// isolation — a <c>COSE_Sign</c> signer's own occurrence, captured via <c>CBAdESSignParseResult.Signers</c> —
/// can decode it without a full message to parse around it.
/// </summary>
/// <remarks>
/// Fail-closed: never throws for malformed input. Reuses the SAME map-decode rules
/// <c>ParseCBAdESSign1</c>'s own body-layer decode applies — RFC 9052's general <c>label: int / tstr</c> union,
/// RFC 8949 §4.2.3 canonical key ordering, <c>alg</c> mandatory (CB-5.1.2-01/CB-6.3-04) — never a forked
/// reading, so a <c>COSE_Sign</c> signer's own protected header is checked by the identical rule body a
/// <c>COSE_Sign1</c> body layer's protected header already is.
/// </remarks>
/// <param name="protectedHeaderBytes">The raw protected-header <c>bstr</c> content bytes (not itself <c>bstr</c>-wrapped).</param>
...
public delegate CBAdESProtectedHeaders? DecodeCBAdESProtectedHeaderDelegate(ReadOnlyMemory<byte> protectedHeaderBytes, BaseMemoryPool pool);


/// <summary>
/// Produces the unprotected-header dictionary
/// <see cref="Cose.SignAsync(EncodedCoseProtectedHeader, IReadOnlyDictionary{int, object}?, ReadOnlyMemory{byte}, BuildSigStructureDelegate, PrivateKeyMemory, BaseMemoryPool, CancellationToken)"/>
/// accepts, from an optional decoded <see cref="CBAdESUnsignedHeaders"/> model (layer placement and
/// header-map composition are COSE-structure semantics, so this seam — not the Pki
/// model — owns the projection onto <see cref="CoseSign1Message.UnprotectedHeader"/>'s shape).
/// </summary>
/// <remarks>
/// <para>
/// <strong>Shape decision, recorded here.</strong> <see cref="CoseSign1Message.UnprotectedHeader"/> is
...
/// CBOR byte string BEFORE being placed in the array — <see cref="CBAdESUnsignedHeaders"/>'s own remarks make
...
/// (<c>Verifiable.Cbor</c>) supplies the per-element bstr encoding through the
...
/// <c>uHeaders</c> member at all: the unprotected map carries AT MOST
...
/// <remarks>
/// <para>
/// <strong>Why this seam exists beside <see cref="EncodeCBAdESUnprotectedHeaderDelegate"/>
...
/// <remarks>
/// <para>
/// Never throws on malformed input (matches the <see cref="ParseCoseSign1Delegate"/>/
...
/// This follows this project's carrier discipline (carriers, not naked bytes): a validation caller typically re-serializes or re-hashes
...
public sealed class CBAdESSign1ParseResult: IDisposable
{
    ...
    /// concern (see the <c>CBAdESValidationResult</c> exemplar).
    /// </summary>
    ...
    /// <see langword="false"/>. Non-null iff <see cref="UnsignedHeaders"/> is non-null (the same
    /// raw-bytes precedent <see cref="RawProtectedHeader"/> follows, extended to <c>uHeaders</c>). The Annex A.1.2.1.2
