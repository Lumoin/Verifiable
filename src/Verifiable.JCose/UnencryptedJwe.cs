...
public sealed class UnencryptedJwe
{
    ...
    /// Optional pre-base64url-encoded value for the <c>apu</c> (Agreement PartyUInfo)
    /// protected-header parameter per RFC 7518 §4.6.1.2. OID4VP mdoc presentations carry
    /// the wallet's <c>mdoc_generated_nonce</c> here per ISO/IEC 18013-7 §B.4.4 so the
