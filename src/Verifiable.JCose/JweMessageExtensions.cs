...
public static class JweMessageExtensions
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        //The epk JWK shape follows the ephemeral key's tag: an EcUncompressed point
        //(0x04 || X || Y) splits into the x and y coordinates of an EC JWK; a Raw key
        //(OKP, e.g. X25519) is a single x coordinate with kty=OKP and no y.
        //Build the coordinate strings now — ReadOnlySpan<byte> cannot cross an await boundary.
        Tag epkTag = agreement.EphemeralPublicKey.Tag;
        ReadOnlySpan<byte> epkKeySpan = agreement.EphemeralPublicKey.AsReadOnlySpan();
        string crv = tagToCrvConverter(epkTag);

        Dictionary<string, object> epkJwk;
        if(epkTag.Get<EncodingScheme>().Equals(EncodingScheme.Raw))
        {
            epkJwk = new Dictionary<string, object>(3)
            {
        ...
        else
        {
            ...
            epkJwk = new Dictionary<string, object>(4)
            {
        ...
        IMemoryOwner<byte> epkOwner = pool.Rent(epkKeySpan.Length);
        epkKeySpan.CopyTo(epkOwner.Memory.Span);
