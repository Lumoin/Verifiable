...
/// <remarks>
/// <para>
/// <strong>No HTTP client ships in this library.</strong> Dereferencing a URI-reference —
...
/// </summary>
public abstract class CBAdESDetachedObjectDereferenceResult
{
...
public sealed class CBAdESDetachedObjectDereferenceSuccess : CBAdESDetachedObjectDereferenceResult
{
    ...
    /// </param>
    public CBAdESDetachedObjectDereferenceSuccess(PooledMemory content)
    {
        Content = content;
    }
...
/// </summary>
/// <remarks>
/// A failed attempt holds nothing but the reason it failed, so equality is that reason under an ordinal
/// comparison. Unlike its sibling success case, which owns rented memory, nothing here can be disposed out
...
public sealed class CBAdESDetachedObjectDereferenceFailure : CBAdESDetachedObjectDereferenceResult, IEquatable<CBAdESDetachedObjectDereferenceFailure>
{
    ...
    public CBAdESDetachedObjectDereferenceFailure(string reason)
    {
        Reason = reason;
    }

    /// <summary>A human-readable statement of why dereferencing failed.</summary>
    public string Reason { get; }

    /// <inheritdoc/>
    public bool Equals(CBAdESDetachedObjectDereferenceFailure? other)
    {
    ...
    {
        return Equals(obj as CBAdESDetachedObjectDereferenceFailure);
    }
    ...
    /// <summary>Reports whether two dereference failures state the same reason.</summary>
    public static bool operator ==(CBAdESDetachedObjectDereferenceFailure? left, CBAdESDetachedObjectDereferenceFailure? right)
    {
        return left is null ? right is null : left.Equals(right);
    }

    /// <summary>Reports whether two dereference failures state a different reason.</summary>
    public static bool operator !=(CBAdESDetachedObjectDereferenceFailure? left, CBAdESDetachedObjectDereferenceFailure? right)
    {
        return !(left == right);
    }
...
/// (no HTTP client ships in this library).
/// </summary>
/// <remarks>
...
/// (creation computes <c>hashV</c> itself under the
...
/// <remarks>
/// <para>
/// <strong>Scope, recorded here.</strong> This extension point covers COSE-Payload retrieval only, matching
...
/// third-party mechanism needs digests is outside what this extension point resolves; a known residue,
/// not a defect (the two mechanisms this document itself defines are fully
/// supported without this delegate).
/// </para>
/// <para>
/// <strong>Failure contract.</strong> Routine retrieval failure — the referenced object is
...
/// from this delegate at all) and <see cref="CBAdESSignatureValidation"/> (whose own catch narrows
...
/// convention: this is trusted-caller-triggered I/O, mirroring
