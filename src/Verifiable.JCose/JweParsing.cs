...
public static class JweParsing
{
            ...
            MemoryPool<byte> pool)
    {
        ...
        //An EC epk carries both coordinates; an OKP epk (e.g. X25519, RFC 8037) carries a
        //single x. y is therefore required only for EC.
        ...
        MemoryPool<byte> pool)
    {
        ...
        //A Raw-encoded exchange tag (OKP, e.g. X25519 per RFC 8037) is a single public
        //key, not an EC point: decode x as the whole key and skip the EC point-on-curve
        //check (Montgomery curves have no such check; clamping handles small subgroups).
        if(epkTag.Get<EncodingScheme>().Equals(EncodingScheme.Raw))
        {
            return new PublicKeyMemory(base64UrlDecoder(xEncoded, pool), epkTag);
        }

        //EC: x and y combine into an uncompressed point 0x04 || X || Y after validation.
        if(yEncoded is null)
        {
            throw new FormatException(
