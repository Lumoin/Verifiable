...
public ref struct JwkJsonWriter
{
    ...
    /// <summary>
    /// Writes a <c>"key":</c> property key — without any value — so the caller
    /// can follow it with <see cref="WriteObjectStart"/>, <see cref="WriteArrayStart"/>,
    /// or another value-writing primitive. Used to build nested JSON
    /// structures (a key whose value is itself an object or array) without
    /// pre-serialising the nested value to a string.
    /// </summary>
    /// <param name="key">The property key.</param>
