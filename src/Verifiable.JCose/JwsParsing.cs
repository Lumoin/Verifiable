...
public static class JwsParsing
{
    ...
    /// <returns>The parsed JWS message.</returns>
    /// <exception cref="ArgumentNullException">
    /// Thrown when any parameter is null.
    /// </exception>
    /// <exception cref="FormatException">
    /// Thrown when the compact serialization is malformed.
    /// </exception>
    public static JwsMessage ParseCompact(
        string compact,
        ...
        MemoryPool<byte> pool)
    {
        ...
        {
            using IMemoryOwner<byte> payloadBytesOwner = base64UrlDecoder(payloadEncoded, pool);
            payload = payloadBytesOwner.Memory.ToArray();
        }

        //Decode signature.
        using IMemoryOwner<byte> signatureBytesOwner = base64UrlDecoder(signatureEncoded, pool);
        byte[] signature = signatureBytesOwner.Memory.ToArray();
        ...
        return new JwsMessage(payload, signatureComponent, isDetached);
    }
    ...
    /// </exception>
    public static JwsMessage ParseFlattenedJson(
    ...
    /// Thrown when the JSON is malformed or missing required properties.
    /// </exception>
    public static JwsMessage ParseGeneralJson(
        ReadOnlySpan<byte> json,
        DecodeDelegate base64UrlDecoder,
        Func<ReadOnlySpan<byte>, IReadOnlyDictionary<string, object>> headerDeserializer,
        Func<ReadOnlySpan<byte>, Dictionary<string, object>> jsonDeserializer,
        MemoryPool<byte> pool)
    {
        ...
        var signatureComponents = new List<JwsSignatureComponent>();
        ...
        foreach(object sigObj in signaturesList)
        {
            ...
            signatureComponents.Add(new JwsSignatureComponent(
                protectedEncoded,
                protectedHeader,
                signature,
                unprotectedHeader));
        ...
        return new JwsMessage(payload, signatureComponents, isDetached);
    }
    ...
    /// <param name="jsonDeserializer">Delegate for deserializing the JSON object.</param>
    /// <param name="pool">Memory pool for allocations.</param>
    /// <param name="message">The parsed JWS message if successful.</param>
