...
/// one of the existing Pki models (<see cref="AdESCertificateThumbprints"/>,
/// <see cref="AdESSignerCommitments"/>, <see cref="AdESSignatureProductionPlace"/>,
/// <see cref="AdESSignerAttributes"/>, <see cref="CBAdESPayloadTimestamp"/>,
/// <see cref="AdESSignaturePolicyIdentifier"/>, <see cref="CBAdESDetachedObjects"/>), reached through the
...
/// the creation path (throw posture) and the validation path (collect posture). This
...
/// disposed-flag field is kept here, mirroring <see cref="CBAdESPayloadTimestamp"/>'s reasoning:
/// <see cref="Dispose"/> forwards unconditionally and relies on every owned member's own <c>Dispose</c> being
/// idempotent.
/// </para>
/// </remarks>
[DebuggerDisplay("CBAdESProtectedHeaders: alg={Algorithm}, iat={CwtClaims?.IssuedAt}")]
public sealed class CBAdESProtectedHeaders: IDisposable
{
    /// <summary>
    ...
    /// member) is a well-formed, non-conformant B-B message, not a malformed one —
    ...
    /// <c>int / tstr</c> label union RFC 9052 §3.1's <c>crit</c> syntax reuses unnarrowed.
    /// </param>
    ...
    /// <c>tstr</c>, the same CDDL shape <see cref="AdESDigestAlgorithmIdentifier"/> models as a two-arm
    ...
    /// clause 5.1.4 (SHOULD be a DER-encoded <c>IssuerSerial</c>, RFC 5035, carried uninterpreted). See
    /// <see cref="CoseKeyIdentifier"/> for the borrowed-view and equality contract. <see langword="null"/> when
    /// absent.
    /// </summary>
    public CoseKeyIdentifier? KeyId { get; }
    ...
    /// digest, reusing the <c>COSE_CertHash</c> model. Owned by this instance when present; disposed via
    /// <see cref="Dispose"/>. Part of the x5t/x5ts/x5chain tri-way (CB-5.2.2-07) enforced by
    /// <see cref="CBAdESHeaderRules"/>. <see langword="null"/> when absent.
    /// </summary>
    public AdESCertificateThumbprint? X5T { get; }
    ...
    /// (<see cref="CBAdESUnsignedHeaderElementCertificateChain"/>). Part of the x5t/x5ts/x5chain tri-way
    /// (CB-5.2.2-07) enforced by <see cref="CBAdESHeaderRules"/>. <see langword="null"/> when absent.
    /// </summary>
    ...
    /// (CB-5.2.2-07) enforced by <see cref="CBAdESHeaderRules"/>. <see langword="null"/> when absent.
    /// </summary>
    public AdESCertificateThumbprints? CertificateDigests { get; }
    ...
    /// claimed signing time (<c>iat</c>), carried signed-only (no legal unsigned path exists; see the
    /// <see cref="CBAdESCwtClaims"/> remarks). <see langword="null"/> when CWT Claims is absent entirely, or
    /// present with no <c>iat</c> member — both legal, non-conformant parsed states that
    ...
    /// entry is a <see cref="CoseHeaderLabel"/> — the general <c>int / tstr</c> union. Must
...
/// convention), and is carried signed-only ("no legal unsigned path exists" for
...
/// construction, exactly like every other required-member CB-AdES component this library models (e.g.
/// <see cref="AdESSignaturePolicyIdentifier"/>'s required <c>id</c>/<c>digAlgVal</c>) — reusing the untyped
...
/// </summary>
/// <remarks>
/// A distinct type from <see cref="CBAdESUnsignedHeaderElementCertificateChain"/>, which carries the
/// UNSIGNED <c>x5chain</c> occurrence's raw encoded bytes opaque pending a structured RFC 9360 COSE_X509
/// decode — a tracked residue, not chartered here: <c>COSE_Sign</c> +
/// RFC 9338 countersignature support ships without touching it, so the residue carries no
/// attribution until one is chartered for it. This SIGNED occurrence gets a structured closed sum now because
/// it is this library's own concern here (clause 5.1.8's <c>x5chain</c> signedness choice, CB-5.1.8-01/02/05), not
/// the same deferred substrate capability.
/// </remarks>
public abstract class CBAdESX5Chain
{
...
/// </summary>
/// <remarks>
/// Equality compares <see cref="Certificate"/>'s bytes rather than buffer identity, so two carriers wrapping
/// the same DER-encoded certificate are equal regardless of which buffer backs them — the same "same value"
/// contract <see cref="CoseKeyIdentifier"/> documents for the COSE <c>kid</c> carrier. <see cref="Certificate"/>
...
public sealed class CBAdESX5ChainSingleCertificate : CBAdESX5Chain, IEquatable<CBAdESX5ChainSingleCertificate>
{
    ...
    /// <inheritdoc/>
    public bool Equals(CBAdESX5ChainSingleCertificate? other)
    {
        return other is not null && Certificate.Span.SequenceEqual(other.Certificate.Span);
    }

    /// <inheritdoc/>
    public override bool Equals(object? obj)
    {
        return Equals(obj as CBAdESX5ChainSingleCertificate);
    }
    ...
    public override int GetHashCode()
    {
        ...
        hash.AddBytes(Certificate.Span);

        return hash.ToHashCode();
    }

    /// <summary>Reports whether two carriers hold the same DER-encoded certificate bytes.</summary>
    public static bool operator ==(CBAdESX5ChainSingleCertificate? left, CBAdESX5ChainSingleCertificate? right)
    {
        return left is null ? right is null : left.Equals(right);
    }

    /// <summary>Reports whether two carriers hold different DER-encoded certificate bytes.</summary>
    public static bool operator !=(CBAdESX5ChainSingleCertificate? left, CBAdESX5ChainSingleCertificate? right)
    {
        return !(left == right);
    }
