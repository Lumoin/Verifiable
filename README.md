...
- **DIDComm Messaging ([v2.1](https://identity.foundation/didcomm-messaging/spec/v2.1/))** — the full message layer (plaintext, signed, anoncrypt, authcrypt, and nested sign-then-encrypt over ECDH-ES / ECDH-1PU JWE), DID rotation via `from_prior`, routing/forward through a mediator (Routing 2.0), mediation coordination (Coordinate Mediation 2.0: mediate request/grant/deny and keylist management), message pickup (Message Pickup 3.0: queue status, batch delivery and acknowledgment, and Live Mode signaling) over the Return-Route extension's same-connection reply channel, out-of-band invitations, ACKs and problem reports, attachment resolution, the Discover Features 2.0 protocol, and a channel-pluggable transport (an HTTPS binding for one-way send and request/reply exchange, plus the seam any non-HTTP channel plugs into).
