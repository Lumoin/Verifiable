...
/// <summary>
/// The multi-server loopback-HTTP wire end-to-end leg for Associated Signature Containers: a
/// Time-Stamping Authority and an OCSP responder each run on their own loopback HTTP host, and every
/// <c>TimeStampReq</c>/<c>TimeStampResp</c> (<see
/// href="https://www.rfc-editor.org/rfc/rfc3161#section-3.4">IETF RFC 3161 §3.4</see>) and
/// <c>OCSPRequest</c>/<c>OCSPResponse</c> (<see href="https://www.rfc-editor.org/rfc/rfc6960#appendix-A">IETF
/// RFC 6960 Appendix A</see>) of the whole container flow crosses those real sockets as DER wire bytes — the
/// signature's time-stamp, the OCSP response placed as B-LT material, the token applied to the
/// <c>ASiCArchiveManifest</c> of Annex A.7 of
...
internal sealed class AsicMultiServerWireFlowTests
{
    ...
    /// Timestamp Renewal — every one of those exchanges crossing a real socket to its own loopback HTTP host — and a
    ...
    public async Task CreatesAugmentsAndRenewsAcrossTwoLoopbackHttpHostsAndTheContainerBytesValidateWithLiveOcsp()
    {
        ...
        using(attachment.EvidenceRecord)
        {
            ...
            Assert.AreEqual(AsicContainerValidationStatus.Valid, result.Status,
                $"A container created, augmented and renewed over two real loopback HTTP hosts validates from its own wire octets ({result.FailureReason}).");
