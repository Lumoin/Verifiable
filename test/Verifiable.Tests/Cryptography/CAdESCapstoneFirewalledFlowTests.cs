...
/// </para>
/// <para>
/// <strong>Leg 3 — "Verifiable creates → independent oracle verifies."</strong> <see
/// cref="MintCapstoneWorldAsync"/> checks its own output, before the firewall closes, against three readers
/// that share no code with the creation/augmentation surfaces: the independently-registered BouncyCastle <see
/// cref="VerifyCmsSignedDataDelegate"/> backend for the outer CMS signature, the independent BouncyCastle TSP
/// validator (<c>Org.BouncyCastle.Tsp.TimeStampToken.Validate</c>) for both time-stamp tokens, and its
/// independent <see cref="AtsHashIndexV3Oracle"/> reimplementation for the <c>ats-hash-index-v3</c> the archive
/// time-stamp carries.
/// </para>
/// <para>
...
/// the root's Subject Key Identifier.
/// </para>
/// </remarks>
[TestClass]
internal sealed class CAdESCapstoneFirewalledFlowTests
{
    ...
    /// ETSI EN 319 122-1 V1.3.1</see> 5.5.3-signedattrs-content-timestamp-poe.
    /// </remarks>
    [TestMethod]
    ...
    /// validator accepts both time-stamp tokens, and its independent hash-index reimplementation recomputes
    ...
    public async Task MintedArtifactsPassTheIndependentBouncyCastleOracleChainAcrossCmsTspAndTheHashIndex()
    {
        ...
        Assert.IsNotEmpty(message.SignedDataObject, "Minting completed and produced a wire Signed Data Object; the assertions above already ran against it.");
    }


    /// <summary>
    /// Post-augmentation property (§5.5.2 NOTE 5): a caller who received the B-LTA signature and augments it
    ...
    public async Task MaterialAddedAfterTheArchiveTimestampStillVerifiesAndLeavesTheEarlierCoverageUnchanged()
    {
        ...
        Assert.AreSequenceEqual(tokensBefore[0], tokensAfter[0], "§5.5.3 preservation: the earlier archive time-stamp's own octets are untouched by a later augmentation.");
    ...
    /// data-to-sign/signature-value split, raises it to B-T, B-LT and B-LTA through
    ...
    private static async ValueTask<CapstoneWireMessage> MintCapstoneWorldAsync(bool includeContentTimestamp, CancellationToken cancellationToken)
    {
        ...
        //B-B, through the data-to-sign/signature-value split: phase (1)'s output is signed by the leaf's
        ...
        // B-LTA, over the same independent TSP oracle; the ats-hash-index-v3 graft is the shipped surface's
        // own work, asserted against its independent reimplementation below.
