...
/// (FX-O) seeds EXACTLY the four this stage's shipped surface actually exercises (<c>CB-6.2.1-02</c>,
/// <c>CB-6.3-10</c>, <c>CB-6.3-a</c>, <c>CB-6.3-b</c>) — the remaining rows are an explicit, recorded follow-up
/// (an S7 candidate), not silently dropped.
/// </para>
/// <para>
...
internal sealed class CBAdESRequirementsMatrixTests
{
    ...
    [
        ...
        ("CB-5.2.8.2.3-07", "If the COSE Payload is required for purposes other than the COSE signature value (e.g. adoTst/arcTst), it shall be generated per clause 5.2.8.2.2.",
            RequirementCoverageStatus.OutOfScope, "Reverted at the wavecb S3 review (FX-Q): CBAdESSignatureCreationTests.ObjectIdByUriPayloadIsOrderPreservingConcatenation proves only the CB-5.2.8.2.2-05 reconstruction primitive itself (that row's own citation; not repeated here, to avoid double-counting a component-level proof as this row's own e2e claim) -- it does not exercise adoTst or arcTst actually consuming that reconstruction as their own message-imprint input, which is what this row states. adoTst's ObjectIdByURIHash-sourced imprint input is deferred to CB-AdES wave stage S4 (the TSA wire seam / B-T augmentation, wavecb-contract.md); arcTst's is deferred to CB-AdES wave stage S5 (arcTst generation orchestration per clause 5.3.5.2)."),
        ...
        //Clause 6 -- baseline-level presence rules and additional requirements (leg 4, wavecb S3 FX-O). Only the
        //four rows this stage's shipped surface actually exercises are seeded here; the remaining ~50 leg-4
        //Table 14 presence-per-level rows are an explicit follow-up (S7 candidate, recorded in the fix report),
        ...
        ("CB-A.1.1-02", "refs shall not contain the signing certificate of the CB-AdES signature itself.",
            RequirementCoverageStatus.OutOfScope, "Re-attributed at the wavecb S3 review: deferred to CB-AdES wave stage S4, not S3 -- the refs mechanism (Annex A) and any cross-check against the actual composed signing certificate are an explicit S4 deliverable (wavecb-contract.md stage plan); S3 ships a real composed B-B signature (CBAdESSignatureCreation) but does not consume refs at all."),
