...
internal sealed class MlDsaCmsVerificationTests
{
    ...
    /// not perform, and is refused rather than resolved in the claimed set's favor. The refusal is the
    ...
    /// attribute, so the patched, unsigned <c>SignerInfo</c> field disagrees with the signed copy — the
    /// substitution §1 describes, caught ahead of the parameter-set binding behind it.
    /// </summary>
    [TestMethod]
    public async Task TheManagedBackendRefusesAnMlDsaParameterSetSubstitution()
    {
        ...
        //The refusal is the RFC 6211 §3.1 item 2 compare's own — the signed attribute pins ML-DSA-65 while
        //the patched SignerInfo names ML-DSA-87 — not the parameter-set binding behind it, and not any other
        //failure sharing the exception type.
    ...
    /// <summary>
    /// The parameter-set binding — the <c>SignerInfo</c> signature algorithm's set must equal the certificate
    /// key's — refuses the substitution even when the RFC 6211 compare cannot see one: patching the LAST TWO
    /// occurrences of the ML-DSA-65 arc (the signed <c>CMSAlgorithmProtection</c> copy inside
    /// <c>signedAttrs</c> and the <c>SignerInfo</c> signature algorithm that follows it, RFC 5652 §5.3's
    /// field order) to ML-DSA-87 leaves the two agreeing with each other while the certificate key stays
    /// ML-DSA-65, so the <see href="https://www.rfc-editor.org/rfc/rfc6211#section-3.1">RFC 6211 §3.1</see>
    /// compare passes and the binding behind it is what refuses — the invariant holds for a shape the
    /// attribute cannot catch.
    /// </summary>
    [TestMethod]
    public async Task TheManagedBackendRefusesAnMlDsaParameterSetSubstitutionWhenTheProtectionAttributeAgrees()
    {
        using MlDsaCmsTestFactory.MlDsaSigningAuthority authority = MlDsaCmsTestFactory.MintSelfSignedAuthority(WellKnownOids.MlDsa65, NotBefore, NotAfter);
        using CmsSignedData carrier = MlDsaCmsTestFactory.SignAsCms("the ml-dsa agreeing-substitution fixture"u8, authority);

        //The certificate's own arcs precede the SignerInfo in the SignedData layout, so the last two
        //occurrences are the protection attribute's copy and the SignerInfo field — the pair that must agree
        //for the §3.1 compare to pass while both disagree with the key.
        ReadOnlySpan<byte> mlDsa65OidValue = WellKnownOids.MlDsa65DerValue;
        ReadOnlySpan<byte> encoded = carrier.AsReadOnlySpan();
        var matchIndexes = new List<int>();
        for(int i = 0; i <= encoded.Length - mlDsa65OidValue.Length; i++)
        {
            if(encoded.Slice(i, mlDsa65OidValue.Length).SequenceEqual(mlDsa65OidValue))
            {
                matchIndexes.Add(i);
            }
        }

        //The ML-DSA-65 arc occurs exactly five times in this fixture: the certificate's SubjectPublicKeyInfo
        //algorithm, the certificate's tbsCertificate signature field, the certificate's outer
        //signatureAlgorithm, the RFC 6211 CMSAlgorithmProtection signed attribute's copy, and the SignerInfo
        //signature algorithm. An exact count — not a floor — so a change in BouncyCastle's emission (an added
        //or dropped occurrence) fails this guard loudly instead of silently changing which two occurrences the
        //patch below reaches.
        Assert.HasCount(5, matchIndexes, "The parameter-set arc must occur exactly five times: the certificate's SubjectPublicKeyInfo algorithm, tbsCertificate signature field and outer signatureAlgorithm, the RFC 6211 CMSAlgorithmProtection signed attribute's copy, and the SignerInfo signature algorithm.");

        byte[] patched = encoded.ToArray();
        patched[matchIndexes[^1] + mlDsa65OidValue.Length - 1] = WellKnownOids.MlDsa87DerValue[^1];
        patched[matchIndexes[^2] + mlDsa65OidValue.Length - 1] = WellKnownOids.MlDsa87DerValue[^1];
        using CmsSignedData substituted = CmsSignedData.FromBytes(patched, BaseMemoryPool.Shared);
        ...
        CryptographicException refusal = await Assert.ThrowsExactlyAsync<CryptographicException>(
            ...
            "An ML-DSA signature claimed under a different parameter set than the key's must be refused by the binding when the protection attribute agrees with the substituted identifier.").ConfigureAwait(false);

        //The parameter-set binding's refusal, reached because the patched attribute and SignerInfo agree —
        //proving the binding does not rest on the RFC 6211 compare alone.
