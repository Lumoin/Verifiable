...
internal sealed class EvidenceRecordRequirementsMatrixTests
{
    ...
    [
        ...
        ("rfc6283-2.1-S1", "The renewal process MUST continue during the whole desired archiving period.",
            RequirementCoverageStatus.OutOfScope, "An operational obligation on the archive's own lifecycle: WHEN to renew is the custodian's monitoring duty, not a library surface. Initial XMLERS creation ships (XmlEvidenceRecords.CreateInitialAsync); RENEWAL creation in this syntax remains an open item, because clause 4.2's procedures digest sub-trees of the document as it stands — a seam-design question of its own. The equivalent monitoring obligation of the ASN.1 form is the rfc4998-5-R22 row."),
        ...
        ("rfc6283-2.1-S8", "Policy or attribute information available at or before an individual renewal step SHOULD be stored in that Archive Time-Stamp's Attributes element rather than in SupportingInformation.",
            RequirementCoverageStatus.OutOfScope, "A placement obligation at RENEWAL time, whose creation in this syntax remains an open item (initial creation ships and places no attributes, which the SHOULD does not oblige it to). The read side of both placements is covered by the S4 and S20 rows."),
        ...
        ("rfc6283-2.2-S23", "Generation of an EvidenceRecord MUST follow the three-step procedure: select the archive object, create the initial Archive Time-Stamp, refresh it by renewal when necessary.",
            RequirementCoverageStatus.Tested, "XmlEvidenceRecordCreationTests.AnInitialRecordOverOneGroupVerifiesFromItsOctetsAndTheOracleRecomputesItsRoot (steps 1 and 2: the archive object is selected and its initial Archive Time-Stamp created, the produced document verified from its octets alone and its root recomputed by the independent oracle). Step 3's RENEWAL creation in this syntax remains an open item — clause 4.2's procedures digest sub-trees of the document as it stands, a seam-design question of its own — while every structure renewal produces IS verified against documents an independent factory minted."),
        ...
        ("rfc6283-3-S25", "An Archive Time-Stamp MUST be renewed before it becomes invalid through a weakening algorithm, certificate invalidation or the Time-Stamping Authority ceasing operation.",
            RequirementCoverageStatus.OutOfScope, "Operationally triggered renewal: the TIMING is the monitoring guidance the contract's Out list excludes, and RENEWAL creation in this syntax remains an open item (initial creation ships). Both renewal procedures' resulting structures are verified (the S68-S75 rows' verification halves)."),
        ...
        ("rfc6283-3.1.1-S27", "When a single Time-Stamp is obtained for a group of archive objects, a hash tree MUST be constructed to bind them.",
            RequirementCoverageStatus.Tested, "XmlEvidenceRecordCreationTests.TwoGroupsShareOneTimeStampAndEachDocumentProvesExactlyItsOwnGroup (two archive objects under one token, bound by one tree the independent oracle recomputes to exactly the token's imprint) — and the verification consequence stays driven: XmlEvidenceRecordsTests.AGroupWithNoHashTreeIsRefusedForTheMissingTree refuses a group whose record carries no tree."),
        ("rfc6283-3.1.1-S28", "A reduced hash tree MUST then be calculated per archive object.",
            RequirementCoverageStatus.Tested, "XmlEvidenceRecordCreationTests.TwoGroupsShareOneTimeStampAndEachDocumentProvesExactlyItsOwnGroup (one reduced tree per archive object: each produced document proves exactly its own group and is no proof about the other's) — the walk that consumes what the reduction produces stays driven by XmlEvidenceRecordHashTreeTests.AFourLevelTreeReachesTheRootTheOracleReaches."),
        ...
        ("rfc6283-3.2-S41", "Initial Archive Time-Stamp generation MUST follow the five-step procedure: collect objects, select the canonicalization method, select the hash algorithm, generate the hash tree, acquire the Time-Stamp Token.",
            RequirementCoverageStatus.Tested, "XmlEvidenceRecordCreationTests.AnInitialRecordOverOneGroupVerifiesFromItsOctetsAndTheOracleRecomputesItsRoot (the five steps end to end, the acquired token's imprint asserted octet for octet against the tree's root before anything is written) with AnXmlDataObjectIsDigestedOverItsCanonicalFormNotItsRawOctets for step 2's canonicalization selection; the reference leg keeps proving verification against documents this project did not produce (ReferenceArtifactXmlEvidenceRecordTests.ARecordOfTheCorpusProvesADataObjectOfTheCorpus)."),
