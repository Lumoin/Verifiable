...
/// <summary>
/// The multi-server loopback-HTTP wire e2e leg for CAdES creation and long-term augmentation: a Time-Stamping
/// Authority and an OCSP responder each run on their own loopback HTTP host, and the signer's
/// and verifier's own <c>TimeStampReq</c>/<c>TimeStampResp</c> (<see
/// href="https://www.rfc-editor.org/rfc/rfc3161#section-3.4">RFC 3161 §3.4</see>) and
/// <c>OCSPRequest</c>/<c>OCSPResponse</c> (<see href="https://www.rfc-editor.org/rfc/rfc6960#appendix-A">RFC 6960
/// Appendix A</see>) exchanges cross those real sockets as DER wire bytes.
/// </summary>
/// <remarks>
/// <para>
/// <strong>Host A (TSA)</strong> answers every <c>TimeStampReq</c> it receives with a genuine token minted
/// through the independent BouncyCastle TSP oracle (<see cref="MintingTimestampResponder"/>, its whole TSA
/// behaviour minus the socket — unmodified, now driven by a real HTTP POST rather than an in-process call).
...
/// </para>
/// <para>
/// <strong>Live, OCSP-driven revocation checking.</strong> <see cref="CAdESCapstoneFirewalledFlowTests"/>'s
/// verifying party decided revocation status from CRL material embedded in the received Signed Data Object.
/// The verifying party here decides it
/// through a real <see cref="OcspRevocationChecker"/> that makes its own live <c>OCSPRequest</c> to Host B
/// during validation — the certificate chains of the CAdES signer AND of the Time-Stamping Authority (both
/// certificates <see cref="BasicSignatureValidation"/> chain-validates) are checked this way, which is why <see
/// cref="MintingOcspResponder"/> answers for a small certificate set matched by serial number rather than one
/// fixed target.
/// </para>
/// <para>
/// <strong>Object-lifetime discipline.</strong> Unlike <see cref="CAdESCapstoneFirewalledFlowTests"/>'s
/// in-process capstone, the signing party's certificates and keys cannot be released before the verifying
/// party runs: Host B must keep answering correctly through the verifying party's OWN live OCSP call, which
/// needs the same signing-key material the responder was configured with. The firewall this leg demonstrates
/// is therefore this: the verifier reconstructs from wire bytes only — no shared objects, keys, or salts —
/// enforced at the level of what the VERIFYING CODE PATH touches (a byte array for the Signed Data Object, a
/// byte array for the trust anchor certificate, and the network), never at the level of process memory
/// lifetime, which a live second network peer cannot honour the way a capstone with only embedded material can.
/// </para>
/// </remarks>
[TestClass]
internal sealed class CAdESMultiServerWireFlowTests
{
    ...
    /// <c>OCSPRequest</c>/<c>OCSPResponse</c> crossing a real socket to its own loopback HTTP host, then a verifying
    ...
    /// </summary>
    /// <remarks>
    /// Proves <see href="https://www.etsi.org/deliver/etsi_en/319100_319199/31912201/01.03.01_60/en_31912201v010301p.pdf">
    /// ETSI EN 319 122-1 V1.3.1</see> 6.3-req-l, 6.3-req-o, 6.3-req-r, 5.5.2-hashIndAlgorithm-identifies,
    /// 5.5.2-hashIndAlgorithm-matches-tst, 5.5.2-certificatesHashIndex, 5.5.2-crlsHashIndex,
    /// 5.5.2-hash-input-tlv, 5.5.3-imprint-concatenation, 5.5.3-step2-hash-algorithm-match,
    /// 5.5.3-single-ats-hash-index, 5.5.3-precondition-extend, 5.5.3-strategy1-root, 5.5.3-preservation.
    /// </remarks>
    [TestMethod]
    ...
    public async Task CreatesAugmentsToBLtaAndReachesTotalPassedAcrossTwoLoopbackHttpHostsWithLiveOcspDrivenRevocation()
    {
        ...
        // Host A (TSA), answering every request with a genuine token minted through its independent oracle.
        ...
        //=== B-B, through the data-to-sign/signature-value split: phase (1)'s output is signed by the
        ...
        // (its CheckRetainingResponseAsync surface, now fed by a real socket) become the B-LT material. ===
        ...
        // === Verifying party: reconstructs from the wire bytes alone (SignedDataObject + trust anchor
        // octets); revocation is decided by the verifier's OWN live OCSPRequest/OCSPResponse round trip to
        // Host B for BOTH the signer's certificate and the Time-Stamping Authority's own certificate —
        // OCSP-DRIVEN, not read from embedded material. ===
        ...
        var constraints = new SignatureValidationConstraints
        {
            ...
            // coverage and proof-of-existence extraction for real, mirroring CAdESCapstoneFirewalledFlowTests's
            // own finding.
        ...
        Assert.AreEqual(SignatureValidationIndication.TotalPassed, outcome.Conclusion.Indication,
            "Creation and augmentation to B-LTA over two real loopback HTTP hosts, verified with live OCSP-driven revocation over a real socket, reaches TOTAL_PASSED.");
    }
    ...
    /// <remarks>
    /// Proves <see href="https://www.etsi.org/deliver/etsi_en/319100_319199/31912201/01.03.01_60/en_31912201v010301p.pdf">
    /// ETSI EN 319 122-1 V1.3.1</see> 6.3-req-r, 5.4.2.2-ocsp-reader-recognition.
    /// </remarks>
    [TestMethod]
