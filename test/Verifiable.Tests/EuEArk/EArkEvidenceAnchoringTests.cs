...
/// <remarks>
/// <para>
/// <strong>This library's sharpest test lives here, and it asserts both directions.</strong> An archival
/// package's provenance is a chain of preservation events pointing at one another in plain text, and the
/// manifest's stated digests over those documents are plain text too — so a producer that rewrites a provenance
/// document and its stated digest together leaves a package that satisfies every structural, metadata and fixity
/// rule of both specifications. The test below builds exactly that package and asserts <em>both</em> facts about
/// it at once: every plain-text rule still passes, and the evidence over the same octets fails. Either half
/// alone would be half a statement.
/// </para>
/// <para>
...
internal sealed class EArkEvidenceAnchoringTests
{
    ...
    public async Task TheManifestIsInsideTheAnchorByDefaultAndLeavingItOutIsTheCallersStatement()
    {
        ...
        EArkProvenanceAnchorContext conformantContext = package.ToAnchorContext();
        EArkProvenanceAnchorPlan withoutManifest = EArkEvidenceAnchoring.StatePlan(new EArkProvenanceAnchorContext
        {
            Snapshot = conformantContext.Snapshot,
            PackageManifest = conformantContext.PackageManifest,
            ManifestEntryName = conformantContext.ManifestEntryName,
            CoverManifest = false,
            AdditionalEntryNames = conformantContext.AdditionalEntryNames,
            MaximumCoveredEntries = conformantContext.MaximumCoveredEntries,
        });
    ...
    public async Task AnEntryNamedTwiceIsCoveredOnce()
    {
        ...
        EArkProvenanceAnchorContext conformantContext = package.ToAnchorContext();
        EArkProvenanceAnchorPlan plan = EArkEvidenceAnchoring.StatePlan(new EArkProvenanceAnchorContext
        {
            Snapshot = conformantContext.Snapshot,
            PackageManifest = conformantContext.PackageManifest,
            ManifestEntryName = conformantContext.ManifestEntryName,
            CoverManifest = conformantContext.CoverManifest,
    ...
    public async Task APlanThatCannotBeStatedCoversNothingAndSaysWhy()
    {
        ...
        MetsDocument noProvenance = new()
        {
            ObjectIdentifier = package.Manifest.ObjectIdentifier,
            ContentCategory = package.Manifest.ContentCategory,
            Profile = package.Manifest.Profile,
            Header = package.Manifest.Header,
            OtherContentCategory = package.Manifest.OtherContentCategory,
            ContentInformationType = package.Manifest.ContentInformationType,
            OtherContentInformationType = package.Manifest.OtherContentInformationType,
            DescriptiveMetadataSections = package.Manifest.DescriptiveMetadataSections,
            AdministrativeMetadata = new()
            {
                DigitalProvenanceSections = [],
                RightsSections = package.Manifest.AdministrativeMetadata!.RightsSections,
            },
            FileSection = package.Manifest.FileSection,
            StructuralMaps = package.Manifest.StructuralMaps,
        };

        EArkProvenanceAnchorPlan none = EArkEvidenceAnchoring.StatePlan(new EArkProvenanceAnchorContext
        {
            Snapshot = conformant.Snapshot,
            PackageManifest = noProvenance,
            ManifestEntryName = conformant.ManifestEntryName,
            CoverManifest = conformant.CoverManifest,
            AdditionalEntryNames = conformant.AdditionalEntryNames,
            MaximumCoveredEntries = conformant.MaximumCoveredEntries,
        });
        ...
        MetsAdministrativeMetadataSection firstProvenanceSection = package.Manifest.AdministrativeMetadata!.DigitalProvenanceSections[0];

        //This section is minted here rather than shared from the package, so it owns the reference it carries
        //and this scope releases it; the manifest below only aliases the package's own sections.
        using MetsAdministrativeMetadataSection danglingProvenanceSection = new()
        {
            Id = firstProvenanceSection.Id,
            Status = firstProvenanceSection.Status,
            Reference = EArkValidationSource.Reference("metadata/preservation/absent.xml"),
        };

        MetsDocument danglingReference = new()
        {
            ObjectIdentifier = package.Manifest.ObjectIdentifier,
            ContentCategory = package.Manifest.ContentCategory,
            Profile = package.Manifest.Profile,
            ...
            AdministrativeMetadata = new()
            {
                DigitalProvenanceSections = [danglingProvenanceSection],
                RightsSections = package.Manifest.AdministrativeMetadata!.RightsSections,
            },
            FileSection = package.Manifest.FileSection,
            StructuralMaps = package.Manifest.StructuralMaps,
        ...
        EArkProvenanceAnchorPlan missing = EArkEvidenceAnchoring.StatePlan(new EArkProvenanceAnchorContext
        {
            Snapshot = conformant.Snapshot,
            PackageManifest = danglingReference,
            ManifestEntryName = conformant.ManifestEntryName,
            CoverManifest = conformant.CoverManifest,
            AdditionalEntryNames = conformant.AdditionalEntryNames,
            MaximumCoveredEntries = conformant.MaximumCoveredEntries,
        ...
        EArkProvenanceAnchorPlan manifestMissing = EArkEvidenceAnchoring.StatePlan(new EArkProvenanceAnchorContext
        {
            Snapshot = conformant.Snapshot,
            PackageManifest = conformant.PackageManifest,
            ManifestEntryName = "SomeOtherManifest.xml",
            CoverManifest = conformant.CoverManifest,
            AdditionalEntryNames = conformant.AdditionalEntryNames,
            MaximumCoveredEntries = conformant.MaximumCoveredEntries,
        });
        ...
        EArkProvenanceAnchorPlan overTheLimit = EArkEvidenceAnchoring.StatePlan(new EArkProvenanceAnchorContext
        {
            Snapshot = conformant.Snapshot,
            PackageManifest = conformant.PackageManifest,
            ManifestEntryName = conformant.ManifestEntryName,
    ...
    public async Task ADocumentAddedToTheCoveredSetFailsTheGroupCheck()
    {
        ...
        EArkProvenanceAnchorContext conformantContext = package.ToAnchorContext();
        EArkProvenanceAnchorContext enlarged = new()
        {
            Snapshot = conformantContext.Snapshot,
            PackageManifest = conformantContext.PackageManifest,
            ManifestEntryName = conformantContext.ManifestEntryName,
    ...
    public async Task APackageNoPlanCanBeStatedFromIsReportedAsSuchAndNeverAsAnchored()
    {
        ...
        using EArkProvenanceAnchorVerification verification = await EArkEvidenceAnchoring.VerifyProvenanceAnchorAsync(
            ...
            {
                EvidenceRecord = creation.EvidenceRecords[0],
                Package = new EArkProvenanceAnchorContext
                {
                    Snapshot = conformantContext.Snapshot,
                    PackageManifest = noProvenance,
                    ManifestEntryName = conformantContext.ManifestEntryName,
    ...
    public async Task TheAnchorRuleReadsWhetherTheProvenanceIsInsideWhatAnArtifactProves()
    {
        ...
        EArkValidationContext anchoredContext = package.ToValidationContext() with
        {
            EvidenceArtifacts =
            [
                new EArkEvidenceArtifactFacts
                {
                    Kind = package.Artifact.Kind,
                    EntryName = package.Artifact.EntryName,
                    RepresentationLabel = package.Artifact.RepresentationLabel,
                    CoveredEntryNames = plan.CoveredEntryNames(),
                    Evidence = creation.EvidenceRecords[0],
                    SelfDescription = package.Artifact.SelfDescription,
                    SelfDescriptionCarrier = package.Artifact.SelfDescriptionCarrier,
                    SelfDescriptionIsProtected = package.Artifact.SelfDescriptionIsProtected,
                }
            ],
        };
    ...
    public async Task AProvenanceRewrittenAfterTheAnchorFailsTheRuleAlthoughEveryCoveredNameStillMatches()
    {
        ...
        EArkEvidenceArtifactFacts statedAnchor = new()
        {
            Kind = anchored.Artifact.Kind,
            EntryName = anchored.Artifact.EntryName,
            RepresentationLabel = anchored.Artifact.RepresentationLabel,
            CoveredEntryNames = coveredNames,
            Evidence = creation.EvidenceRecords[0],
            SelfDescription = anchored.Artifact.SelfDescription,
            SelfDescriptionCarrier = anchored.Artifact.SelfDescriptionCarrier,
            SelfDescriptionIsProtected = anchored.Artifact.SelfDescriptionIsProtected,
        };
        ...
        EArkValidationContext context = package.ToValidationContext() with
        {
            ...
            [
                new EArkEvidenceArtifactFacts
                {
                    ...
                    RepresentationLabel = package.Artifact.RepresentationLabel,
                    CoveredEntryNames = plan.CoveredEntryNames(),
                    Evidence = package.Artifact.Evidence,
                    SelfDescription = package.Artifact.SelfDescription,
                    SelfDescriptionCarrier = package.Artifact.SelfDescriptionCarrier,
                    SelfDescriptionIsProtected = package.Artifact.SelfDescriptionIsProtected,
                }
