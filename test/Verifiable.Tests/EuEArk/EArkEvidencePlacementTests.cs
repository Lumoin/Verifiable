...
internal sealed class EArkEvidencePlacementTests
{
    ...
    public void TheIdentifierFoldIsNotInjectiveAndTheConventionSaysSo()
    {
        ...
        Assert.AreNotEqual(
            ...
            EArkEvidenceWellKnown.ToIdentifierToken("metadata/other/A.ers"),
            "Comparison is ordinal everywhere in this codebase, and the fold keeps what the production admits.");
    }
    ...
    public async Task APlacementIsRefusedForWhatTheConventionCannotState()
    {
        ...
        EArkEvidencePlacementContext ingestionContext = ContextWith(new EArkEvidenceArtifactFacts { Kind = EArkEvidenceKind.EvidenceRecord, EntryName = entryName }, file);
        _ = await Assert.ThrowsAsync<ArgumentException>(async () => await EArkEvidencePlacement.StatePlacementAsync(
            new EArkEvidencePlacementContext
            {
                Artifact = ingestionContext.Artifact,
                Entry = ingestionContext.Entry,
                Instant = ingestionContext.Instant,
                AttestedObjectIdentifiers = ingestionContext.AttestedObjectIdentifiers,
                AgentIdentifiers = ingestionContext.AgentIdentifiers,
                DigestAlgorithm = ingestionContext.DigestAlgorithm,
                EventType = "ingestion",
                EventOutcome = ingestionContext.EventOutcome,
                RelatedEventIdentifier = ingestionContext.RelatedEventIdentifier,
            },
            BaseMemoryPool.Shared,
            TestContext.CancellationToken).ConfigureAwait(false)).ConfigureAwait(false);
    ...
    /// </summary>
    /// <remarks>
    /// Proves <see href="https://www.etsi.org/deliver/etsi_ts/119500_119599/119511/01.02.01_60/ts_119511v010201p.pdf">ETSI TS 119 511 V1.2.1</see>
    /// OVR-9.2-04.
    /// </remarks>
    ...
    public async Task AnArtifactSittingWhereTheConventionDoesNotPutOneFailsThePlacementRule()
    {
        ...
        EArkEvidenceArtifactFacts misplaced = new()
        {
            Kind = artifact.Facts.Kind,
            EntryName = EArkWellKnown.PreservationMetadataFolderName + "/provenance.ers",
            RepresentationLabel = artifact.Facts.RepresentationLabel,
            CoveredEntryNames = artifact.Facts.CoveredEntryNames,
            Evidence = artifact.Facts.Evidence,
            SelfDescription = artifact.Facts.SelfDescription,
            SelfDescriptionCarrier = artifact.Facts.SelfDescriptionCarrier,
            SelfDescriptionIsProtected = artifact.Facts.SelfDescriptionIsProtected,
        };
