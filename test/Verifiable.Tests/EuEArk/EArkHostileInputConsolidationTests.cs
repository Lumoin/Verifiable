...
/// <summary>
/// The hostile-input prong of this library, consolidated where the per-component tests left a gap: the two
/// ways a package can arrive answering the same refusals, the container-format edge the archive path inherits,
/// one attack shape carried by three different mechanisms, and what the rule lists do when the document they
/// judge never reaches them.
/// </summary>
/// <remarks>
/// <para>
...
/// </para>
/// <para>
/// <strong>The shape this library's ancestor established.</strong> An attack is built once and run through
/// every mechanism that could answer it, and the assertions are about how the answers relate — identical where
/// the two package paths must agree, different where three mechanisms answer three different questions. A test
/// that exercised each mechanism in isolation would pass just as well if two of them had been collapsed into
/// one.
/// </para>
/// </remarks>
[TestClass]
internal sealed class EArkHostileInputConsolidationTests
{
    ...
    /// eight refusals of the generic archive-safety layer this library reuses —
    ...
    /// name bound of its own — and this library's "ONE shared limits record" is exactly the promise that they do
