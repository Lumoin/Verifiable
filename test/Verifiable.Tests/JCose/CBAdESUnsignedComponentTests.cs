...
/// <summary>
/// Model-invariant tests for the CB-AdES unsigned components this agent owns — <c>sigPSt</c>
/// (<see cref="CBAdESSignaturePolicyStore"/>), <c>valData</c> (<see cref="CBAdESValidationData"/>), <c>refs</c>
/// (<see cref="CBAdESReferences"/> and its sub-types), and the four <see cref="AdESTimestampContainer"/> alias
...
/// </summary>
/// <remarks>
/// This file exercises construction-time invariants (guard clauses, disposal idempotency) and value-equality
/// behavior — the CDDL wire shape, round-trip byte-exactness, and strict-parse negatives for the same
/// components live in <c>CBAdESUnsignedComponentSerializationTests</c>. <see cref="DigestValue"/> fixtures are
/// real SHA-256 digests computed through the registered <see cref="CryptographicKeyEvents"/> digest delegate
/// seam (via <see cref="CreateDigestAsync"/>), never a hand-rolled hash, matching
/// <c>CBAdESSignedHeaderModelTests</c>'s own convention.
/// </remarks>
[TestClass]
internal sealed class CBAdESUnsignedComponentTests
{
    ...
    /// <summary>
    /// CB-5.3.2-04: two <see cref="CBAdESSignaturePolicyStore"/> instances built from separately-allocated but
    /// value-equal <see cref="CBAdESSignaturePolicyStoreDocument"/> content and <see cref="AdESObjectIdentifier"/>
    /// <c>spDSpec</c> (itself carrying separately-allocated but element-equal <c>docRefs</c>) compare equal —
    /// proving in-process, without wire decoding, the type's own doc comment claim that "two stores decoded
    /// from the same wire bytes therefore compare equal".
    /// </summary>
    /// <remarks>
    /// Proves <see href="https://www.etsi.org/deliver/etsi_ts/119100_119199/11915201/01.01.01_60/ts_11915201v010101p.pdf">
    /// ETSI TS 119 152-1 V1.1.1</see> clause 5.3.2.
    /// </remarks>
    [TestMethod]
    public void SignaturePolicyStoresBuiltFromSeparatelyAllocatedEqualPartsCompareEqual()
    {
        var left = new CBAdESSignaturePolicyStore(
            new CBAdESSignaturePolicyStoreDocument(new byte[] { 0x30, 0x03, 0x02, 0x01, 0x01 }),
            new AdESObjectIdentifier(
                "https://example.org/cbades/oid/spdspec",
                "spDSpec description.",
                new Uri[] { new("https://example.org/docs/spec-1"), new("https://example.org/docs/spec-2") }));
        var right = new CBAdESSignaturePolicyStore(
            new CBAdESSignaturePolicyStoreDocument(new byte[] { 0x30, 0x03, 0x02, 0x01, 0x01 }),
            new AdESObjectIdentifier(
                "https://example.org/cbades/oid/spdspec",
                "spDSpec description.",
        ...
        Assert.AreNotSame(left.SpDSpec!.DocRefs, right.SpDSpec!.DocRefs, "The fixture must exercise separately-allocated docRefs list instances.");
        Assert.IsTrue(left.Equals(right), "Stores built from value-equal parts must compare equal.");
        Assert.IsTrue(left == right, "The operator== must agree with Equals.");
        Assert.AreEqual(left.GetHashCode(), right.GetHashCode(), "Equal stores must produce equal hash codes.");
    }


    /// <summary>
    /// The <c>sigPSt</c> arm of <c>UHeaderInstance</c> wraps a mandatory <see cref="CBAdESSignaturePolicyStore"/>
    /// (clause 5.3.1, Table 8, label 7); constructing <see cref="CBAdESUnsignedHeaderElementSignaturePolicyStore"/>
    /// with a <see langword="null"/> store throws at construction time, so no null-membered instance can exist
    /// for <see cref="CBAdESUnsignedHeaderElementSignaturePolicyStore.Equals(CBAdESUnsignedHeaderElementSignaturePolicyStore?)"/>
    /// to dereference — <see cref="object.Equals(object?)"/>'s contract requires returning
    /// <see langword="false"/>, never throwing <see cref="NullReferenceException"/>.
    /// </summary>
    [TestMethod]
    ...
    /// <summary>
    /// The CDDL's <c>+</c> cardinality on <c>otherRefs</c> (not OCSP-only): a present member
    /// shall be non-empty; constructing <see cref="CBAdESRevocationReferences"/> with an empty
    /// <c>otherReferences</c> array throws.
    /// </summary>
    /// <remarks>
    /// Proves <see href="https://www.etsi.org/deliver/etsi_ts/119100_119199/11915201/01.01.01_60/ts_11915201v010101p.pdf">
    /// ETSI TS 119 152-1 V1.1.1</see> CB-A.1.1-29.
    /// </remarks>
    [TestMethod]
    ...
    /// <remarks>
    /// Proves <see href="https://www.etsi.org/deliver/etsi_ts/119100_119199/11915201/01.01.01_60/ts_11915201v010101p.pdf">
    /// ETSI TS 119 152-1 V1.1.1</see> CB-A.1.1-06.
    /// </remarks>
    [TestMethod]
...
#pragma warning disable CA2000 // Dispose objects before losing scope
        var certificateReference = new CBAdESCertificateReference(
            new AdESCertificateThumbprint(new AdESDigestAlgorithmIntegerIdentifier(WellKnownCoseAlgorithms.Sha256), certificateDigest));
...
#pragma warning restore CA2000 // Dispose objects before losing scope
    ...
    /// <summary>
    /// <see cref="CBAdESSignatureTimestamp.Dispose"/> forwards to its <see cref="AdESTimestampContainer"/>
    ...
    {
        //CA2000: ownership of the inline AdESTimestampContainer transfers immediately into 'wrapper';
...
#pragma warning restore CA2000 // Dispose objects before losing scope
    ...
    /// <summary>
    /// <see cref="CBAdESArchiveTimestamp.Dispose"/> forwards to its <see cref="AdESTimestampContainer"/>
...
#pragma warning restore CA2000 // Dispose objects before losing scope
    ...
    /// <see cref="AdESTimestampContainer"/> unconditionally; calling it twice must not throw.
    /// </summary>
    [TestMethod]
...
#pragma warning restore CA2000 // Dispose objects before losing scope
    ...
    /// <summary>
    /// <see cref="CBAdESReferencesTimestamp.Dispose"/> forwards to its <see cref="AdESTimestampContainer"/>
    /// unconditionally; calling it twice must not throw.
    /// </summary>
    [TestMethod]
...
#pragma warning restore CA2000 // Dispose objects before losing scope
    ...
    /// <summary>
    /// Builds a minimal, single-token <see cref="AdESTimestampContainer"/> fixture — the legacy RFC 3161
