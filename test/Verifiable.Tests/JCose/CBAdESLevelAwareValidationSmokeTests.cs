...
/// is covered elsewhere in this suite; this file only proves the new overloads exist with the
...
internal sealed class CBAdESLevelAwareValidationSmokeTests
{
    ...
    /// <summary>A <see langword="null"/> <c>buildArchiveTimestampImprintInput</c> delegate raises <see cref="ArgumentNullException"/>.</summary>
    [TestMethod]
    public async Task ValidateAsyncWithLevelThrowsArgumentNullExceptionWhenBuildArchiveTimestampImprintInputIsNull()
    {
        var keyMaterial = TestKeyMaterialProvider.CreateP256KeyMaterial();
        using var publicKey = keyMaterial.PublicKey;
        ...
        await Assert.ThrowsExactlyAsync<ArgumentNullException>(async () =>
            await CBAdESSignatureValidation.ValidateAsync(
                ...
                CBAdESLevelMessageImprintAdapters.TryBuildSignatureAndReferencesTimestampMessageImprintInput,
                CBAdESLevelMessageImprintAdapters.TryBuildReferencesOnlyTimestampMessageImprintInput,
                buildArchiveTimestampImprintInput: null!,
                BaseMemoryPool.Shared,
                cancellationToken: TestContext.CancellationToken).ConfigureAwait(false)).ConfigureAwait(false);
    }
    ...
    /// overload, exactly like the B-B-only overload — the level pass never runs because the shared core
    ...
    /// the level-aware overload at <see cref="AdESBaselineLevel.BB"/>, end to end against the SHIPPED
    /// <see cref="Verifiable.Cbor.CBAdESLevelMessageImprintAdapters"/> seams — proof the new plumbing composes,
    /// not a substitute for the dedicated positive/negative level-rule and token-binding coverage elsewhere in this suite.
    /// </summary>
    ...
    public async Task ValidateAsyncWithLevelSucceedsForMinimalConformantBBMessageWithNoTimestamps()
    {
        ...
        byte[] payloadBytes = "CBAdESLevelAwareValidationSmokeTests level-aware smoke test payload"u8.ToArray();
        ...
        Assert.IsNull(result.Verified!.Value.Value.UnsignedHeaders, "No uHeaders element was minted, so none should decode.");
