...
/// ETSI TS 119 152-1 V1.1.1</see>, clauses 4-6 and 5.2.8 (the B-B creation e2e).
/// </summary>
/// <remarks>
/// <para>
...
internal sealed class CBAdESSignatureCreationTests
{
    ...
    /// <summary>
    /// Creation with none of <c>x5t</c>, <c>x5ts</c>, or <c>x5chain</c> present throws, citing CB-5.2.2-07.
    /// </summary>
    [TestMethod]
    ...
    [SuppressMessage("Reliability", "CA2000:Dispose objects before losing scope",
        Justification = "headers is deliberately not using-scoped: ownership transfers into the returned " +
            "CBAdESSignatureCreationResult on a successful SignCreateAsync call (see that type's own ownership " +
            "remarks), which this test disposes via 'using result'. The nested AdESCertificateThumbprints/" +
            "AdESCertificateThumbprint fixtures are constructor arguments passed straight into headers's own " +
    ...
    public async Task CreationWithOnlyCertificateDigestsSatisfiesTriWay()
    {
        ...
        var headers = new CBAdESProtectedHeaders(
            WellKnownCoseAlgorithms.Es256,
            new CBAdESCwtClaims(TestClock.CanonicalEpoch),
            certificateDigests: new AdESCertificateThumbprints(
            [
                new AdESCertificateThumbprint(
    ...
    /// ETSI TS 119 152-1 V1.1.1</see> CB-5.1.8-04.
    /// </remarks>
    [SuppressMessage("Reliability", "CA2000:Dispose objects before losing scope",
        Justification = "headers is deliberately not using-scoped: ownership transfers into the returned " +
            "CBAdESSignatureCreationResult on a successful SignCreateAsync call (see that type's own ownership " +
    ...
    /// path-writing branch and <see cref="Verifiable.Cbor.CBAdESSignatureSerialization.ParseCBAdESSign1"/>'s
    /// corresponding array-decode branch, both reached only via this arm.
    /// </summary>
    [SuppressMessage("Reliability", "CA2000:Dispose objects before losing scope",
        Justification = "headers is deliberately not using-scoped: ownership transfers into the returned " +
            "CBAdESSignatureCreationResult on a successful SignCreateAsync call (see that type's own ownership " +
            ...
            "CBAdESProtectedHeaders(...)' cannot see across that async call boundary into the transfer.")]
    [TestMethod]
    public async Task X5ChainCertificatePathArmRoundTripsByteExactThroughCreateAndValidate()
    {
        var keyPair = TestKeyMaterialProvider.CreateP256KeyMaterial();
        using PublicKeyMemory publicKey = keyPair.PublicKey;
        using PrivateKeyMemory privateKey = keyPair.PrivateKey;
        ...
        [
            new byte[] { 0x30, 0x01, 0xAA },
            new byte[] { 0x30, 0x02, 0xBB, 0xBB },
            new byte[] { 0x30, 0x03, 0xCC, 0xCC, 0xCC }
        ];

        var headers = new CBAdESProtectedHeaders(
            WellKnownCoseAlgorithms.Es256,
            new CBAdESCwtClaims(TestClock.CanonicalEpoch),
            x5chain: new CBAdESX5ChainCertificatePath(certificates));
        ...
        Assert.IsTrue(validationResult.IsValid, "The certificate-path x5chain arm must self-round-trip through creation and validation exactly like the single-certificate arm already does.");
        var decodedPath = validationResult.Verified!.Value.Value.Headers.X5Chain as CBAdESX5ChainCertificatePath;
        Assert.IsNotNull(decodedPath, "ParseCBAdESSign1 must decode the [2*certs:bstr] array form as CBAdESX5ChainCertificatePath, not CBAdESX5ChainSingleCertificate.");
        Assert.HasCount(certificates.Length, decodedPath!.Certificates, "Every certificate of the path survives the round trip -- none dropped, none duplicated.");
        for(int i = 0; i < certificates.Length; ++i)
        {
            Assert.IsTrue(certificates[i].Span.SequenceEqual(decodedPath.Certificates[i].Span),
                $"Certificate at position {i} must round-trip byte-exact (CB-5.1.8-04): the signing certificate stays first, entry order is preserved.");
        }
    }


    /// <summary>
    /// A missing CWT Claims member throws, citing CB-6.3-10 -- asserted directly against
    /// <see cref="CBAdESHeaderRules.EnsureConformant"/> (see the class remarks for why this rule is exercised
    /// directly rather than through <see cref="CBAdESSignatureCreation"/>). Previously,
    ...
    /// <remarks>
    /// Proves <see href="https://www.etsi.org/deliver/etsi_ts/119100_119199/11915201/01.01.01_60/ts_11915201v010101p.pdf">
    /// ETSI TS 119 152-1 V1.1.1</see> CB-5.1.9-01, CB-5.1.9-02, CB-5.1.9-03, CB-5.1.9-04, CB-5.1.9-05, CB-6.3-a.
    /// </remarks>
    [SuppressMessage("Reliability", "CA2000:Dispose objects before losing scope",
        Justification = "headers is deliberately not using-scoped: ownership transfers into the returned " +
            "CBAdESSignatureCreationResult on a successful SignCreateAsync call (see that type's own ownership " +
    ...
    /// <see cref="CBAdESSignatureCreation.SignAsync(CBAdESProtectedHeaders, CBAdESSigningPayloadInput, CBAdESUnsignedHeaders?, EncodeCBAdESProtectedHeaderDelegate, EncodeCBAdESUnprotectedHeaderDelegate, BuildSigStructureDelegate, PrivateKeyMemory, SigningDelegate, CBAdESDetachedObjectDereferenceDelegate?, CBAdESDetachedObjectDereferenceContext?, CBAdESUnknownDetachedObjectMechanismDelegate?, BaseMemoryPool, CryptoEventSink?, CancellationToken)"/>
    /// itself (a load-bearing design fact, recorded here): <see cref="CBAdESDetachedSigDPayloadInput"/> always
    ...
    /// listed it (CB-5.1.10-04) — verified via an INDEPENDENT <see cref="CborReader"/>
    /// over the encoded protected header bytes — while a caller-supplied extra <c>crit</c> label survives
    /// alongside it.
    /// </summary>
    /// <remarks>
    /// Proves <see href="https://www.etsi.org/deliver/etsi_ts/119100_119199/11915201/01.01.01_60/ts_11915201v010101p.pdf">
    /// ETSI TS 119 152-1 V1.1.1</see> CB-5.1.10-01, CB-5.1.10-02, CB-5.1.10-03, CB-6.3-09.
    /// </remarks>
    [SuppressMessage("Reliability", "CA2000:Dispose objects before losing scope",
        Justification = "headers is deliberately not using-scoped: ownership transfers into the returned " +
            "CBAdESSignatureCreationResult on a successful SignCreateAsync call (see that type's own ownership " +
    ...
    /// </summary>
    [SuppressMessage("Reliability", "CA2000:Dispose objects before losing scope",
        Justification = "The flagged AdESCertificateThumbprint construction is a constructor argument passed " +
    ...
    public async Task X5TWithMd5DigestThrows()
    {
        ...
        using var headers = new CBAdESProtectedHeaders(
            WellKnownCoseAlgorithms.Es256,
            new CBAdESCwtClaims(TestClock.CanonicalEpoch),
            x5t: new AdESCertificateThumbprint(
                new AdESDigestAlgorithmTextIdentifier("MD5"),
    ...
    /// </summary>
    [SuppressMessage("Reliability", "CA2000:Dispose objects before losing scope",
        Justification = "The flagged AdESCertificateThumbprints/AdESCertificateThumbprint constructions are " +
            "constructor arguments passed straight into the enclosing 'using var headers' aggregate's own " +
            "construction; ownership passes to headers, which this test disposes. Roslyn's CA2000 analysis " +
            "flags each nested 'new' expression independently of the enclosing aggregate that actually owns " +
            "and disposes them.")]
    ...
    /// </summary>
    [SuppressMessage("Reliability", "CA2000:Dispose objects before losing scope",
        Justification = "The flagged AdESSignaturePolicyIdentifier construction is a constructor argument " +
    ...
    public async Task SignaturePolicyIdentifierDigestWithMd5DigestThrows()
    {
        ...
        using var headers = new CBAdESProtectedHeaders(
            ...
            signaturePolicyIdentifier: new AdESSignaturePolicyIdentifier(
                new AdESObjectIdentifier("urn:test:policy"),
                new AdESDigestAlgorithmTextIdentifier("MD5"),
    ...
    /// <see cref="CBAdESSignatureCreation.SignAsync(CBAdESProtectedHeaders, CBAdESSigningPayloadInput, CBAdESUnsignedHeaders?, EncodeCBAdESProtectedHeaderDelegate, EncodeCBAdESUnprotectedHeaderDelegate, BuildSigStructureDelegate, PrivateKeyMemory, SigningDelegate, CBAdESDetachedObjectDereferenceDelegate?, CBAdESDetachedObjectDereferenceContext?, CBAdESUnknownDetachedObjectMechanismDelegate?, BaseMemoryPool, CryptoEventSink?, CancellationToken)"/>.
    /// A load-bearing design fact, recorded here (see also the class remarks): under the
        ...
        using var headers = new CBAdESProtectedHeaders(
            ...
            detachedObjects: new CBAdESDetachedObjects(
                ...
                [new CBAdESDetachedObjectEntry(
                    ...
                    contentType: null)],
                hashAlgorithm: new AdESDigestAlgorithmTextIdentifier("MD5")),
            criticalLabels: [new CoseHeaderIntegerLabel(CBAdESHeaderParameters.SigD)]);
    ...
    public async Task ObjectIdByUriWithHashAlgorithmThrows()
    {
        ...
        var payloadInput = new CBAdESDetachedSigDPayloadInput(
            ...
            hashAlgorithm: new AdESDigestAlgorithmIntegerIdentifier(WellKnownCoseAlgorithms.Sha256));
    ...
    /// <remarks>
    /// Proves <see href="https://www.etsi.org/deliver/etsi_ts/119100_119199/11915201/01.01.01_60/ts_11915201v010101p.pdf">
    /// ETSI TS 119 152-1 V1.1.1</see> CB-5.2.8-07.
    /// </remarks>
    [SuppressMessage("Reliability", "CA2000:Dispose objects before losing scope",
        Justification = "headers is deliberately not using-scoped: ownership transfers into the returned " +
            "CBAdESSignatureCreationResult on a successful SignCreateAsync call (see that type's own ownership " +
    ...
    /// <summary>
    /// A leak regression: <c>content type</c> is caller-supplied ALONGSIDE an
    ...
    /// clause disposes <c>resolution.DetachedObjects</c> on exactly this path. A
    ...
    public async Task CreationDisposesDetachedObjectsOnPass2RuleViolationLeavingNoOutstandingPoolRentals()
    {
        ...
        Assert.AreEqual(0, pool.OutstandingCount,
            "The internally-built DetachedObjects' hashV digest rental -- computed via one real dereference+digest before PASS 2's throw -- must be disposed by the catch clause, not leaked.");
    }
    ...
    /// <remarks>
    /// Proves <see href="https://www.etsi.org/deliver/etsi_ts/119100_119199/11915201/01.01.01_60/ts_11915201v010101p.pdf">
    /// ETSI TS 119 152-1 V1.1.1</see> CB-5.2.6-07, CB-5.2.8-07.
    /// </remarks>
    [SuppressMessage("Reliability", "CA2000:Dispose objects before losing scope",
        Justification = "headers is deliberately not using-scoped: ownership transfers into the returned " +
            "CBAdESSignatureCreationResult on a successful SignCreateAsync call (see that type's own ownership " +
    ...
    /// <remarks>
    /// Proves <see href="https://www.etsi.org/deliver/etsi_ts/119100_119199/11915201/01.01.01_60/ts_11915201v010101p.pdf">
    /// ETSI TS 119 152-1 V1.1.1</see> CB-6.3-20.
    /// </remarks>
    [TestMethod]
    ...
    [SuppressMessage("Reliability", "CA2000:Dispose objects before losing scope",
        Justification = "headers is deliberately not using-scoped: ownership transfers into the returned " +
            "CBAdESSignatureCreationResult on a successful SignCreateAsync call (see that type's own ownership " +
            "remarks), which this test disposes via 'using result'. The nested AdESSignaturePolicyIdentifier " +
    ...
    public async Task UnprofiledX5BagLabelRoundTripsByteExactThroughCreateAndValidate()
    {
        ...
        Assert.IsNotNull(validationResult.Verified!.Value.Value.Headers.UnprofiledHeaders);
        Assert.IsTrue(validationResult.Verified.Value.Value.Headers.UnprofiledHeaders!.ContainsKey(x5BagLabel));
        Assert.IsTrue(unprofiledX5BagBytes.AsSpan().SequenceEqual(validationResult.Verified.Value.Value.Headers.UnprofiledHeaders![x5BagLabel].Span));
    }
    ...
    /// order (major type 1's magnitude encoding does not correspond to signed integer order).
    /// <see cref="Verifiable.Cbor.CBAdESSignatureSerialization.ParseCBAdESSign1"/>'s top-level
    ...
    /// principle <see cref="CborReaderExtensions.ReadAscendingMapKey"/> uses -- so this
    /// round-trip exercises that shared logic end to end.
    /// </summary>
    [SuppressMessage("Reliability", "CA2000:Dispose objects before losing scope",
        Justification = "headers is deliberately not using-scoped: ownership transfers into the returned " +
            "CBAdESSignatureCreationResult on a successful SignCreateAsync call (see that type's own ownership " +
    ...
    public async Task UnprofiledNegativeLabelRoundTripsThroughCreateAndValidate()
    {
        ...
        Assert.IsNotNull(validationResult.Verified!.Value.Value.Headers.UnprofiledHeaders);
        Assert.IsTrue(validationResult.Verified.Value.Value.Headers.UnprofiledHeaders!.ContainsKey(negativeLabel));
        Assert.IsTrue(unprofiledNegativeLabelBytes.AsSpan().SequenceEqual(validationResult.Verified.Value.Value.Headers.UnprofiledHeaders![negativeLabel].Span));
    }
    ...
    public async Task TextArmCriticalLabelAndUnprofiledHeaderRoundTripThroughCreateAndValidate()
    {
        ...
        Assert.IsTrue(validationResult.IsValid);
        Assert.IsNotNull(validationResult.Verified!.Value.Value.Headers.CriticalLabels);
        Assert.Contains(criticalLabel, validationResult.Verified.Value.Value.Headers.CriticalLabels!, "The tstr-arm crit element must round-trip.");
        Assert.IsNotNull(validationResult.Verified.Value.Value.Headers.UnprofiledHeaders);
        Assert.IsTrue(validationResult.Verified.Value.Value.Headers.UnprofiledHeaders!.ContainsKey(unprofiledLabel));
        Assert.IsTrue(unprofiledBytes.AsSpan().SequenceEqual(validationResult.Verified.Value.Value.Headers.UnprofiledHeaders![unprofiledLabel].Span));
    }
    ...
    private static async ValueTask<AdESCertificateThumbprint> CreateX5TAsync(CancellationToken cancellationToken)
    {
        ...
        return new AdESCertificateThumbprint(new AdESDigestAlgorithmIntegerIdentifier(WellKnownCoseAlgorithms.Sha256), digest);
    }
