...
/// </para>
/// <para>
/// <strong>New production seam, recorded here.</strong>
...
/// <see cref="CoseSerialization.SerializeCoseSign1"/>; flow 6
/// (detached, no <c>sigD</c>, clause 4.5's out-of-band-agreement arm,
...
internal sealed class CBAdESSignatureFlowTests
{
    ...
    /// CB-4.2-02, CB-4.3-02, CB-4.4-05, CB-4.5-01, CB-5.1.2-05.
    /// </remarks>
    ...
    public async Task AttachedBaselineFlowRoundTripsAndVerifiesWithEs256()
    {
        ...
        (AdESCertificateThumbprint thumbprint, byte[] expectedDigestBytes) =
        ...
        Assert.IsFalse(validationResult.Verified!.Value.Value.PayloadIsDetached, "The attached flow's payload must decode as not detached.");
        Assert.AreEqual(expectedAlgorithm, validationResult.Verified.Value.Value.Headers.Algorithm);
        Assert.AreEqual(expectedIssuedAt, validationResult.Verified.Value.Value.Headers.CwtClaims!.IssuedAt);
        Assert.IsNotNull(validationResult.Verified.Value.Value.Headers.X5T);
        Assert.IsTrue(expectedDigestBytes.AsSpan().SequenceEqual(validationResult.Verified.Value.Value.Headers.X5T!.Digest.AsReadOnlySpan()));
    }
    ...
    /// <c>x5chain</c> element (label 33, CB-5.1.8-02), carried opaque per the
    ...
    public async Task AttachedFlowWithUnsignedCertificateChainElementRoundTripsExactlyOneUHeadersElement()
    {
        ...
        Assert.IsNotNull(validationResult.Verified!.Value.Value.UnsignedHeaders);
        Assert.HasCount(1, validationResult.Verified.Value.Value.UnsignedHeaders!);
        var chainElement = Assert.IsInstanceOfType<CBAdESUnsignedHeaderElementCertificateChain>(validationResult.Verified.Value.Value.UnsignedHeaders![0]);
    ...
    /// <remarks>
    /// Proves <see href="https://www.etsi.org/deliver/etsi_ts/119100_119199/11915201/01.01.01_60/ts_11915201v010101p.pdf">ETSI TS 119 152-1 V1.1.1</see>
    /// CB-4.5-01, CB-4.5-02, CB-5.2.8-01, CB-5.2.8-04, CB-5.2.8-05, CB-5.2.8-10, CB-5.2.8-11.
    /// </remarks>
    [SuppressMessage("Reliability", "CA2000:Dispose objects before losing scope",
        Justification = "headers is deliberately not using-scoped: ownership transfers into the returned " +
            "CBAdESSignatureCreationResult on a successful CBAdESSignatureCreation.SignAsync call (see that " +
    ...
    public async Task DetachedObjectIdByUriFlowReconstructsAndValidatesFromWireBytesOnly()
    {
        ...
        Assert.IsTrue(validationResult.Verified!.Value.Value.PayloadIsDetached, "The ObjectIdByURI flow's payload must decode as detached (the wire nil sentinel).");
    }


    /// <summary>
    /// Flow 3, negative leg: the verifier's OWN store is missing one referenced object -- reconstruction must
    /// fail closed as <see cref="CBAdESDetachedObjectUnresolvableFailure"/>, never a thrown exception.
    /// </summary>
    [SuppressMessage("Reliability", "CA2000:Dispose objects before losing scope",
        Justification = "headers is deliberately not using-scoped: ownership transfers into the returned " +
            "CBAdESSignatureCreationResult on a successful CBAdESSignatureCreation.SignAsync call (see that " +
    ...
    /// referenced objects, each digested by creation itself; the verifier
    ...
    /// <remarks>
    /// Proves <see href="https://www.etsi.org/deliver/etsi_ts/119100_119199/11915201/01.01.01_60/ts_11915201v010101p.pdf">ETSI TS 119 152-1 V1.1.1</see>
    /// CB-5.2.8-01, CB-5.2.8-04, CB-5.2.8.2.3-05, CB-5.2.8.2.3-06.
    /// </remarks>
    [SuppressMessage("Reliability", "CA2000:Dispose objects before losing scope",
        Justification = "headers is deliberately not using-scoped: ownership transfers into the returned " +
            "CBAdESSignatureCreationResult on a successful CBAdESSignatureCreation.SignAsync call (see that " +
    ...
    public async Task DetachedObjectIdByUriHashFlowVerifiesEveryDigestFromWireBytesOnly()
    {
        ...
        Assert.IsTrue(validationResult.Verified!.Value.Value.PayloadIsDetached);
    }
    ...
    /// <summary>
    /// Flow 5 (attached B-B, full-house aggregate): every clause 5.1/5.2 signed header this suite
    /// models that flows 1/2 leave unpopulated -- <c>kid</c>, <c>x5u</c>, <c>x5t</c> (the tri-way),
    ...
    /// discipline). <c>content type</c> joins this flow because it is legal on an attached,
    ...
    /// <remarks>
    /// Proves <see href="https://www.etsi.org/deliver/etsi_ts/119100_119199/11915201/01.01.01_60/ts_11915201v010101p.pdf">ETSI TS 119 152-1 V1.1.1</see>
    /// CB-5.1.3-01, CB-5.1.3-02, CB-5.1.3-06, CB-5.1.4-01, CB-5.1.4-02, CB-5.1.4-05, CB-5.1.5-01, CB-5.1.5-02,
    /// CB-5.1.5-04, CB-5.2.3-01, CB-5.2.4-01, CB-5.2.5-01, CB-5.2.6-01.
    /// </remarks>
    [SuppressMessage("Reliability", "CA2000:Dispose objects before losing scope",
        Justification = "headers is deliberately not using-scoped: ownership transfers into the returned " +
            "CBAdESSignatureCreationResult on a successful CBAdESSignatureCreation.SignAsync call (see that " +
            "type's own ownership remarks), which this test disposes via 'using creationResult'. The nested " +
            "adoTst (CBAdESPayloadTimestamp/AdESTimestampContainer) construction is a constructor argument " +
    ...
    public async Task FullHouseAttachedFlowRoundTripsAndVerifiesEveryClause5SignedHeader()
    {
        ...
        var expectedCommitmentId = "urn:cbades:flow5:commitment:proof-of-origin";
        ...
        byte[] wireCopy;
        {
            var srCms = new AdESSignerCommitments(
                [new AdESCommitment(new AdESObjectIdentifier(expectedCommitmentId))]);
        ...
        Assert.IsFalse(validationResult.Verified!.Value.Value.PayloadIsDetached);
        CBAdESProtectedHeaders decoded = validationResult.Verified.Value.Value.Headers;
    ...
    /// <see cref="CBAdESDetachedExternalPayloadInput"/>): the wire payload is the RFC 9052
    ...
    /// <summary>
    /// Flow 6, negative leg: the same detached-external message, but the verifier supplies no
    ...
    /// rather than the creation-side <see cref="AdESCertificateThumbprint"/> object.
    /// </summary>
