...
/// <see cref="CBAdESSignatureCreation"/>, the negative-quartet precedent this file follows
...
/// Algorithms registry) IS referenced directly, matching the sibling exemplar's own precedent for that specific
...
internal sealed class CBAdESSignatureValidationTests
{
    ...
    /// ETSI TS 119 152-1 V1.1.1</see> CB-5.1.2-01, CB-5.1.2-02, CB-5.1.2-03, CB-5.1.3-04, CB-5.1.7-01,
    /// CB-5.1.7-02, CB-5.1.7-03, CB-5.1.9-05.
    /// </remarks>
    [TestMethod]
    public async Task ValidateAsyncSucceedsForMinimalConformantBBMessage()
    {
        ...
        Assert.IsNull(result.Verified.Value.Value.UnsignedHeaders);
        Assert.IsNotNull(result.Verified.Value.Value.Headers);
        Assert.AreEqual(WellKnownCoseAlgorithms.Es256, result.Verified.Value.Value.Headers.Algorithm);
        Assert.AreEqual(TestClock.CanonicalEpoch, result.Verified.Value.Value.Headers.CwtClaims!.IssuedAt);
        Assert.IsNotNull(result.Verified.Value.Value.Headers.X5T);
        Assert.AreEqual(new AdESDigestAlgorithmIntegerIdentifier(WellKnownCoseAlgorithms.Sha256), result.Verified.Value.Value.Headers.X5T!.HashAlgorithm);
        Assert.IsTrue(digestBytes.AsSpan().SequenceEqual(result.Verified.Value.Value.Headers.X5T!.Digest.AsReadOnlySpan()));
    }


    /// <summary>
    /// A CWT-Claims <c>iat</c> encoded with the CBOR floating-point arm of <c>NumericDate</c>
    /// (RFC 8392 section 2's <c>int / float</c> union) validates successfully -- a
    ...
    public async Task ValidateAsyncSucceedsWhenCwtClaimsIatIsEncodedAsFloatNumericDate()
    {
        ...
        Assert.IsFalse(result.Verified!.Value.Value.PayloadIsDetached);
        Assert.IsNotNull(result.Verified.Value.Value.Headers);
        Assert.AreEqual(expectedIssuedAt, result.Verified.Value.Value.Headers.CwtClaims!.IssuedAt, "The float-form iat must still decode to the exact signing time.");
        Assert.IsTrue(digestBytes.AsSpan().SequenceEqual(result.Verified.Value.Value.Headers.X5T!.Digest.AsReadOnlySpan()), "The verification path must have consumed the actual wire bytes -- the x5t digest decoded from the result must match what was minted.");
    }


    /// <summary>
    /// An untagged <c>COSE_Sign1</c> (no <c>COSE_Sign1_Tagged</c> prefix) parses and validates successfully.
    /// Clause 4.3 states CB-AdES signatures MAY be encoded untagged as well as
    ...
    /// <remarks>
    /// Proves <see href="https://www.etsi.org/deliver/etsi_ts/119100_119199/11915201/01.01.01_60/ts_11915201v010101p.pdf">
    /// ETSI TS 119 152-1 V1.1.1</see> CB-4.3-01.
    /// </remarks>
    [TestMethod]
    ...
    /// <remarks>
    /// Proves <see href="https://www.etsi.org/deliver/etsi_ts/119100_119199/11915201/01.01.01_60/ts_11915201v010101p.pdf">
    /// ETSI TS 119 152-1 V1.1.1</see> CB-5.3.1-10.
    /// </remarks>
    [TestMethod]
    ...
    /// <remarks>
    /// Proves <see href="https://www.etsi.org/deliver/etsi_ts/119100_119199/11915201/01.01.01_60/ts_11915201v010101p.pdf">
    /// ETSI TS 119 152-1 V1.1.1</see> CB-4.4-01.
    /// </remarks>
    [TestMethod]
    ...
    /// <see cref="CBAdESCwtClaimsMissingViolation"/> (CB-6.3-10).
    ...
    /// reports BOTH violations -- the collect posture is finally observable for this
    /// pairing now that <see cref="CBAdESCwtClaimsMissingViolation"/> is live.
    /// </summary>
    [TestMethod]
    ...
    /// 2) parses successfully, decodes <c>iat</c> correctly, and the message validates -- a
    ...
    /// because the Sig_structure is built from the raw wire bytes captured at parse, never a
    ...
    public async Task ValidateAsyncSucceedsWhenCwtClaimsCarriesASiblingClaimAlongsideIat()
    {
        ...
        Assert.IsNotNull(result.Verified!.Value.Value.Headers);
        Assert.AreEqual(expectedIssuedAt, result.Verified.Value.Value.Headers.CwtClaims!.IssuedAt, "iat must decode correctly even with a sibling claim present in the same map.");
    }


    /// <summary>
    /// An independently minted message whose protected header carries a <c>tstr</c>-arm <c>crit</c> element and
    /// a <c>tstr</c>-keyed unprofiled header entry parses and validates successfully, exercising
    /// <see cref="CBAdESSignatureSerialization.ParseCBAdESSign1"/>'s general
    /// COSE <c>label: int / tstr</c> union support (RFC 9052 §1.4/§3.1, clause 4.4 NOTE 4) purely from
    /// independently minted wire bytes, never through this file's own encoder -- the creation-side round trip
    ...
    public async Task ParseAcceptsIndependentlyMintedTextArmCritAndUnprofiledHeader()
    {
        ...
        Assert.IsNotNull(result.Verified!.Value.Value.Headers);
        Assert.IsNotNull(result.Verified.Value.Value.Headers.CriticalLabels);
        Assert.Contains(new CoseHeaderTextLabel("x-test-crit"), result.Verified.Value.Value.Headers.CriticalLabels!, "The tstr-arm crit element must decode.");
        Assert.IsNotNull(result.Verified.Value.Value.Headers.UnprofiledHeaders);
        var unprofiledLabel = new CoseHeaderTextLabel("x-test-ext");
        Assert.IsTrue(result.Verified.Value.Value.Headers.UnprofiledHeaders!.ContainsKey(unprofiledLabel));
        Assert.IsTrue(expectedUnprofiledBytes.AsSpan().SequenceEqual(result.Verified.Value.Value.Headers.UnprofiledHeaders![unprofiledLabel].Span));
    }


    /// <summary>
    /// A protected header whose top-level map keys mix an integer and a text-string label in NON-CANONICAL order
    /// fails closed: a naive "every
    ...
    /// <see cref="CBAdESCertificateReferenceTriWayViolation"/> (CB-5.2.2-07).
    /// </summary>
    [TestMethod]
    ...
    public async Task ValidateAsyncReportsContentTypeDetachedObjectsExclusivityViolation()
    {
        ...
        byte[] protectedHeader = BuildProtectedHeaderBytes(
            ...
            (new AdESDigestAlgorithmIntegerIdentifier(WellKnownCoseAlgorithms.Sha256), digestBytes),
    ...
    public async Task ValidateAsyncReportsMd5DigestAlgorithmViolationOnTextIdentifier()
    {
        ...
        byte[] protectedHeader = BuildProtectedHeaderBytes(
            WellKnownCoseAlgorithms.Es256,
            TestClock.CanonicalEpoch,
            (new AdESDigestAlgorithmTextIdentifier("MD5"), md5DigestBytes),
            criticalLabels: null,
            contentType: null,
            sigD: null);
    ...
    /// <summary>
    /// An otherwise-fully-conformant third-party-<c>mId</c> message reaches dispatch (crit
    ...
    /// <remarks>
    /// Proves <see href="https://www.etsi.org/deliver/etsi_ts/119100_119199/11915201/01.01.01_60/ts_11915201v010101p.pdf">
    /// ETSI TS 119 152-1 V1.1.1</see> CB-5.2.6-07, CB-5.2.8-08.
    /// </remarks>
    [TestMethod]
    public async Task ValidateAsyncSucceedsWhenUnknownMechanismHandlerResolvesPayload()
    {
        ...
        const string reference = "https://example.org/dispatch-object";
        ...
        Assert.IsTrue(result.Verified!.Value.Value.PayloadIsDetached);
    }


    /// <summary>
    /// The routine leg: the unknown-mechanism handler throws
    /// <see cref="CBAdESDetachedObjectDereferenceException"/> -- the delegate's own documented routine-failure
    /// signal -- and validation reports <see cref="CBAdESDetachedObjectUnresolvableFailure"/>
    ...
    public async Task ValidateAsyncFailsWithDetachedObjectUnresolvableWhenUnknownMechanismHandlerThrowsRoutineDereferenceException()
    {
        ...
        const string reference = "https://example.org/dispatch-object-routine-failure";
    ...
    /// <summary>
    /// The non-routine leg: the unknown-mechanism handler throws
    /// <see cref="InvalidOperationException"/> -- NOT the delegate's documented routine-failure signal -- and
    /// <see cref="CBAdESSignatureValidation.ValidateAsync(ReadOnlyMemory{byte}, ParseCBAdESSign1Delegate, BuildSigStructureDelegate, PublicKeyMemory, VerificationDelegate, CBAdESDetachedObjectDereferenceDelegate?, CBAdESDetachedObjectDereferenceContext?, ReadOnlyMemory{byte}?, CBAdESUnknownDetachedObjectMechanismDelegate?, BaseMemoryPool, CancellationToken)"/>
    /// propagates it unmodified (the narrowed catch names only <see cref="CBAdESDetachedObjectDereferenceException"/>).
    /// </summary>
    [TestMethod]
    public async Task ValidateAsyncPropagatesNonRoutineExceptionFromUnknownMechanismHandler()
    {
        ...
        const string reference = "https://example.org/dispatch-object-non-routine-failure";
    ...
    /// <summary>
    /// The same third-party-<c>mId</c> dispatch-reaching message, but no unknown-mechanism
    ...
    public async Task ValidateAsyncFailsWithDetachedObjectUnresolvableWhenNoUnknownMechanismHandlerIsSupplied()
    {
        ...
        const string reference = "https://example.org/dispatch-object-no-handler";
        byte[] wireBytes = await CreateThirdPartyMechanismDispatchMessageAsync(
            privateKey, reference, [], TestContext.CancellationToken).ConfigureAwait(false);
    ...
    /// only through the catch on this exact path, since resolution itself never fails — are disposed rather
    /// than leaked.
    /// </summary>
    [TestMethod]
    public async Task ValidateAsyncPropagatesCancellationObservedDuringResolutionWithoutLeakingParseResultCarriers()
    {
        ...
        Assert.AreEqual(0, pool.OutstandingCount,
            "The catch clause must dispose parseResult's owned carriers when Cose.VerifyAsync observes the cancellation the dereference delegate triggered mid-resolution, not leak them.");
    }
    ...
    /// throws — the explicit no-throw assertion every negative test in this file relies on (validating
    ...
    {
        //MSTEST0058: the failure report must not live textually inside the catch block --
    ...
    /// <summary>
    /// Builds the shared "reaches unknown-mechanism dispatch" fixture the four tests above perturb only
        ...
        PrivateKeyMemory privateKey, byte[] x5tDigestBytes, CancellationToken cancellationToken)
    {
        byte[] protectedHeader = BuildProtectedHeaderBytes(
            WellKnownCoseAlgorithms.Es256,
            TestClock.CanonicalEpoch,
            (new AdESDigestAlgorithmIntegerIdentifier(WellKnownCoseAlgorithms.Sha256), x5tDigestBytes),
            criticalLabels: null,
            contentType: null,
            sigD: null);
    ...
    private static byte[] BuildProtectedHeaderBytes(
        ...
        (AdESDigestAlgorithmIdentifier Algorithm, byte[] Digest)? x5t,
