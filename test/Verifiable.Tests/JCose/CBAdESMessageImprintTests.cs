...
/// (<c>sigRTst</c>), and Annex A.1.2.2.2 (<c>rfsTst</c>).
/// </summary>
/// <remarks>
/// <para>
...
/// </para>
/// <para>
/// <strong>Step-12 reading.</strong> Every <c>arcTst</c> expected byte sequence below is the fully
/// assembled twelve-step array's OWN canonical CBOR encoding, with no additional outer <c>bstr</c> header —
/// matching this library's recorded reading of the spec's closing sentence (see
/// <see cref="CBAdESMessageImprints"/>'s own remarks for the step-12 reading).
/// </para>
/// </remarks>
[TestClass]
internal sealed class CBAdESMessageImprintTests
{
    ...
    /// ETSI TS 119 152-1 V1.1.1</see> CB-5.3.5.1-01, CB-5.3.5.2-02, CB-5.3.5.3-01, CB-5.3.5.3-02, CB-5.3.5.3-03,
    /// CB-5.3.5.3-04, CB-5.3.5.3-06, CB-5.3.5.3-07, CB-5.3.5.3-10, CB-5.3.5.3-12, CB-5.3.5.3-13, CB-5.3.5.3-16.
    /// </remarks>
    [TestMethod]
    ...
    /// <remarks>
    /// Proves <see href="https://www.etsi.org/deliver/etsi_ts/119100_119199/11915201/01.01.01_60/ts_11915201v010101p.pdf">
    /// ETSI TS 119 152-1 V1.1.1</see> CB-5.3.5.3-04, CB-5.3.5.3-06, CB-5.3.5.3-07.
    /// </remarks>
    [TestMethod]
    ...
    /// <remarks>
    /// Proves <see href="https://www.etsi.org/deliver/etsi_ts/119100_119199/11915201/01.01.01_60/ts_11915201v010101p.pdf">
    /// ETSI TS 119 152-1 V1.1.1</see> CB-5.3.5.3-03, CB-5.3.5.3-05, CB-5.3.5.3-11.
    /// </remarks>
    [TestMethod]
    ...
    /// <remarks>
    /// Proves <see href="https://www.etsi.org/deliver/etsi_ts/119100_119199/11915201/01.01.01_60/ts_11915201v010101p.pdf">
    /// ETSI TS 119 152-1 V1.1.1</see> CB-5.3.5.3-05.
    /// </remarks>
    [TestMethod]
    ...
    /// <summary>
    /// RFC 9338 <c>other_fields</c>: <c>arcTst</c> step 8 exercised end to end
    /// for a genuinely countersigned target, closing the gap where every prior
    /// test's <see cref="CBAdESArchiveTimestampImprintContext.CountersignatureOtherFields"/> was hardcoded
    /// <see langword="null"/>. The <c>other_fields</c> bytes themselves are derived from the REAL
    /// <see cref="CountersignTarget"/>/<see cref="CountersignStructureInput"/> substrate (never hand-derived
    /// here) — a <see cref="CoseSign1CountersignTarget"/>'s own <c>OtherFieldsSignature</c> (RFC 9338 §3.3:
    /// "an array of one element... containing the signature value") is embedded, unwrapped, as step 8's own
    /// contribution — proving the "embedded as the CBOR array value it already is, never bstr-wrapped"
    /// reading holds against genuine countersignature-derived bytes, not merely a structurally-absent
    /// placeholder. Every OTHER field (body-layer protected header, external data, payload, signature value)
    /// remains a literal, controlled fixture, matching this file's own independent-oracle convention exactly
    /// as every <c>COSE_Sign</c> scenario above it does.
    /// </summary>
    [TestMethod]
    public void ArcTstGenerationCountersignatureStructureWithOtherFieldsMatchesIndependentOracleForACountersignedTarget()
    {
        byte[] countersignedTargetProtectedHeader = [0x01, 0x02];
        byte[] countersignedTargetPayload = [0xAA, 0xBB];
        byte[] countersignedTargetSignature = [0x10, 0x20, 0x30];
        var countersignedTarget = new CoseSign1CountersignTarget(countersignedTargetProtectedHeader, countersignedTargetPayload, countersignedTargetSignature);

        CountersignStructureInput countersignInput = CountersignStructureInput.ForTarget(
        ...
        writer.WriteByteString(bodyProtectedHeader); //step 3.
        writer.WriteByteString(externallySuppliedData); //step 5 (this context has no COSE_Sign signer layer -- step 4 omitted).
        writer.WriteByteString(payload); //steps 6/7.
        writer.WriteEncodedValue(otherFieldsBytesOwner.Memory.Span); //step 8: the CBOR array value itself, never bstr-wrapped.
        writer.WriteByteString(signatureValue); //step 9.
        writer.WriteByteString(ReadOnlySpan<byte>.Empty); //steps 10/11: no uHeaders at all.
        writer.WriteEndArray();
        byte[] expected = writer.Encode();
        ...
        {
            StructureContext = new CBAdESCountersignatureStructureContext("CounterSignature0V2"),
            BodyProtectedHeader = bodyProtectedHeader,
            SignerProtectedHeader = null,
            ExternallySuppliedData = externallySuppliedData,
            PayloadSource = new CBAdESAttachedPayloadImprintSource(payload),
            CountersignatureOtherFields = otherFieldsBytesOwner.Memory,
        ...
        using(result)
        {
            Assert.IsTrue(expected.AsSpan().SequenceEqual(result!.AsReadOnlySpan()),
                "Encode must reproduce the independent oracle's bytes exactly, with step 8's other_fields bytes end-to-end derived from a real CountersignTarget.");
        }
    }
    ...
    /// <remarks>
    /// Proves <see href="https://www.etsi.org/deliver/etsi_ts/119100_119199/11915201/01.01.01_60/ts_11915201v010101p.pdf">
    /// ETSI TS 119 152-1 V1.1.1</see> CB-5.3.5.3-08.
    /// </remarks>
    [TestMethod]
    ...
    /// <remarks>
    /// Proves <see href="https://www.etsi.org/deliver/etsi_ts/119100_119199/11915201/01.01.01_60/ts_11915201v010101p.pdf">
    /// ETSI TS 119 152-1 V1.1.1</see> CB-5.3.5.3-11, CB-5.3.5.3-12.
    /// </remarks>
    [TestMethod]
    ...
    /// <remarks>
    /// Proves <see href="https://www.etsi.org/deliver/etsi_ts/119100_119199/11915201/01.01.01_60/ts_11915201v010101p.pdf">
    /// ETSI TS 119 152-1 V1.1.1</see> CB-5.3.5.3-14, CB-5.3.5.3-15.
    /// </remarks>
    [TestMethod]
    ...
    /// <remarks>
    /// Proves <see href="https://www.etsi.org/deliver/etsi_ts/119100_119199/11915201/01.01.01_60/ts_11915201v010101p.pdf">
    /// ETSI TS 119 152-1 V1.1.1</see> CB-5.2.6-04.
    /// </remarks>
    [TestMethod]
    ...
    /// <remarks>
    /// Proves <see href="https://www.etsi.org/deliver/etsi_ts/119100_119199/11915201/01.01.01_60/ts_11915201v010101p.pdf">
    /// ETSI TS 119 152-1 V1.1.1</see> CB-A.1.2.1-01, CB-A.1.2.1-02, CB-A.1.2.1.2-01, CB-A.1.2.1.2-02,
    /// CB-A.1.2.1.2-03, CB-A.1.2.1.2-04, CB-A.1.2.1.2-05.
    /// </remarks>
    [TestMethod]
    ...
    /// <remarks>
    /// Proves <see href="https://www.etsi.org/deliver/etsi_ts/119100_119199/11915201/01.01.01_60/ts_11915201v010101p.pdf">
    /// ETSI TS 119 152-1 V1.1.1</see> CB-A.1.2.1.2-03.
    /// </remarks>
    [TestMethod]
    ...
    /// <remarks>
    /// Proves <see href="https://www.etsi.org/deliver/etsi_ts/119100_119199/11915201/01.01.01_60/ts_11915201v010101p.pdf">
    /// ETSI TS 119 152-1 V1.1.1</see> CB-A.1.2.1.2-02, CB-A.1.2.2-01, CB-A.1.2.2-02, CB-A.1.2.2.2-01,
    /// CB-A.1.2.2.2-02, CB-A.1.2.2.2-03.
    /// </remarks>
    [TestMethod]
    ...
    /// <remarks>
    /// Proves <see href="https://www.etsi.org/deliver/etsi_ts/119100_119199/11915201/01.01.01_60/ts_11915201v010101p.pdf">
    /// ETSI TS 119 152-1 V1.1.1</see> CB-A.1.2.2.2-02, CB-A.1.2.2.2-04.
    /// </remarks>
    [TestMethod]
    ...
    /// </param>
    /// <returns>The built context. <see cref="CBAdESArchiveTimestampImprintContext.CountersignatureOtherFields"/> is always <see langword="null"/> (step 8 is out of scope for this helper).</returns>
