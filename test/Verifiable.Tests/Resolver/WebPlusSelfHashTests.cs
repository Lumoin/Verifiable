...
internal sealed class WebPlusSelfHashTests
{
    ...
    public async Task VerifiesSpecificationRootDocumentSelfHash()
    {
        ...
        bool valid = await WebPlusSelfHash.VerifyAsync(
            jcs, RootSelfHash.AsMemory(), MultihashHeaders.Blake3.ToArray(), Blake3DigestLength, BouncyCastleCryptographicFunctionsAdapter.ComputeBlake3DigestAsync, CryptoTags.Blake3Digest, Base64UrlEncoder, BaseMemoryPool.Shared, CancellationToken.None);
    ...
    public async Task VerifiesSpecificationNonRootDocumentSelfHash()
    {
        ...
        bool valid = await WebPlusSelfHash.VerifyAsync(
            jcs, NonRootSelfHash.AsMemory(), MultihashHeaders.Blake3.ToArray(), Blake3DigestLength, BouncyCastleCryptographicFunctionsAdapter.ComputeBlake3DigestAsync, CryptoTags.Blake3Digest, Base64UrlEncoder, BaseMemoryPool.Shared, CancellationToken.None);
