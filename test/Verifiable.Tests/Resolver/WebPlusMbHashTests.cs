...
internal sealed class WebPlusMbHashTests
{
    ...
    public async Task ComputesDeterministicSha256MbHash()
    {
        string first = await WebPlusMbHash.ComputeAsync(
            "did:webplus test input"u8.ToArray(), MultihashHeaders.Sha2Bits256.ToArray(), Bits256DigestLength, MicrosoftCryptographicFunctionsAdapter.ComputeDigestAsync, CryptoTags.Sha256Digest, Base64UrlEncoder, BaseMemoryPool.Shared, CancellationToken.None);
    ...
    public async Task PlaceholderDiffersFromComputedHash()
    {
        ...
        string computed = await WebPlusMbHash.ComputeAsync(
            "content"u8.ToArray(), MultihashHeaders.Sha2Bits256.ToArray(), Bits256DigestLength, MicrosoftCryptographicFunctionsAdapter.ComputeDigestAsync, CryptoTags.Sha256Digest, Base64UrlEncoder, BaseMemoryPool.Shared, CancellationToken.None);
