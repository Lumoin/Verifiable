...
internal sealed class WebPlusHashedKeyTests
{
    ...
    public async Task ComputesSpecificationHashedKeyVector()
    {
        string hashedKey = await WebPlusHashedKey.ComputeAsync(
            MbPubKey, MultihashHeaders.Blake3.ToArray(), Blake3DigestLength, BouncyCastleCryptographicFunctionsAdapter.ComputeBlake3DigestAsync, CryptoTags.Blake3Digest, TestSetup.Base64UrlEncoder, BaseMemoryPool.Shared, CancellationToken.None);
    ...
    public async Task MatcherAcceptsCommittedKeyAndRejectsOthers()
    {
        HashedKeyMatcher matcher = WebPlusHashedKey.CreateMatcher(
            MultihashHeaders.Blake3.ToArray(), Blake3DigestLength, BouncyCastleCryptographicFunctionsAdapter.ComputeBlake3DigestAsync, CryptoTags.Blake3Digest, TestSetup.Base64UrlEncoder, BaseMemoryPool.Shared);
