...
namespace Verifiable.Tests.Jose
{
    ...
    {
        /// <summary>
        /// Decodes a Base64Url string to bytes. This adapter bridges from the pool-aware decoder to a simple byte array return.
        /// </summary>
        private static ReadOnlySpan<byte> DecodeBase64Url(string input)
        {
            using var decoded = TestSetup.Base64UrlDecoder(input, SensitiveMemoryPool<byte>.Shared);
        ...
        public async Task HsJwtTokenEncodingSigningAndVerifyingSucceeds(HsTestData testData)
        {
            ...
            var isSignatureValid = await JwtExtensions.VerifyCompactSignature(
                ...
                base64UrlDecoder: DecodeBase64Url,
                partDecoder: section =>
                {
            ...
            //There is no random components, so all values should match with the given test values.
    ...
    public static class DefaultVerifierSelector
    {
        ...
        {
            string alg = context.Algorithm ?? (string)context.Parameters["alg"];
            VerifyImplementation<byte> operation = alg switch
            {
                "ES256" => MicrosoftCryptographicFunctions.VerifyP256,
                "ES384" => MicrosoftCryptographicFunctions.VerifyP384,
                "ES512" => MicrosoftCryptographicFunctions.VerifyP521,
                _ => throw new NotSupportedException("Unsupported algorithm: " + alg)
            };
    ...
    public static class MicrosoftCryptographicFunctions
    {
        ...
        {
            return VerifyEcdsa(dataToVerify, signature, publicKeyMaterial, HashAlgorithmName.SHA256, ECCurve.NamedCurves.nistP256);
        }
        ...
        {
            return VerifyEcdsa(dataToVerify, signature, publicKeyMaterial, HashAlgorithmName.SHA384, ECCurve.NamedCurves.nistP384);
        }
        ...
        {
            return VerifyEcdsa(dataToVerify, signature, publicKeyMaterial, HashAlgorithmName.SHA512, ECCurve.NamedCurves.nistP521);
        }
