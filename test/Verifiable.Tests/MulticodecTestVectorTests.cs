...
namespace Verifiable.Tests
{
    ...
    public class MulticodecTestVectorTests
    {
        ...
        public void RountripEd25519()
        {
            ...
            var decodedVector1 = MultibaseSerializer.Decode(Vector1, ExactSizeMemoryPool<byte>.Shared, TestBase58Decoder).Memory.Span;
            
            var multibaseEncodedPublicKey = MultibaseSerializer.Encode(decodedVector1, MulticodecHeaders.Ed25519PublicKey, MultibaseAlgorithms.Base58Btc, TestBase58Encoder);
            var reEncodedVector1 = MultibaseSerializer.Encode(decodedVector1, MulticodecHeaders.Ed25519PublicKey, MultibaseAlgorithms.Base58Btc, TestBase58Encoder);
            Assert.Equal(Vector1, reEncodedVector1);

            var decodedVector2 = MultibaseSerializer.Decode(Vector2, ExactSizeMemoryPool<byte>.Shared, TestBase58Decoder).Memory.Span;
            var reEncodedVector2 = MultibaseSerializer.Encode(decodedVector2, MulticodecHeaders.Ed25519PublicKey, MultibaseAlgorithms.Base58Btc, TestBase58Encoder);
            Assert.Equal(Vector2, reEncodedVector2);

            var decodedVector3 = MultibaseSerializer.Decode(Vector3, ExactSizeMemoryPool<byte>.Shared, TestBase58Decoder).Memory.Span;
            var reEncodedVector3 = MultibaseSerializer.Encode(decodedVector3, MulticodecHeaders.Ed25519PublicKey, MultibaseAlgorithms.Base58Btc, TestBase58Encoder);
            Assert.Equal(Vector3, reEncodedVector3);

            //A sanity check, some (in practice 'any') other vector type should not work.
            var reEncodedVector1WithWrongHeader = MultibaseSerializer.Encode(decodedVector1, MulticodecHeaders.P256PublicKey, MultibaseAlgorithms.Base58Btc, TestBase58Encoder);
        ...
        public void RountripX25519()
        {
            ...
            var decodedVector1 = MultibaseSerializer.Decode(Vector1, ExactSizeMemoryPool<byte>.Shared, TestBase58Decoder).Memory.Span;            
            var reEncodedVector1 = MultibaseSerializer.Encode(decodedVector1, MulticodecHeaders.X25519PublicKey, MultibaseAlgorithms.Base58Btc, TestBase58Encoder);
            Assert.Equal(Vector1, reEncodedVector1);

            var decodedVector2 = MultibaseSerializer.Decode(Vector2, ExactSizeMemoryPool<byte>.Shared, TestBase58Decoder).Memory.Span;
            var reEncodedVector2 = MultibaseSerializer.Encode(decodedVector2, MulticodecHeaders.X25519PublicKey, MultibaseAlgorithms.Base58Btc, TestBase58Encoder);
            Assert.Equal(Vector2, reEncodedVector2);

            var decodedVector3 = MultibaseSerializer.Decode(Vector3, ExactSizeMemoryPool<byte>.Shared, TestBase58Decoder).Memory.Span;
            var reEncodedVector3 = MultibaseSerializer.Encode(decodedVector3, MulticodecHeaders.X25519PublicKey, MultibaseAlgorithms.Base58Btc, TestBase58Encoder);
            Assert.Equal(Vector3, reEncodedVector3);
        ...
        public void RountripSecp256k1()
        {
            ...
            var decodedVector1 = MultibaseSerializer.Decode(Vector1, ExactSizeMemoryPool<byte>.Shared, TestBase58Decoder).Memory.Span;
            var reEncodedVector1 = MultibaseSerializer.Encode(decodedVector1, MulticodecHeaders.Secp256k1PublicKey, MultibaseAlgorithms.Base58Btc, TestBase58Encoder);
            Assert.Equal(Vector1, reEncodedVector1);

            var decodedVector2 = MultibaseSerializer.Decode(Vector2, ExactSizeMemoryPool<byte>.Shared, TestBase58Decoder).Memory.Span;
            var reEncodedVector2 = MultibaseSerializer.Encode(decodedVector2, MulticodecHeaders.Secp256k1PublicKey, MultibaseAlgorithms.Base58Btc, TestBase58Encoder);
            Assert.Equal(Vector2, reEncodedVector2);

            var decodedVector3 = MultibaseSerializer.Decode(Vector3, ExactSizeMemoryPool<byte>.Shared, TestBase58Decoder).Memory.Span;
            var reEncodedVector3 = MultibaseSerializer.Encode(decodedVector3, MulticodecHeaders.Secp256k1PublicKey, MultibaseAlgorithms.Base58Btc, TestBase58Encoder);
            Assert.Equal(Vector3, reEncodedVector3);
        ...
        public void RountripBls12381()
        {
            ...
            var decodedVector1 = MultibaseSerializer.Decode(Vector1, ExactSizeMemoryPool<byte>.Shared, TestBase58Decoder).Memory.Span;
            var reEncodedVector1 = MultibaseSerializer.Encode(decodedVector1, MulticodecHeaders.Bls12381G2PublicKey, MultibaseAlgorithms.Base58Btc, TestBase58Encoder);
            Assert.Equal(Vector1, reEncodedVector1);

            var decodedVector2 = MultibaseSerializer.Decode(Vector2, ExactSizeMemoryPool<byte>.Shared, TestBase58Decoder).Memory.Span;
            var reEncodedVector2 = MultibaseSerializer.Encode(decodedVector2, MulticodecHeaders.Bls12381G2PublicKey, MultibaseAlgorithms.Base58Btc, TestBase58Encoder);
            Assert.Equal(Vector2, reEncodedVector2);
        ...
        public void RountripP256()
        {
            ...
            var decodedVector1 = MultibaseSerializer.Decode(Vector1, ExactSizeMemoryPool<byte>.Shared, TestBase58Decoder).Memory.Span;
            var reEncodedVector1 = MultibaseSerializer.Encode(decodedVector1, MulticodecHeaders.P256PublicKey, MultibaseAlgorithms.Base58Btc, TestBase58Encoder);
            Assert.Equal(Vector1, reEncodedVector1);

            var decodedVector2 = MultibaseSerializer.Decode(Vector2, ExactSizeMemoryPool<byte>.Shared, TestBase58Decoder).Memory.Span;
            var reEncodedVector2 = MultibaseSerializer.Encode(decodedVector2, MulticodecHeaders.P256PublicKey, MultibaseAlgorithms.Base58Btc, TestBase58Encoder);
            Assert.Equal(Vector2, reEncodedVector2);

            //A sanity check, some (in practice 'any') other vector type should not work.
            var reEncodedVector1WithWrongHeader = MultibaseSerializer.Encode(decodedVector1, MulticodecHeaders.Bls12381G2PublicKey, MultibaseAlgorithms.Base58Btc, TestBase58Encoder);
            Assert.NotEqual(Vector1, reEncodedVector1WithWrongHeader);
        }
        ...
        public void RountripP384()
        {
            ...
            var decodedVector1 = MultibaseSerializer.Decode(Vector1, ExactSizeMemoryPool<byte>.Shared, TestBase58Decoder).Memory.Span;
            var reEncodedVector1 = MultibaseSerializer.Encode(decodedVector1, MulticodecHeaders.P384PublicKey, MultibaseAlgorithms.Base58Btc, TestBase58Encoder);
            Assert.Equal(Vector1, reEncodedVector1);

            var decodedVector2 = MultibaseSerializer.Decode(Vector2, ExactSizeMemoryPool<byte>.Shared, TestBase58Decoder).Memory.Span;
            var reEncodedVector2 = MultibaseSerializer.Encode(decodedVector2, MulticodecHeaders.P384PublicKey, MultibaseAlgorithms.Base58Btc, TestBase58Encoder);
            Assert.Equal(Vector2, reEncodedVector2);
        ...
        public void RountripP521()
        {
            ...
            var decodedVector1 = MultibaseSerializer.Decode(Vector1, ExactSizeMemoryPool<byte>.Shared, TestBase58Decoder).Memory.Span;
            var reEncodedVector1 = MultibaseSerializer.Encode(decodedVector1, MulticodecHeaders.P521PublicKey, MultibaseAlgorithms.Base58Btc, TestBase58Encoder);
            Assert.Equal(Vector1, reEncodedVector1);

            var decodedVector2 = MultibaseSerializer.Decode(Vector2, ExactSizeMemoryPool<byte>.Shared, TestBase58Decoder).Memory.Span;
            var reEncodedVector2 = MultibaseSerializer.Encode(decodedVector2, MulticodecHeaders.P521PublicKey, MultibaseAlgorithms.Base58Btc, TestBase58Encoder);
            Assert.Equal(Vector2, reEncodedVector2);
        ...
        public void RountripRsa2048()
        {
            ...
            var decodedVector1 = MultibaseSerializer.Decode(Vector1, ExactSizeMemoryPool<byte>.Shared, TestBase58Decoder).Memory.Span;
            var reEncodedVector1 = MultibaseSerializer.Encode(decodedVector1, MulticodecHeaders.RsaPublicKey, MultibaseAlgorithms.Base58Btc, TestBase58Encoder);
            Assert.Equal(Vector1, reEncodedVector1);
