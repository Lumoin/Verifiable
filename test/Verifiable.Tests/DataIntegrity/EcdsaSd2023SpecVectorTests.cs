...
public sealed class EcdsaSd2023W3cVectorTests
{
    ...
    /// <summary>
    /// Base proof configuration from Example 78.
    /// </summary>
    private static string BaseProofConfiguration { get; } = /*lang=json,strict*/ """
    {
    ...
    /// </summary>
    private static IReadOnlyDictionary<string, string> DerivedLabelMap { get; } = new Dictionary<string, string>
    {
    ...
    /// <summary>
    /// Debug test that mirrors DeriveProofVerboseAsync exactly.
    /// </summary>
    [TestMethod]
    public async ValueTask IssuerHolderVerifierFlowWithVerboseApisDebug()
    {
        var cancellationToken = TestContext.CancellationToken;
        ...
        TestContext.WriteLine($"=== ISSUER BASE PROOF ===");
        TestContext.WriteLine($"Signed statements count: {baseProofResult.SignedStatements.Count}");
        TestContext.WriteLine($"Non-mandatory indexes from issuer: [{string.Join(", ", baseProofResult.NonMandatoryIndexes)}]");
        TestContext.WriteLine($"Label map from issuer: {string.Join(", ", baseProofResult.LabelMap.Select(kv => $"{kv.Key}->{kv.Value}"))}");

        baseProofResult.Dispose();

        //=== Now simulate DeriveProofVerboseAsync exactly ===
        var proof = signedCredential.Proof![0];

        using var parsedProof = EcdsaSd2023CborSerializer.ParseBaseProof(
            proof.ProofValue!,
            TestSetup.Base64UrlDecoder,
            SensitiveMemoryPool<byte>.Shared);

        TestContext.WriteLine($"=== PARSED BASE PROOF ===");
        TestContext.WriteLine($"Signatures count: {parsedProof.Signatures.Count}");
        TestContext.WriteLine($"MandatoryPointers: [{string.Join(", ", parsedProof.MandatoryPointers)}]");

        //Clone without proof.
        var credentialWithoutProof = new VerifiableCredential
        {
            Context = signedCredential.Context,
            Id = signedCredential.Id,
            Type = signedCredential.Type,
            Name = signedCredential.Name,
            Description = signedCredential.Description,
            Issuer = signedCredential.Issuer,
            CredentialSubject = signedCredential.CredentialSubject,
            ValidFrom = signedCredential.ValidFrom,
            ValidUntil = signedCredential.ValidUntil,
            CredentialStatus = signedCredential.CredentialStatus,
            CredentialSchema = signedCredential.CredentialSchema,
            RelatedResource = signedCredential.RelatedResource,
            RefreshService = signedCredential.RefreshService,
            TermsOfUse = signedCredential.TermsOfUse,
            Evidence = signedCredential.Evidence,
            AdditionalData = signedCredential.AdditionalData,
            Proof = null
        };
        var fullCredentialJson = SerializeCredential(credentialWithoutProof);

        string canonicalizeSync(string json) => rdfcCanonicalizer(json, contextResolver, cancellationToken).AsTask().GetAwaiter().GetResult();

        var fullPartition = JsonLdSelection.PartitionStatements(
            fullCredentialJson,
            parsedProof.MandatoryPointers,
            canonicalizeSync);

        TestContext.WriteLine($"=== FULL CREDENTIAL ===");
        TestContext.WriteLine($"Full partition - AllStatements count: {fullPartition.AllStatements.Count}");
        ...
        TestContext.WriteLine($"Full prepared - LabelMap: {string.Join(", ", fullPrepared.LabelMap.Select(kv => $"{kv.Key}->{kv.Value}"))}");
        TestContext.WriteLine($"Full prepared - NonMandatoryIndexes: [{string.Join(", ", fullPrepared.NonMandatoryIndexes.OrderBy(i => i))}]");

        //Show full HMAC statements.
        TestContext.WriteLine($"=== FULL HMAC SORTED STATEMENTS ===");
        for(int i = 0; i < fullPrepared.SortedStatements.Count; i++)
        {
            var marker = fullPrepared.NonMandatoryIndexes.Contains(i) ? "(non-mand)" : "(mandatory)";
            TestContext.WriteLine($"  [{i}] {marker}: {fullPrepared.SortedStatements[i].TrimEnd()}");
        }

        //Build statement to signature map from full credential.
        ...
        TestContext.WriteLine($"Statement to signature map size: {statementToSignature.Count}");

        //Create reduced credential.
        var verifierRequestedPaths = new HashSet<CredentialPath>
    {
        CredentialPath.FromJsonPointer("/credentialSubject/givenName"),
        CredentialPath.FromJsonPointer("/credentialSubject/familyName"),
        CredentialPath.FromJsonPointer("/credentialSubject/birthCountry"),
        CredentialPath.FromJsonPointer("/validFrom"),
        CredentialPath.FromJsonPointer("/validUntil")
    };
        ...
        var requestedPointers = verifierRequestedPaths
            .Select(p => Verifiable.JsonPointer.JsonPointer.Parse(p.ToJsonPointerString()))
            .ToList();
        var disclosedPointers = parsedProof.MandatoryPointers.ToList();
        disclosedPointers.AddRange(requestedPointers);

        var reducedCredentialJson = JsonLdSelection.SelectFragments(fullCredentialJson, disclosedPointers);

        TestContext.WriteLine($"=== REDUCED CREDENTIAL ===");
        TestContext.WriteLine($"JSON: {reducedCredentialJson}");

        var reducedPartition = JsonLdSelection.PartitionStatements(
            reducedCredentialJson,
            parsedProof.MandatoryPointers,
            canonicalizeSync);

        TestContext.WriteLine($"Reduced partition - AllStatements count: {reducedPartition.AllStatements.Count}");
        TestContext.WriteLine($"Reduced partition - AllStatements:");
        for(int i = 0; i < reducedPartition.AllStatements.Count; i++)
        {
            TestContext.WriteLine($"  [{i}]: {reducedPartition.AllStatements[i].TrimEnd()}");
        }

        //Compute correct label map for reduced credential (matching what DeriveProofVerboseAsync should do).
        var reducedLabelMap = ComputeReducedLabelMapForTest(
            reducedPartition.AllStatements,
            fullPrepared.SortedStatements,
            fullPrepared.LabelMap);

        TestContext.WriteLine($"=== COMPUTED REDUCED LABEL MAP ===");
        TestContext.WriteLine($"Reduced label map: {string.Join(", ", reducedLabelMap.Select(kv => $"{kv.Key}->{kv.Value}"))}");

        var reducedPrepared = NQuadStatementPreparation.PrepareWithLabelMap(
            reducedPartition.AllStatements,
            reducedPartition.MandatoryIndexes,
            reducedLabelMap);

        TestContext.WriteLine($"=== REDUCED PREPARED (using computed label map) ===");
        TestContext.WriteLine($"Reduced prepared - SortedStatements count: {reducedPrepared.SortedStatements.Count}");
        TestContext.WriteLine($"Reduced prepared - NonMandatoryIndexes: [{string.Join(", ", reducedPrepared.NonMandatoryIndexes.OrderBy(i => i))}]");
        TestContext.WriteLine($"Reduced prepared - SortedStatements:");
        for(int i = 0; i < reducedPrepared.SortedStatements.Count; i++)
        {
            var marker = reducedPrepared.NonMandatoryIndexes.Contains(i) ? "(non-mand)" : "(mandatory)";
            TestContext.WriteLine($"  [{i}] {marker}: {reducedPrepared.SortedStatements[i].TrimEnd()}");
        }

        //Match signatures.
        var reducedSortedNonMandatoryIndexes = reducedPrepared.NonMandatoryIndexes.OrderBy(i => i).ToList();
        var selectedSignatures = new List<byte[]>();

        TestContext.WriteLine($"=== SIGNATURE MATCHING ===");
        foreach(int reducedIdx in reducedSortedNonMandatoryIndexes)
        {
            var statement = reducedPrepared.SortedStatements[reducedIdx];
            TestContext.WriteLine($"Looking for [{reducedIdx}]: {statement.TrimEnd()}");
            ...
            {
                TestContext.WriteLine($"  -> NOT FOUND");
            }
        }

        TestContext.WriteLine($"=== RESULTS ===");
        TestContext.WriteLine($"Selected signatures count: {selectedSignatures.Count}");
        TestContext.WriteLine($"Expected (W3C DerivedFilteredSignaturesHex): {DerivedFilteredSignaturesHex.Length}");

        //Call the actual derive.
        var (derivedCredential, selectionResult) = await signedCredential.DeriveProofVerboseAsync(
            verifierRequestedPaths,
            userExclusions: null,
            JsonLdSelection.PartitionStatements,
        ...
        TestContext.WriteLine($"=== ACTUAL DERIVED PROOF (from DeriveProofVerboseAsync) ===");
        TestContext.WriteLine($"Signatures count: {parsedDerivedProof.Signatures.Count}");
        TestContext.WriteLine($"MandatoryIndexes: [{string.Join(", ", parsedDerivedProof.MandatoryIndexes)}]");
        TestContext.WriteLine($"LabelMap: {string.Join(", ", parsedDerivedProof.LabelMap.Select(kv => $"{kv.Key}->{kv.Value}"))}");
        ...
        TestContext.WriteLine($"Verifier result: {verifierResult}");

        Assert.AreEqual(CredentialVerificationResult.Success(), verifierResult, "Verifier must successfully verify derived proof.");

        verifierContext?.Dispose();
    }


    private static Dictionary<string, string> ComputeReducedLabelMapForTest(
    ...
    private static string SerializeCredential(VerifiableCredential credential) =>
        JsonSerializer.Serialize(credential, TestSetup.DefaultSerializationOptions);

    private static VerifiableCredential DeserializeCredential(string json) =>
        JsonSerializer.Deserialize<VerifiableCredential>(json, TestSetup.DefaultSerializationOptions)!;
