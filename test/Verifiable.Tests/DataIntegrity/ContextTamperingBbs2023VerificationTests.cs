...
internal sealed class ContextTamperingBbs2023VerificationTests
{
    ...
    /// equivalent protections, to validate contexts in a conforming secured document." A bbs-2023 base proof over a
    ...
    /// <see href="https://www.w3.org/TR/vc-data-integrity/#validating-contexts">Data Integrity §2.4.1 Validating
    /// Contexts</see>: "Applications MUST use the algorithm in Section 4.6 Context Validation, or one that achieves
    /// equivalent protections, to validate contexts in a conforming secured document." A bbs-2023 derived proof over a
    /// credential whose <c>@context</c> was altered after derivation is refused for the reason the alteration produces.
    /// </summary>
    /// <param name="variantName">The name of the alteration.</param>
    /// <param name="tamperedContextJson">The altered <c>@context</c>.</param>
    /// <param name="expectedReason">The reason the alteration is refused for.</param>
    ...
    /// Verify Proof or Section 4.5 Verify Proof Sets and Chains." An untampered bbs-2023 base proof, checked against its
    /// own <c>@context</c>, passes both and verifies.
    /// </summary>
    [TestMethod]
    ...
    /// <see href="https://www.w3.org/TR/vc-data-integrity/#validating-contexts">Data Integrity §2.4.1 Validating
    /// Contexts</see>: "Context validation MUST be run after running the applicable algorithm in either Section 4.4
    /// Verify Proof or Section 4.5 Verify Proof Sets and Chains." An untampered bbs-2023 derived proof, checked against
    ...
    /// </remarks>
    [TestMethod]
    public async Task BbsBaseProofWithoutTypeIsRejected()
    {
        var cancellationToken = TestContext.CancellationToken;
        using var bbs = Bbs2023ResolvingBindingTests.ResolvingBbsOperations.Generate();
    ...
    /// </remarks>
    [TestMethod]
    public async Task BbsDerivedProofWithoutTypeIsRejected()
    {
        var cancellationToken = TestContext.CancellationToken;
        using var bbs = Bbs2023ResolvingBindingTests.ResolvingBbsOperations.Generate();
    ...
    /// missing verificationMethod, exactly as <see cref="BbsBaseProofWithoutTypeIsRejected"/>
    ...
    /// </remarks>
    [TestMethod]
    public async Task BbsBaseProofWithoutVerificationMethodIsRejected()
    {
        var cancellationToken = TestContext.CancellationToken;
        using var bbs = Bbs2023ResolvingBindingTests.ResolvingBbsOperations.Generate();
    ...
    /// </remarks>
    [TestMethod]
    public async Task BbsBaseProofWithoutProofPurposeIsRejected()
    {
        var cancellationToken = TestContext.CancellationToken;
        using var bbs = Bbs2023ResolvingBindingTests.ResolvingBbsOperations.Generate();
    ...
    /// </remarks>
    [TestMethod]
    public async Task BbsDerivedProofWithoutVerificationMethodIsRejected()
    {
        var cancellationToken = TestContext.CancellationToken;
        using var bbs = Bbs2023ResolvingBindingTests.ResolvingBbsOperations.Generate();
    ...
    /// </remarks>
    [TestMethod]
    public async Task BbsDerivedProofWithoutProofPurposeIsRejected()
    {
        var cancellationToken = TestContext.CancellationToken;
        using var bbs = Bbs2023ResolvingBindingTests.ResolvingBbsOperations.Generate();
    ...
    /// bbs-2023 base proof verifier that takes its signature verifier from its caller refuses a proof declaring the
    /// purpose <c>x</c>, one no verification relationship models, before it verifies anything.
    /// </summary>
    [TestMethod]
    public async Task BbsBaseProofWithUnexpectedPurposeIsRejected()
    {
        var cancellationToken = TestContext.CancellationToken;
        using var bbs = Bbs2023ResolvingBindingTests.ResolvingBbsOperations.Generate();

        var (signed, _) = await DataIntegrityContextTamperingFixture.CreateBbsBaseProofAsync(
            bbs, DataIntegrityContextTamperingFixture.CreateUnsignedCredentialJson(BaseCredentialId), ProofCreated, cancellationToken).ConfigureAwait(false);
        ...
        Assert.AreEqual(VerificationFailureReason.ProofPurposeMismatch, result.FailureReason,
            "The proof is refused for its purpose, whichever verifier the caller supplied.");
    ...
    /// so the bbs-2023 derived proof verifier that takes its proof verifier from its caller refuses a derived proof
    /// declaring the purpose <c>x</c>, one no verification relationship models, before it verifies anything.
    /// </summary>
    [TestMethod]
    public async Task BbsDerivedProofWithUnexpectedPurposeIsRejected()
    {
        var cancellationToken = TestContext.CancellationToken;
        using var bbs = Bbs2023ResolvingBindingTests.ResolvingBbsOperations.Generate();
