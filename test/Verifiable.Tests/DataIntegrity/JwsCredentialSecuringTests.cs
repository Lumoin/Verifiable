...
/// <item><description>Signing via <see cref="CredentialJwsExtensions.SignJwsAsync(VerifiableCredential, PrivateKeyMemory, string, CredentialToJsonBytesDelegate, Verifiable.Core.Model.Credentials.JwtHeaderSerializer, EncodeDelegate, BaseMemoryPool, string?, string?, CancellationToken)"/>.</description></item>
/// <item><description>Verification via <see cref="CredentialJwsExtensions.VerifyJwsAsync(string, PublicKeyMemory, DecodeDelegate, JwtHeaderDeserializer, CredentialFromJsonBytesDelegate, BaseMemoryPool, CancellationToken)"/>.</description></item>
...
internal sealed class JwsCredentialSecuringTests
{
    ...
    public async Task SignJwsAndVerifyFromCompactSerializationSucceeds()
    {
        ...
        Assert.AreEqual(credential.Id, result.Credential!.Value.Value.Id);
    }
