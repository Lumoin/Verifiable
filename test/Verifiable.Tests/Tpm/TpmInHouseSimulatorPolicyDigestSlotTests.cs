...
/// <summary>
/// Drives <c>TPM2_PolicyCpHash()</c> (TPM 2.0 Library Part 3, clause 23.13), <c>TPM2_PolicyNameHash()</c>
/// (clause 23.14), <c>TPM2_PolicyTemplate()</c> (clause 23.21), and <c>TPM2_PolicyParameters()</c> (Section
...
internal sealed class TpmInHouseSimulatorPolicyDigestSlotTests
{
    ...
    private static byte[] DigestB { get; } = Convert.FromHexString("B2B2B2B2B2B2B2B2B2B2B2B2B2B2B2B2B2B2B2B2B2B2B2B2B2B2B2B2B2B2B2B2");
    ...
    /// (Part 3, clause 23.13), matching <see cref="TpmPolicyDigest.ExtendForCpHash"/> from a fresh policyDigest.
    /// </summary>
    [TestMethod]
    public async Task PolicyCpHashFoldsTheExpectedDigest()
    {
        ...
        zero.Clear();
        _ = TpmPolicyDigest.ExtendForCpHash(zero, DigestA, PolicySweepHarness.SessionAlg, predicted, pool);
    ...
    /// width with <c>TPM_RC_SIZE</c> (Part 3, clause 23.13: "If cpHashA does not have the size of the
    ...
    public async Task PolicyCpHashRefusesAWrongSizeDigest()
    {
        ...
        try
        {
            ...
            Assert.AreEqual(HmacKeyHarness.ParameterEncodedRc(TpmRcConstants.TPM_RC_SIZE, 0), result.ResponseCode, "A wrong-size cpHashA must refuse TPM_RC_SIZE.");
        }
        finally
        {
            await PolicySweepHarness.FlushIfPresentAsync(tpm, sessionHandle, TestContext.CancellationToken).ConfigureAwait(false);
    ...
    /// clause 23.13: "If policySession→cpHash is already set and not the same as cpHashA... TPM_RC_CPHASH"),
    ...
    /// (Part 3, clause 23.14), matching <see cref="TpmPolicyDigest.ExtendForNameHash"/> from a fresh
    /// policyDigest.
    /// </summary>
    [TestMethod]
    public async Task PolicyNameHashFoldsTheExpectedDigest()
    {
        ...
        Span<byte> zero = stackalloc byte[size];
        zero.Clear();
        _ = TpmPolicyDigest.ExtendForNameHash(zero, DigestA, PolicySweepHarness.SessionAlg, predicted, pool);
    ...
    /// <c>TPM_RC_CPHASH</c> (Part 3, clause 23.14: "If policySession→cpHash is already set, the TPM shall
    ...
    /// templateHash)</c> (Part 3, clause 23.21), matching <see cref="TpmPolicyDigest.ExtendForTemplate"/> from a
    /// fresh policyDigest.
    /// </summary>
    [TestMethod]
    public async Task PolicyTemplateFoldsTheExpectedDigest()
    {
        ...
        Span<byte> zero = stackalloc byte[size];
        zero.Clear();
        _ = TpmPolicyDigest.ExtendForTemplate(zero, DigestA, PolicySweepHarness.SessionAlg, predicted, pool);
    ...
    /// (Part 3, clause 23.21: "If policySession→isTemplateSet is SET and policySession→cpHash is not equal to
    ...
    public async Task PolicyTemplateRefusesADifferentTemplateWithValue()
    {
        ...
        try
        {
            ...
            Assert.AreEqual(TpmRcConstants.TPM_RC_CPHASH, conflicting.ResponseCode, "A conflicting templateHash must refuse TPM_RC_CPHASH: the templateHash slot, once latched, is the same occupied-slot check every Policy*Hash command shares.");
    ...
    /// <c>TPM_RC_CPHASH</c> (Part 3, clause 23.21: "if policySession→cpHash is already set, the TPM shall
    ...
    /// command parameter digest) refuses <c>TPM2_Unseal()</c> with a <c>TPM_RC_POLICY_FAIL</c>, session-encoded to the same index even though
    /// its accumulated policyDigest matches the sealed object's authPolicy: the use-time binding check recomputes
    /// the REAL cpHash of the authorized command and finds it unequal to the latched value (Part 3, clause
    ...
    public async Task PolicyCpHashBindingRefusesANonMatchingCommandAtUnseal()
    {
        ...
        Span<byte> zero = stackalloc byte[size];
        zero.Clear();
        _ = TpmPolicyDigest.ExtendForCpHash(zero, DigestA, PolicySweepHarness.SessionAlg, authPolicy, pool);
        ...
        try
        {
            ...
            using TpmPolicySession policySession = TpmPolicySession.ForSession(sessionHandle, PolicySweepHarness.SessionAlg, TestEntropy.NewCounterStream(), pool);
            ...
            Assert.AreEqual(HmacKeyHarness.SessionEncodedRc(TpmRcConstants.TPM_RC_POLICY_FAIL, 0), unsealResult.ResponseCode, "The cpHash binding mismatch is the policy's own TPM_RC_POLICY_FAIL at itemHandle, session 1 of Table 30.");
        }
        finally
        {
            await PolicySweepHarness.FlushIfPresentAsync(tpm, sessionHandle, TestContext.CancellationToken).ConfigureAwait(false);
    ...
    /// <summary>
    /// Verifies a policy session bound to an arbitrary nameHash refuses <c>TPM2_Unseal()</c> with
    /// <c>TPM_RC_POLICY_FAIL</c>, session-encoded to the same index, even though its accumulated policyDigest matches the sealed object's authPolicy:
    /// the use-time binding check recomputes the REAL nameHash (the digest of the authorized command's target
    /// Name) and finds it unequal to the latched value (Part 3, clause 23.14).
    /// </summary>
    [TestMethod]
    public async Task PolicyNameHashBindingRefusesANonMatchingCommandAtUnseal()
    {
        ...
        Span<byte> zero = stackalloc byte[size];
        zero.Clear();
        _ = TpmPolicyDigest.ExtendForNameHash(zero, DigestA, PolicySweepHarness.SessionAlg, authPolicy, pool);
        ...
        try
        {
            ...
            Assert.AreEqual(HmacKeyHarness.SessionEncodedRc(TpmRcConstants.TPM_RC_POLICY_FAIL, 0), unsealResult.ResponseCode, "The nameHash binding mismatch is the policy's own TPM_RC_POLICY_FAIL at itemHandle, session 1 of Table 30.");
        }
        finally
        {
            await PolicySweepHarness.FlushIfPresentAsync(tpm, sessionHandle, TestContext.CancellationToken).ConfigureAwait(false);
    ...
    /// <summary>
    /// Verifies a policy session bound to a creation template ALWAYS refuses <c>TPM2_Unseal()</c> at the
    /// itemHandle authorization, session 1 of Table 30, with <c>TPM_RC_POLICY_FAIL</c>, even though its
    /// accumulated policyDigest matches the sealed object's authPolicy: a template binding can only be satisfied
    /// by an object-creation command (Part 3, clause 23.21), and Unseal is not one, so the deferred use-time
    /// check refuses unconditionally rather than comparing a recomputed digest.
    /// </summary>
    [TestMethod]
    public async Task PolicyTemplateAtUseAlwaysFailsUnseal()
    {
        ...
        Span<byte> zero = stackalloc byte[size];
        zero.Clear();
        _ = TpmPolicyDigest.ExtendForTemplate(zero, DigestA, PolicySweepHarness.SessionAlg, authPolicy, pool);
        ...
        try
        {
            ...
            Assert.AreEqual(HmacKeyHarness.SessionEncodedRc(TpmRcConstants.TPM_RC_POLICY_FAIL, 0), unsealResult.ResponseCode, "A template binding at use fails the itemHandle authorization, session 1 of Table 30, regardless of the digest match.");
        }
        finally
        {
            await PolicySweepHarness.FlushIfPresentAsync(tpm, sessionHandle, TestContext.CancellationToken).ConfigureAwait(false);
    ...
    /// (Part 3, clause 23.24), matching <see cref="TpmPolicyDigest.ExtendForParameters"/> from a fresh policyDigest.
    ...
    public async Task PolicyParametersFoldsTheExpectedDigest()
    {
        ...
        Span<byte> zero = stackalloc byte[size];
        zero.Clear();
        _ = TpmPolicyDigest.ExtendForParameters(zero, DigestA, PolicySweepHarness.SessionAlg, predicted, pool);
    ...
    public async Task PolicyParametersRefusesAWrongSizeDigest()
    {
        ...
        try
        {
            ...
            Assert.AreEqual(HmacKeyHarness.ParameterEncodedRc(TpmRcConstants.TPM_RC_SIZE, 0), result.ResponseCode, "A pHash of the wrong width is TPM_RC_SIZE.");
        }
        finally
        {
            await PolicySweepHarness.FlushIfPresentAsync(tpm, sessionHandle, TestContext.CancellationToken).ConfigureAwait(false);
    ...
    /// and the reference's own checks answer it first too.
    /// <see href="https://trustedcomputinggroup.org/resource/tpm-library-specification/">TPM 2.0 Library Part 3, clause 23.24</see>.
    /// </summary>
    [TestMethod]
    public async Task PolicyParametersAnswersSizeAheadOfAnOccupiedSlot()
    {
        ...
        try
        {
            ...
            Assert.AreEqual(HmacKeyHarness.ParameterEncodedRc(TpmRcConstants.TPM_RC_SIZE, 0), result.ResponseCode, "The size rule answers before the occupied slot does.");
        }
        finally
        {
            await PolicySweepHarness.FlushIfPresentAsync(tpm, sessionHandle, TestContext.CancellationToken).ConfigureAwait(false);
    ...
    /// <summary>
    /// Verifies a policy session bound to an arbitrary pHash refuses <c>TPM2_Unseal()</c> with
    /// <c>TPM_RC_POLICY_FAIL</c>, session-encoded to the same index, even though its accumulated policyDigest matches the sealed object's authPolicy:
    ...
    public async Task PolicyParametersBindingRefusesANonMatchingCommandAtUnseal()
    {
        ...
        Span<byte> zero = stackalloc byte[size];
        zero.Clear();
        _ = TpmPolicyDigest.ExtendForParameters(zero, DigestA, PolicySweepHarness.SessionAlg, authPolicy, pool);
        ...
        try
        {
            ...
            Assert.AreEqual(HmacKeyHarness.SessionEncodedRc(TpmRcConstants.TPM_RC_POLICY_FAIL, 0), unsealResult.ResponseCode, "The pHash binding mismatch is the policy's own TPM_RC_POLICY_FAIL at itemHandle, session 1 of Table 30.");
        }
        finally
        {
            await PolicySweepHarness.FlushIfPresentAsync(tpm, sessionHandle, TestContext.CancellationToken).ConfigureAwait(false);
    ...
    public async Task PolicyParametersBindingAdmitsUnsealWhosePHashMatches()
    {
        ...
        Span<byte> zero = stackalloc byte[size];
        zero.Clear();
        _ = TpmPolicyDigest.ExtendForParameters(zero, unsealParametersHash, PolicySweepHarness.SessionAlg, authPolicy, pool);
    ...
    public async Task UnsealOverAPolicySessionRefusesAnObjectWithAnEmptyAuthPolicy()
    {
        ...
        try
        {
            ...
            Assert.AreEqual(HmacKeyHarness.SessionEncodedRc(TpmRcConstants.TPM_RC_POLICY_FAIL, 0), unsealResult.ResponseCode, "An empty authPolicy never equals a session's policyDigest: TPM_RC_POLICY_FAIL.");
        }
        finally
        {
            await PolicySweepHarness.FlushIfPresentAsync(tpm, sessionHandle, TestContext.CancellationToken).ConfigureAwait(false);
