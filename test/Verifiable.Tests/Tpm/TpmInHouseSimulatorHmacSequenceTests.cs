...
internal sealed class TpmInHouseSimulatorHmacSequenceTests
{
    ...
    private static byte[] KeyPassword { get; } = "hmac-start-key-auth"u8.ToArray();

    /// <summary>A value that is not the key's authorization value.</summary>
    private static byte[] WrongKeyPassword { get; } = "hmac-start-key-wrong"u8.ToArray();
    ...
    private static byte[] WrongSequencePassword { get; } = "hmac-sequence-wrong"u8.ToArray();
    ...
    /// completion names a TRANSIENT-range <c>sequenceHandle</c> resolving to no loaded sequence, refused
    /// <c>TPM_RC_REFERENCE_H0</c> (TPM 2.0 Library Part 3, clause 5.4, step 2.1).
    ...
    public async Task HmacSequenceIsFlushedAfterCompletion()
    {
        ...
        Assert.AreEqual(TpmRcConstants.TPM_RC_REFERENCE_H0, second.ResponseCode, "sequenceHandle is TPM2_SequenceComplete()'s sole handle (index 0); a flushed sequence handle is a TRANSIENT-range value resolving to nothing loaded, TPM_RC_REFERENCE_H0 (TPM 2.0 Library Part 3, clause 5.4, step 2.1).");
    }

    /// <summary>
    /// <c>TPM2_FlushContext()</c> releases an open HMAC sequence: the flush succeeds, a completion of the flushed
    /// handle names a TRANSIENT-range <c>sequenceHandle</c> resolving to no loaded sequence (refused
    /// <c>TPM_RC_REFERENCE_H0</c>, TPM 2.0 Library Part 3, clause 5.4, step 2.1), and the pool returns every
    /// carrier the sequence owned — its key copy among them.
    ...
    public async Task FlushContextReleasesAnOpenHmacSequence()
    {
        ...
        TpmResult<SequenceCompleteResponse> completeResult = await CompleteAsync(tpm, registry, pool, sequence, Rfc4231Case3Data).ConfigureAwait(false);
        Assert.AreEqual(TpmRcConstants.TPM_RC_REFERENCE_H0, completeResult.ResponseCode, "sequenceHandle is TPM2_SequenceComplete()'s sole handle (index 0); a flushed sequence handle no longer resolves, a TRANSIENT-range value naming nothing loaded, TPM_RC_REFERENCE_H0 (TPM 2.0 Library Part 3, clause 5.4, step 2.1).");
    ...
    public async Task HmacSequenceSurvivesTheKeyBeingFlushedAfterStart()
    {
        ...
        Assert.AreEqual(HmacKeyHarness.HandleEncodedRc(TpmRcConstants.TPM_RC_HANDLE, 0), gone.ResponseCode, "handle, handle 1 of Table 71, must no longer resolve before the sequence completes, refused with handle-encoded TPM_RC_HANDLE.");
    ...
    public async Task HmacSequenceWithAWrongSequencePasswordIsBadAuthUnchargedAndSurvives()
    {
        ...
        Assert.AreEqual(HmacKeyHarness.SessionEncodedRc(TpmRcConstants.TPM_RC_BAD_AUTH, sessionIndex: 0), refused.ResponseCode, "sequenceHandle's authorizing session, session 1 of Table 93, is refused with session-encoded TPM_RC_BAD_AUTH on a wrong sequence password.");
    ...
    public async Task HmacStartOverAnAsymmetricKeyHandleIsRefusedWithType()
    {
        ...
        Assert.AreEqual(HmacKeyHarness.HandleEncodedRc(TpmRcConstants.TPM_RC_TYPE, 0), result.ResponseCode, "handle, handle 1 of Table 80, naming an asymmetric key must be refused with handle-encoded TPM_RC_TYPE.");
    }
    ...
    public async Task HmacStartOverASequenceHandleIsRefusedWithType()
    {
        ...
        Assert.AreEqual(HmacKeyHarness.HandleEncodedRc(TpmRcConstants.TPM_RC_TYPE, 0), result.ResponseCode, "handle, handle 1 of Table 80, naming a sequence object must be refused with handle-encoded TPM_RC_TYPE.");
    }
    ...
    public async Task HmacStartOverASealedDataObjectIsRefusedWithKey()
    {
        ...
        Assert.AreEqual(HmacKeyHarness.HandleEncodedRc(TpmRcConstants.TPM_RC_KEY, 0), result.ResponseCode, "handle, handle 1 of Table 80, naming a sealed data object must be refused with handle-encoded TPM_RC_KEY.");
    }
    ...
    public async Task HmacStartOverARestrictedKeyIsRefusedWithAttributes()
    {
        ...
        Assert.AreEqual(HmacKeyHarness.HandleEncodedRc(TpmRcConstants.TPM_RC_ATTRIBUTES, 0), result.ResponseCode, "handle, handle 1 of Table 80, naming a restricted KEYEDHASH key must be refused with handle-encoded TPM_RC_ATTRIBUTES.");
    }
    ...
    public async Task HmacStartWithAWrongPasswordAndAMismatchedHashAlgIsAuthFailAndCharged()
    {
        ...
        Assert.AreEqual(HmacKeyHarness.SessionEncodedRc(TpmRcConstants.TPM_RC_AUTH_FAIL, sessionIndex: 0), result.ResponseCode, "handle's authorizing session, session 1 of Table 80, is refused with session-encoded TPM_RC_AUTH_FAIL: the password compare must answer before the hashAlg selection.");
    ...
    public async Task HmacStartWithAMismatchedHashAlgIsRefusedWithValue()
    {
        ...
        Assert.AreEqual(HmacKeyHarness.ParameterEncodedRc(TpmRcConstants.TPM_RC_VALUE, 1), result.ResponseCode, "hashAlg, parameter 2 of Table 80, other than the key's scheme hash must be refused with parameter-encoded TPM_RC_VALUE.");
    }
    ...
    public async Task HmacStartWithAWrongPasswordOnADaProtectedKeyIsAuthFailAndCharged()
    {
        ...
        Assert.AreEqual(HmacKeyHarness.SessionEncodedRc(TpmRcConstants.TPM_RC_AUTH_FAIL, sessionIndex: 0), result.ResponseCode, "handle's authorizing session, session 1 of Table 80, is refused with session-encoded TPM_RC_AUTH_FAIL on a wrong password against a DA-protected key.");
    ...
    public async Task HmacStartWithAWrongPasswordOnANoDaKeyIsBadAuthAndUncharged()
    {
        ...
        TpmResult<HmacStartResponse> result = await HmacKeyHarness.HmacStartAsync(
            tpm, registry, pool, key.Handle, TpmAlgIdConstants.TPM_ALG_SHA256, WrongKeyPassword, ReadOnlyMemory<byte>.Empty, TestContext.CancellationToken).ConfigureAwait(false);
        Assert.AreEqual(HmacKeyHarness.SessionEncodedRc(TpmRcConstants.TPM_RC_BAD_AUTH, sessionIndex: 0), result.ResponseCode, "handle's authorizing session, session 1 of Table 80, is refused with session-encoded TPM_RC_BAD_AUTH on a wrong password against a noDA key.");
    ...
    public async Task HmacStartOverAUserWithAuthClearKeyIsRefusedWithPolicyFail()
    {
        ...
        Assert.AreEqual(HmacKeyHarness.SessionEncodedRc(TpmRcConstants.TPM_RC_POLICY_FAIL, 0), result.ResponseCode, "handle's authorizing session, session 1 of Table 80, is refused with session-encoded TPM_RC_POLICY_FAIL when a password session presents against a userWithAuth-CLEAR key.");
    }
    ...
    public async Task HmacStartOverAUserWithAuthClearRestrictedKeyAnswersPolicyFailBeforeAttributes()
    {
        ...
        Assert.AreEqual(HmacKeyHarness.SessionEncodedRc(TpmRcConstants.TPM_RC_POLICY_FAIL, 0), result.ResponseCode, "handle's authorizing session, session 1 of Table 80, is refused with session-encoded TPM_RC_POLICY_FAIL: the userWithAuth gate must answer before the restricted-key gate.");
    }
    ...
    public async Task EventSequenceCompleteOnAnHmacSequenceIsRefusedWithModeAndTheSequenceSurvives()
    {
        ...
        Assert.AreEqual(HmacKeyHarness.HandleEncodedRc(TpmRcConstants.TPM_RC_MODE, 1), refused.ResponseCode, "sequenceHandle, handle 2 of Table 95, naming an HMAC sequence must be refused with handle-encoded TPM_RC_MODE.");
    ...
    public async Task SignSequenceCompleteOnAnHmacSequenceIsRefusedWithModeAndTheSequenceSurvives()
    {
        ...
        Assert.AreEqual(HmacKeyHarness.HandleEncodedRc(TpmRcConstants.TPM_RC_MODE, 0), refused.ResponseCode, "sequenceHandle, handle 1 of Table 124, naming an HMAC sequence must be refused with handle-encoded TPM_RC_MODE.");
    ...
    public async Task HmacStartFramedWithANonHashAlgorithmIsRefusedWithHash()
    {
        ...
        Assert.AreEqual(HmacKeyHarness.ParameterEncodedRc(TpmRcConstants.TPM_RC_HASH, 1), code, "hashAlg is TPM2_HMAC_Start()'s second parameter (Table 80, index 1); one that is not a hash identifier must fail the unmarshal with parameter-encoded TPM_RC_HASH.");
    }
