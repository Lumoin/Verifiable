...
internal sealed class TpmInHouseSimulatorPcrExtendTests
{
    ...
    /// <see href="https://trustedcomputinggroup.org/resource/pc-client-platform-tpm-profile-ptp-specification/">PC Client PTP 1.07, clause 4.7.2, Table 15; clause 4.7.1, Table 14</see>.
    /// </summary>
    [TestMethod]
    ...
    public async Task PcrExtendWithAnUnimplementedOrNullHashEntryHandFramedReturnsHash()
    {
        ...
        Assert.AreEqual(TpmRcConstants.TPM_RC_HASH, sm3, "An SM3-256 entry must be refused with TPM_RC_HASH.");
        ...
        Assert.AreEqual(TpmRcConstants.TPM_RC_HASH, nullAlg, "A TPM_ALG_NULL entry must be refused with TPM_RC_HASH.");
    }
    ...
    public async Task PcrExtendWithFiveEntriesHandFramedReturnsSizeAndLeavesTheRegister()
    {
        ...
        Assert.AreEqual(TpmRcConstants.TPM_RC_SIZE, code, "A list longer than the implemented hash count must be refused with TPM_RC_SIZE.");
    ...
    /// <see href="https://trustedcomputinggroup.org/resource/tpm-library-specification/">TPM 2.0 Library Part 3, clause 22.2.1, Table 130</see>.
    /// </summary>
    [TestMethod]
    ...
    /// <see href="https://trustedcomputinggroup.org/resource/pc-client-platform-tpm-profile-ptp-specification/">PC Client PTP 1.07, clause 4.7.1, Table 14; TPM 2.0 Library Part 4, PCR_Extend.c</see>.
    /// </summary>
    [TestMethod]
    ...
    public async Task PcrExtendOnPcr24ReturnsValue()
    {
        ...
        Assert.AreEqual(TpmRcConstants.TPM_RC_VALUE, result.ResponseCode, "A handle past PCR_LAST must be refused with TPM_RC_VALUE.");
    }
    ...
    public async Task PcrResetWithTheNullHandleOrPcr24ReturnsValueAndTheApplicationPcrResets()
    {
        ...
        Assert.AreEqual(TpmRcConstants.TPM_RC_VALUE, nullHandle.ResponseCode, "TPM_RH_NULL is not admitted by TPMI_DH_PCR without the + form.");

        TpmResult<PcrResetResponse> outOfRange = await ResetAsync(tpm, registry, pool, OutOfRangePcr, []).ConfigureAwait(false);
        Assert.AreEqual(TpmRcConstants.TPM_RC_VALUE, outOfRange.ResponseCode, "PCR 24 is past PCR_LAST.");
    ...
    /// <see href="https://trustedcomputinggroup.org/resource/tpm-library-specification/">TPM 2.0 Library Part 3, clause 5.2, Table 142; Part 1, clause 14.7</see>.
    /// </summary>
    [TestMethod]
