...
internal sealed class TpmInHouseSimulatorSignDigestTests
{
    ...
    public async Task SignDigestAgainstANullSchemeRsaKeyReturnsScheme()
    {
        ...
        Assert.AreEqual(
            TpmRcConstants.TPM_RC_SCHEME, signResult.ResponseCode,
    ...
    /// 2 clause 4.16 / Part 3 clause 5.8's general size-parameter fallback].
    /// </summary>
    [TestMethod]
    public async Task SignDigestWithMismatchedDigestSizeAgainstAnEccKeyReturnsSize()
    {
        ...
        Assert.AreEqual(TpmRcConstants.TPM_RC_SIZE, signResult.ResponseCode, "A digest whose size mismatches the ECC key's SHA-256 scheme must be refused with TPM_RC_SIZE.");
    }
    ...
    public async Task SignDigestWithMismatchedDigestSizeAgainstAnRsaKeyReturnsSize()
    {
        ...
        Assert.AreEqual(TpmRcConstants.TPM_RC_SIZE, signResult.ResponseCode, "A digest whose size mismatches the RSA key's SHA-256 scheme must be refused with TPM_RC_SIZE.");
    }
    ...
    public async Task SignDigestWithNonEmptyContextReturnsSize()
    {
        ...
        Assert.AreEqual(TpmRcConstants.TPM_RC_SIZE, code, "A non-empty context under ECDSA must be refused with TPM_RC_SIZE (Table 220's empty[0] arm).");
    }
    ...
    public async Task SignDigestOverARestrictedKeyWithANullTicketReturnsTicket()
    {
        ...
        Assert.AreEqual(TpmRcConstants.TPM_RC_TICKET, signResult.ResponseCode, "A restricted key given a NULL validation ticket must be refused with TPM_RC_TICKET.");
    }
    ...
    /// exactly as the simulator's own <c>VerifyHashcheckTicketAsync</c> recomputes it — signs successfully
    /// (<see href="https://trustedcomputinggroup.org/resource/tpm-library-specification/">TPM 2.0 Library
    ...
    public async Task SignDigestOverARestrictedKeyWithATamperedTicketReturnsTicket()
    {
        ...
        Assert.AreEqual(TpmRcConstants.TPM_RC_TICKET, signResult.ResponseCode, "A single-bit-tampered validation ticket must be refused with TPM_RC_TICKET.");
    }
    ...
    public async Task SignDigestOverAnUnrestrictedKeyWithAForgedTicketReturnsTicket()
    {
        ...
        Assert.AreEqual(TpmRcConstants.TPM_RC_TICKET, signResult.ResponseCode, "An unrestricted key given a forged non-NULL ticket must still be refused with TPM_RC_TICKET.");
    }
    ...
    public async Task SignDigestWithAHashcheckTicketTaggedCreationReturnsTag()
    {
        ...
        Assert.AreEqual(
            ...
            "A validation ticket tagged TPM_ST_CREATION instead of TPM_ST_HASHCHECK must be refused with TPM_RC_TAG.");
    }
    ...
    /// lives in the simulator's sealed-object table rather than among its signing keys, so <c>OnSignDigest</c>
    ...
    public async Task SignDigestAgainstAKeyedhashKeyReturnsScheme()
    {
        ...
        Assert.AreEqual(TpmRcConstants.TPM_RC_SCHEME, signResult.ResponseCode, "A KEYEDHASH object has no digest-signing default scheme and must be refused with TPM_RC_SCHEME.");
    }
