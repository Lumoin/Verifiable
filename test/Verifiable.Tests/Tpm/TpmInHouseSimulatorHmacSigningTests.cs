...
internal sealed class TpmInHouseSimulatorHmacSigningTests
{
    ...
    private static byte[] SealedSecretBytes { get; } = [1, 2, 3, 4];
    ...
    private static byte[] TicketSeed { get; } = Convert.FromHexString("c1a2b3d4e5f60718293a4b5c6d7e8f9001122334455667788990aabbccddeeff");

    /// <summary>A password that never matches any fixture's authValue.</summary>
    private static byte[] WrongKeyPassword { get; } = "hmac-signing-wrong"u8.ToArray();
    ...
    public async Task SignWithAMismatchedOrForeignSchemeOnAnHmacKeyIsRefusedWithScheme()
    {
        ...
        Assert.AreEqual(HmacKeyHarness.ParameterEncodedRc(TpmRcConstants.TPM_RC_SCHEME, 1), mismatchedHashResult.ResponseCode, "HMAC with a hash other than the key's own must be refused with TPM_RC_SCHEME.");
        ...
        Assert.AreEqual(HmacKeyHarness.ParameterEncodedRc(TpmRcConstants.TPM_RC_SCHEME, 1), ecdsaResult.ResponseCode, "An ECDSA inScheme against a KEYEDHASH key must be refused with TPM_RC_SCHEME.");
    }
    ...
    public async Task SignWithAWrongWidthDigestOnAnHmacKeyIsRefusedWithSize()
    {
        ...
        Assert.AreEqual(HmacKeyHarness.ParameterEncodedRc(TpmRcConstants.TPM_RC_SIZE, 0), shortResult.ResponseCode, "A 31-octet digest against a SHA-256 HMAC scheme must be refused with TPM_RC_SIZE.");
        ...
        Assert.AreEqual(HmacKeyHarness.ParameterEncodedRc(TpmRcConstants.TPM_RC_SIZE, 0), wideResult.ResponseCode, "A 33-octet digest against a SHA-256 HMAC scheme must be refused with TPM_RC_SIZE.");
    }
    ...
    public async Task SignOverASealedDataObjectIsRefusedWithKey()
    {
        ...
        Assert.AreEqual(HmacKeyHarness.HandleEncodedRc(TpmRcConstants.TPM_RC_KEY, 0), result.ResponseCode, "A sealed data object (sign CLEAR) at TPM2_Sign() must be refused with TPM_RC_KEY.");
    }
    ...
    public async Task SignOverARestrictedHmacKeyIsRefusedWithTicket()
    {
        ...
        Assert.AreEqual(HmacKeyHarness.ParameterEncodedRc(TpmRcConstants.TPM_RC_TICKET, 2), result.ResponseCode, "A restricted HMAC key with the modelled NULL validation ticket must be refused with TPM_RC_TICKET.");
    }
    ...
    public async Task SignOverAUserWithAuthClearHmacKeyIsRefusedWithPolicyFail()
    {
        ...
        Assert.AreEqual(HmacKeyHarness.SessionEncodedRc(TpmRcConstants.TPM_RC_POLICY_FAIL, 0), result.ResponseCode, "A password against a userWithAuth-CLEAR HMAC key must be refused with TPM_RC_POLICY_FAIL.");
    }
    ...
    public async Task VerifySignatureOverATamperedOrMismatchedHmacSignatureIsRefusedWithSignature()
    {
        ...
        Assert.AreEqual(HmacKeyHarness.ParameterEncodedRc(TpmRcConstants.TPM_RC_SIGNATURE, 1), tamperedDigestResult.ResponseCode, "A signature over a different digest must be refused with TPM_RC_SIGNATURE at signature, parameter 2 of Table 116.");
        ...
        Assert.AreEqual(HmacKeyHarness.ParameterEncodedRc(TpmRcConstants.TPM_RC_SIGNATURE, 1), tamperedSignatureResult.ResponseCode, "Tampered signature octets must be refused with TPM_RC_SIGNATURE at signature, parameter 2 of Table 116.");
        ...
        Assert.AreEqual(HmacKeyHarness.ParameterEncodedRc(TpmRcConstants.TPM_RC_SIGNATURE, 1), mismatchedHashResult.ResponseCode, "A signature hash disagreeing with the key's scheme hash must be refused with TPM_RC_SIGNATURE.");
    }
    ...
    public async Task VerifySignatureWithASchemeForeignToTheKeysTypeIsRefusedWithScheme()
    {
        ...
        Assert.AreEqual(HmacKeyHarness.ParameterEncodedRc(TpmRcConstants.TPM_RC_SCHEME, 1), hmacAgainstEccResult.ResponseCode, "An HMAC signature against an ECC signing key must be refused with TPM_RC_SCHEME.");
        ...
        Assert.AreEqual(HmacKeyHarness.ParameterEncodedRc(TpmRcConstants.TPM_RC_SCHEME, 1), ecdsaAgainstHmacResult.ResponseCode, "An ECDSA signature against a KEYEDHASH key must be refused with TPM_RC_SCHEME.");
    }
    ...
    public async Task VerifySignatureOverARestrictedHmacKeyReachesTheSignatureCompare()
    {
        ...
        Assert.AreEqual(
            HmacKeyHarness.ParameterEncodedRc(TpmRcConstants.TPM_RC_SIGNATURE, 1), result.ResponseCode,
            "A restricted HMAC key must reach the compare and answer TPM_RC_SIGNATURE at signature, parameter 2 of Table 116 — verification is not restricted-gated.");
    }
    ...
    /// (<see href="https://trustedcomputinggroup.org/resource/tpm-library-specification/">TPM 2.0 Library
    /// Specification</see>, Part 3: Commands, clause 20.2, Detailed Actions;
    ...
    public async Task VerifySignatureOverANonSigningKeyedHashObjectIsRefusedWithAttributes()
    {
        ...
        Assert.AreEqual(
            HmacKeyHarness.HandleEncodedRc(TpmRcConstants.TPM_RC_ATTRIBUTES, 0), sealedResult.ResponseCode,
        ...
        Assert.AreEqual(HmacKeyHarness.HandleEncodedRc(TpmRcConstants.TPM_RC_ATTRIBUTES, 0), xorResult.ResponseCode, "An XOR-scheme decryption key (sign CLEAR) must be refused with TPM_RC_ATTRIBUTES — its bits are keying material, not a verification key.");
    }
    ...
    /// segments chain in order), and the sequence is flushed on success (<c>{F}</c>: a second Complete names a
    /// TRANSIENT-range <c>sequenceHandle</c> resolving to nothing loaded, <c>TPM_RC_REFERENCE_H0</c>, TPM 2.0
    ...
    public async Task SignSequenceOverAnHmacKeyReproducesRfc4231Case3AndFlushesTheSequence()
    {
        ...
        TpmResult<SignSequenceCompleteResponse> replayResult = await SignSequenceCompleteAsync(
            tpm, registry, pool, sequenceHandle, key.Handle, trailingBlock).ConfigureAwait(false);
        Assert.AreEqual(TpmRcConstants.TPM_RC_REFERENCE_H0, replayResult.ResponseCode, "sequenceHandle is TPM2_SignSequenceComplete()'s sole handle (index 0); a successful Complete flushes the sequence, so a second Complete names a TRANSIENT-range value resolving to nothing loaded, TPM_RC_REFERENCE_H0 (TPM 2.0 Library Part 3, clause 5.4, step 2.1).");
    }
    ...
    public async Task SignSequenceStartOverARestrictedHmacKeyIsRefusedWithAttributes()
    {
        ...
        Assert.AreEqual(HmacKeyHarness.HandleEncodedRc(TpmRcConstants.TPM_RC_ATTRIBUTES, 0), result.ResponseCode, "A restricted HMAC key must be refused at TPM2_SignSequenceStart() with TPM_RC_ATTRIBUTES.");
    }
    ...
    public async Task SequenceStartsOverASealedDataObjectAreRefusedWithKey()
    {
        ...
        Assert.AreEqual(HmacKeyHarness.HandleEncodedRc(TpmRcConstants.TPM_RC_KEY, 0), signStartResult.ResponseCode, "A sealed data object at TPM2_SignSequenceStart() must be refused with TPM_RC_KEY.");
        ...
        Assert.AreEqual(HmacKeyHarness.HandleEncodedRc(TpmRcConstants.TPM_RC_KEY, 0), verifyStartResult.ResponseCode, "A sealed data object at TPM2_VerifySequenceStart() must be refused with TPM_RC_KEY.");
    }
    ...
    public async Task SignSequenceCompleteOnAnHmacStartSequenceIsRefusedWithMode()
    {
        ...
        Assert.AreEqual(HmacKeyHarness.HandleEncodedRc(TpmRcConstants.TPM_RC_MODE, 0), result.ResponseCode, "TPM2_SignSequenceComplete() on an HMAC sequence must be refused with TPM_RC_MODE.");
    }
    ...
    public async Task SignSequenceCompleteWithASwappedHmacKeyIsRefusedWithSignContextKey()
    {
        ...
        Assert.AreEqual(HmacKeyHarness.HandleEncodedRc(TpmRcConstants.TPM_RC_SIGN_CONTEXT_KEY, 1), result.ResponseCode, "A key other than the sequence's starting key must be refused with TPM_RC_SIGN_CONTEXT_KEY.");
    }
    ...
    public async Task VerifySequenceOverAnHmacKeyVerifiesTheRfcVectorAndMintsAMessageVerifiedTicket()
    {
        ...
        TpmResult<VerifySequenceCompleteResponse> replayResult = await VerifySequenceCompleteAsync(
            tpm, registry, pool, sequenceHandle, key.Handle, Rfc4231Case3Sha256, TpmAlgIdConstants.TPM_ALG_SHA256).ConfigureAwait(false);
        Assert.AreEqual(TpmRcConstants.TPM_RC_REFERENCE_H0, replayResult.ResponseCode, "sequenceHandle is TPM2_VerifySequenceComplete()'s sole handle (index 0); a successful Complete flushes the sequence, so a second Complete names a TRANSIENT-range value resolving to nothing loaded, TPM_RC_REFERENCE_H0 (TPM 2.0 Library Part 3, clause 5.4, step 2.1).");
    }
    ...
    public async Task VerifySequenceCompleteOverAWrongSignatureIsRefusedWithSignatureAndTheSequenceSurvives()
    {
        ...
        Assert.AreEqual(HmacKeyHarness.ParameterEncodedRc(TpmRcConstants.TPM_RC_SIGNATURE, 0), wrongResult.ResponseCode, "A wrong HMAC signature must be refused with TPM_RC_SIGNATURE at signature, parameter 1 of Table 118.");
    ...
    public async Task VerifySequenceCompleteWithAMismatchedSignatureHashIsRefusedWithScheme()
    {
        ...
        Assert.AreEqual(HmacKeyHarness.HandleEncodedRc(TpmRcConstants.TPM_RC_SCHEME, 1), result.ResponseCode, "A signature hash disagreeing with the sequence's retained scheme must be refused with TPM_RC_SCHEME.");
    }
    ...
    public async Task SignDigestAndVerifyDigestSignatureOverAnHmacKeyAreRefusedWithScheme()
    {
        ...
        Assert.AreEqual(HmacKeyHarness.HandleEncodedRc(TpmRcConstants.TPM_RC_SCHEME, 0), signDigestResult.ResponseCode, "TPM2_SignDigest() over an HMAC key must be refused with TPM_RC_SCHEME.");
        ...
        //sigAlg = TPM_ALG_HMAC is refused when the TPMT_SIGNATURE member is unmarshaled, since TPM_ALG_HMAC
        //is not a valid digest-signature scheme selector. Table 120: signature is
        //TPM2_VerifyDigestSignature()'s third parameter, index 2.
        Assert.AreEqual(
            HmacKeyHarness.ParameterEncodedRc(TpmRcConstants.TPM_RC_SCHEME, 2), verifyDigestResult.ResponseCode,
            "Table 120: signature is TPM2_VerifyDigestSignature()'s third parameter (index 2); an HMAC signature must be refused with TPM_RC_SCHEME there.");
    }

    /// <summary>
    /// The wire HMAC signature member is a <c>TPMT_HA</c> whose <c>hashAlg</c> both selects and sizes the
    /// digest; an unrecognized hashAlg cannot size it and is refused at the unmarshal with parameter-encoded
    /// <c>TPM_RC_HASH</c> (Table 116: <c>signature</c> is the second parameter, index 1), before any pooled
    /// rental — proved by the metered pool's balance
    /// (<see href="https://trustedcomputinggroup.org/resource/tpm-library-specification/">TPM 2.0 Library
    ...
    public async Task VerifySignatureWithAGarbageSignatureHashOnTheWireIsRefusedWithHashBeforeAnyRental()
    {
        ...
        Assert.AreEqual(HmacKeyHarness.ParameterEncodedRc(TpmRcConstants.TPM_RC_HASH, 1), code, "signature is TPM2_VerifySignature()'s second parameter (Table 116, index 1); an unrecognized TPMT_HA hashAlg must be refused with parameter-encoded TPM_RC_HASH.");
    ...
    public async Task HmacSigningCommandsBalanceTheMeteredPoolAcrossSuccessAndRefusal()
    {
        ...
        TpmResult<SignResponse> refusedResult = await SignAsync(
            tpm, registry, pool, key.Handle, Sha256WidthDigest, TpmAlgIdConstants.TPM_ALG_ECDSA, TpmAlgIdConstants.TPM_ALG_SHA256).ConfigureAwait(false);
        Assert.AreEqual(HmacKeyHarness.ParameterEncodedRc(TpmRcConstants.TPM_RC_SCHEME, 1), refusedResult.ResponseCode, "The scheme refusal must hold under the metered pool.");
        ...
        Assert.AreEqual(HmacKeyHarness.ParameterEncodedRc(TpmRcConstants.TPM_RC_SIGNATURE, 1), refusedVerifyResult.ResponseCode, "The post-parse SIGNATURE refusal at signature, parameter 2 of Table 116 must hold under the metered pool.");
    ...
    public async Task SignOverAnXorSchemeDecryptObjectIsRefusedWithKey()
    {
        ...
        Assert.AreEqual(HmacKeyHarness.HandleEncodedRc(TpmRcConstants.TPM_RC_KEY, 0), result.ResponseCode, "An XOR-scheme decryption object (sign CLEAR) at TPM2_Sign() must be refused with TPM_RC_KEY.");
    }
    ...
    public async Task VerifySequenceStartOverAnHmacKeyWithANonEmptyHintIsRefusedWithSize()
    {
        ...
        Assert.AreEqual(HmacKeyHarness.ParameterEncodedRc(TpmRcConstants.TPM_RC_SIZE, 1), code, "A non-empty hint at TPM2_VerifySequenceStart() over an HMAC key must be refused with TPM_RC_SIZE.");
    ...
    public async Task VerifySequenceStartOverAnHmacKeyWithANonEmptyContextIsRefusedWithSize()
    {
        ...
        Assert.AreEqual(HmacKeyHarness.ParameterEncodedRc(TpmRcConstants.TPM_RC_SIZE, 2), code, "A non-empty context at TPM2_VerifySequenceStart() over an HMAC key must be refused with TPM_RC_SIZE.");
        Assert.AreEqual(baseline, trackingPool.OutstandingCount, "The refused Start must release every parse-rented carrier.");
    }
    ...
    public async Task SignSequenceStartOverAnHmacKeyWithANonEmptyContextIsRefusedWithSize()
    {
        ...
        Assert.AreEqual(HmacKeyHarness.ParameterEncodedRc(TpmRcConstants.TPM_RC_SIZE, 1), code, "A non-empty context at TPM2_SignSequenceStart() over an HMAC key must be refused with TPM_RC_SIZE.");
        Assert.AreEqual(baseline, trackingPool.OutstandingCount, "The refused Start must release every parse-rented carrier.");
    }
    ...
    /// 2.0 Library Specification</see>, Part 3: Commands, clause 20.3.2, Table 118; Part 1: Architecture, clause 13, Table 9 for the Name).
    /// </summary>
    [TestMethod]
    public async Task VerifySequenceCompleteWithASwappedHmacKeyIsRefusedWithSignContextKey()
    {
        ...
        Assert.AreEqual(HmacKeyHarness.HandleEncodedRc(TpmRcConstants.TPM_RC_SIGN_CONTEXT_KEY, 1), swappedResult.ResponseCode, "A key other than the sequence's starting key must be refused with TPM_RC_SIGN_CONTEXT_KEY.");
    ...
    public async Task VerifySignatureWithASha1SignatureHashIsRefusedWithHash()
    {
        ...
        Assert.AreEqual(HmacKeyHarness.ParameterEncodedRc(TpmRcConstants.TPM_RC_HASH, 1), result.ResponseCode, "A SHA-1 signature hash must be refused with TPM_RC_HASH ahead of the scheme and signature gates.");
    }
    ...
    public async Task VerifySequenceStartOverARestrictedHmacKeyOpensASequenceThatReachesTheSignatureCompare()
    {
        ...
        Assert.AreEqual(HmacKeyHarness.ParameterEncodedRc(TpmRcConstants.TPM_RC_SIGNATURE, 0), result.ResponseCode, "A restricted HMAC key must reach the compare on the sequence form and answer TPM_RC_SIGNATURE at signature, parameter 1 of Table 118 — verification is not restricted-gated.");
    }
    ...
    public async Task VerifySignatureWithATruncatedHmacMemberOnTheWireIsRefusedWithInsufficientBeforeAnyRental()
    {
        ...
        Assert.AreEqual(HmacKeyHarness.ParameterEncodedRc(TpmRcConstants.TPM_RC_INSUFFICIENT, 1), code, "signature is TPM2_VerifySignature()'s second parameter (Table 116, index 1); a TPMT_HA member shorter than its hashAlg sizes must be refused with parameter-encoded TPM_RC_INSUFFICIENT.");
        Assert.AreEqual(baseline, trackingPool.OutstandingCount, "The refusal fires at the wire read, so a refused parse must rent nothing.");
    }
    ...
    public async Task HmacSignSequenceCommandsBalanceTheMeteredPoolAcrossRefusalAndSuccess()
    {
        ...
        Assert.AreEqual(HmacKeyHarness.HandleEncodedRc(TpmRcConstants.TPM_RC_SIGN_CONTEXT_KEY, 1), refused.ResponseCode, "The swapped-key refusal must hold under the metered pool.");
    ...
    public async Task HmacVerifySequenceCommandsBalanceTheMeteredPoolAcrossRefusalAndSuccess()
    {
        ...
        Assert.AreEqual(HmacKeyHarness.ParameterEncodedRc(TpmRcConstants.TPM_RC_SIGNATURE, 0), refused.ResponseCode, "The signature refusal at signature, parameter 1 of Table 118 must hold under the metered pool.");
