...
internal sealed class TpmInHouseSimulatorHashTests
{
    ...
    /// <c>TPM_ALG_NULL</c> fails the unmarshal with <c>#TPM_RC_HASH</c> (Part 2, Table 66); an unimplemented TCG
    /// hash (<c>TPM_ALG_SHA3_256</c>) is refused with the same code.
    /// <see href="https://trustedcomputinggroup.org/resource/tpm-library-specification/">TPM 2.0 Library Part 3, clause 15.4.2, Table 69; Part 2, clause 9.27, Table 66</see>.
    /// </summary>
    [TestMethod]
    public async Task HashWithNullOrAnUnimplementedHashAlgorithmReturnsHash()
    {
        ...
        Assert.AreEqual(TpmRcConstants.TPM_RC_HASH, nullResult.ResponseCode, "TPM_ALG_NULL must be refused with TPM_RC_HASH.");
        ...
        Assert.AreEqual(TpmRcConstants.TPM_RC_HASH, sha3Result.ResponseCode, "An unimplemented hash algorithm must be refused with TPM_RC_HASH.");
    }
    ...
    /// <summary>
    /// Part 4 <c>Hash.c</c>: the ticket is withheld only when <c>data.size &gt;= sizeof(TPM_GENERATED_VALUE)</c>
    ...
    /// the simulator models none and refuses a session-tagged frame with <c>TPM_RC_BAD_TAG</c>. A trailing octet
    /// after <c>hierarchy</c> is <c>TPM_RC_SIZE</c> (clause 5.2); a hierarchy outside Part 2, Table 59 is
    /// <c>TPM_RC_VALUE</c>.
    /// <see href="https://trustedcomputinggroup.org/resource/tpm-library-specification/">TPM 2.0 Library Part 3, clause 15.4.2, Table 69; Part 2, clause 9.13, Table 59</see>.
    /// </summary>
    [TestMethod]
    public async Task HashHandFramedWithSessionsReturnsBadTagATrailingOctetReturnsSizeAndAnOutOfTableHierarchyReturnsValue()
    {
        BaseMemoryPool pool = BaseMemoryPool.Shared;
        using TpmSimulator simulator = await CreateOperationalAsync(pool).ConfigureAwait(false);
