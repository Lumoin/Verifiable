...
internal sealed class TpmaCcTests
{
    ...
    public void HandleReturningCommandsReportRHandle()
    {
        ...
        Assert.IsTrue(TpmCcConstants.TPM_CC_LoadExternal.GetCommandAttributes().R_HANDLE, "TPM2_LoadExternal() returns objectHandle (Part 3, clause 12.3, Table 23).");
        ...
        Assert.IsFalse(TpmCcConstants.TPM_CC_ObjectChangeAuth.GetCommandAttributes().R_HANDLE, "TPM2_ObjectChangeAuth() answers outPrivate, no handle (Part 3, clause 12.8, Table 33).");
        Assert.IsFalse(TpmCcConstants.TPM_CC_RSA_Encrypt.GetCommandAttributes().R_HANDLE, "TPM2_RSA_Encrypt() answers outData, no handle (Part 3, clause 14.2, Table 45).");
        Assert.IsFalse(TpmCcConstants.TPM_CC_RSA_Decrypt.GetCommandAttributes().R_HANDLE, "TPM2_RSA_Decrypt() answers message, no handle (Part 3, clause 14.3, Table 47).");
        Assert.IsFalse(TpmCcConstants.TPM_CC_ContextSave.GetCommandAttributes().R_HANDLE, "TPM2_ContextSave() answers the TPMS_CONTEXT, no handle (Part 3, clause 28.2, Table 225).");
        Assert.IsTrue(TpmCcConstants.TPM_CC_ContextLoad.GetCommandAttributes().R_HANDLE, "TPM2_ContextLoad() returns loadedHandle (Part 3, clause 28.3, Table 227).");
        Assert.IsFalse(TpmCcConstants.TPM_CC_GetSessionAuditDigest.GetCommandAttributes().R_HANDLE, "TPM2_GetSessionAuditDigest() answers auditInfo and signature, no handle (Part 3, clause 18.5, Table 104).");
    }
    ...
    public void NvWritingCommandsReportNv()
    {
        ...
        Assert.IsTrue(TpmCcConstants.TPM_CC_StirRandom.GetCommandAttributes().NV, "TPM2_StirRandom() is {NV} (Table 77).");
        ...
        Assert.IsFalse(TpmCcConstants.TPM_CC_ClockRateAdjust.GetCommandAttributes().NV, "TPM2_ClockRateAdjust() carries no {NV} (Table 236).");
        Assert.IsFalse(TpmCcConstants.TPM_CC_TestParms.GetCommandAttributes().NV, "TPM2_TestParms() carries no {NV} (Table 240).");
    }
    ...
    public void HandleCountsStayIntactBesideTheAttributeBits()
    {
        ...
        Assert.AreEqual((byte)0, TpmCcConstants.TPM_CC_LoadExternal.GetCommandAttributes().C_HANDLES, "TPM2_LoadExternal() takes no handle: inPrivate, inPublic and hierarchy are all parameters (Part 3, clause 12.3, Table 22).");
        Assert.AreEqual((byte)2, TpmCcConstants.TPM_CC_ObjectChangeAuth.GetCommandAttributes().C_HANDLES, "TPM2_ObjectChangeAuth() takes @objectHandle and parentHandle (Part 3, clause 12.8, Table 32).");
        Assert.AreEqual((byte)1, TpmCcConstants.TPM_CC_RSA_Encrypt.GetCommandAttributes().C_HANDLES, "TPM2_RSA_Encrypt() takes keyHandle with no authorization (Part 3, clause 14.2, Table 44).");
        Assert.AreEqual((byte)1, TpmCcConstants.TPM_CC_RSA_Decrypt.GetCommandAttributes().C_HANDLES, "TPM2_RSA_Decrypt() takes @keyHandle (Part 3, clause 14.3, Table 46).");
        Assert.AreEqual((byte)1, TpmCcConstants.TPM_CC_ContextSave.GetCommandAttributes().C_HANDLES, "TPM2_ContextSave() takes saveHandle with no authorization (Part 3, clause 28.2, Table 224).");
        Assert.AreEqual((byte)0, TpmCcConstants.TPM_CC_ContextLoad.GetCommandAttributes().C_HANDLES, "TPM2_ContextLoad() takes no handle: the TPMS_CONTEXT is a parameter (Part 3, clause 28.3, Table 226).");
        Assert.AreEqual((byte)3, TpmCcConstants.TPM_CC_GetSessionAuditDigest.GetCommandAttributes().C_HANDLES, "TPM2_GetSessionAuditDigest() takes @privacyAdminHandle, @signHandle and the unauthorized sessionHandle (Part 3, clause 18.5, Table 103).");
    }
}
