...
internal sealed class TpmInHouseSimulatorReadPublicTests
{
    ...
    /// <summary>
    /// An in-range TRANSIENT handle nothing is loaded at is <c>TPM_RC_REFERENCE_H0</c> (Part 3, clause 5.4, step
    /// 2.1: "the handle shall reference a loaded object") — both a never-assigned transient value and a
    /// primary's handle after <c>TPM2_FlushContext()</c>, which must stop answering the instant the object is
    /// gone.
    /// </summary>
    [TestMethod]
    public async Task ReadPublicOfAnUnloadedOrFlushedHandleReturnsReferenceH0()
    {
        BaseMemoryPool pool = BaseMemoryPool.Shared;
        using TpmSimulator simulator = await CreateOperationalAsync(pool).ConfigureAwait(false);
        using TpmDevice tpm = TpmDevice.Create(simulator.SubmitAsync, BaseMemoryPool.Shared, TestEntropy.NewCounterStream());
        ...
        Assert.AreEqual(TpmRcConstants.TPM_RC_REFERENCE_H0, unknown.ResponseCode);
        ...
        Assert.AreEqual(TpmRcConstants.TPM_RC_REFERENCE_H0, flushed.ResponseCode);
    }

    /// <summary>
    /// The persistent-range complement of <see cref="ReadPublicOfAnUnloadedOrFlushedHandleReturnsReferenceH0"/>:
    /// an unallocated persistent handle answers <c>TPM_RC_HANDLE</c> designated to <c>objectHandle</c>, handle 1
    /// of Table 24 (clause 5.4, step 2.2), not the transient-only step 2.1 code.
    /// </summary>
    [TestMethod]
    public async Task ReadPublicOfAnUnallocatedPersistentHandleReturnsHandle()
    {
        ...
        TpmResult<ReadPublicResponse> unallocated = await SubmitReadPublicAsync(tpm, registry, pool, UnallocatedPersistentHandle).ConfigureAwait(false);
        Assert.IsFalse(unallocated.IsSuccess, "A persistent handle nothing is evicted to must not answer.");
        Assert.AreEqual(HmacKeyHarness.HandleEncodedRc(TpmRcConstants.TPM_RC_HANDLE, 0), unallocated.ResponseCode, "Table 24: objectHandle is TPM2_ReadPublic()'s sole handle (handle 1); a persistent handle nothing is evicted to is handle-encoded TPM_RC_HANDLE at index 0.");
    }
    ...
    public async Task ReadPublicOfAnOutOfRangeHandleReturnsValue()
    {
        ...
        Assert.AreEqual(
            HmacKeyHarness.HandleEncodedRc(TpmRcConstants.TPM_RC_VALUE, 0), nullHandle.ResponseCode,
        ...
        Assert.AreEqual(
            HmacKeyHarness.HandleEncodedRc(TpmRcConstants.TPM_RC_VALUE, 0), nvRange.ResponseCode,
            "Table 24: objectHandle is TPM2_ReadPublic()'s sole handle (index 0).");
    }
    ...
    /// <summary>
    /// Table 24's tag is <c>TPM_ST_SESSIONS</c> "if an audit or encrypt session is present" (TPM 2.0 Library
    /// Part 3, clause 4.3; Part 1, clause 15.6.1): a session-tagged frame carrying one <c>TPM_RS_PW</c> slot that
    /// claims none of <c>decrypt</c>, <c>encrypt</c>, or <c>audit</c> is admitted into the shared
    /// no-authorization ladder and refused session-index-encoded <c>TPM_RC_ATTRIBUTES</c> at that slot (Part 3, clause
    /// 5.5, step 4.4.2: "If a session is not being used for authorization, at least one of decrypt, encrypt, or
    /// audit must be SET") — not the bare <c>TPM_RC_SIZE</c> a trailing-octet parse failure would answer.
    /// </summary>
    [TestMethod]
    public async Task ReadPublicWithASessionTaggedFrameOverAnAttributelessPasswordSlotIsRefusedWithAttributes()
    {
        BaseMemoryPool pool = BaseMemoryPool.Shared;
        using TpmSimulator simulator = await CreateOperationalAsync(pool).ConfigureAwait(false);
        using TpmDevice tpm = TpmDevice.Create(simulator.SubmitAsync, BaseMemoryPool.Shared, TestEntropy.NewCounterStream());
        ...
        Assert.AreEqual(
            TpmInHouseSimulatorZeroHandleSessionTests.SessionEncodedRc(TpmRcConstants.TPM_RC_ATTRIBUTES, sessionIndex: 0), (TpmRcConstants)responseHeader.Code,
            "A TPM_RS_PW companion claiming none of decrypt/encrypt/audit is refused session-encoded TPM_RC_ATTRIBUTES.");
    }
    ...
    {
        var simulator = new TpmSimulator("tpm-in-house-read-public", signingBackend: BouncyCastleTpmEccSigningBackend.Create(), rng: TestEntropy.NewCounterStream(), timeProvider: new FakeTimeProvider(TestClock.CanonicalEpoch));
