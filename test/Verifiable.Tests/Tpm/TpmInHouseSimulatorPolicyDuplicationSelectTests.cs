...
/// <summary>
/// Drives <c>TPM2_PolicyDuplicationSelect()</c> (TPM 2.0 Library Part 3, Section 23.15) against the in-house
...
/// <c>TPM2_PolicyParameters()</c> binding (Section 23.24) judged against <c>TPM2_Duplicate()</c>'s real
...
internal sealed class TpmInHouseSimulatorPolicyDuplicationSelectTests
{
    ...
    public async Task PolicyDuplicationSelectOnATrialSessionLatchesTheSlotAndSetsTheCommandCode()
    {
        ...
        try
        {
            ...
            Assert.AreEqual(TpmRcConstants.TPM_RC_VALUE, commandCodeResult.ResponseCode, "The trial session's commandCode must be set to TPM_CC_Duplicate by the assertion.");
        }
        finally
        {
            await PolicySweepHarness.FlushIfPresentAsync(tpm, sessionHandle, TestContext.CancellationToken).ConfigureAwait(false);
    ...
    /// the reference's answer for any prior value (Part 4 <c>PolicyDuplicationSelect.c</c>: "commandCode in
    /// session context must be empty").
    ...
    public async Task PolicyCommandCodeForDuplicateIsAcceptedAfterPolicyDuplicationSelect()
    {
        ...
        _ = TpmPolicyDigest.ExtendForCommandCode(afterSelect, TpmCcConstants.TPM_CC_Duplicate, SessionAlg, predicted);
    ...
    public async Task PolicyCommandCodeForAnotherCommandIsRefusedAfterPolicyDuplicationSelect()
    {
        ...
        try
        {
            ...
            Assert.AreEqual(TpmRcConstants.TPM_RC_VALUE, commandCodeResult.ResponseCode, "A different prior commandCode is TPM_RC_VALUE.");
        }
        finally
        {
            await PolicySweepHarness.FlushIfPresentAsync(tpm, sessionHandle, TestContext.CancellationToken).ConfigureAwait(false);
    ...
    /// <summary>
    /// Verifies <c>TPM2_PolicyDuplicationSelect()</c> against a handle naming no policy session is refused with
    /// <c>TPM_RC_HANDLE</c> and that the refusal releases the two parse-rented Name carriers, leaving the pool
    /// balanced.
    /// <see href="https://trustedcomputinggroup.org/resource/tpm-library-specification/">TPM 2.0 Library Part 3, clause 23.15, Table 168; Part 2, clause 9.10, Table 56</see>.
    /// </summary>
    [TestMethod]
    public async Task PolicyDuplicationSelectRefusesAnUnknownSessionAndBalancesThePool()
    {
        ...
        Assert.AreEqual(TpmRcConstants.TPM_RC_HANDLE, result.ResponseCode, "An unknown policy session is TPM_RC_HANDLE.");
    ...
    public async Task HandFramedPolicyDuplicationSelectRefusesAnIncludeObjectOctetAboveOne()
    {
        ...
        try
        {
            ...
            Assert.AreEqual(TpmRcConstants.TPM_RC_VALUE, code, "An includeObject octet outside {0, 1} is TPM_RC_VALUE.");
        }
        finally
        {
            await PolicySweepHarness.FlushIfPresentAsync(tpm, sessionHandle, TestContext.CancellationToken).ConfigureAwait(false);
    ...
    public async Task HandFramedPolicyDuplicationSelectRefusesANameOverTheTpm2bNameBound()
    {
        ...
        try
        {
            ...
            Assert.AreEqual(TpmRcConstants.TPM_RC_SIZE, code, "A Name beyond sizeof(TPMU_NAME) is TPM_RC_SIZE.");
        }
        finally
        {
            await PolicySweepHarness.FlushIfPresentAsync(tpm, sessionHandle, TestContext.CancellationToken).ConfigureAwait(false);
    ...
    /// <c>TPM2_Duplicate()</c> toward a THIRD storage parent with a bare <c>TPM_RC_POLICY_FAIL</c> — the
    ...
    public async Task DuplicationSelectedNewParentRefusesDuplicateToAnotherParent()
    {
        ...
        try
        {
            ...
            Assert.AreEqual(TpmRcConstants.TPM_RC_POLICY_FAIL, duplicateResult.ResponseCode, "The nameHash binding mismatch is the policy's own bare TPM_RC_POLICY_FAIL.");
        }
        finally
        {
            await PolicySweepHarness.FlushIfPresentAsync(tpm, sessionHandle, TestContext.CancellationToken).ConfigureAwait(false);
    ...
    public async Task DuplicationSelectWithIncludeObjectComposesWithPolicyAuthorizeToDuplicateTheSelectedPair()
    {
        ...
        //The FIXED authPolicy the object is created under depends only on keySign and policyRef (Part 3, Section
    ...
    /// refuses <c>TPM2_Duplicate()</c> with a bare <c>TPM_RC_POLICY_FAIL</c> even though the session's
    ...
    public async Task PolicyParametersBoundToAnotherParameterAreaRefusesDuplicate()
    {
        ...
        try
        {
            ...
            Assert.AreEqual(TpmRcConstants.TPM_RC_POLICY_FAIL, duplicateResult.ResponseCode, "The pHash binding mismatch is the policy's own bare TPM_RC_POLICY_FAIL.");
        }
        finally
        {
            await PolicySweepHarness.FlushIfPresentAsync(tpm, sessionHandle, TestContext.CancellationToken).ConfigureAwait(false);
    ...
    private static byte[] PredictDuplicationSelect(ReadOnlySpan<byte> objectName, ReadOnlySpan<byte> newParentName, bool isObjectIncluded)
    {
        ...
        _ = TpmPolicyDigest.ExtendForDuplicationSelect(new byte[size], objectName, newParentName, isObjectIncluded, SessionAlg, predicted);
    ...
    private static byte[] PredictCommandCodeThenParameters(TpmCcConstants commandCode, ReadOnlySpan<byte> parametersHash)
    {
        ...
        _ = TpmPolicyDigest.ExtendForCommandCode(new byte[size], commandCode, SessionAlg, afterCommandCode);
        byte[] predicted = new byte[size];
        _ = TpmPolicyDigest.ExtendForParameters(afterCommandCode, parametersHash, SessionAlg, predicted);
    ...
    /// 3, Section 23.16, equation 33) as an in-test SHA-256 oracle.
    /// </summary>
        ...
        TpmDevice tpm, TpmResponseRegistry registry, BaseMemoryPool pool, uint objectHandle, uint newParentHandle, uint sessionHandle, byte[] objectName, byte[] newParentName)
    {
        ...
        using TpmPolicySession policySession = TpmPolicySession.ForSession(sessionHandle, SessionAlg, pool);
