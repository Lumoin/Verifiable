...
/// <c>TPM_RC_SIZE</c> carry the same handle- or parameter-index modifier on the session form that they carry on
/// the password form, while a failure
...
internal sealed class TpmInHouseSimulatorSignDigestSessionTests
{
    ...
    public async Task SignDigestOverASessionWithATamperedTicketIsBareTicketAndLeavesTheSessionUsable()
    {
        ...
        try
        {
            ...
            using(SignDigestInput refusedInput = SignDigestInput.CreateForRestrictedKey(key.ObjectHandle, digest, tampered, pool))
            {
                ...
                Assert.AreEqual(
                    HmacKeyHarness.ParameterEncodedRc(TpmRcConstants.TPM_RC_TICKET, 2), refused.ResponseCode,
                    "The ticket check is one of the command's own rules, decided after the authorization, so it is designated to validation, TPM2_SignDigest()'s third parameter (Table 126, index 2), rather than session-index-encoded.");
            }
    ...
    /// NULL ticket has no valid <c>TPMT_TK_HASHCHECK</c> and is refused <c>TPM_RC_TICKET</c> naming validation,
    /// parameter 3 of Table 126, over a session exactly as over a password: the rule is one of the command's own,
    /// run after the authorization has proved the authValue (Part 3, clauses 5.6 and 5.8).
    /// </summary>
    [TestMethod]
    public async Task SignDigestOverASessionAgainstARestrictedKeyWithANullTicketIsBareTicket()
    {
        ...
        try
        {
            ...
            Assert.AreEqual(
                ...
                "A restricted key given a NULL validation ticket is refused at validation, parameter 3 of Table 126, on the session form too.");
        }
        finally
        {
            session.Dispose();
    ...
    /// 2.0 Library Specification</see>, Part 1: Architecture, clause 18.1): <c>context</c> is Part 3's Table 126's first
    ...
    /// the empty one (Part 2: Structures, clause 11.3.8, Table 221), which is what the protected field
    /// carries.
    /// </summary>
    [TestMethod]
    ...
    /// (<see href="https://trustedcomputinggroup.org/resource/tpm-library-specification/">TPM 2.0 Library
    /// Specification</see>, Part 1: Architecture, clause 18.3) — over the same first parameter of Part 3's Table 126.
    /// </summary>
    [TestMethod]
    ...
    /// recovered on the session form is refused with <c>TPM_RC_SIZE</c> naming context, parameter 1 of Table 126,
    /// because the emptiness rule is one of the command's own rules and not a slot's (Part 3: Commands, clause
    /// 5.8).
    /// </summary>
    [TestMethod]
    public async Task SignDigestWithANonEmptyContextOverASessionIsBareSize()
    {
        ...
        try
        {
            ...
            Assert.AreEqual(
                ...
                "A non-empty context is refused at context, parameter 1 of Table 126, with no session-index modifier.");
        }
        finally
        {
            session.Dispose();
    ...
    /// <summary>
    /// The bound on a decrypt-protected parameter belongs to the recovered PLAINTEXT, and a wider recovered
    /// value is a failure of that PARAMETER'S OWN content, never of the decrypt step
    /// (<see href="https://trustedcomputinggroup.org/resource/tpm-library-specification/">TPM 2.0 Library
    /// Specification</see>, Part 2: Structures, clause 6.6.2): a <c>context</c> one octet wider than
    /// <c>sizeof(TPMU_SIGNATURE_CTX)</c> recovered over a <c>decrypt</c> session is refused with
    /// <c>TPM_RC_SIZE</c> parameter-encoded to <c>context</c> itself, <c>TPM2_SignDigest()</c>'s first
    /// parameter (Part 3, clause 20.7, Table 126, index 0) — the same designation the emptiness rule above
    /// answers.
    /// </summary>
    [TestMethod]
    public async Task SignDigestWithAnOverBoundContextOverADecryptSessionReturnsParameterEncodedSize()
    {
        BaseMemoryPool pool = BaseMemoryPool.Shared;
        using TpmSimulator simulator = await CreateOperationalAsync(nameof(SignDigestWithAnOverBoundContextOverADecryptSessionReturnsParameterEncodedSize), pool).ConfigureAwait(false);
        using TpmDevice tpm = TpmDevice.Create(simulator.SubmitAsync, BaseMemoryPool.Shared, TestEntropy.NewCounterStream());
        TpmResponseRegistry registry = CreateRegistry();
        ...
        try
        {
            ...
            Assert.AreEqual(
                ...
                "context is TPM2_SignDigest()'s first parameter (Table 126, index 0); an over-bound recovered value is that parameter's own content failure, not the decrypt slot's, so the wire code carries the parameter-index modifier.");
        }
        finally
        {
            session.Dispose();
    ...
    /// <c>@keyHandle</c> is refused <c>TPM_RC_SCHEME</c> naming keyHandle, handle 1 of Table 126, at handle
    /// resolution — before the authorization ladder runs at all — so the session form answers exactly what the
    /// password form answers.
    /// </summary>
    [TestMethod]
    public async Task SignDigestAgainstAKeyedHashKeyOverASessionIsBareScheme()
    {
        ...
        try
        {
            ...
            Assert.AreEqual(
                HmacKeyHarness.HandleEncodedRc(TpmRcConstants.TPM_RC_SCHEME, 0), result.ResponseCode,
                "A KEYEDHASH object has no digest-signing scheme, and the refusal designates keyHandle, handle 1 of Table 126, at resolution.");
        }
        finally
        {
            session.Dispose();
    ...
    public async Task SignDigestOverASessionBoundToTheSigningKeyItselfSignsWithoutAPerCommandAuthValue()
    {
        ...
        StartAuthSessionInput startInput = StartAuthSessionInput.CreateBoundUnsaltedHmacSession(key.ObjectHandle.Value, SessionAlg, TestEntropy.NewCounterStream(), pool);
        ...
        try
        {
            using TpmSession session = await TpmSession.CreateBoundAsync(
                new TpmHandle(sessionHandle), SigningKeyAuth, startInput.NonceCaller, started.NonceTPM, SessionAlg, TestEntropy.NewCounterStream(), pool,
