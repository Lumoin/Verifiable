...
/// <summary>
/// Drives <c>TPM2_PolicyLocality()</c> (TPM 2.0 Library Part 3, clause 23.8; Part 2, clause 8.5, Table 39)
...
internal sealed class TpmInHouseSimulatorPolicyLocalityTests
{
    ...
    /// over the locality octet AS SENT (Part 3, clause 23.8), matching <see cref="TpmPolicyDigest.ExtendForLocality"/>.
    /// </summary>
    [TestMethod]
    public async Task PolicyLocalityFoldsTheOctetAsSent()
    {
        ...
        zero.Clear();
        _ = TpmPolicyDigest.ExtendForLocality(zero, TpmaLocality.TPM_LOC_ZERO, PolicySweepHarness.SessionAlg, predicted, pool);
    ...
    /// clause 8.5: a zero octet selects no locality at all).
    /// </summary>
    [TestMethod]
    ...
    /// ANDs against the prior setting to an empty set (Part 3, clause 23.8: normal localities intersect, and an
    ...
    public async Task PolicyLocalityRefusesWhenNarrowingToNothing()
    {
        ...
        try
        {
            ...
            Assert.AreEqual(HmacKeyHarness.ParameterEncodedRc(TpmRcConstants.TPM_RC_RANGE, 0), second.ResponseCode, "locality, parameter 1 of Table 154, narrowed to an empty set must refuse parameter-encoded TPM_RC_RANGE.");
        }
        finally
        {
            await PolicySweepHarness.FlushIfPresentAsync(tpm, sessionHandle, TestContext.CancellationToken).ConfigureAwait(false);
    ...
    /// <c>TPM_RC_RANGE</c> (Part 3, clause 23.8: a prior setting and the new request must both be normal or
    /// both be extended).
    /// </summary>
    [TestMethod]
    public async Task PolicyLocalityRefusesANormalAfterAnExtended()
    {
        ...
        try
        {
            ...
            Assert.AreEqual(HmacKeyHarness.ParameterEncodedRc(TpmRcConstants.TPM_RC_RANGE, 0), normal.ResponseCode, "locality, parameter 1 of Table 154, mixing normal and extended localities must refuse parameter-encoded TPM_RC_RANGE.");
        }
        finally
        {
            await PolicySweepHarness.FlushIfPresentAsync(tpm, sessionHandle, TestContext.CancellationToken).ConfigureAwait(false);
    ...
    /// 3, clause 23.8: an extended request must equal any prior extended setting), while the SAME extended
    ...
    public async Task PolicyLocalityRefusesADifferentExtended()
    {
        ...
        try
        {
            ...
            Assert.AreEqual(HmacKeyHarness.ParameterEncodedRc(TpmRcConstants.TPM_RC_RANGE, 0), conflicting.ResponseCode, "locality, parameter 1 of Table 154, given a conflicting extended locality must refuse parameter-encoded TPM_RC_RANGE.");
    ...
    public async Task PolicyLocalityZeroEnabledSessionUnsealsAtLocalityZero()
    {
        ...
        Span<byte> zero = stackalloc byte[size];
        zero.Clear();
        _ = TpmPolicyDigest.ExtendForLocality(zero, TpmaLocality.TPM_LOC_ZERO, PolicySweepHarness.SessionAlg, authPolicy, pool);
    ...
    public async Task PolicyLocalityOneOnlySessionFailsUnseal()
    {
        ...
        Span<byte> zero = stackalloc byte[size];
        zero.Clear();
        _ = TpmPolicyDigest.ExtendForLocality(zero, TpmaLocality.TPM_LOC_ONE, PolicySweepHarness.SessionAlg, authPolicy, pool);
