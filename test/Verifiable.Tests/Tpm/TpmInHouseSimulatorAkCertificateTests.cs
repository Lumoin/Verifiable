...
internal sealed class TpmInHouseSimulatorAkCertificateTests
{
    ...
    public async Task AkCertificateChainsAndVerifiesAttestation()
    {
        ...
        try
        {
            ...
            using PublicKeyMemory validatedLeafKey = await MicrosoftX509Functions.ValidateChainAsync(
                [akCertMemory], [caCertMemory], time.GetUtcNow(), pool, cancellationToken: TestContext.CancellationToken).ConfigureAwait(false);
        ...
        try
        {
            ...
            try
            {
                using PublicKeyMemory _ = await MicrosoftX509Functions.ValidateChainAsync(
                    [akCertMemory], [unrelatedCaMemory], time.GetUtcNow(), pool, cancellationToken: TestContext.CancellationToken).ConfigureAwait(false);
            }
    ...
    {
        var simulator = new TpmSimulator("tpm-in-house-ak-certificate", signingBackend: BouncyCastleTpmEccSigningBackend.Create(), rng: TestEntropy.NewCounterStream(), timeProvider: new FakeTimeProvider(TestClock.CanonicalEpoch));
