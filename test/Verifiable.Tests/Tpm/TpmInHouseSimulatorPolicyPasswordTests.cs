...
/// session's <c>hmac</c> field (TPM 2.0 Library Part 3, clause 23.18), folding the same policyDigest value
...
internal sealed class TpmInHouseSimulatorPolicyPasswordTests
{
    ...
    /// (clause 23.18: "This is the same extend value as used with TPM2_PolicyAuthValue()"), so one authPolicy
    ...
    public async Task PolicyPasswordFoldsTheSameDigestAsPolicyAuthValue()
    {
        ...
        zero.Clear();
        _ = TpmPolicyDigest.ExtendForAuthValue(zero, PolicySweepHarness.SessionAlg, predicted, pool);
    ...
    /// password in the clear in its hmac field (Part 1, clause 16.6.16) — proving the item's authValue is checked
    ...
    public async Task PolicyPasswordUnsealsWhenTheCorrectPasswordIsPresented()
    {
        ...
        Span<byte> zero = stackalloc byte[size];
        zero.Clear();
        _ = TpmPolicyDigest.ExtendForAuthValue(zero, PolicySweepHarness.SessionAlg, authPolicy, pool);
        ...
        try
        {
            ...
            using TpmPolicySession policySession = TpmPolicySession.ForSessionWithPassword(sessionHandle, PolicySweepHarness.SessionAlg, password, TestEntropy.NewCounterStream(), pool);
    ...
    /// compared, not merely folded (Part 3, clause 23.18).
    /// </summary>
    [TestMethod]
    public async Task PolicyPasswordRefusesAWrongPassword()
    {
        ...
        try
        {
            ...
            using TpmPolicySession policySession = TpmPolicySession.ForSessionWithPassword(sessionHandle, PolicySweepHarness.SessionAlg, wrong, TestEntropy.NewCounterStream(), pool);
            UnsealInput unsealInput = UnsealInput.ForItem(TpmiDhObject.FromValue(itemHandle));
            TpmResult<UnsealResponse> unsealResult = await TpmCommandExecutor.ExecuteAsync<UnsealResponse>(
                tpm, unsealInput, [policySession], handleNames, pool, registry, TestContext.CancellationToken).ConfigureAwait(false);
    ...
    /// <summary>
    /// Verifies that when both assertions appear, the LAST one determines the presentation format (Part 1, clause
    ...
    public async Task PolicyAuthValueThenPolicyPasswordEnforcesThePasswordFormat()
    {
        ...
        zero.Clear();
        _ = TpmPolicyDigest.ExtendForAuthValue(zero, PolicySweepHarness.SessionAlg, authPolicy, pool);
        _ = TpmPolicyDigest.ExtendForAuthValue(authPolicy, PolicySweepHarness.SessionAlg, authPolicy, pool);
