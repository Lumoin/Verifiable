...
internal sealed class TpmInHouseSimulatorNvExtendTests
{
    ...
    /// read with the bare <c>TPM_RC_SIZE</c> the structure's own unmarshal answers (Part 2, Table 97), ahead
    /// of any rental — proved by the metered pool's balance — so the command body's rules are never reached.
    ...
    /// the 48-octet width clause 31.3.1 requires, reads back as <c>SHA384(0⁴⁸ ‖ data)</c> — the nameAlg
    ...
    /// AFTER authorization (Part 4 <c>NV_Extend.c</c>: <c>NvWriteAccessChecks</c> before <c>IsNvExtendIndex</c>):
    ...
    public async Task NvExtendOfOrdinaryIndexReturnsAttributesUnderCorrectAuthAndAuthFailUnderWrongAuth()
    {
        ...
        Assert.AreEqual(TpmRcConstants.TPM_RC_ATTRIBUTES, correctResult.ResponseCode, "TPM2_NV_Extend() must refuse a non-Extend Index once authorization has succeeded.");
    ...
    public async Task NvExtendOfCounterIndexReturnsAttributesUnderCorrectAuthAndAuthFailUnderWrongAuth()
    {
        ...
        Assert.AreEqual(TpmRcConstants.TPM_RC_ATTRIBUTES, correctResult.ResponseCode, "TPM2_NV_Extend() must refuse a Counter Index once authorization has succeeded.");
        Assert.AreEqual(TpmRcConstants.TPM_RC_AUTH_FAIL, wrongResult.ResponseCode, "The authorization is checked before the type, so a wrong authValue answers the auth-failure, not TPM_RC_ATTRIBUTES.");
    }
    ...
    /// the session — no audit trail is modelled — proven the same hand-framed way as the encrypt half.
    /// <see href="https://trustedcomputinggroup.org/resource/tpm-library-specification/">TPM 2.0 Library Part 3, clause 5.7</see>.
    /// </summary>
    [TestMethod]
    public async Task NvExtendOverSessionWithAuditAttributeReturnsAttributes()
    {
        BaseMemoryPool pool = BaseMemoryPool.Shared;
        using TpmSimulator simulator = await CreateOperationalAsync().ConfigureAwait(false);
        using TpmDevice device = TpmDevice.Create(simulator.SubmitAsync);
        ...
        TpmRcConstants rawAuditCode = await ExtendOverHmacHandFramedAsync(
            device, pool, registry, ExtendIndexHandle, CorrectAuth, FiveOctets, TpmaSession.AUDIT).ConfigureAwait(false);
        TpmResult<NvExtendResponse> auditResult = TpmResult<NvExtendResponse>.TpmError(rawAuditCode);
    ...
    public async Task NvDefineSpaceOfExtendIndexOverHmacSessionWithMismatchedDataSizeReturnsSize()
    {
        ...
        Assert.AreEqual(TpmRcConstants.TPM_RC_SIZE, result.ResponseCode, "The session arm applies the same digest-width rule as the password arm.");
    }
    ...
    public async Task NvExtendOverIndexBoundAndSaltedSessionForTheBoundEntityOmitsAuthValueAndSucceeds()
    {
        ...
        (StartAuthSessionInput Input, IMemoryOwner<byte> Salt, int SaltLength) salted = await StartAuthSessionInput.CreateBoundAndSaltedHmacSession(
            tpmKeyHandle, ExtendIndexHandle, modulus, DefaultRsaExponent, RsaKeyNameAlg, HmacSessionAlg, rsaBackend.EncryptOaep, pool, TestContext.CancellationToken).ConfigureAwait(false);
    ...
    private async Task<TpmSimulator> CreateOperationalAsync(bool withRsaBackend = false)
    {
        var simulator = withRsaBackend
            ? new TpmSimulator(
                "tpm-in-house-nv-extend", signingBackend: BouncyCastleTpmEccSigningBackend.Create(), rsaSigningBackend: MicrosoftTpmRsaSigningBackend.Create())
            : new TpmSimulator("tpm-in-house-nv-extend");
        await simulator.PowerOnAsync(TestContext.CancellationToken).ConfigureAwait(false);
