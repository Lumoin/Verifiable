...
/// <summary>
/// Drives <c>TPM2_PolicyAuthorizeNV()</c> (TPM 2.0 Library Part 3, Section 23.22) against the in-house
...
internal sealed class TpmInHouseSimulatorPolicyAuthorizeNvTests
{
    ...
    /// approved digest, replacing the session's digest with the fold over the Index's own Name (Part 3, Section
    ...
    public async Task PolicyAuthorizeNvFoldsTheIndexNameAfterAMatch()
    {
        ...
        zero.Clear();
        _ = TpmPolicyDigest.ExtendForCommandCode(zero, TpmCcConstants.TPM_CC_Unseal, PolicySweepHarness.SessionAlg, approvedDigest);
    ...
    /// does not equal the Index's held approved digest (Part 3, Section 23.22).
    /// </summary>
    [TestMethod]
    ...
    /// never been written (Part 3, Section 23.22: a real session requires the Index to have been written).
    /// </summary>
    [TestMethod]
    ...
    /// (SHA-256) (Part 3, Section 23.22: the digest's algorithm must equal the session's own).
    /// </summary>
    [TestMethod]
    ...
    /// <summary>
    /// Verifies <c>TPM2_PolicyAuthorizeNV()</c> refuses with <c>TPM_RC_HASH</c> when the Index's held
    /// <c>TPMT_HA</c> names a hash algorithm (SM3_256) this simulator does not implement (Part 3, Section 23.22).
    /// </summary>
    [TestMethod]
    ...
    /// fewer octets than a <c>TPM_ALG_ID</c> needs (Part 3, Section 23.22: the data must hold a properly
    ...
    /// <summary>Marshals a <c>TPMT_HA</c>: a two-octet big-endian <c>TPM_ALG_ID</c> followed by the digest octets, with no length prefix (Part 2, Section 10.2.2, Table 89).</summary>
