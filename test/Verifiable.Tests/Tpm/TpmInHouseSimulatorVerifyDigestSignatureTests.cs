...
/// clause 10.6.4, Table 111's <c>digestVerified</c> arm).
/// </para>
/// <para>
...
internal sealed class TpmInHouseSimulatorVerifyDigestSignatureTests
{
    ...
    public async Task VerifyDigestSignatureWithMismatchedHashAlgorithmReturnsScheme()
    {
        ...
        Assert.AreEqual(HmacKeyHarness.ParameterEncodedRc(TpmRcConstants.TPM_RC_SCHEME, 2), verifyResult.ResponseCode, "A signature declaring SHA-384 against a SHA-256-schemed key must be refused with TPM_RC_SCHEME.");
    }
    ...
    public async Task VerifyDigestSignatureWithSchemeIncompatibleWithKeyTypeReturnsScheme()
    {
        ...
        Assert.AreEqual(HmacKeyHarness.ParameterEncodedRc(TpmRcConstants.TPM_RC_SCHEME, 2), verifyResult.ResponseCode, "An RSASSA-shaped signature against an ECC key must be refused with TPM_RC_SCHEME.");
    }
    ...
    public async Task VerifyDigestSignatureWithCorruptedSignatureReturnsSignature()
    {
        ...
        Assert.AreEqual(HmacKeyHarness.ParameterEncodedRc(TpmRcConstants.TPM_RC_SIGNATURE, 2), verifyResult.ResponseCode, "A corrupted signature must be refused with TPM_RC_SIGNATURE at signature, parameter 3 of Table 120.");
    }
    ...
    /// clause 20.4 itself; Part 2 clause 4.15 / Part 3 clause 5.8's general size-parameter fallback]
    /// (<see href="https://trustedcomputinggroup.org/resource/tpm-library-specification/">TPM 2.0 Library
    ...
    public async Task VerifyDigestSignatureWithMismatchedDigestSizeReturnsSize()
    {
        ...
        Assert.AreEqual(HmacKeyHarness.ParameterEncodedRc(TpmRcConstants.TPM_RC_SIZE, 1), verifyResult.ResponseCode, "A digest whose size mismatches the resolved scheme's hash width must be refused with TPM_RC_SIZE.");
    }
    ...
    public async Task VerifyDigestSignatureAgainstAnEccStorageParentHandleReturnsScheme()
    {
        ...
        Assert.AreEqual(
            HmacKeyHarness.ParameterEncodedRc(TpmRcConstants.TPM_RC_SCHEME, 2), verifyResult.ResponseCode,
    ...
    public async Task VerifyDigestSignatureWithNonEmptyContextReturnsSize()
    {
        ...
        Assert.AreEqual(HmacKeyHarness.ParameterEncodedRc(TpmRcConstants.TPM_RC_SIZE, 0), code, "A non-empty context must be refused with TPM_RC_SIZE (Table 220's empty[0] arm), ahead of the digest-size check.");
    }

    /// <summary>
    /// Table 120 admits a <c>TPM_ST_SESSIONS</c> tag on a TPM carrying an audit or decrypt session
    /// (<see href="https://trustedcomputinggroup.org/resource/tpm-library-specification/">TPM 2.0 Library
    /// Specification</see>, Part 3: Commands, clause 20.4.2, Table 120). This frame's own
    /// <c>digest</c>/<c>signature</c> octets, read as the authorization area, describe a session block whose
    /// declared width runs past what the frame actually carries — refused
    /// with that slot's own structural code, session-encoded <c>TPM_RC_SIZE</c> at index 0 (clause 5.5), rather
    /// than misread as the head of the <c>context</c> parameter.
    /// </summary>
    [TestMethod]
    public async Task VerifyDigestSignatureWithASessionsTagAndAMalformedAreaIsRefusedSessionEncodedSize()
    {
        BaseMemoryPool pool = BaseMemoryPool.Shared;
        using TpmSimulator simulator = await CreateOperationalAsync(pool).ConfigureAwait(false);
        ...
        Assert.AreEqual(
            ...
            "A TPM_ST_SESSIONS frame whose authorization area is malformed must be refused with the slot's own session-encoded TPM_RC_SIZE.");
    }
