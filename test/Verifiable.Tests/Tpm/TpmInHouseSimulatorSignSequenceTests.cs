...
internal sealed class TpmInHouseSimulatorSignSequenceTests
{
    ...
    public async Task SignSequenceStartOnANullSchemeRsaSigningTemplateReturnsScheme()
    {
        ...
        Assert.AreEqual(TpmRcConstants.TPM_RC_SCHEME, startResult.ResponseCode, "A key created with a NULL template scheme retains none; TPM2_SignSequenceStart() must refuse it with TPM_RC_SCHEME.");
    }
    ...
    public async Task SignSequenceStartOnALoadedSealedObjectHandleReturnsKey()
    {
        ...
        Assert.AreEqual(TpmRcConstants.TPM_RC_KEY, startResult.ResponseCode, "A sealed (KEYEDHASH data) object is not a signing key; TPM2_SignSequenceStart() must refuse it with TPM_RC_KEY.");
    }
    ...
    /// <summary>
    /// <c>TPM2_SignSequenceStart()</c> is framed <c>TPM_ST_NO_SESSIONS</c> — no authorization of <c>keyHandle</c>
    /// is required at Start — so a command framed <c>TPM_ST_SESSIONS</c> is refused with <c>TPM_RC_BAD_TAG</c>: a
    /// simulator limitation, since Table 87 conditions the tag only on an audit or decrypt session, neither of
    /// which this simulator executes
    /// (<see href="https://trustedcomputinggroup.org/resource/tpm-library-specification/">TPM 2.0 Library
    /// Specification</see>, Part 3: Commands, clause 17.5, Table 87).
    /// </summary>
    [TestMethod]
    public async Task SignSequenceStartFramedWithSessionsReturnsBadTag()
    {
        BaseMemoryPool pool = BaseMemoryPool.Shared;
        using TpmSimulator simulator = await CreateOperationalAsync(pool).ConfigureAwait(false);
        using TpmDevice tpm = TpmDevice.Create(simulator.SubmitAsync);
        ...
        Assert.AreEqual(TpmRcConstants.TPM_RC_BAD_TAG, code, "TPM2_SignSequenceStart() framed with TPM_ST_SESSIONS must be refused with TPM_RC_BAD_TAG.");
    }

    /// <summary>
    /// A key whose <c>x509sign</c> attribute (Part 2, Table 37, bit 19) is SET is refused at Start — the errata
    /// section 2.2 fail-fast check, ahead of the same refusal <c>TPM2_SignSequenceComplete()</c> also carries
    /// normatively
    /// (<see href="https://trustedcomputinggroup.org/resource/tpm-library-specification/">TPM 2.0 Library
    ...
    public async Task SignSequenceStartOnAnX509SignKeyReturnsAttributes()
    {
        ...
        Assert.AreEqual(TpmRcConstants.TPM_RC_ATTRIBUTES, startResult.ResponseCode, "A key whose x509sign attribute is SET must be refused with TPM_RC_ATTRIBUTES.");
    }
    ...
    /// <summary>
    /// <c>@sequenceHandle</c> resolving to a loaded key rather than a sequence, or to a handle naming no loaded
    /// object at all, is refused with <c>TPM_RC_HANDLE</c> — "the handle is not correct for the use" (Table 17);
    /// clause 17.7 itself names no response code for this rule, so Table 17's general handle-mismatch entry
    /// applies
    /// (<see href="https://trustedcomputinggroup.org/resource/tpm-library-specification/">TPM 2.0 Library
    /// Specification</see>, Part 3: Commands, clause 17.7).
    /// </summary>
    [TestMethod]
    public async Task SequenceUpdateOnAKeyHandleAndOnAnUnknownHandleReturnHandle()
    {
        BaseMemoryPool pool = BaseMemoryPool.Shared;
        using TpmSimulator simulator = await CreateOperationalAsync(pool).ConfigureAwait(false);
        using TpmDevice tpm = TpmDevice.Create(simulator.SubmitAsync);
        ...
        Assert.AreEqual(TpmRcConstants.TPM_RC_HANDLE, onKeyResult.ResponseCode, "TPM2_SequenceUpdate() against a signing key's own handle (not a sequence) must be refused with TPM_RC_HANDLE.");
        ...
        Assert.AreEqual(TpmRcConstants.TPM_RC_HANDLE, unknownResult.ResponseCode, "TPM2_SequenceUpdate() against a handle naming no loaded object must be refused with TPM_RC_HANDLE.");
    }
    ...
    public async Task SequenceUpdateWithABufferOverMaxSizeHandFramedReturnsSize()
    {
        ...
        Assert.AreEqual(TpmRcConstants.TPM_RC_SIZE, code, "A buffer over Tpm2bMaxBuffer.MaxSize must be refused with TPM_RC_SIZE (Table 91's floor).");
    ...
    /// the TPM": a second Complete, and a FlushContext, on the same handle both answer <c>TPM_RC_HANDLE</c>
    /// (<see href="https://trustedcomputinggroup.org/resource/tpm-library-specification/">TPM 2.0 Library
    ...
    public async Task SignSequenceCompleteFlushesTheSequenceOnSuccess()
    {
        ...
        TpmResult<SignSequenceCompleteResponse> secondComplete = await SubmitCompleteAsync(
            tpm, registry, pool, sequenceHandle, primary.ObjectHandle, [], [], "message"u8.ToArray()).ConfigureAwait(false);
        Assert.AreEqual(TpmRcConstants.TPM_RC_HANDLE, secondComplete.ResponseCode, "A second TPM2_SignSequenceComplete() on an already-flushed sequence must be refused with TPM_RC_HANDLE.");
    ...
    /// <c>TPMA_OBJECT.userWithAuth</c> is CLEAR is refused with a bare <c>TPM_RC_POLICY_FAIL</c> — even though
    ...
    public async Task SignSequenceCompleteOnAUserWithAuthClearKeyReturnsPolicyFail()
    {
        ...
        Assert.AreEqual(
            TpmRcConstants.TPM_RC_POLICY_FAIL, completeResult.ResponseCode,
            "A userWithAuth-CLEAR signing key must refuse TPM2_SignSequenceComplete()'s password session with a bare TPM_RC_POLICY_FAIL, even though the sequence auth and the key password are both correct.");
    }
    ...
    /// simulator flushes EVERY open sequence, not merely one — two concurrently open sequences are both refused
    /// with <c>TPM_RC_HANDLE</c> on the next <c>TPM2_SequenceUpdate()</c>
    /// (<see href="https://trustedcomputinggroup.org/resource/tpm-library-specification/">TPM 2.0 Library
    ...
    public async Task StartupFlushesEveryOpenSequence()
    {
        ...
        Assert.AreEqual(TpmRcConstants.TPM_RC_HANDLE, firstUpdateResult.ResponseCode, "TPM2_Startup() must flush every open sequence; the first of two concurrently open sequences must be refused with TPM_RC_HANDLE.");
        ...
        Assert.AreEqual(TpmRcConstants.TPM_RC_HANDLE, secondUpdateResult.ResponseCode, "TPM2_Startup() must flush every open sequence; the second, concurrently open sequence must also be refused with TPM_RC_HANDLE.");
    }
