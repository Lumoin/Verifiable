...
/// 16.6.10's own Note: "The session can continue to be used, but it, in effect, is no longer bound") — so a
...
internal sealed class TpmInHouseSimulatorBoundEntityBindingLifecycleTests
{
    ...
    /// 16.6.10: "The TPM will record the fact that the authValue was not used in the HMAC computation of the
    ...
    public async Task DefiningAnNvIndexWithAnAuthValueWiderThanItsNameAlgDigestIsRefusedWithSize()
    {
        ...
        Assert.AreEqual(HmacKeyHarness.ParameterEncodedRc(TpmRcConstants.TPM_RC_SIZE, 0), refused.ResponseCode, "An auth wider than the nameAlg digest must be refused with TPM_RC_SIZE (Part 3, clause 31.3.1).");
    ...
    public async Task SealingWithAUserAuthWiderThanTheNameAlgDigestIsRefusedWithSize()
    {
        ...
        Assert.AreEqual(HmacKeyHarness.ParameterEncodedRc(TpmRcConstants.TPM_RC_SIZE, 0), refused.ResponseCode, "A userAuth wider than the nameAlg digest must be refused with TPM_RC_SIZE (Part 1, clause 16.6.4.2).");
    ...
    /// Library Part 3, clause 31.4) — the deletion step of Part 1, clause 16.6.10's squatting attack.
    /// </summary>
    /// <param name="device">The TPM device.</param>
    /// <param name="pool">The memory pool.</param>
    /// <param name="registry">The response codec registry.</param>
    ...
    {
        var simulator = new TpmSimulator("tpm-in-house-bound-entity-lifecycle", signingBackend: BouncyCastleTpmEccSigningBackend.Create(), rng: TestEntropy.NewCounterStream(), timeProvider: new FakeTimeProvider(TestClock.CanonicalEpoch));
