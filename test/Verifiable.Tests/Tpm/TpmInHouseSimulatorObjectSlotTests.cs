...
internal sealed class TpmInHouseSimulatorObjectSlotTests
{
    ...
    /// own actions, after every handle and parameter check — so with the slots full an unknown <c>keyHandle</c>
    /// still answers <c>TPM_RC_HANDLE</c> and a non-signing key still answers <c>TPM_RC_KEY</c>, never
    /// <c>TPM_RC_OBJECT_MEMORY</c> (Part 3, clauses 5.4 and 17.5).
    /// </summary>
    [TestMethod]
    public async Task TheSlotGateIsDecidedAfterEveryOtherRefusal()
    {
        ...
        Assert.AreEqual(TpmRcConstants.TPM_RC_HANDLE, unknown.ResponseCode, "Handle resolution precedes the slot gate.");
        ...
        Assert.AreEqual(TpmRcConstants.TPM_RC_KEY, nonSigning.ResponseCode, "The key-attribute gate precedes the slot gate.");
    ...
    public async Task ClearFreesALoadedSealedObjectsSlot()
    {
        ...
        Assert.AreEqual(TpmRcConstants.TPM_RC_HANDLE, flushed.ResponseCode, "The sealed object's handle names nothing after the clear.");
    }
    ...
    public async Task DisablingTheStorageHierarchyFreesALoadedSealedObjectsSlot()
    {
        ...
        TpmResult<FlushContextResponse> flushed = await SubmitFlushAsync(tpm, registry, pool, sealedHandle.Value).ConfigureAwait(false);
        Assert.AreEqual(TpmRcConstants.TPM_RC_HANDLE, flushed.ResponseCode, "The sealed object's handle names nothing once its hierarchy is disabled.");
    }
    ...
    {
        var simulator = new TpmSimulator("tpm-in-house-object-slots", signingBackend: BouncyCastleTpmEccSigningBackend.Create());
        await simulator.PowerOnAsync(TestContext.CancellationToken).ConfigureAwait(false);
        await BringOperationalAsync(simulator, pool).ConfigureAwait(false);
