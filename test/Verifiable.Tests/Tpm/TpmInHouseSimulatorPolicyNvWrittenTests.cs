...
/// <summary>
/// Drives <c>TPM2_PolicyNvWritten()</c> (TPM 2.0 Library Part 3, clause 23.20) against the in-house
...
internal sealed class TpmInHouseSimulatorPolicyNvWrittenTests
{
    ...
    /// than for NO (octet 0x00) (Part 3, clause 23.20), matching <see cref="TpmPolicyDigest.ExtendForNvWritten"/>
    ...
    public async Task PolicyNvWrittenFoldsYesAndNoDifferently()
    {
        ...
        zero.Clear();
        _ = TpmPolicyDigest.ExtendForNvWritten(zero, isWrittenSet: true, PolicySweepHarness.SessionAlg, predictedYes, pool);
        _ = TpmPolicyDigest.ExtendForNvWritten(zero, isWrittenSet: false, PolicySweepHarness.SessionAlg, predictedNo, pool);
    ...
    /// (Part 3, clause 23.20: "If policySession→checkNvWritten is SET, the TPM will return TPM_RC_VALUE if
    ...
    public async Task PolicyNvWrittenRefusesAConflictingSecondAssertion()
    {
        ...
        try
        {
            ...
            Assert.AreEqual(HmacKeyHarness.ParameterEncodedRc(TpmRcConstants.TPM_RC_VALUE, 0), conflicting.ResponseCode, "writtenSet, parameter 1 of Table 178, asserted a second time with a conflicting value must refuse parameter-encoded TPM_RC_VALUE.");
    ...
    /// <summary>
    /// Verifies a session carrying an nvWritten check refuses <c>TPM2_Unseal()</c> at the itemHandle
    /// authorization, session 1 of Table 30, with <c>TPM_RC_POLICY_FAIL</c> when the authorized command has no
    /// NV Index at all (Part 3, clause 23.20: "If this is not an NV index, the policy makes no sense so fail
    /// it") — even though the accumulated policyDigest matches the sealed object's authPolicy.
    /// </summary>
    [TestMethod]
    public async Task PolicyNvWrittenAtUseFailsUnsealWithNoNvIndex()
    {
        ...
        Span<byte> zero = stackalloc byte[size];
        zero.Clear();
        _ = TpmPolicyDigest.ExtendForNvWritten(zero, isWrittenSet: false, PolicySweepHarness.SessionAlg, authPolicy, pool);
        ...
        try
        {
            ...
            Assert.AreEqual(HmacKeyHarness.SessionEncodedRc(TpmRcConstants.TPM_RC_POLICY_FAIL, 0), unsealResult.ResponseCode, "An nvWritten check with no NV Index authorized fails the itemHandle authorization, session 1 of Table 30.");
        }
        finally
        {
