...
internal sealed class TpmInHouseSimulatorNvWriteLockTests
{
    ...
    public async Task NvWriteLockOfUndefinedIndexReturnsHandle()
    {
        ...
        Assert.AreEqual(HmacKeyHarness.HandleEncodedRc(TpmRcConstants.TPM_RC_HANDLE, 1), result.ResponseCode, "Table 261: nvIndex is TPM2_NV_WriteLock()'s second handle (handle 2); an undefined Index is handle-encoded TPM_RC_HANDLE at index 1.");
    }
    ...
    public async Task NvWriteLockOfIndexWithNeitherLockAttributeReturnsAttributesUnderCorrectAuthAndAuthFailUnderWrongAuth()
    {
        ...
        Assert.AreEqual(HmacKeyHarness.HandleEncodedRc(TpmRcConstants.TPM_RC_ATTRIBUTES, 1), correctResult.ResponseCode, "An Index carrying neither lockability attribute cannot be write-locked.");
    ...
    public async Task NvWriteLockOfAlreadyLockedIndexWithWrongAuthReturnsAuthFailAndChargesFailedTries()
    {
        ...
        Assert.AreEqual(HmacKeyHarness.SessionEncodedRc(TpmRcConstants.TPM_RC_AUTH_FAIL, 0), wrongResult.ResponseCode, "An already-locked Index still authorizes the command before answering it.");
        Assert.AreEqual(
    ...
    public async Task NvWriteLockWithWrongAuthOnDaProtectedIndexReturnsAuthFailAndChargesFailedTries()
    {
        ...
        Assert.AreEqual(HmacKeyHarness.SessionEncodedRc(TpmRcConstants.TPM_RC_AUTH_FAIL, 0), result.ResponseCode, "authHandle's authorizing session is session 1 of Table 261 (TPM 2.0 Library Part 2, clause 6.6.2); a wrong authValue on a DA-protected Index is session-encoded TPM_RC_AUTH_FAIL there.");
    ...
    public async Task NvWriteLockWithWrongAuthOnNoDaIndexReturnsBadAuthUncharged()
    {
        ...
        Assert.AreEqual(HmacKeyHarness.SessionEncodedRc(TpmRcConstants.TPM_RC_BAD_AUTH, 0), result.ResponseCode, "authHandle's authorizing session is session 1 of Table 261 (TPM 2.0 Library Part 2, clause 6.6.2); a wrong authValue on a non-DA Index is session-encoded TPM_RC_BAD_AUTH there.");
    ...
    /// </summary>
    [TestMethod]
    public async Task NvWriteLockOverSessionWithAuditAttributeSucceeds()
    {
        BaseMemoryPool pool = BaseMemoryPool.Shared;
        using TpmSimulator simulator = await CreateOperationalAsync().ConfigureAwait(false);
        using TpmDevice device = TpmDevice.Create(simulator.SubmitAsync, BaseMemoryPool.Shared, TestEntropy.NewCounterStream());
        ...
        try
        {
            ...
            byte[] rpHash = await ComputeRpHashAsync(TpmCcConstants.TPM_CC_NV_WriteLock, responseParameters, pool).ConfigureAwait(false);
            ...
            Assert.IsTrue(
                ...
                "The session's audit digest must equal H(0…0 ‖ cpHash ‖ rpHash) chained from the NV_WriteLock exchange's own wire octets (TPM 2.0 Library Part 1, clause 17.1, equation 30).");
        }
        finally
        {
    ...
    public async Task NvWriteLockReturnsItsCarriersAcrossRefusalsAndASuccess()
    {
        ...
        TpmResult<NvWriteLockResponse> undefinedResult = await WriteLockAsync(
            device, pool, registry, WriteDefineIndexHandle, WriteDefineIndexHandle, CorrectAuth).ConfigureAwait(false);
        Assert.AreEqual(HmacKeyHarness.HandleEncodedRc(TpmRcConstants.TPM_RC_HANDLE, 1), undefinedResult.ResponseCode, "Table 261: nvIndex is TPM2_NV_WriteLock()'s second handle (handle 2); an undefined Index is handle-encoded TPM_RC_HANDLE at index 1.");
        ...
        TpmResult<NvWriteLockResponse> plainResult = await WriteLockAsync(
            device, pool, registry, PlainIndexHandle, PlainIndexHandle, CorrectAuth).ConfigureAwait(false);
        Assert.AreEqual(HmacKeyHarness.HandleEncodedRc(TpmRcConstants.TPM_RC_ATTRIBUTES, 1), plainResult.ResponseCode, "Table 261: nvIndex is TPM2_NV_WriteLock()'s second handle (handle 2); an Index with neither TPMA_NV_WRITEDEFINE nor TPMA_NV_WRITE_STCLEAR SET is handle-encoded TPM_RC_ATTRIBUTES at index 1.");
        ...
        TpmDevice device, BaseMemoryPool pool, TpmResponseRegistry registry, uint nvIndex, ReadOnlyMemory<byte> suppliedAuth, TpmaSession attribute)
    {
        ...
        try
        {
            ...
            //15.7, equation 15, with an EMPTY parameters term because Part 3's Table 261 defines no command parameters.
    ...
    /// <summary>
    /// The audit twin of <see cref="WriteLockOverHmacHandFramedAsync"/>: hand-frames a raw
    /// <c>TPM2_NV_WriteLock()</c> authorized by a single unbound, unsalted HMAC session carrying
    ...
    private async Task<(byte[] Response, uint SessionHandle, byte[] CpHash)> WriteLockOverHmacHandFramedForAuditAsync(
    ...
    private async Task<TpmSimulator> CreateOperationalAsync(bool withEccSigningBackend = false)
    {
        var simulator = withEccSigningBackend
            ? new TpmSimulator("tpm-in-house-nv-writelock", signingBackend: BouncyCastleTpmEccSigningBackend.Create(), rng: TestEntropy.NewCounterStream(), timeProvider: new FakeTimeProvider(TestClock.CanonicalEpoch))
            : new TpmSimulator("tpm-in-house-nv-writelock", rng: TestEntropy.NewCounterStream(), timeProvider: new FakeTimeProvider(TestClock.CanonicalEpoch));
        await simulator.PowerOnAsync(TestContext.CancellationToken).ConfigureAwait(false);
