...
internal sealed class TpmInHouseSimulatorHmacTests
{
    ...
    private static byte[] Rfc4231Case2Key { get; } = Convert.FromHexString("4a656665");

    /// <summary>RFC 4231 test case 2 data: "what do ya want for nothing?".</summary>
    private static byte[] Rfc4231Case2Data { get; } = Convert.FromHexString("7768617420646f2079612077616e7420666f72206e6f7468696e673f");
    ...
    public async Task HmacWithAMismatchedHashAlgIsRefusedWithValue()
    {
        ...
        Assert.AreEqual(HmacKeyHarness.ParameterEncodedRc(TpmRcConstants.TPM_RC_VALUE, 1), result.ResponseCode, "hashAlg, parameter 2 of Table 71, other than the key's scheme hash must be refused with parameter-encoded TPM_RC_VALUE.");
    }
    ...
    public async Task HmacOverAnAsymmetricKeyHandleIsRefusedWithType()
    {
        ...
        Assert.AreEqual(HmacKeyHarness.HandleEncodedRc(TpmRcConstants.TPM_RC_TYPE, 0), result.ResponseCode, "handle, handle 1 of Table 71, naming an asymmetric key must be refused with handle-encoded TPM_RC_TYPE.");
    }
    ...
    public async Task HmacOverASequenceHandleIsRefusedWithType()
    {
        ...
        Assert.AreEqual(HmacKeyHarness.HandleEncodedRc(TpmRcConstants.TPM_RC_TYPE, 0), result.ResponseCode, "handle, handle 1 of Table 71, naming a sequence object must be refused with handle-encoded TPM_RC_TYPE.");
    }
    ...
    public async Task HmacOverASealedDataObjectIsRefusedWithKey()
    {
        ...
        Assert.AreEqual(HmacKeyHarness.HandleEncodedRc(TpmRcConstants.TPM_RC_KEY, 0), result.ResponseCode, "handle, handle 1 of Table 71, naming a sealed data object must be refused with handle-encoded TPM_RC_KEY.");
    }
    ...
    public async Task HmacOverARestrictedKeyIsRefusedWithAttributes()
    {
        ...
        Assert.AreEqual(HmacKeyHarness.HandleEncodedRc(TpmRcConstants.TPM_RC_ATTRIBUTES, 0), result.ResponseCode, "handle, handle 1 of Table 71, naming a restricted KEYEDHASH key must be refused with handle-encoded TPM_RC_ATTRIBUTES.");
    }
    ...
    public async Task HmacOverAnUnloadedHandleIsRefusedWithHandle()
    {
        ...
        Assert.AreEqual(HmacKeyHarness.HandleEncodedRc(TpmRcConstants.TPM_RC_HANDLE, 0), result.ResponseCode, "handle, handle 1 of Table 71, naming nothing loaded must be refused with handle-encoded TPM_RC_HANDLE.");
    }
    ...
    public async Task HmacWithAWrongPasswordOnADaProtectedKeyIsAuthFailAndCharged()
    {
        ...
        Assert.AreEqual(HmacKeyHarness.SessionEncodedRc(TpmRcConstants.TPM_RC_AUTH_FAIL, sessionIndex: 0), result.ResponseCode, "handle's authorizing session, session 1 of Table 71, is refused with session-encoded TPM_RC_AUTH_FAIL on a wrong password against a DA-protected key.");
    ...
    public async Task HmacWithAWrongPasswordOnANoDaKeyIsBadAuthAndUncharged()
    {
        ...
        TpmResult<HmacResponse> result = await HmacKeyHarness.HmacAsync(
            tpm, registry, pool, key.Handle, Rfc4231Case1Data, TpmAlgIdConstants.TPM_ALG_SHA256, WrongKeyPassword, TestContext.CancellationToken).ConfigureAwait(false);
        Assert.AreEqual(HmacKeyHarness.SessionEncodedRc(TpmRcConstants.TPM_RC_BAD_AUTH, sessionIndex: 0), result.ResponseCode, "handle's authorizing session, session 1 of Table 71, is refused with session-encoded TPM_RC_BAD_AUTH on a wrong password against a noDA key.");
    ...
    public async Task HmacOverAUserWithAuthClearKeyIsRefusedWithPolicyFail()
    {
        ...
        Assert.AreEqual(HmacKeyHarness.SessionEncodedRc(TpmRcConstants.TPM_RC_POLICY_FAIL, 0), result.ResponseCode, "handle's authorizing session, session 1 of Table 71, is refused with session-encoded TPM_RC_POLICY_FAIL when a password session presents against a userWithAuth-CLEAR key.");
    }
    ...
    public async Task HmacOverAUserWithAuthClearRestrictedKeyAnswersPolicyFailBeforeAttributes()
    {
        ...
        Assert.AreEqual(HmacKeyHarness.SessionEncodedRc(TpmRcConstants.TPM_RC_POLICY_FAIL, 0), result.ResponseCode, "handle's authorizing session, session 1 of Table 71, is refused with session-encoded TPM_RC_POLICY_FAIL: the userWithAuth gate must answer before the restricted-key gate.");
    }
    ...
    public async Task HmacWithAWrongPasswordAndAMismatchedHashAlgIsAuthFailAndCharged()
    {
        ...
        Assert.AreEqual(HmacKeyHarness.SessionEncodedRc(TpmRcConstants.TPM_RC_AUTH_FAIL, sessionIndex: 0), result.ResponseCode, "handle's authorizing session, session 1 of Table 71, is refused with session-encoded TPM_RC_AUTH_FAIL: the password compare must answer before the hashAlg selection.");
