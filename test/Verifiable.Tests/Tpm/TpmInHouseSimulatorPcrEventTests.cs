...
internal sealed class TpmInHouseSimulatorPcrEventTests
{
    ...
    /// <see href="https://trustedcomputinggroup.org/resource/tpm-library-specification/">TPM 2.0 Library Part 3, clause 22.3.1, Table 132</see>.
    /// </summary>
    [TestMethod]
    ...
    /// and Table 132's <c>TPM_ST_SESSIONS</c> tag makes a no-sessions frame <c>TPM_RC_AUTH_MISSING</c> (clause 5.5).
    ...
    public async Task PcrEventHandFramedOverTheEventBoundReturnsSizeAndWithoutSessionsReturnsAuthMissing()
    {
        ...
        Assert.AreEqual(TpmRcConstants.TPM_RC_SIZE, size, "An event over 1,024 octets must be refused with TPM_RC_SIZE.");
    ...
    /// <summary>
    /// PTP 1.07 Table 14, "Extended by TPM2_PCR_Extend, Locality = 0": N for PCR 18 (Locality 3) — Part 4
    /// <c>PCR_Event.c</c> answers <c>TPM_RC_LOCALITY</c> before hashing anything.
    /// <see href="https://trustedcomputinggroup.org/resource/pc-client-platform-tpm-profile-ptp-specification/">PC Client PTP 1.07, clause 4.7.1, Table 14; TPM 2.0 Library Part 4, PCR_Event.c</see>.
    /// </summary>
    [TestMethod]
    ...
    {
        var simulator = new TpmSimulator("tpm-in-house-pcr-event");
