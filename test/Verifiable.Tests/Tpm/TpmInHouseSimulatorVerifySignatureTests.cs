...
internal sealed class TpmInHouseSimulatorVerifySignatureTests
{
    ...
    public async Task EcdsaVerifySignatureProducesAVerifiableTicket()
    {
        ...
        using var simulator = new TpmSimulator("tpm-in-house-verify-signature-seed", signingBackend: BouncyCastleTpmEccSigningBackend.Create(), seed: seed, rng: TestEntropy.NewCounterStream(), timeProvider: new FakeTimeProvider(TestClock.CanonicalEpoch));
    ...
    public async Task VerifySignatureWithCorruptedSignatureReturnsSignature()
    {
        ...
        Assert.AreEqual(HmacKeyHarness.ParameterEncodedRc(TpmRcConstants.TPM_RC_SIGNATURE, 1), verifyResult.ResponseCode, "A tampered signature must be refused with TPM_RC_SIGNATURE at signature, parameter 2 of Table 116.");
    }
    ...
    public async Task VerifySignatureWithSchemeIncompatibleWithKeyTypeReturnsScheme()
    {
        ...
        Assert.AreEqual(HmacKeyHarness.ParameterEncodedRc(TpmRcConstants.TPM_RC_SCHEME, 1), verifyResult.ResponseCode, "signature, parameter 2 of Table 116, whose algorithm is incompatible with the resolved key's type must be refused with parameter-encoded TPM_RC_SCHEME.");
    }

    /// <summary>
    /// Verifies that an unloaded transient-range <c>keyHandle</c> is rejected with <c>TPM_RC_REFERENCE_H0</c>
    ...
    /// </summary>
    [TestMethod]
    public async Task VerifySignatureWithUnknownKeyHandleReturnsReferenceH0()
    {
        BaseMemoryPool pool = BaseMemoryPool.Shared;
        using TpmSimulator simulator = await CreateOperationalAsync(pool).ConfigureAwait(false);
        using TpmDevice tpm = TpmDevice.Create(simulator.SubmitAsync, BaseMemoryPool.Shared, TestEntropy.NewCounterStream());
        ...
        Assert.AreEqual(TpmRcConstants.TPM_RC_REFERENCE_H0, verifyResult.ResponseCode);
    }
    ...
    public async Task VerifySignatureWithNullSigAlgReturnsScheme()
    {
        ...
        Assert.AreEqual(
            ...
            "Table 116: signature is TPM2_VerifySignature()'s second parameter (index 1); a NULL signature.sigAlg must be refused there at the wire (Table 219).");
    }
    ...
    public async Task VerifySignatureWithUnsupportedSigAlgReturnsScheme()
    {
        ...
        Assert.AreEqual(
            HmacKeyHarness.ParameterEncodedRc(TpmRcConstants.TPM_RC_SCHEME, 1), code,
            "Table 116: signature is TPM2_VerifySignature()'s second parameter (index 1); a sigAlg naming no signing scheme at all must be refused there (Table 219).");
    }
    ...
    public async Task VerifySignatureWithSignatureROverBoundReturnsSize()
    {
        ...
        Assert.AreEqual(HmacKeyHarness.ParameterEncodedRc(TpmRcConstants.TPM_RC_SIZE, 1), code, "signature is TPM2_VerifySignature()'s second parameter (Table 116, index 1); signatureR over Tpm2bEccParameter.MaxSize must be parameter-encoded TPM_RC_SIZE (Table 214/197).");
    }
    ...
    public async Task VerifySignatureWithSignatureSOverBoundReturnsSize()
    {
        ...
        Assert.AreEqual(HmacKeyHarness.ParameterEncodedRc(TpmRcConstants.TPM_RC_SIZE, 1), code, "signature is TPM2_VerifySignature()'s second parameter (Table 116, index 1); signatureS over Tpm2bEccParameter.MaxSize must be parameter-encoded TPM_RC_SIZE (Table 214/197).");
    }
    ...
    public async Task VerifySignatureWithRsaSignatureOverBoundReturnsSize()
    {
        ...
        Assert.AreEqual(HmacKeyHarness.ParameterEncodedRc(TpmRcConstants.TPM_RC_SIZE, 1), code, "signature is TPM2_VerifySignature()'s second parameter (Table 116, index 1); rsaSignature over Tpm2bPublicKeyRsa.MaxRsaKeyBytes must be parameter-encoded TPM_RC_SIZE (Table 212/194).");
    }
    ...
    public async Task VerifySignatureWithOverBoundAndTruncatedSignatureRReturnsSize()
    {
        ...
        Assert.AreEqual(HmacKeyHarness.ParameterEncodedRc(TpmRcConstants.TPM_RC_SIZE, 1), code, "signature is TPM2_VerifySignature()'s second parameter (Table 116, index 1); an over-bound signatureR is parameter-encoded TPM_RC_SIZE even when the frame is also too short to supply it.");
    }
    ...
    public async Task VerifySignatureWithWithinBoundTruncatedSignatureRReturnsInsufficient()
    {
        ...
        Assert.AreEqual(
            ...
            "signature is TPM2_VerifySignature()'s second parameter (Table 116, index 1); a within-bound signatureR whose frame is too short to supply it is parameter-encoded TPM_RC_INSUFFICIENT, not TPM_RC_SIZE.");
    }
    ...
    /// parse cleanly through to the next check (here <c>TPM_RC_REFERENCE_H0</c>, from the unloaded transient
    /// key, TPM 2.0 Library Part 3, clause 5.4, step 2.1).
    /// </summary>
    [TestMethod]
    public async Task VerifySignatureWithOddCombinedSignatureRAndSLengthDoesNotReturnSize()
    {
        ...
        Assert.AreEqual(
            ...
            "An ECDSA signature whose r and s independently-supplied lengths combine to an odd total must parse cleanly, reaching the unloaded-transient-key check rather than being refused with TPM_RC_SIZE.");
    }
    ...
    /// genuinely non-empty, in-bound signature has been fully rented (unknown <c>keyHandle</c>, refused with
    /// <c>TPM_RC_REFERENCE_H0</c>) still releases it.
    /// </summary>
    [TestMethod]
    public async Task VerifySignatureOverBoundSignatureRRefusalBalancesThePool()
    {
        ...
        Assert.AreEqual(HmacKeyHarness.ParameterEncodedRc(TpmRcConstants.TPM_RC_SIZE, 1), code, "signature is TPM2_VerifySignature()'s second parameter (Table 116, index 1); an over-bound signatureR must still answer parameter-encoded TPM_RC_SIZE under the metered pool.");
    ...
    /// rented as the parse's last acts), then the unloaded transient <c>keyHandle</c> is refused with
    /// <c>TPM_RC_REFERENCE_H0</c> (TPM 2.0 Library Part 3, clause 5.4, step 2.1) and every parsed carrier is
    /// released — the signature, not merely the digest.
    /// </summary>
    [TestMethod]
    public async Task VerifySignatureWithUnknownKeyHandleReleasesTheParsedSignature()
    {
        ...
        Assert.AreEqual(TpmRcConstants.TPM_RC_REFERENCE_H0, code, "An unloaded transient keyHandle must be refused after a clean parse.");
        Assert.AreEqual(
            baseline, trackingPool.OutstandingCount,
            "The TPM_RC_REFERENCE_H0 refusal must release the parsed digest and signature carriers alike.");
    }
    ...
    public async Task VerifySignatureRefusesASignatureRWiderThanTheCurveFieldWithSignature()
    {
        ...
        TpmRcConstants code = await SubmitVerifySignatureCommandAsync(simulator, pool, primary.ObjectHandle.Value, digest, body).ConfigureAwait(false);
        Assert.AreEqual(HmacKeyHarness.ParameterEncodedRc(TpmRcConstants.TPM_RC_SIGNATURE, 1), code, "A signatureR above the field width is no curve element and must fail verification, not parsing, at signature, parameter 2 of Table 116.");
    }
    ...
    public async Task VerifySignatureOverANonSigningAsymmetricKeyIsRefusedWithAttributes()
    {
        ...
        Assert.AreEqual(HmacKeyHarness.HandleEncodedRc(TpmRcConstants.TPM_RC_ATTRIBUTES, 0), result.ResponseCode, "keyHandle, handle 1 of Table 116, naming a non-signing asymmetric key must be refused with handle-encoded TPM_RC_ATTRIBUTES.");
    }
    ...
    public async Task VerifySignatureOverANonSigningAsymmetricKeyIsRefusedWithAttributesBeforeTheHashRule()
    {
        ...
        Assert.AreEqual(HmacKeyHarness.HandleEncodedRc(TpmRcConstants.TPM_RC_ATTRIBUTES, 0), result.ResponseCode, "keyHandle, handle 1 of Table 116, is refused with handle-encoded TPM_RC_ATTRIBUTES: the sign gate must answer before the hash bound, so a non-signing key with a SHA-1 signature hash is ATTRIBUTES, not HASH.");
    }
