...
/// (<c>TPM2B_DATA</c>, TPM 2.0 Library Part 2, clause 10.3.3, Table 91; the command is Part 3, clause 18.2,
/// Table 97) against the in-house behavioural <see cref="TpmSimulator"/>: the caller nonce rides a carrier the
...
internal sealed class TpmInHouseSimulatorCertifyCarrierTests
{
    ...
    /// <summary>
    /// A <c>TPM2_Certify()</c> refused by the entry transition — the objectHandle (the 1st handle in the handle
    /// area, index 0) is transient-range but resolves to no loaded object, so TPM 2.0 Library Part 3, clause 5.4
    /// step 2.1 answers <c>TPM_RC_REFERENCE_H0</c> before either slot's credential is compared and before any
    /// parameter is looked at — returns every carrier the parser rented: both slots' supplied passwords and the
    /// qualifying data. The refusing arm reaches them through the request record's own <c>IDisposable.Dispose</c>,
    /// which is the only owner they ever had, because a handle refusal transfers nothing into an action.
    /// </summary>
    [TestMethod]
    public async Task RefusedCertifyAtTheEntryTransitionReturnsTheParseRentedCarriersToPool()
    {
        ...
        long baseline = trackingPool.OutstandingCount;
        {
            ...
            Assert.AreEqual(
                ...
                "A transient-range objectHandle (the 1st handle in the handle area) that resolves to no loaded object is TPM_RC_REFERENCE_H0 (TPM 2.0 Library Part 3, clause 5.4, step 2.1).");
    ...
    /// 2.0 Library Part 3, clause 18.2; Part 1, clause 15.6.1 for the response entries).
    /// </summary>
    [TestMethod]
    ...
    public async Task CertifyWithQualifyingDataOverTheDataBoundIsRefusedAtParseAndRentsNothing()
    {
        ...
        long baseline = trackingPool.OutstandingCount;
        {
            ...
            Assert.AreEqual(
                ...
                "A qualifyingData wider than sizeof(TPMT_HA) is TPM_RC_SIZE at qualifyingData, parameter 1 of the Certify command table (TPM 2.0 Library Part 2, clause 10.3.3, Table 91).");
    ...
    /// 2.0 Library Part 1, clause 15.6.1) are released before a caller reads the pool balance.
    /// </summary>
        ...
        TpmAlgIdConstants schemeHashAlg, ReadOnlyMemory<byte> objectSlotAuthValue, ReadOnlyMemory<byte> signSlotAuthValue)
    {
        ...
        try
        {
            using TpmSession objectSession = new(new TpmHandle(objectSessionHandle), objectStarted.NonceTPM, HmacSessionAlg, TestEntropy.NewCounterStream(), trackingPool.Pool);
