...
internal sealed class TpmInHouseSimulatorVerifySequenceHardeningTests
{
    ...
    /// <c>TPM2_VerifySequenceComplete()</c>'s own parser would have produced from a real wire frame. No private member,
    ...
    public async Task VerifySequenceCompleteWithASequenceRetainedPairDifferentFromTheKeysRetainedPairReturnsScheme()
    {
        ...
        Assert.AreEqual(
            HmacKeyHarness.HandleEncodedRc(TpmRcConstants.TPM_RC_SCHEME, 1), rejection.ResponseCode,
            "keyHandle, handle 2 of Table 118, is refused with handle-encoded TPM_RC_SCHEME when a sequence whose retained HashAlg no longer matches the Name-matched key's own retained SigningSchemeHashAlg.");
    }
    ...
    public async Task VerifySequenceCompleteAgainstAKeyForcedNullSigningSchemeReturnsScheme()
    {
        ...
        Assert.AreEqual(
            HmacKeyHarness.HandleEncodedRc(TpmRcConstants.TPM_RC_SCHEME, 1), rejection.ResponseCode,
            "keyHandle, handle 2 of Table 118, whose retained signing scheme is NULL must be refused with handle-encoded TPM_RC_SCHEME at TPM2_VerifySequenceComplete(), even though the completing request's own SignatureScheme/SchemeHashAlg agree with the sequence's retained pair.");
    }
    ...
    public async Task VerifySequenceStartWithANonEmptyHintAndContextHandFramedReturnsSizeAndReturnsEveryParsedCarrierToPool()
    {
        ...
        Assert.AreEqual(HmacKeyHarness.ParameterEncodedRc(TpmRcConstants.TPM_RC_SIZE, 1), code, "hint, parameter 2 of Table 89, non-empty under ECDSA (and, independently, a non-empty context) must be refused with parameter-encoded TPM_RC_SIZE.");
        Assert.AreEqual(
    ...
    public async Task PolicyAuthorizeWithACheckTicketMetadataOfAnUnimplementedTcgHashAlgorithmReturnsHashWithNoExceptionEscaping()
    {
        ...
        Assert.AreEqual(
            HmacKeyHarness.ParameterEncodedRc(TpmRcConstants.TPM_RC_HASH, 3), code,
            "checkTicket, parameter 4 of Table 170, naming a TCG-registered hash algorithm this simulator does not implement (SHA3-256) must be refused with parameter-encoded TPM_RC_HASH, never an escaping exception.");
    }
    ...
    public async Task PolicyAuthorizeWithACheckTicketMetadataOfSha1ReturnsHash()
    {
        ...
        Assert.AreEqual(HmacKeyHarness.ParameterEncodedRc(TpmRcConstants.TPM_RC_HASH, 3), code, "checkTicket, parameter 4 of Table 170, naming SHA-1 must be refused with parameter-encoded TPM_RC_HASH.");
    }
    ...
    private static byte[] BuildApprovedPolicyForUnseal()
    {
        ...
        zero.Clear();
        _ = TpmPolicyDigest.ExtendForCommandCode(zero, TpmCcConstants.TPM_CC_Unseal, SessionAlg, approvedPolicy, BaseMemoryPool.Shared);
