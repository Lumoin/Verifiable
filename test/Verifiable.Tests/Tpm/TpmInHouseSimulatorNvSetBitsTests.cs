...
internal sealed class TpmInHouseSimulatorNvSetBitsTests
{
    ...
    public async Task NvDefineSpaceOfBitsIndexOverHmacSessionWithSixteenOctetDataSizeReturnsSize()
    {
        ...
        Assert.AreEqual(TpmRcConstants.TPM_RC_SIZE, result.ResponseCode, "The session arm applies the same eight-octet rule as the password arm.");
    }
    ...
    /// authorization (Part 4 <c>NV_SetBits.c</c>: <c>NvWriteAccessChecks</c> before <c>IsNvBitsIndex</c>): an
    ...
    public async Task NvSetBitsOfOrdinaryIndexReturnsAttributesUnderCorrectAuthAndAuthFailUnderWrongAuth()
    {
        ...
        Assert.AreEqual(TpmRcConstants.TPM_RC_ATTRIBUTES, correctResult.ResponseCode, "TPM2_NV_SetBits() must refuse a non-Bit-Field Index once authorization has succeeded.");
        Assert.AreEqual(TpmRcConstants.TPM_RC_AUTH_FAIL, wrongResult.ResponseCode, "The authorization is checked before the type, so a wrong authValue answers the auth-failure, not TPM_RC_ATTRIBUTES.");
    }
    ...
    public async Task NvSetBitsOfCounterIndexReturnsAttributesUnderCorrectAuthAndAuthFailUnderWrongAuth()
    {
        ...
        Assert.AreEqual(TpmRcConstants.TPM_RC_ATTRIBUTES, correctResult.ResponseCode, "TPM2_NV_SetBits() must refuse a Counter Index once authorization has succeeded.");
        Assert.AreEqual(TpmRcConstants.TPM_RC_AUTH_FAIL, wrongResult.ResponseCode, "The authorization is checked before the type, so a wrong authValue answers the auth-failure, not TPM_RC_ATTRIBUTES.");
    }
    ...
    public async Task NvSetBitsOverHmacSessionInLockoutModeReturnsLockout()
    {
        ...
        for(uint attempt = 1; attempt <= LoweredMaxTries; attempt++)
        {
            TpmResult<NvSetBitsResponse> wrongResult = await SetBitsAsync(
                device, pool, registry, BitsIndexHandle, BitsIndexHandle, WrongAuth, FirstBits).ConfigureAwait(false);
            Assert.AreEqual(TpmRcConstants.TPM_RC_AUTH_FAIL, wrongResult.ResponseCode, $"Attempt {attempt} of {LoweredMaxTries} must count as an auth-failure while driving the TPM into Lockout mode.");
        }
    ...
    /// the session — no audit trail is modelled — proven the same hand-framed way, with the identical session
    /// without the attribute succeeding.
    /// <see href="https://trustedcomputinggroup.org/resource/tpm-library-specification/">TPM 2.0 Library Part 3, clause 5.7</see>.
    /// </summary>
    [TestMethod]
    public async Task NvSetBitsOverSessionWithAuditAttributeReturnsAttributesWhileTheSameSessionWithoutItSucceeds()
    {
        BaseMemoryPool pool = BaseMemoryPool.Shared;
        using TpmSimulator simulator = await CreateOperationalAsync().ConfigureAwait(false);
        using TpmDevice device = TpmDevice.Create(simulator.SubmitAsync);
        ...
        TpmRcConstants rawAuditCode = await SetBitsOverHmacHandFramedAsync(
            device, pool, registry, BitsIndexHandle, CorrectAuth, FirstBits, TpmaSession.AUDIT).ConfigureAwait(false);
        TpmResult<NvSetBitsResponse> auditResult = TpmResult<NvSetBitsResponse>.TpmError(rawAuditCode);
        Assert.AreEqual(TpmRcConstants.TPM_RC_ATTRIBUTES, auditResult.BaseError, "No audit trail is modelled, so an audit-attributed session must fail closed.");
        Assert.AreNotEqual(TpmRcConstants.TPM_RC_ATTRIBUTES, auditResult.ResponseCode, "The refusal names the offending session, so the raw wire code carries the session-index modifier (TPM 2.0 Library Part 2, clause 6.6.2).");

        TpmResult<NvSetBitsResponse> declinedResult = await SetBitsOverHmacAsync(
            device, pool, registry, BitsIndexHandle, BitsIndexHandle, CorrectAuth, FirstBits).ConfigureAwait(false);
        Assert.IsTrue(declinedResult.IsSuccess, $"The identical authValue over an otherwise identical session with audit left CLEAR must succeed: '{declinedResult.ResponseCode}'.");
    }
    ...
    public async Task NvSetBitsOverIndexBoundAndSaltedSessionForTheBoundEntityOmitsAuthValueAndSucceeds()
    {
        ...
        (StartAuthSessionInput Input, IMemoryOwner<byte> Salt, int SaltLength) salted = await StartAuthSessionInput.CreateBoundAndSaltedHmacSession(
            tpmKeyHandle, BitsIndexHandle, modulus, DefaultRsaExponent, RsaKeyNameAlg, HmacSessionAlg, rsaBackend.EncryptOaep, pool, TestContext.CancellationToken).ConfigureAwait(false);
    ...
    public async Task NvSetBitsReturnsItsCarriersAcrossRefusalsAndASuccess()
    {
        ...
        TpmResult<NvSetBitsResponse> ordinaryResult = await SetBitsAsync(
            device, pool, registry, OrdinaryIndexHandle, OrdinaryIndexHandle, CorrectAuth, FirstBits).ConfigureAwait(false);
        Assert.AreEqual(TpmRcConstants.TPM_RC_ATTRIBUTES, ordinaryResult.ResponseCode);
    ...
    /// authorization area is a truncated integer: the wire read answers the bare <c>TPM_RC_INSUFFICIENT</c> the
    /// unmarshalling of a too-short parameter area produces, before the command body's own rules are reached.
    ...
    public async Task NvSetBitsWithSevenOctetsOfBitsReturnsInsufficient()
    {
        ...
        Assert.AreEqual(TpmRcConstants.TPM_RC_INSUFFICIENT, code, "A truncated UINT64 parameter is refused at the wire read with TPM_RC_INSUFFICIENT.");
    }

    /// <summary>
    /// <c>bits</c> is the command's only parameter (Table 259), so a ninth octet after it leaves the command
    /// buffer longer than the structures the command consumes: the wire read answers the bare
    /// <c>TPM_RC_SIZE</c> clause 5.2's "commandSize ... shall match" rule produces.
    /// <see href="https://trustedcomputinggroup.org/resource/tpm-library-specification/">TPM 2.0 Library Part 3, clause 31.10.2, Table 259; clause 5.2</see>.
    /// </summary>
    [TestMethod]
    ...
    private async Task<TpmSimulator> CreateOperationalAsync(bool withRsaBackend = false)
    {
        var simulator = withRsaBackend
            ? new TpmSimulator(
                "tpm-in-house-nv-setbits", signingBackend: BouncyCastleTpmEccSigningBackend.Create(), rsaSigningBackend: MicrosoftTpmRsaSigningBackend.Create())
            : new TpmSimulator("tpm-in-house-nv-setbits");
        await simulator.PowerOnAsync(TestContext.CancellationToken).ConfigureAwait(false);
