...
internal sealed class TpmInHouseSimulatorSealedDataTemplateTests
{
    ...
    private static readonly byte[] ShortSecret = "sealed"u8.ToArray();
    ...
    public async Task CreateDataObjectWithSensitiveDataOriginAndDataIsRefusedWithAttributes()
    {
        ...
        Assert.AreEqual(TpmRcConstants.TPM_RC_ATTRIBUTES, result.ResponseCode, "A data object template with sensitiveDataOrigin SET and data supplied must be refused with TPM_RC_ATTRIBUTES.");
    }
    ...
    public async Task CreateDataObjectWithSensitiveDataOriginAndNoDataIsRefusedWithAttributes()
    {
        ...
        Assert.AreEqual(TpmRcConstants.TPM_RC_ATTRIBUTES, result.ResponseCode, "A data object template with sensitiveDataOrigin SET and no data must be refused with TPM_RC_ATTRIBUTES.");
    }
    ...
    public async Task SealOf129OctetsIsRefusedWithSize()
    {
        ...
        Assert.AreEqual(TpmRcConstants.TPM_RC_SIZE, code, "Sealing 129 octets must be refused with TPM_RC_SIZE.");
    }
    ...
    /// <c>MAX_SYM_DATA</c> bound: sealing 129 octets over a bound HMAC session is refused with <c>TPM_RC_SIZE</c>.
    /// <see href="https://trustedcomputinggroup.org/resource/tpm-library-specification/">TPM 2.0 Library Part 2, clause 11.1.14, Table 170; Part 3, clauses 5.8 and 12.1</see>.
    /// </summary>
    [TestMethod]
    public async Task SealOf129OctetsOverAnHmacSessionIsRefusedWithSize()
    {
        BaseMemoryPool pool = BaseMemoryPool.Shared;
        using TpmSimulator simulator = await HmacKeyHarness.CreateOperationalAsync(nameof(SealOf129OctetsOverAnHmacSessionIsRefusedWithSize), pool, TestContext.CancellationToken).ConfigureAwait(false);
        using TpmDevice tpm = TpmDevice.Create(simulator.SubmitAsync);
        TpmResponseRegistry registry = HmacKeyHarness.CreateRegistry();
        ...
        try
        {
            using(session)
            {
                ...
                Assert.AreEqual(TpmRcConstants.TPM_RC_SIZE, code, "Sealing 129 octets over a session must be refused with TPM_RC_SIZE.");
            }
        }
        finally
        {
    ...
    public async Task ATruncatedInSensitiveIsRefusedWithInsufficient()
    {
        ...
        Assert.AreEqual(TpmRcConstants.TPM_RC_INSUFFICIENT, code, "A truncated inSensitive must be refused with TPM_RC_INSUFFICIENT.");
    }
    ...
    /// <see href="https://trustedcomputinggroup.org/resource/tpm-library-specification/">TPM 2.0 Library Part 3, clause 5.8.2, Table 2; Part 2, clause 12.2.5, Table 229</see>.
    /// </summary>
    [TestMethod]
    public async Task ATruncatedInPublicIsRefusedWithInsufficient()
    {
        ...
        Assert.AreEqual(TpmRcConstants.TPM_RC_INSUFFICIENT, code, "A truncated inPublic must be refused with TPM_RC_INSUFFICIENT.");
    }
