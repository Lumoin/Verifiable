...
internal sealed class TpmInHouseSimulatorSequenceUpdateSessionTests
{
    ...
    /// <summary>
    /// <c>@sequenceHandle</c> is a <c>TPMI_DH_OBJECT</c> that must name an OPEN SEQUENCE: a loaded KEYEDHASH
    /// object's handle at that slot names something that is not a sequence at all — <c>TPM2_SequenceUpdate()</c>'s
    /// own clause 17.7 names no RC of its own for this exact case, but <c>TPM2_SequenceComplete()</c>'s clause
    /// 17.8.1 states <c>TPM_RC_MODE</c> for a sequence handle of the wrong KIND, and the reference code's
    /// <c>ObjectIsSequence</c> check generalizes it to this non-sequence-object case too, handle-encoded to
    /// index 0 (TPM 2.0 Library Part 2, clause 6.6.2, Table 16's one-based N field) —
    /// decided before any authorization is evaluated, so the refusal carries no session-index modifier
    /// (<see href="https://trustedcomputinggroup.org/resource/tpm-library-specification/">TPM 2.0 Library
    /// Specification</see>, Part 3: Commands, clause 17.7, 17.8.1; Table 91; Part 2: Structures, clause 6.6.2,
    /// Table 16).
    /// </summary>
    [TestMethod]
    public async Task SequenceUpdateNamingALoadedKeyAtTheSequenceSlotIsRefusedWithMode()
    {
        BaseMemoryPool pool = BaseMemoryPool.Shared;
        using TpmSimulator simulator = await HmacKeyHarness.CreateOperationalAsync(
            nameof(SequenceUpdateNamingALoadedKeyAtTheSequenceSlotIsRefusedWithMode), pool, TestContext.CancellationToken).ConfigureAwait(false);
        using TpmDevice tpm = TpmDevice.Create(simulator.SubmitAsync, BaseMemoryPool.Shared, TestEntropy.NewCounterStream());
        TpmResponseRegistry registry = CreateRegistry();
        ...
        try
        {
            ...
            Assert.AreEqual(
                HmacKeyHarness.HandleEncodedRc(TpmRcConstants.TPM_RC_MODE, handleIndex: 0), refused.ResponseCode,
                "A loaded KEYEDHASH object at @sequenceHandle must be refused with TPM_RC_MODE handle-encoded to index 0 before any authorization is read.");
        }
        finally
        {
            session.Dispose();
    ...
    /// the first parameter's framing, and a recovered <c>buffer</c> wider than the 1,024-octet bound is that
    /// parameter's own content failure, never the decrypt step's — parameter-encoded to <c>buffer</c> itself,
    /// <c>TPM2_SequenceUpdate()</c>'s sole parameter (Part 3, clause 17.7, Table 91, index 0)
    /// (<see href="https://trustedcomputinggroup.org/resource/tpm-library-specification/">TPM 2.0 Library
    /// Specification</see>, Part 3: Commands, clause 5.7; Part 2: Structures, clause 6.6.2).
    /// </summary>
    [TestMethod]
    public async Task SequenceUpdateWithAnOverBoundPlaintextBufferReturnsParameterEncodedSize()
    {
        BaseMemoryPool pool = BaseMemoryPool.Shared;
        using TpmSimulator simulator = await HmacKeyHarness.CreateOperationalAsync(
            nameof(SequenceUpdateWithAnOverBoundPlaintextBufferReturnsParameterEncodedSize), pool, TestContext.CancellationToken).ConfigureAwait(false);
        using TpmDevice tpm = TpmDevice.Create(simulator.SubmitAsync, BaseMemoryPool.Shared, TestEntropy.NewCounterStream());
        TpmResponseRegistry registry = CreateRegistry();
        ...
        try
        {
            ...
            Assert.AreEqual(
                HmacKeyHarness.ParameterEncodedRc(TpmRcConstants.TPM_RC_SIZE, parameterIndex: 0), refused.ResponseCode,
                "buffer is TPM2_SequenceUpdate()'s sole parameter (Table 91, index 0); a recovered value wider than TPM2B_MAX_BUFFER is that parameter's own content failure, not the decrypt slot's.");
        }
        finally
        {
            session.Dispose();
    ...
    /// <summary>
    /// An audit companion beside <c>@sequenceHandle</c>'s own password authorization is admitted exactly like
    /// the decrypt companion above, and the session's audit digest chains
    /// <c>H(0…0 ‖ cpHash ‖ rpHash)</c> with the sequence handle's Name term the Empty Buffer in cpHash — "the
    /// Name associated with sequenceHandle will be the Empty Buffer" — never the handle's own four octets, which
    /// a caller or a TPM that folded them would compute a different, non-matching digest for.
    /// <see href="https://trustedcomputinggroup.org/resource/tpm-library-specification/">TPM 2.0 Library Part 1,
    /// Table 9 footnote (1), clause 29.4.6 and clause 17.1, equation 30; Part 3, clause 17.7.1</see>.
    /// </summary>
    [TestMethod]
    public async Task SequenceUpdateWithAnAuditCompanionBesideAPasswordSequenceSlotChainsTheEmptyBufferNameIntoTheDigest()
    {
        BaseMemoryPool pool = BaseMemoryPool.Shared;
        using TpmSimulator simulator = await HmacKeyHarness.CreateOperationalAsync(
            nameof(SequenceUpdateWithAnAuditCompanionBesideAPasswordSequenceSlotChainsTheEmptyBufferNameIntoTheDigest), pool, TestContext.CancellationToken).ConfigureAwait(false);
        using TpmDevice tpm = TpmDevice.Create(simulator.SubmitAsync, BaseMemoryPool.Shared, TestEntropy.NewCounterStream());
        TpmResponseRegistry registry = CreateRegistry();
        _ = registry.Register(TpmCcConstants.TPM_CC_GetSessionAuditDigest, TpmResponseCodec.GetSessionAuditDigest);

        TpmiDhObject sequenceHandle = await StartHashSequenceAsync(tpm, registry, pool, SequenceAuth).ConfigureAwait(false);
        (uint auditHandle, TpmSession auditSession) = await StartUnboundSessionAsync(tpm, registry, pool, TpmtSymDef.Null).ConfigureAwait(false);
        ...
        try
        {
            ...
            TpmResult<SequenceUpdateResponse> update = await TpmCommandExecutor.ExecuteAsync<SequenceUpdateResponse>(
                tpm, input, [sequenceSession, auditSession], null, pool, registry, TestContext.CancellationToken).ConfigureAwait(false);
            Assert.IsTrue(update.IsSuccess, $"An audit companion beside a password sequence slot must be admitted: '{update.ResponseCode}'.");

            //cpHash = H(commandCode ‖ Name ‖ parameters) with Name the Empty Buffer (zero octets contributed) and
            //parameters Table 91's own TPM2B_MAX_BUFFER (Part 1, clause 15.7, equation 15).
            byte[] parameters = new byte[sizeof(ushort) + Message.Length];
            BinaryPrimitives.WriteUInt16BigEndian(parameters, (ushort)Message.Length);
            Message.CopyTo(parameters.AsSpan(sizeof(ushort)));

            byte[] cpHashInput = new byte[sizeof(uint) + parameters.Length];
            BinaryPrimitives.WriteUInt32BigEndian(cpHashInput, (uint)TpmCcConstants.TPM_CC_SequenceUpdate);
            parameters.CopyTo(cpHashInput.AsSpan(sizeof(uint)));
            byte[] cpHash = SHA256.HashData(cpHashInput);

            //rpHash = H(responseCode ‖ commandCode ‖ parameters), Table 92's response carrying none (Part 1,
            //clause 15.8, equation 16).
            byte[] rpHashInput = new byte[sizeof(uint) + sizeof(uint)];
            BinaryPrimitives.WriteUInt32BigEndian(rpHashInput, (uint)TpmRcConstants.TPM_RC_SUCCESS);
            BinaryPrimitives.WriteUInt32BigEndian(rpHashInput.AsSpan(sizeof(uint)), (uint)TpmCcConstants.TPM_CC_SequenceUpdate);
            byte[] rpHash = SHA256.HashData(rpHashInput);

            byte[] extendInput = new byte[32 + cpHash.Length + rpHash.Length];
            cpHash.CopyTo(extendInput.AsSpan(32));
            rpHash.CopyTo(extendInput.AsSpan(32 + cpHash.Length));
            byte[] expectedDigest = SHA256.HashData(extendInput);
            ...
            Assert.IsTrue(
                ...
                "The audit digest must equal H(0…0 ‖ cpHash ‖ rpHash) with the sequence handle's Empty Buffer Name folded into cpHash — never its four handle octets.");
        }
        finally
        {
            auditSession.Dispose();
            await HmacKeyHarness.FlushIfPresentAsync(tpm, registry, pool, auditHandle, TestContext.CancellationToken).ConfigureAwait(false);
        }
    }
    ...
    public async Task SequenceUpdateWithALoadedPolicySessionAtTheSequenceSlotIsRefusedWithAuthType()
    {
        ...
        try
        {
            using TpmSession policySlotSession = new(new TpmHandle(policySessionHandle), started.NonceTPM, SessionAlg, TestEntropy.NewCounterStream(), pool);
    ...
    public async Task SequenceUpdateRoundTripsReturnEveryRentedCarrierToThePool()
    {
        ...
        try
        {
            ...
            Assert.AreEqual(
                ...
                "sequenceHandle is TPM2_SequenceUpdate()'s sole handle (index 0); an unloaded TRANSIENT-range value is TPM_RC_REFERENCE_H0 (TPM 2.0 Library Part 3, clause 5.4, step 2.1) — the balance below proves nothing unless the refused exchange really was refused after its carriers were rented.");
