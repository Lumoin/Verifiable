...
internal sealed class TpmInHouseSimulatorHashSequenceTests
{
    ...
    public async Task FlushContextOnAHashSequenceSucceedsAndASecondFlushReturnsHandle()
    {
        ...
        TpmResult<FlushContextResponse> secondFlush = await TpmCommandExecutor.ExecuteAsync<FlushContextResponse>(
            tpm, flushInput, [], null, pool, registry, TestContext.CancellationToken).ConfigureAwait(false);
        Assert.AreEqual(TpmRcConstants.TPM_RC_HANDLE, secondFlush.ResponseCode, "A second flush of the already-flushed hash sequence must be refused with TPM_RC_HANDLE.");
    }
    ...
    public async Task HashSequenceStartWithAnUnimplementedHashReturnsHash()
    {
        ...
        Assert.AreEqual(TpmRcConstants.TPM_RC_HASH, result.ResponseCode, "An unimplemented hash algorithm must be refused with TPM_RC_HASH.");
    }

    /// <summary>
    /// Part 2, Table 66: a value that is not a hash identifier fails the <c>TPMI_ALG_HASH</c> unmarshal with
    /// <c>#TPM_RC_HASH</c>. The simulator does not distinguish this from clause 17.4.1's refusal — both are
    /// <c>TPM_RC_HASH</c>.
    /// <see href="https://trustedcomputinggroup.org/resource/tpm-library-specification/">TPM 2.0 Library Part 2, clause 9.27, Table 66</see>.
    /// </summary>
    [TestMethod]
    public async Task HashSequenceStartWithANonHashIdentifierHandFramedReturnsHash()
    {
        ...
        Assert.AreEqual(TpmRcConstants.TPM_RC_HASH, code, "TPM_ALG_ECDSA is not a hash identifier and must be refused with TPM_RC_HASH.");
    }

    /// <summary>
    /// Table 85: the tag is <c>TPM_ST_SESSIONS</c> only when an audit or decrypt session is present, otherwise
    /// <c>TPM_ST_NO_SESSIONS</c>; the simulator models neither and refuses a session-tagged frame with
    /// <c>TPM_RC_BAD_TAG</c>. A trailing octet after <c>hashAlg</c> is <c>TPM_RC_SIZE</c> (clause 5.2).
    ...
    /// </summary>
    [TestMethod]
    public async Task HashSequenceStartFramedWithSessionsReturnsBadTagAndATrailingOctetReturnsSize()
    {
        BaseMemoryPool pool = BaseMemoryPool.Shared;
        using TpmSimulator simulator = await CreateOperationalAsync(pool).ConfigureAwait(false);
        ...
        Assert.AreEqual(TpmRcConstants.TPM_RC_BAD_TAG, sessionTagged, "A TPM_ST_SESSIONS frame must be refused with TPM_RC_BAD_TAG.");
    ...
    /// clause 12.4.1: a sequence object answers <c>TPM_RC_SEQUENCE</c>.
    ...
    public async Task SequenceCompleteWhoseFirstBlockBeginsWithTpmGeneratedReturnsTheNullTicketAndARestrictedKeyRefuses()
    {
        ...
        TpmResult<SignDigestResponse> signResult = await TpmCommandExecutor.ExecuteAsync<SignDigestResponse>(
            tpm, signInput, [keySession], null, pool, registry, TestContext.CancellationToken).ConfigureAwait(false);
        Assert.AreEqual(TpmRcConstants.TPM_RC_TICKET, signResult.ResponseCode, "A restricted key must refuse a digest that carries only the NULL ticket.");
    }
    ...
    public async Task SequenceCompleteOnAnEventSequenceReturnsModeAndTheSequenceSurvives()
    {
        ...
        Assert.AreEqual(TpmRcConstants.TPM_RC_MODE, refused.ResponseCode, "TPM2_SequenceComplete() on an Event Sequence must be refused with TPM_RC_MODE.");
    ...
    /// with <c>TPM2_SequenceComplete()</c> is the same kind mismatch clause 17.8.1 answers with
    ...
    public async Task SequenceCompleteOnASigningSequenceReturnsModeAndTheSequenceSurvives()
    {
        ...
        Assert.AreEqual(TpmRcConstants.TPM_RC_MODE, refused.ResponseCode, "TPM2_SequenceComplete() on a signing sequence must be refused with TPM_RC_MODE.");
    ...
    /// (<c>{F}</c>, Table 93) — a second completion of the same handle finds nothing.
    /// <see href="https://trustedcomputinggroup.org/resource/tpm-library-specification/">TPM 2.0 Library Part 3, clause 17.8.1, Table 93; Part 1, clause 29.4.6</see>.
    /// </summary>
    [TestMethod]
    public async Task SequenceCompleteFlushesTheSequenceOnSuccess()
    {
        ...
        Assert.AreEqual(TpmRcConstants.TPM_RC_HANDLE, second.ResponseCode, "The completed sequence must have been flushed, so its handle no longer resolves.");
    }

    /// <summary>
    /// Part 2, Table 17: a handle that is "not correct for the use" is <c>TPM_RC_HANDLE</c> — a signing key's
    /// handle, and an unknown transient handle, are refused alike by <c>TPM2_SequenceComplete()</c>.
    /// <see href="https://trustedcomputinggroup.org/resource/tpm-library-specification/">TPM 2.0 Library Part 2, clause 6.6.3, Table 17</see>.
    /// </summary>
    [TestMethod]
    public async Task SequenceCompleteOnAKeyHandleAndOnAnUnknownHandleReturnHandle()
    {
        BaseMemoryPool pool = BaseMemoryPool.Shared;
        using TpmSimulator simulator = await CreateOperationalAsync(pool).ConfigureAwait(false);
        using TpmDevice tpm = TpmDevice.Create(simulator.SubmitAsync);
        ...
        Assert.AreEqual(TpmRcConstants.TPM_RC_HANDLE, onKey.ResponseCode, "A key handle is not a sequence and must be refused with TPM_RC_HANDLE.");
        ...
        Assert.AreEqual(TpmRcConstants.TPM_RC_HANDLE, onUnknown.ResponseCode, "An unknown handle must be refused with TPM_RC_HANDLE.");
    }
    ...
    public async Task SequenceCompleteHandFramedWithAnOutOfTableHierarchyReturnsValueAndAnOversizedBufferReturnsSize()
    {
        ...
        Assert.AreEqual(TpmRcConstants.TPM_RC_VALUE, badHierarchy, "A hierarchy outside Table 59 must be refused with TPM_RC_VALUE.");
        ...
        Assert.AreEqual(TpmRcConstants.TPM_RC_SIZE, oversized, "A buffer over MAX_2B_BUFFER_SIZE must be refused with TPM_RC_SIZE.");
