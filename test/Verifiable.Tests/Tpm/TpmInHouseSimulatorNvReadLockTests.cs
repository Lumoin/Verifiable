...
internal sealed class TpmInHouseSimulatorNvReadLockTests
{
    ...
    public async Task NvReadLockWithoutReadStclearReturnsAttributesUnderCorrectAuthAndAuthFailUnderWrongAuth()
    {
        ...
        Assert.AreEqual(TpmRcConstants.TPM_RC_ATTRIBUTES, correctResult.ResponseCode, "An Index without TPMA_NV_READ_STCLEAR cannot be read-locked at all.");
        Assert.AreEqual(TpmRcConstants.TPM_RC_AUTH_FAIL, wrongResult.ResponseCode, "The authorization is resolved first, so a wrong authValue answers the auth-failure rather than the attribute refusal.");
    }
    ...
    public async Task NvReadLockOfPinFailIndexWithWrongAuthIncrementsPinCountByOne()
    {
        ...
        Assert.AreEqual(TpmRcConstants.TPM_RC_BAD_AUTH, result.ResponseCode, "A wrong PIN on a NO_DA PIN Fail Index is a plain bad-authorization.");
    ...
    public async Task NvReadOfReadLockedIndexWithWrongAuthReturnsAuthFail()
    {
        ...
        Assert.AreEqual(TpmRcConstants.TPM_RC_AUTH_FAIL, result.ResponseCode, "Authorization is resolved before the read-lock status, so the wrong authValue answers first.");
    ...
    /// TPM shall return TPM_RC_NV_LOCKED", answered bare after the command HMAC has verified against the
    /// post-lock Name (Part 1, clause 15.7, equation 15).
    ...
    /// <summary>
    /// The <c>audit</c> attribute on the authorizing session fails closed with <c>TPM_RC_ATTRIBUTES</c> naming the
    /// session — no audit trail is modelled — proven the same hand-framed way as the encrypt half.
    /// <see href="https://trustedcomputinggroup.org/resource/tpm-library-specification/">TPM 2.0 Library Part 3, clause 5.7</see>.
    /// </summary>
    [TestMethod]
    public async Task NvReadLockOverSessionWithAuditAttributeReturnsAttributes()
    {
        BaseMemoryPool pool = BaseMemoryPool.Shared;
        using TpmSimulator simulator = await CreateOperationalAsync().ConfigureAwait(false);
        using TpmDevice device = TpmDevice.Create(simulator.SubmitAsync);
        ...
        TpmRcConstants rawAuditCode = await ReadLockOverHmacHandFramedAsync(
            device, pool, registry, ReadLockIndexHandle, CorrectAuth, TpmaSession.AUDIT).ConfigureAwait(false);
        TpmResult<NvReadLockResponse> auditResult = TpmResult<NvReadLockResponse>.TpmError(rawAuditCode);
        ...
        TpmDevice device, BaseMemoryPool pool, TpmResponseRegistry registry, uint nvIndex, ReadOnlyMemory<byte> suppliedAuth, TpmaSession attribute)
    {
        ...
        try
        {
            ...
            //15.7, equation 15, with no parameters term at all, since Table 267 gives this command none. This
    ...
    private async Task<TpmSimulator> CreateOperationalAsync(bool withSigningBackend = false)
    {
        var simulator = withSigningBackend
            ? new TpmSimulator("tpm-in-house-nv-read-lock", signingBackend: BouncyCastleTpmEccSigningBackend.Create())
            : new TpmSimulator("tpm-in-house-nv-read-lock");
        await simulator.PowerOnAsync(TestContext.CancellationToken).ConfigureAwait(false);
