...
internal sealed class TpmInHouseSimulatorDuplicationTests
{
    ...
    public async Task DuplicateRefusalLadderAnswersTheClauseCodes()
    {
        ...
        await AssertDuplicateRefusalAsync(tpm, registry, pool, duplicableHandle, newParentHandle, unlatchedSession, [duplicableName, newParentName], TpmRcConstants.TPM_RC_POLICY_FAIL,
        ...
        await AssertDuplicateRefusalAsync(tpm, registry, pool, duplicableHandle, newParentHandle, mislatchedSession, [duplicableName, newParentName], TpmRcConstants.TPM_RC_POLICY_FAIL,
        ...
        await AssertDuplicateRefusalAsync(tpm, registry, pool, boundHandle, newParentHandle, latchedSession, [boundName, newParentName], TpmRcConstants.TPM_RC_ATTRIBUTES,
        ...
        await AssertDuplicateRefusalAsync(tpm, registry, pool, policyFreeHandle, newParentHandle, latchedSecondSession, [policyFreeName, newParentName], TpmRcConstants.TPM_RC_POLICY_FAIL,
    ...
    public async Task DuplicateWithAnInnerWrapperAlgorithmIsRefusedWithSymmetric()
    {
        ...
        Assert.AreEqual(
            TpmRcConstants.TPM_RC_SYMMETRIC, responseCode,
            "An inner-wrapper symmetricAlg is refused with TPM_RC_SYMMETRIC while only the NULL form is modeled.");
    }
    ...
    public async Task TamperedDuplicateOrSeedIsRefusedWithIntegrityUnderBothParentTypes()
    {
        ...
        await AssertImportRefusalAsync(tpm, registry, pool, eccParentHandle, publicOctets, tamperedEccHmac, eccSeed,
            TpmRcConstants.TPM_RC_INTEGRITY, "A corrupted outer HMAC must fail its own compare.").ConfigureAwait(false);
        ...
        await AssertImportRefusalAsync(tpm, registry, pool, eccParentHandle, publicOctets, tamperedEccDuplicate, eccSeed,
            TpmRcConstants.TPM_RC_INTEGRITY, "A corrupted duplication blob must fail its outer-HMAC compare.").ConfigureAwait(false);
        ...
        await AssertImportRefusalAsync(tpm, registry, pool, rsaParent.ObjectHandle.Value, publicOctets, rsaDuplicate, tamperedRsaSeed,
            TpmRcConstants.TPM_RC_INTEGRITY, "A failed RSA-OAEP decode substitutes an unpredictable seed, so the refusal is the same TPM_RC_INTEGRITY.").ConfigureAwait(false);
    ...
    public async Task IndependentlyConstructedDuplicateImportsAndTheIdentityLabelIsRefused()
    {
        ...
        //authValue ‖ 32-octet obfuscation ‖ the test's own data) — Part 2, clause 12.3, Tables 224/225 by hand.
        ...
        await AssertImportRefusalAsync(tpm, registry, pool, rsaParent.ObjectHandle.Value, publicOctets, duplicateBlob, identityLabelSeed,
            TpmRcConstants.TPM_RC_INTEGRITY, "The IDENTITY label must not recover a DUPLICATE-use seed (Part 1, clause 20.3.2.3).").ConfigureAwait(false);
    ...
    private static ReadOnlyMemory<byte> DuplicationPolicyDigest()
    {
        ...
        _ = TpmPolicyDigest.ExtendForCommandCode(new byte[DigestSize], TpmCcConstants.TPM_CC_Duplicate, SessionAlg, digest);
    ...
    private async Task<TpmSimulator> CreateOperationalAsync(BaseMemoryPool pool)
    {
        var simulator = new TpmSimulator(
            "tpm-in-house-duplication", signingBackend: BouncyCastleTpmEccSigningBackend.Create(), rsaSigningBackend: MicrosoftTpmRsaSigningBackend.Create());
