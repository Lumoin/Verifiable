...
/// policyDigest of TPM 2.0 Library Part 1, clause 16.7 and the latched cpHash of Part 3, Section 23.2.4 — and of
...
internal sealed class TpmInHouseSimulatorPolicyCarrierTests
{
    ...
    /// Table 90) is refused with a bare <c>TPM_RC_SIZE</c> at the wire read, ahead of the rental and ahead of
    ...
    /// refused with a bare <c>TPM_RC_TAG</c> at the wire read. TPM 2.0 Library Part 3, clause 23.16, printed page
    /// 227 states the requirement: "The unmarshaling process requires that a proper TPMT_TK_VERIFIED be provided
    ...
    /// (<c>TPM2_VerifyDigestSignature()</c>, clause 23.16.2's preferred producer) — and names <c>TPM_RC_TAG</c>
    ...
    public async Task PolicyAuthorizeWithACheckTicketTagOutsideTheAdmittedSetIsRefusedAtTheParseWithoutRenting()
    {
        ...
        try
        {
            ...
            Assert.AreEqual(
                ...
                "A checkTicket tag outside Table 112's three-tag admitted set is a bare TPM_RC_TAG, answered at the parse.");
            Assert.AreEqual(
                baseline, trackingPool.OutstandingCount,
    ...
    /// 10.7.4, Table 113), whose own table names <c>TPM_RC_VALUE</c> for a value outside
    ...
    public async Task PolicyAuthorizeWithAnOutOfSetCheckTicketHierarchyIsRefusedAtTheParseWithoutRenting()
    {
        ...
        try
        {
            ...
            Assert.AreEqual(
                ...
                "A checkTicket hierarchy outside TPMI_RH_HIERARCHY is a bare TPM_RC_VALUE, answered at the parse (Part 2, Table 59).");
            Assert.AreEqual(
                baseline, trackingPool.OutstandingCount,
                "The three admitted fields ahead of it are rented only after every wire check has passed, so nothing is outstanding.");
        }
    ...
    /// <c>TPM2B_DIGEST</c> buffer may carry (<c>{:sizeof(TPMU_HA)}</c>, TPM 2.0 Library Part 2, clause 10.3.2,
    /// Table 90, over the <c>TPML_DIGEST</c> of clause 10.8.5, Table 126) is refused with a bare
    ...
    public async Task PolicyOrWithAMidListOverBoundAndTruncatedBranchAnswersSizeNotInsufficientAndRentsNothing()
    {
        ...
        try
        {
            ...
            Assert.AreEqual(
                TpmRcConstants.TPM_RC_SIZE, code,
                "A branch that is both over-bound and truncated is TPM_RC_SIZE — the bound is checked before the remaining-buffer comparison.");
            Assert.AreEqual(
                baseline, trackingPool.OutstandingCount,
    ...
    /// 125, whose out-of-range count is <c>#TPM_RC_SIZE</c>) — with <c>TPM_RC_SIZE</c> at the parse, the same
    ...
    public async Task PcrReadAndPolicyPcrRejectASelectionNamingMoreThanSixteenBanks()
    {
        ...
        Assert.AreEqual(
            TpmRcConstants.TPM_RC_SIZE, pcrReadCode,
            "A selection naming more than HASH_COUNT banks is TPM_RC_SIZE for PCR_Read (Part 2, clause 10.8.7, Table 128).");
        ...
        try
        {
            ...
            Assert.AreEqual(
                TpmRcConstants.TPM_RC_SIZE, policyPcrCode,
                "The same count bound applies to PolicyPCR's pcrs parameter.");
    ...
    public async Task PcrReadPreservesTheSizeofSelectValueBoundThroughTheCarrier()
    {
        ...
        Assert.AreEqual(
            TpmRcConstants.TPM_RC_VALUE, belowMinCode,
            "sizeofSelect below PCR_SELECT_MIN (3) is TPM_RC_VALUE (Part 2, clause 10.5.2, Table 107).");
        ...
        Assert.AreEqual(
            TpmRcConstants.TPM_RC_VALUE, aboveMaxCode,
            "sizeofSelect above PCR_SELECT_MAX (32) is TPM_RC_VALUE (Part 2, clause 10.5.2, Table 107).");
    ...
    public async Task PcrReadAndPolicyPcrAnswerSizeNotInsufficientForAnOverBoundAndTruncatedSelection()
    {
        ...
        try
        {
            ...
            Assert.AreEqual(
                TpmRcConstants.TPM_RC_SIZE, policyPcrCode,
                "The same precedence applies to PolicyPCR's pcrs parameter.");
            Assert.AreEqual(policyPcrBaseline, trackingPool.OutstandingCount, "The refused probe must rent nothing, including the already-read pcrDigest.");
        }
        finally
        {
    ...
    public async Task PolicyPcrFoldMatchesTheSpecFormulaOverTheMarshaledSelectionOnATrialSession()
    {
        ...
        try
        {
            ...
            byte[] expected = new byte[DigestSize];
            _ = TpmPolicyDigest.ExtendForPcr(new byte[DigestSize], marshaledSelection, callerDigest, SessionAlg, expected);
    ...
    public async Task PolicyPcrTrialFoldUsesThePcrsInputParameterWithoutModification()
    {
        ...
        try
        {
            ...
            byte[] expected = new byte[DigestSize];
            _ = TpmPolicyDigest.ExtendForPcr(new byte[DigestSize], rawSelection, callerDigest, SessionAlg, expected);
    ...
    public async Task PolicyPcrRealFoldClearsUnimplementedPcrBitsInThePcrsTerm()
    {
        ...
        try
        {
            ...
            byte[] expected = new byte[DigestSize];
            _ = TpmPolicyDigest.ExtendForPcr(new byte[DigestSize], expectedFoldedSelection, liveComposite.AsReadOnlySpan(), SessionAlg, expected);
    ...
    /// clause 10.6.5 restates that <c>TPM2_PolicyAuthorize()</c> is the ticket's consumer. Otherwise mirrors
    ...
    /// <summary>
    /// Frames a <c>TPM2_PolicyTicket()</c> body (TPM 2.0 Library Part 3, Section 23.5, Table 148): the policy
    ...
    /// <summary>
    /// Frames a <c>TPM2_PolicySigned()</c> body (TPM 2.0 Library Part 3, Section 23.3, Table 144): the two
    ...
    /// <summary>
    /// Frames a <c>TPM2_PolicyAuthorize()</c> body (TPM 2.0 Library Part 3, Section 23.16, Table 170): the
