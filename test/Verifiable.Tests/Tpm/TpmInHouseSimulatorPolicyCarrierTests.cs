...
/// policyDigest of TPM 2.0 Library Part 1, clause 17.7 and the latched cpHash of Part 3, Section 23.2.4 — and of
...
internal sealed class TpmInHouseSimulatorPolicyCarrierTests
{
    ...
    /// Part 2, clause 10.4.2, Table 92) — the width the hand-framed parse-bound cases drive.
    /// </summary>
    ...
    /// Part 2, clause 10.4.10, Table 100).
    /// </summary>
    ...
    /// <summary>
    /// The Zero Digest a session's policyDigest starts at (TPM 2.0 Library Part 1, clause 17.7) is a shared
    ...
    {
        Tpm2bDigest zero = Tpm2bDigest.Zero(TpmiAlgHash.FromValue(SessionAlg));
    ...
    /// from the current digest's own length (TPM 2.0 Library Part 1, clause 17.7), so folding over a zero-LENGTH
    ...
    /// buffer (TPM 2.0 Library Part 1, clause 17.7): the value <c>TPM2_PolicyGetDigest()</c> reports after one
    ...
    public async Task FirstAssertionFoldsOverAZeroDigestOfTheSessionHashWidth()
    {
        ...
        try
        {
            ...
            _ = TpmPolicyDigest.ExtendForCommandCode(new byte[DigestSize], TpmCcConstants.TPM_CC_Unseal, SessionAlg, expected);
    ...
    /// 19.7's wholesale replacement), so three assertions leave the balance one above the session's own start,
    ...
    public async Task PolicyGetDigestBorrowsTheSessionDigestSoTheNextAssertionStillFoldsOverIt()
    {
        ...
        try
        {
            ...
            byte[] expected = new byte[DigestSize];
            _ = TpmPolicyDigest.ExtendForAuthValue(afterFirst, SessionAlg, expected);
    ...
    /// <summary>
    /// The first-writer-wins cpHash latch of TPM 2.0 Library Part 3, Section 23.2.4 is a conditional TRANSFER: an
    ...
    /// Part 3, Section 23.2.4's immutability rule), and the refusing arm returns both the rejected cpHash carrier
    ...
    /// Section 23.4) folds inside the minting effect, so that effect owns the qualifier it consumed and returns
    ...
    public async Task PolicySecretWithATicketReturnsEveryCarrierTheMintingEffectConsumed()
    {
        ...
        try
        {
            ...
            byte[] expected = new byte[DigestSize];
            _ = TpmPolicyDigest.ExtendForSecret(new byte[DigestSize], PermanentHandleName(TpmRh.TPM_RH_OWNER), PolicyRef, SessionAlg, expected);
    ...
    /// plain <c>TPM_RC_BAD_AUTH</c>, TPM 2.0 Library Part 1, clause 17.8.1) never reaches the fold, so the
    ...
    /// clause 10.9.5, Table 123), so every branch carrier the parser rented reaches the pool again — on the
    ...
    public async Task PolicyOrReturnsEveryBranchCarrierOnBothArms()
    {
        ...
        try
        {
            ...
            Assert.AreEqual(
                TpmRcConstants.TPM_RC_VALUE, realResult.ResponseCode,
    ...
    /// Part 1, clause 17.6.17's "clear all associated context"): the advanced policyDigest and the latched
    ...
    /// caller's cpHash, and then, in the same command, the context reset of TPM 2.0 Library Part 3, Section
    ...
    /// refused with <c>TPM_RC_AUTH_UNAVAILABLE</c> before any of this is reached (Part 1, clause 11.2, Table 5).
    /// </remarks>
    [TestMethod]
    public async Task SelfReferentialPolicySecretOverAPolicySessionIsNetZeroOnThePool()
    {
        ...
        _ = TpmPolicyDigest.ExtendForAuthValue(new byte[DigestSize], SessionAlg, authValueDigest);
        ...
        using var authorizer = new TpmSession(new TpmHandle(sessionHandle), startResponse.NonceTPM, SessionAlg, trackingPool.Pool);
        try
        {
            long baseline = trackingPool.OutstandingCount;
    ...
    /// may carry is refused with a bare <c>TPM_RC_SIZE</c> at the wire read — the <c>{:sizeof(UINT64)}</c> bound
    /// of TPM 2.0 Library Part 2, clause 10.4.10, Table 100, answered ahead of the command body exactly where
    ...
    public async Task PolicyTicketWithAnOverWideTimeoutIsRefusedAtTheParseWithoutRenting()
    {
        ...
        try
        {
            ...
            Assert.AreEqual(
                ...
                "A timeout wider than sizeof(UINT64) is a bare TPM_RC_SIZE, answered at the parse whatever the session is.");
            Assert.AreEqual(
                baseline, trackingPool.OutstandingCount,
    ...
    /// ever evaluated — the order Part 4's <c>TPM2_PolicyTicket()</c> gives on printed page 654, where
    ...
    public async Task PolicyTicketWithAShortTimeoutOnATrialSessionIsRefusedOnTheSessionAttributes()
    {
        ...
        try
        {
            ...
            Assert.AreEqual(
                TpmRcConstants.TPM_RC_ATTRIBUTES, code,
    ...
    /// may carry (<c>{:sizeof(TPMU_HA)}</c>, TPM 2.0 Library Part 2, clause 10.4.2, Table 92) is refused with a
    /// bare <c>TPM_RC_SIZE</c> at the wire read, ahead of the rental whose own factory would refuse the same
    ...
    public async Task PolicySecretWithAnOverWideCpHashIsRefusedAtTheParseWithoutRenting()
    {
        ...
        try
        {
            ...
            Assert.AreEqual(
                TpmRcConstants.TPM_RC_SIZE, code,
                "A cpHashA wider than sizeof(TPMU_HA) is a bare TPM_RC_SIZE, answered at the parse.");
            Assert.AreEqual(
                baseline, trackingPool.OutstandingCount,
                "A parse refused on a wire bound must rent nothing, so the balance cannot move.");
        }
    ...
    /// may carry (<c>{:sizeof(TPMU_HA)}</c>, TPM 2.0 Library Part 2, clause 10.4.2, Table 92) is refused with a
    /// bare <c>TPM_RC_SIZE</c> at the wire read, ahead of the rental and ahead of the signature the same frame
    ...
    /// may carry (<c>{:sizeof(TPMU_HA)}</c>, TPM 2.0 Library Part 2, clause 10.4.2, Table 92) is refused with a
    /// bare <c>TPM_RC_SIZE</c> at the wire read — after the timeout the same frame carries has already been
    ...
    /// <c>TPM2B_DIGEST</c> buffer may carry (<c>{:sizeof(TPMU_HA)}</c>, TPM 2.0 Library Part 2, clause 10.4.2,
    /// Table 92) is refused with a bare <c>TPM_RC_SIZE</c> at the wire read, ahead of the rental and ahead of
    ...
    public async Task PolicyAuthorizeWithAnOverWideApprovedPolicyIsRefusedAtTheParseWithoutRenting()
    {
        ...
        try
        {
            ...
            Assert.AreEqual(
                TpmRcConstants.TPM_RC_SIZE, code,
                "An approvedPolicy wider than sizeof(TPMU_HA) is a bare TPM_RC_SIZE, answered at the parse.");
            Assert.AreEqual(
                baseline, trackingPool.OutstandingCount,
                "A parse refused on a wire bound must rent nothing, so the balance cannot move.");
        }
    ...
    /// <c>TPM2B_DIGEST</c> buffer may carry is refused with a bare <c>TPM_RC_SIZE</c> at the wire read: the
    /// ticket's digest field is a <c>TPM2B_DIGEST</c> in its own right (<c>TPMT_TK_VERIFIED</c>, TPM 2.0 Library
    /// Part 2, clause 10.7.4, Table 110), so it takes the <c>{:sizeof(TPMU_HA)}</c> bound of Table 92 too — and
    ...
    public async Task PolicyAuthorizeWithAnOverWideCheckTicketDigestIsRefusedAtTheParseWithoutRenting()
    {
        ...
        try
        {
            ...
            Assert.AreEqual(
                TpmRcConstants.TPM_RC_SIZE, code,
                "A checkTicket digest wider than sizeof(TPMU_HA) is a bare TPM_RC_SIZE, answered at the parse.");
            Assert.AreEqual(
                baseline, trackingPool.OutstandingCount,
    ...
    /// <summary>
    /// A <c>TPM2_PolicyAuthorize()</c> whose <c>checkTicket</c> carries a tag other than <c>TPM_ST_VERIFIED</c>
    /// is refused with a bare <c>TPM_RC_TAG</c> at the wire read. TPM 2.0 Library Part 3, clause 23.16, printed
    /// page 227 states the requirement: "The unmarshaling process requires that a proper TPMT_TK_VERIFIED be
    /// provided for checkTicket but it may be a NULL Ticket." Part 2, clause 10.7.4, Table 110 fixes the tag to
    /// <c>TPM_ST_VERIFIED</c> and names <c>TPM_RC_TAG</c> for anything else — and the NULL Ticket carries that
    /// same tag, so the gate admits it.
    /// </summary>
    [TestMethod]
    public async Task PolicyAuthorizeWithACheckTicketTagOtherThanVerifiedIsRefusedAtTheParseWithoutRenting()
    {
        using var trackingPool = new MeteredHousePool();
        using TpmSimulator simulator = await CreateOperationalAsync(trackingPool.Pool).ConfigureAwait(false);
        using TpmDevice tpm = TpmDevice.Create(simulator.SubmitAsync);
        ...
        try
        {
            ...
            Assert.AreEqual(
                TpmRcConstants.TPM_RC_TAG, code,
                "A checkTicket tag other than TPM_ST_VERIFIED is a bare TPM_RC_TAG, answered at the parse (Part 2, Table 110).");
            Assert.AreEqual(
                baseline, trackingPool.OutstandingCount,
    ...
    /// <c>TPMI_RH_HIERARCHY</c>'s admitted set is refused with a bare <c>TPM_RC_VALUE</c> at the wire read.
    /// <c>TPMT_TK_VERIFIED.hierarchy</c> is typed <c>TPMI_RH_HIERARCHY+</c> (TPM 2.0 Library Part 2, clause
    /// 10.7.4, Table 110), whose own table names <c>TPM_RC_VALUE</c> for a value outside
    /// <c>{TPM_RH_OWNER, TPM_RH_PLATFORM, TPM_RH_ENDORSEMENT, TPM_RH_NULL}</c> (clause 9.13, Table 60) — the
    /// unmarshaling requirement clause 23.16, printed page 227 states for this parameter.
    /// </summary>
    [TestMethod]
    public async Task PolicyAuthorizeWithAnOutOfSetCheckTicketHierarchyIsRefusedAtTheParseWithoutRenting()
    {
        ...
        try
        {
            ...
            Assert.AreEqual(
                TpmRcConstants.TPM_RC_VALUE, code,
                "A checkTicket hierarchy outside TPMI_RH_HIERARCHY is a bare TPM_RC_VALUE, answered at the parse (Part 2, Table 60).");
            Assert.AreEqual(
                baseline, trackingPool.OutstandingCount,
                "The three admitted fields ahead of it are rented only after every wire check has passed, so nothing is outstanding.");
        }
    ...
    /// <c>TPM2B_DIGEST</c> buffer may carry (<c>{:sizeof(TPMU_HA)}</c>, TPM 2.0 Library Part 2, clause 10.4.2,
    /// Table 92, over the <c>TPML_DIGEST</c> of clause 10.9.5, Table 123) is refused with a bare
    /// <c>TPM_RC_SIZE</c> — and the first branch, already read and admitted, leaves nothing orphaned, because
    ...
    public async Task PolicyOrWithAnOverWideSecondBranchIsRefusedAtTheParseWithoutRenting()
    {
        ...
        try
        {
            ...
            Assert.AreEqual(
                TpmRcConstants.TPM_RC_SIZE, code,
                "A branch wider than sizeof(TPMU_HA) is a bare TPM_RC_SIZE, answered at the parse.");
            Assert.AreEqual(
                baseline, trackingPool.OutstandingCount,
    ...
    /// Part 3, Section 23.3) folds inside the verification effect, so that effect returns the qualifier and the
    ...
    /// (TPM 2.0 Library Part 3, Section 23.3), and the refusing continuation is the terminal owner of every
        ...
        try
        {
...

            {
                ...
                Assert.AreEqual(
                    ...
                    "A signature that does not verify against aHash is TPM_RC_SIGNATURE.");
            }
    ...
    /// Part 3, Section 23.5) folds inside the re-verification effect, which is the terminal owner of the ticket
    ...
    /// with <c>TPM_RC_TICKET</c> (TPM 2.0 Library Part 3, Section 23.5), and the refusing continuation returns
        ...
        try
        {
...

            {
                ...
                Assert.AreEqual(
                    ...
                    "A ticket whose digest does not reproduce the TPM's recompute is TPM_RC_TICKET.");
            }
    ...
    /// Part 3, Section 23.16) folds inside the ticket-verification effect: the reset-and-refold replaces the
    ...
    public async Task PolicyAuthorizeVerifiedAgainstARealTicketLeavesExactlyOneLivePolicyDigest()
    {
        ...
        _ = TpmPolicyDigest.ExtendForCommandCode(new byte[DigestSize], TpmCcConstants.TPM_CC_Unseal, SessionAlg, approvedPolicy);
...

        {
            using PolicyAuthorizeInput input = PolicyAuthorizeInput.Create(
                sessionHandle, approvedPolicy, PolicyRef, keySign, approval.TicketTag, approval.TicketHierarchy, approval.TicketDigest, trackingPool.Pool);
    ...
    /// recompute is refused with <c>TPM_RC_VALUE</c> (TPM 2.0 Library Part 3, Section 23.16) — but only after
    /// the verification effect has run, which is the arm where all four wire carriers have already travelled
    /// request to action to effect. The refusing continuation returns them, leaving the session's own digest in
    /// place and the balance exactly where the command found it.
    /// </summary>
    [TestMethod]
    public async Task PolicyAuthorizeRefusedOnTheTicketLeavesTheSessionsBalanceUntouched()
    {
        ...
        try
        {
...

            {
                ...
                Assert.AreEqual(
                    TpmRcConstants.TPM_RC_VALUE, result.ResponseCode,
                    "A checkTicket that does not reproduce the TPM's recompute is TPM_RC_VALUE.");
            }
    ...
    /// Part 1, clause 14, Table 6) — the term <c>PolicySecret</c>'s <c>PolicyUpdate</c> folds for a hierarchy.
    /// </summary>
    ...
    /// identifier followed by a digest of that hash's width (TPM 2.0 Library Part 2, clause 10.5.3, Table 104).
    /// </summary>
    /// <returns>The Name octets.</returns>
    ...
    /// <summary>
    /// Frames a <c>TPM2_PolicyTicket()</c> body (TPM 2.0 Library Part 3, Section 23.5, Table 131): the policy
    ...
    /// <summary>
    /// Frames a <c>TPM2_PolicySecret()</c> body (TPM 2.0 Library Part 3, Section 23.4): the two handles, a
    ...
    /// <summary>
    /// Frames a <c>TPM2_PolicySigned()</c> body (TPM 2.0 Library Part 3, Section 23.3, Table 124): the two
    ...
    /// <summary>
    /// Frames a <c>TPM2_PolicyAuthorize()</c> body (TPM 2.0 Library Part 3, Section 23.16, Table 150): the
    ...
    /// TPM 2.0 Library Part 3, Section 23.3), and hands the octets back so every carrier the signing step
    ...
    /// policyRef)</c>, TPM 2.0 Library Part 3, Section 23.16), returning the ticket as plain octets so the
        ...
        TpmDevice tpm, TpmResponseRegistry registry, BaseMemoryPool pool, TpmiDhObject authorityHandle, byte[] approvedPolicy)
    {
        ...
        return new VerifiedApproval(
            (ushort)verified.Validation.Tag, verified.Validation.Hierarchy.Value, verified.Validation.Digest.ToArray());
    }

    /// <summary>
    /// Mints a real <c>TPM_ST_AUTH_SECRET</c> ticket against the owner hierarchy over a session-bound
    /// <c>TPM2_PolicySecret()</c> (a negative expiration, TPM 2.0 Library Part 3, Section 23.4), then flushes
    ...
    private sealed record VerifiedApproval(ushort TicketTag, uint TicketHierarchy, byte[] TicketDigest);
    ...
    {
        var simulator = new TpmSimulator("tpm-in-house-policy-carriers", signingBackend: BouncyCastleTpmEccSigningBackend.Create());
