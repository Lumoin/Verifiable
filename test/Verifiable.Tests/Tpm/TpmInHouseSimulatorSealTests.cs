...
internal sealed class TpmInHouseSimulatorSealTests
{
    ...
    public async Task SealedSecretUnsealsUnderAnRsaStorageParent()
    {
        ...
        var simulator = new TpmSimulator(
            "tpm-in-house-seal-rsa", signingBackend: BouncyCastleTpmEccSigningBackend.Create(), rsaSigningBackend: MicrosoftTpmRsaSigningBackend.Create(), rng: TestEntropy.NewCounterStream(), timeProvider: new FakeTimeProvider(TestClock.CanonicalEpoch));
    ...
    /// <c>TPM_RC_ATTRIBUTES</c> before any object is built (TPM 2.0 Library Part 3, clause 12.1, Table 18:
    ...
    public async Task CreateTemplateWithFixedTpmSetAndFixedParentClearIsRefusedWithAttributes()
    {
        ...
        TpmResult<CreateResponse> createResult = await TpmCommandExecutor.ExecuteAsync<CreateResponse>(
            tpm, createInput, [createParentAuth], null, pool, registry, TestContext.CancellationToken).ConfigureAwait(false);
        Assert.AreEqual(
            HmacKeyHarness.ParameterEncodedRc(TpmRcConstants.TPM_RC_ATTRIBUTES, 1), createResult.ResponseCode,
    ...
    public async Task LoadWithTruncatedPrivateBlobReturnsInsufficient()
    {
        ...
        Assert.AreEqual(HmacKeyHarness.ParameterEncodedRc(TpmRcConstants.TPM_RC_INSUFFICIENT, 0), code, "inPrivate is TPM2_Load()'s first parameter (Table 243, index 0); a declared size past the frame's own end must be refused with parameter-encoded TPM_RC_INSUFFICIENT.");
    }
    ...
    public async Task LoadWithZeroSizePrivateBlobFails()
    {
        ...
        Assert.AreEqual(HmacKeyHarness.ParameterEncodedRc(TpmRcConstants.TPM_RC_SIZE, 0), loadResult.ResponseCode, "A zero-size inPrivate cannot supply the simulator's own userAuth-length prefix.");
    }
    ...
    /// with <c>TPM_RC_REFERENCE_H0</c> (TPM 2.0 Library Part 3, clause 5.4, step 2.1) and releases the request's
    ...
    public async Task LoadWithUnknownParentHandleReleasesTheParsedPrivateBlob()
    {
        ...
        //carriers, would be what the assertion is really observing.
        {
            ...
            Assert.AreEqual(TpmRcConstants.TPM_RC_REFERENCE_H0, result.ResponseCode, "An unloaded transient parentHandle at index 0 answers TPM_RC_REFERENCE_H0 (TPM 2.0 Library Part 3, clause 5.4, step 2.1).");
        }
    ...
    /// The HMAC-authorized path enforces the identical check at the same
    ...
    public async Task PlainPasswordUnsealRejectsUserWithAuthClearSealedObject()
    {
        ...
        Assert.AreEqual(HmacKeyHarness.SessionEncodedRc(TpmRcConstants.TPM_RC_POLICY_FAIL, 0), unsealResult.ResponseCode,
    ...
    /// <summary>
    /// <c>itemHandle</c> is <c>TPM2_Unseal()</c>'s sole handle (index 0, TPM 2.0 Library Part 3, Table 30); a
    /// TRANSIENT-range value naming no loaded object is <c>TPM_RC_REFERENCE_H0</c> (clause 5.4, step 2.1).
    /// </summary>
    [TestMethod]
    public async Task UnsealWithUnknownItemHandleAnswersReferenceH0()
    {
        BaseMemoryPool pool = BaseMemoryPool.Shared;
        using TpmSimulator simulator = await CreateOperationalAsync(pool).ConfigureAwait(false);
        using TpmDevice tpm = TpmDevice.Create(simulator.SubmitAsync, BaseMemoryPool.Shared, TestEntropy.NewCounterStream());
        ...
        //No object was loaded, so the transient handle resolves nowhere (TPM 2.0 Library Part 3, clause 5.4, step 2.1).
        using TpmPasswordSession itemAuth = TpmPasswordSession.CreateEmpty(pool);
        ...
        Assert.AreEqual(
            TpmRcConstants.TPM_RC_REFERENCE_H0, unsealResult.ResponseCode,
            "itemHandle is TPM2_Unseal()'s sole handle (index 0); a TRANSIENT-range value naming no loaded object is TPM_RC_REFERENCE_H0 (TPM 2.0 Library Part 3, clause 5.4, step 2.1).");
    }
    ...
    public async Task SealUnderNonStorageParentReturnsType()
    {
        ...
        //rejected with TPM_RC_TYPE (TPM 2.0 Library Part 3, clause 12.1).
        ...
        Assert.AreEqual(HmacKeyHarness.HandleEncodedRc(TpmRcConstants.TPM_RC_TYPE, 0), createResult.ResponseCode, "Table 18: parentHandle is TPM2_Create()'s sole handle (handle 1); a non-storage parent is handle-encoded TPM_RC_TYPE at index 0.");
    }
    ...
    /// <c>TPM2_Create()</c>'s single-session form with <c>TPM_RC_LOCKOUT</c> when the parent is
    ...
    /// which covers the same gate in the two-session form.
    /// </summary>
    [TestMethod]
    public async Task LockedOutTpmRejectsPlainPasswordCreateOverDaProtectedParent()
    {
        ...
        try
        {
            ...
            //locked-out) DA-protected parent — the single-session form handles directly.
    ...
    public async Task CreateOverUnboundHmacSessionVerifiesTheParentsAuthValue()
    {
        ...
        Assert.AreEqual(SessionEncodedRc(TpmRcConstants.TPM_RC_AUTH_FAIL, sessionIndex: 0), wrong.ResponseCode, "parentHandle's authorizing session is session 1 of Table 18 (TPM 2.0 Library Part 2, clause 6.6.2); a session folding the wrong parent authValue is session-encoded TPM_RC_AUTH_FAIL there (the parent is DA-protected).");
    ...
    /// 16.6.10, equation 20), so the command HMAC omits it (equations 21/22) — the same bind-omission rule
    /// this simulator applies at every bound session slot, parent or signing key alike. Proves TPM2_Create()'s
    ...
    public async Task CreateOverSessionBoundToTheParentItselfSeals()
    {
        ...
        StartAuthSessionInput startInput = StartAuthSessionInput.CreateBoundUnsaltedHmacSession(parentHandle, SessionAlg, TestEntropy.NewCounterStream(), pool);
    ...
    /// clause 12.1) — the same inline-compare discipline the single-session plain form applies: a session
    /// pair carrying the CORRECT password succeeds and moves no
    ...
    /// <c>TPM_RS_PW</c> session is refused with the <c>TPM_RC_POLICY_FAIL</c>, session-encoded to the same index even when it carries the
    ...
    public async Task PlainPasswordCreateUnderUserWithAuthClearParentIsRefusedWithoutComparingThePassword()
    {
        ...
        Assert.AreEqual(
            ...
            "A userWithAuth-CLEAR parent must refuse a plain TPM_RS_PW TPM2_Create() at parentHandle, session 1 of Table 18 " +
    ...
    /// <summary>
    /// TPM2_Create()'s parent slot, over an UNBOUND unsalted HMAC session (the two-session form),
    /// applies check 7.1 of clause 5.6 BEFORE the queued command
    /// HMAC verification (checks 9/10): a userWithAuth-CLEAR parent refuses at parentHandle, session 1 of
    /// Table 18, even when the session folds a WRONG authValue guess as its entity term (TPM
    ...
    public async Task CreateOverUnboundHmacSessionUnderUserWithAuthClearParentIsRefusedWithoutComparingTheGuess()
    {
        ...
        Assert.AreEqual(
            HmacKeyHarness.SessionEncodedRc(TpmRcConstants.TPM_RC_POLICY_FAIL, 0), result.ResponseCode,
            "A userWithAuth-CLEAR parent must refuse an over-session TPM2_Create() at parentHandle, session 1 of Table 18, " +
    ...
    /// separate decrypt-attributed HMAC companion (slot 1) protecting <c>inSensitive</c> — the two-session
    /// form's inline-compare arm — applies check 7.1 of clause 5.6 identically to the
    /// single-session plain form: a userWithAuth-CLEAR parent refuses at parentHandle, session 1 of Table 18,
    ...
    public async Task CreateOverPasswordSlotWithDecryptSessionUnderUserWithAuthClearParentIsRefused()
    {
        ...
        Assert.AreEqual(
            ...
            $"at parentHandle, session 1 of Table 18 (Part 3, clause 5.6, check 7.1) even though the TPM_RS_PW slot " +
    ...
    /// parent refuses a plain <c>TPM_RS_PW</c> Load at parentHandle, session 1 of Table 20, even when the
    ...
    public async Task LoadUnderUserWithAuthClearParentIsRefusedWithoutComparingThePassword()
    {
        ...
        Assert.AreEqual(
            HmacKeyHarness.SessionEncodedRc(TpmRcConstants.TPM_RC_POLICY_FAIL, 0), loadResult.ResponseCode,
            "A userWithAuth-CLEAR parent must refuse a plain TPM_RS_PW TPM2_Load() at parentHandle, session 1 of Table 20, " +
            $"(Part 3, clause 5.6, check 7.1) even though the session carries the parent's genuine password " +
    ...
    /// guess as the folded entity term with the <c>TPM_RC_POLICY_FAIL</c>, session-encoded to the same index — never the
    ...
    public async Task UnsealOverHmacSessionAgainstUserWithAuthClearObjectIsRefusedWithoutComparingTheGuess()
    {
        ...
        try
        {
            using TpmSession itemAuthSession = new(new TpmHandle(sessionHandle), started.NonceTPM, SessionAlg, TestEntropy.NewCounterStream(), pool);
            ...
            Assert.AreEqual(
                HmacKeyHarness.SessionEncodedRc(TpmRcConstants.TPM_RC_POLICY_FAIL, 0), unsealResult.ResponseCode,
                "A userWithAuth-CLEAR sealed object must refuse an over-session Unseal at itemHandle, session 1 of Table 30, " +
    ...
    /// since <c>inPrivate</c> is <c>TPM2_Load()</c>'s first parameter to be read and a truncated declaration refuses
        ...
        TpmDevice tpm, TpmResponseRegistry registry, BaseMemoryPool pool, uint parentHandle, ReadOnlyMemory<byte> parentName, ReadOnlyMemory<byte> suppliedParentAuth)
    {
        ...
        try
        {
            using TpmSession parentAuthSession = new(new TpmHandle(sessionHandle), started.NonceTPM, SessionAlg, TestEntropy.NewCounterStream(), pool);
    ...
    /// <c>inSensitive</c> — the two-session shape routes to the over-sessions form's password-slot arm. The
    /// companion session is flushed before
    /// return.
    /// </summary>
    /// <param name="tpm">The TPM device.</param>
    /// <param name="registry">The response codec registry.</param>
    ...
    {
        StartAuthSessionInput decryptStartInput = StartAuthSessionInput.CreateUnboundUnsaltedHmacSession(SessionAlg, TestEntropy.NewCounterStream(), pool, TpmtSymDef.Xor(SessionAlg));
        ...
        try
        {
            using TpmSession decryptSession = new(
                new TpmHandle(decryptSessionHandle), decryptStarted.NonceTPM, SessionAlg, TestEntropy.NewCounterStream(), pool, TpmtSymDef.Xor(SessionAlg));
    ...
    {
        var simulator = new TpmSimulator("tpm-in-house-seal", signingBackend: BouncyCastleTpmEccSigningBackend.Create(), rng: TestEntropy.NewCounterStream(), timeProvider: new FakeTimeProvider(TestClock.CanonicalEpoch));
        await simulator.PowerOnAsync(TestContext.CancellationToken).ConfigureAwait(false);
        await BringOperationalAsync(simulator, pool).ConfigureAwait(false);
