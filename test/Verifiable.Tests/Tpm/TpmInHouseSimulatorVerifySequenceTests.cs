...
internal sealed class TpmInHouseSimulatorVerifySequenceTests
{
    ...
    public async Task VerifySequenceStartOnAStorageParentReturnsKey()
    {
        ...
        Assert.AreEqual(HmacKeyHarness.HandleEncodedRc(TpmRcConstants.TPM_RC_KEY, 0), startResult.ResponseCode, "A storage parent's sign attribute is CLEAR; TPM2_VerifySequenceStart() must refuse it with TPM_RC_KEY.");
    }
    ...
    public async Task VerifySequenceStartOnANullSchemeRsaTemplateReturnsScheme()
    {
        ...
        Assert.AreEqual(HmacKeyHarness.HandleEncodedRc(TpmRcConstants.TPM_RC_SCHEME, 0), startResult.ResponseCode, "A key created with a NULL template scheme retains none; TPM2_VerifySequenceStart() must refuse it with TPM_RC_SCHEME.");
    }
    ...
    public async Task VerifySequenceStartOnALoadedSealedObjectHandleReturnsKey()
    {
        ...
        Assert.AreEqual(HmacKeyHarness.HandleEncodedRc(TpmRcConstants.TPM_RC_KEY, 0), startResult.ResponseCode, "A sealed (KEYEDHASH data) object is not a verification key; TPM2_VerifySequenceStart() must refuse it with TPM_RC_KEY.");
    }

    /// <summary>
    /// An unloaded transient-range <c>keyHandle</c> is refused with <c>TPM_RC_REFERENCE_H0</c>
    /// (<see href="https://trustedcomputinggroup.org/resource/tpm-library-specification/">TPM 2.0 Library Part 3,
    /// clause 5.4, step 2.1</see>: "the handle shall reference a loaded object (TPM_RC_REFERENCE_H0 + N …)").
    /// </summary>
    [TestMethod]
    public async Task VerifySequenceStartOnAnUnknownHandleReturnsReferenceH0()
    {
        BaseMemoryPool pool = BaseMemoryPool.Shared;
        using TpmSimulator simulator = await CreateOperationalAsync(pool).ConfigureAwait(false);
        using TpmDevice tpm = TpmDevice.Create(simulator.SubmitAsync, BaseMemoryPool.Shared, TestEntropy.NewCounterStream());
        ...
        Assert.AreEqual(TpmRcConstants.TPM_RC_REFERENCE_H0, startResult.ResponseCode, "An unloaded transient-range keyHandle must be refused with TPM_RC_REFERENCE_H0.");
    }
    ...
    public async Task VerifySequenceStartWithANonEmptyHintHandFramedReturnsSize()
    {
        ...
        Assert.AreEqual(HmacKeyHarness.ParameterEncodedRc(TpmRcConstants.TPM_RC_SIZE, 1), code, "A non-empty hint under ECDSA must be refused with TPM_RC_SIZE.");
    }
    ...
    public async Task VerifySequenceStartWithANonEmptyContextHandFramedReturnsSize()
    {
        ...
        Assert.AreEqual(HmacKeyHarness.ParameterEncodedRc(TpmRcConstants.TPM_RC_SIZE, 2), code, "A non-empty context under any scheme this simulator executes must be refused with TPM_RC_SIZE (Table 220's empty[0] arm).");
    }

    /// <summary>
    /// Table 89 conditions the tag on an audit or decrypt session, matching
    /// <c>TPM2_SignSequenceStart()</c>'s own posture for the identical Auth Index None handle: a
    /// <c>TPM_ST_SESSIONS</c> frame with no well-formed authorization area behind it reads its own (empty)
    /// <c>auth</c>/<c>hint</c>/<c>context</c> octets as a bogus <c>authorizationSize</c> and is refused with the
    /// area's own structural code, <c>TPM_RC_AUTHSIZE</c> (clause 5.5, step 4.3)
    /// (<see href="https://trustedcomputinggroup.org/resource/tpm-library-specification/">TPM 2.0 Library
    /// Specification</see>, Part 3: Commands, clause 17.6, Table 89, clause 5.5).
    /// </summary>
    [TestMethod]
    public async Task VerifySequenceStartFramedWithSessionsReturnsAuthsize()
    {
        BaseMemoryPool pool = BaseMemoryPool.Shared;
        using TpmSimulator simulator = await CreateOperationalAsync(pool).ConfigureAwait(false);
        using TpmDevice tpm = TpmDevice.Create(simulator.SubmitAsync, BaseMemoryPool.Shared, TestEntropy.NewCounterStream());
    ...
    /// (<see href="https://trustedcomputinggroup.org/resource/tpm-library-specification/">TPM 2.0 Library
    /// Specification</see>, Part 2: Structures, clause 8.3.2, Table 37; clause 8.3.3.15, Bit[19]).
    /// </summary>
    [TestMethod]
    ...
    public async Task VerifySequenceCompleteWithACorruptedSignatureReturnsSignatureAndTheSequenceSurvives()
    {
        ...
        Assert.AreEqual(HmacKeyHarness.ParameterEncodedRc(TpmRcConstants.TPM_RC_SIGNATURE, 0), badResult.ResponseCode, "A corrupted signature must be refused with TPM_RC_SIGNATURE at signature, parameter 1 of Table 118.");
    ...
    public async Task VerifySequenceCompleteWithKeyHandleResolvingToASealedObjectReturnsKey()
    {
        ...
        Assert.AreEqual(HmacKeyHarness.HandleEncodedRc(TpmRcConstants.TPM_RC_KEY, 1), result.ResponseCode, "A keyHandle resolving to a sealed object is not a verification key; TPM2_VerifySequenceComplete() must refuse it with TPM_RC_KEY.");
    }
    ...
    public async Task VerifySequenceCompleteWithKeyHandleResolvingToTheSequenceItselfReturnsKey()
    {
        ...
        Assert.AreEqual(HmacKeyHarness.HandleEncodedRc(TpmRcConstants.TPM_RC_KEY, 1), result.ResponseCode, "A keyHandle resolving to the sequence's own handle is not a verification key; TPM2_VerifySequenceComplete() must refuse it with TPM_RC_KEY.");
    }
    ...
    public async Task VerifySequenceCompleteWithAnUnknownKeyHandleReturnsHandle()
    {
        ...
        Assert.AreEqual(HmacKeyHarness.HandleEncodedRc(TpmRcConstants.TPM_RC_HANDLE, 1), result.ResponseCode, "A keyHandle resolving nowhere must be refused with TPM_RC_HANDLE.");
    }
    ...
    public async Task VerifySequenceCompleteWithAnRsassaSignaturePresentedAsRsapssReturnsScheme()
    {
        ...
        Assert.AreEqual(HmacKeyHarness.HandleEncodedRc(TpmRcConstants.TPM_RC_SCHEME, 1), result.ResponseCode, "A genuine RSASSA signature presented under the claimed RSAPSS scheme must be refused with TPM_RC_SCHEME.");
    }
    ...
    public async Task VerifySequenceCompleteWithMismatchedHashAlgorithmReturnsScheme()
    {
        ...
        Assert.AreEqual(HmacKeyHarness.HandleEncodedRc(TpmRcConstants.TPM_RC_SCHEME, 1), result.ResponseCode, "A signature declaring SHA-384 against a SHA-256-schemed key must be refused with TPM_RC_SCHEME.");
    }
    ...
    public async Task VerifySequenceCompleteOnASigningSequenceReturnsMode()
    {
        ...
        Assert.AreEqual(HmacKeyHarness.HandleEncodedRc(TpmRcConstants.TPM_RC_MODE, 0), result.ResponseCode, "TPM2_VerifySequenceComplete() against a SIGNING sequence must be refused with TPM_RC_MODE.");
    }
    ...
    public async Task SignSequenceCompleteOnAVerificationSequenceReturnsMode()
    {
        ...
        Assert.AreEqual(HmacKeyHarness.HandleEncodedRc(TpmRcConstants.TPM_RC_MODE, 0), result.ResponseCode, "TPM2_SignSequenceComplete() against a VERIFICATION sequence must be refused with TPM_RC_MODE.");
    }
    ...
    /// TPM": a second Complete names <c>sequenceHandle</c> — its sole handle, index 0 — as a TRANSIENT-range value
    /// resolving to nothing loaded, <c>TPM_RC_REFERENCE_H0</c> (TPM 2.0 Library Part 3, clause 5.4, step 2.1);
    /// <c>TPM2_FlushContext()</c> on the same handle keeps its own clause's <c>TPM_RC_HANDLE</c>, parameter-encoded to the same index
    /// (<see href="https://trustedcomputinggroup.org/resource/tpm-library-specification/">TPM 2.0 Library
    ...
    public async Task VerifySequenceCompleteFlushesTheSequenceOnSuccess()
    {
        ...
        Assert.AreEqual(TpmRcConstants.TPM_RC_REFERENCE_H0, secondComplete.ResponseCode, "sequenceHandle is TPM2_VerifySequenceComplete()'s sole handle (index 0); an already-flushed sequence is a TRANSIENT-range value resolving to nothing loaded, TPM_RC_REFERENCE_H0 (TPM 2.0 Library Part 3, clause 5.4, step 2.1).");
    ...
    /// (<see href="https://trustedcomputinggroup.org/resource/tpm-library-specification/">TPM 2.0 Library
    /// Specification</see>, Part 3: Commands, clause 23.16.1; Part 2: Structures, clause 10.6.4, Table 111).
    /// </summary>
    [TestMethod]
    ...
    public async Task VerifySequenceCompleteRefusedOnABadSignatureReturnsTheParsedCarriersToPool()
    {
        ...
        Assert.AreEqual(HmacKeyHarness.ParameterEncodedRc(TpmRcConstants.TPM_RC_SIGNATURE, 0), badResult.ResponseCode, "A placeholder (non-matching) signature must be refused with TPM_RC_SIGNATURE at signature, parameter 1 of Table 118.");
    ...
    public async Task VerifySequenceStartWithAnAuthOver64OctetsOnTheWireReturnsSize()
    {
        ...
        Assert.AreEqual(HmacKeyHarness.ParameterEncodedRc(TpmRcConstants.TPM_RC_SIZE, 0), code, "auth is TPM2_VerifySequenceStart()'s first parameter (Table 89, index 0); one declared one octet over Tpm2bAuth.MaxSize must be refused with parameter-encoded TPM_RC_SIZE.");
    }
    ...
    public async Task VerifySequenceStartWithAHintOver57OctetsOnTheWireReturnsSize()
    {
        ...
        Assert.AreEqual(
            ...
            "Table 89: hint is TPM2_VerifySequenceStart()'s second parameter (index 1); one octet over Tpm2bSignatureHint.MaxSize must be refused with TPM_RC_SIZE there.");
    }
    ...
    public async Task VerifySequenceCompleteWithAnUnsupportedSigAlgReturnsScheme()
    {
        ...
        Assert.AreEqual(
            HmacKeyHarness.ParameterEncodedRc(TpmRcConstants.TPM_RC_SCHEME, 0), code,
            "Table 118: signature is TPM2_VerifySequenceComplete()'s sole parameter (index 0); a sigAlg outside {ECDSA, RSASSA, RSAPSS, HMAC} must be refused with parameter-encoded TPM_RC_SCHEME there at the TPMT_SIGNATURE parse.");
    }
