...
/// 246) against the in-house behavioural <see cref="TpmSimulator"/> — entirely in-process, with no external
...
/// state, and a parent's Qualified Name is hand-computed per Part 1, clause 22.5, mirroring the sibling
...
internal sealed class TpmInHouseSimulatorCreationDataTests
{
    ...
    /// Specification</see>, Part 2: Structures, clause 14.1, Table 261; Part 1: Architecture, clause 23.5;
    ...
    public async Task CreateChildUnderOwnerEccStorageParentCarriesTheParentsOwnNameAndQualifiedName()
    {
        ...
        Assert.IsTrue(
            ...
            "parentQualifiedName must be the parent's own Qualified Name: nameAlg || H_nameAlg(OWNER handle || parentName) (Part 1, clause 22.5) — every parent this model holds is a primary.");
    ...
    /// Name)</c> (TPM 2.0 Library Part 1, clause 22.5) — reading the nameAlg back out of <paramref name="name"/>'s
    ...
    {
        var simulator = new TpmSimulator("tpm-in-house-creation-data", signingBackend: BouncyCastleTpmEccSigningBackend.Create());
