...
internal sealed class TpmInHouseSimulatorEventSequenceTests
{
    ...
    /// <see href="https://trustedcomputinggroup.org/resource/tpm-library-specification/">TPM 2.0 Library Part 3, clause 17.9.1, Table 95</see>.
    /// </summary>
    [TestMethod]
    public async Task EventSequenceCompleteWithTheNullPcrReturnsTheDigestsFlushesAndExtendsNothing()
    {
        ...
        Assert.AreEqual(TpmRcConstants.TPM_RC_HANDLE, flush.ResponseCode, "The completed sequence must have been flushed.");
    }
    ...
    public async Task EventSequenceCompleteOnAHashSequenceAndOnASigningSequenceReturnsModeAndTheSequencesSurvive()
    {
        ...
        Assert.AreEqual(TpmRcConstants.TPM_RC_MODE, onHash.ResponseCode, "TPM2_EventSequenceComplete() on a hash sequence must be refused with TPM_RC_MODE.");
        ...
        Assert.AreEqual(TpmRcConstants.TPM_RC_MODE, onSigning.ResponseCode, "TPM2_EventSequenceComplete() on a signing sequence must be refused with TPM_RC_MODE.");
    ...
    /// <summary>
    /// Part 4 <c>EventSequenceComplete.c</c>: after the kind check, a non-NULL <c>pcrHandle</c> that locality 0 may
    /// not extend (PCR 17, PTP 1.07 Table 14) is <c>TPM_RC_LOCALITY</c>, and the sequence is left as it was — a
    /// later update still succeeds.
    /// <see href="https://trustedcomputinggroup.org/resource/pc-client-platform-tpm-profile-ptp-specification/">PC Client PTP 1.07, Table 14; TPM 2.0 Library Part 4, EventSequenceComplete.c</see>.
    /// </summary>
    [TestMethod]
    ...
    /// <summary>
    /// Part 2, Table 17: a handle that is "not correct for the use" is <c>TPM_RC_HANDLE</c> — an unknown transient
    /// handle and a signing key's handle alike; and clause 17.9.1: "If this command completes successfully, the
    /// sequenceHandle object will be flushed" — a second completion finds nothing.
    /// <see href="https://trustedcomputinggroup.org/resource/tpm-library-specification/">TPM 2.0 Library Part 2, clause 6.6.3, Table 17; Part 3, clause 17.9.1</see>.
    /// </summary>
    [TestMethod]
    public async Task EventSequenceCompleteOnAnUnknownOrKeyHandleReturnsHandleAndACompletedSequenceIsFlushed()
    {
        BaseMemoryPool pool = BaseMemoryPool.Shared;
        using TpmSimulator simulator = await CreateOperationalAsync(pool).ConfigureAwait(false);
        using TpmDevice tpm = TpmDevice.Create(simulator.SubmitAsync);
