...
internal sealed class TpmInHouseSimulatorHmacSessionTests
{
    ...
    /// <summary>
    /// <c>TPM2_HMAC()</c> authorized by a policy session that asserted <c>TPM2_PolicyAuthValue()</c> folds
    /// the key's authorization value into the command HMAC at use (TPM 2.0 Library Part 1, clause 16.6.12,
    /// equation 26) and returns the published value — the key's authValue is checked through a real HMAC at a
    /// command that consumes the key, which the password-only forms could not do.
    /// </summary>
    [TestMethod]
    ...
    /// Library Part 3, clause 23.18), and returns the published value.
    /// </summary>
    [TestMethod]
    public async Task HmacOverAPolicyPasswordSessionReturnsThePublishedValue()
    {
        ...
        try
        {
            ...
            using TpmPolicySession session = TpmPolicySession.ForSessionWithPassword(sessionHandle, SessionAlg, KeyPassword, TestEntropy.NewCounterStream(), pool);
    ...
    /// does not match (TPM 2.0 Library Part 3, clause 23.18; Part 4 <c>CheckPWAuthSession</c>).
    /// </summary>
    [TestMethod]
    public async Task HmacOverAPolicyPasswordSessionWithAWrongPasswordIsBadAuth()
    {
        ...
        try
        {
            ...
            using TpmPolicySession session = TpmPolicySession.ForSessionWithPassword(sessionHandle, SessionAlg, WrongKeyPassword, TestEntropy.NewCounterStream(), pool);
    ...
    public async Task HmacOverAPlainPolicyCommandCodeSessionReturnsThePublishedValue()
    {
        ...
        try
        {
            ...
            using TpmPolicySession session = TpmPolicySession.ForSession(sessionHandle, SessionAlg, TestEntropy.NewCounterStream(), pool);
    ...
    /// checked against the command it authorizes (TPM 2.0 Library Part 3, clause 23.4; Part 4
    ...
    public async Task HmacOverAPolicySessionBoundToAnotherCommandCodeIsRefusedWithPolicyCc()
    {
        ...
        try
        {
            ...
            Assert.AreEqual(HmacKeyHarness.SessionEncodedRc(TpmRcConstants.TPM_RC_POLICY_CC, 0), result.ResponseCode, "A command-code mismatch must be refused with TPM_RC_POLICY_CC.");
        }
        finally
        {
            await HmacKeyHarness.FlushIfPresentAsync(tpm, registry, pool, sessionHandle, TestContext.CancellationToken).ConfigureAwait(false);
    ...
    public async Task HmacOverAPolicySessionWhoseDigestDoesNotMatchIsRefusedWithPolicyFail()
    {
        ...
        try
        {
            ...
            Assert.AreEqual(HmacKeyHarness.SessionEncodedRc(TpmRcConstants.TPM_RC_POLICY_FAIL, 0), result.ResponseCode, "A policyDigest mismatch must be refused with TPM_RC_POLICY_FAIL.");
        }
        finally
        {
            await HmacKeyHarness.FlushIfPresentAsync(tpm, registry, pool, sessionHandle, TestContext.CancellationToken).ConfigureAwait(false);
    ...
    public async Task HmacOverAnHmacSessionOnAUserWithAuthClearKeyIsRefusedWithPolicyFail()
    {
        ...
        try
        {
            ...
            Assert.AreEqual(HmacKeyHarness.SessionEncodedRc(TpmRcConstants.TPM_RC_POLICY_FAIL, 0), result.ResponseCode, "An HMAC session on a userWithAuth-CLEAR key must be refused with TPM_RC_POLICY_FAIL.");
        }
        finally
        {
            session.Dispose();
    ...
    public async Task HmacOverAPolicySessionOnAKeyWithNoAuthPolicyIsRefusedWithPolicyFail()
    {
        ...
        try
        {
            ...
            Assert.AreEqual(HmacKeyHarness.SessionEncodedRc(TpmRcConstants.TPM_RC_POLICY_FAIL, 0), result.ResponseCode, "An empty authPolicy never equals a session's policyDigest: TPM_RC_POLICY_FAIL.");
        }
        finally
        {
            await HmacKeyHarness.FlushIfPresentAsync(tpm, registry, pool, sessionHandle, TestContext.CancellationToken).ConfigureAwait(false);
    ...
    /// <summary>
    /// <c>audit</c> is admitted on <c>TPM2_HMAC()</c>'s authorizing slot (TPM 2.0 Library Part 1, clause 17.1),
    /// so planting the bit onto the wire AFTER the command HMAC was computed over the original (audit-clear)
    /// octet is refused for the HMAC mismatch that rewrite creates — session-encoded <c>TPM_RC_BAD_AUTH</c> —
    /// never for the attribute itself.
    /// </summary>
    [TestMethod]
    public async Task HmacWithAPlantedAuditAttributeIsRefusedWithBadAuth()
    {
        TpmRcConstants responseCode = await PlantAttributeOnKeyedHashCommandAsync(
            nameof(HmacWithAPlantedAuditAttributeIsRefusedWithBadAuth), TpmCcConstants.TPM_CC_HMAC, TpmtSymDef.Null, TpmaSession.AUDIT).ConfigureAwait(false);

        Assert.AreEqual(
            HmacKeyHarness.SessionEncodedRc(TpmRcConstants.TPM_RC_BAD_AUTH, sessionIndex: 0), responseCode,
            "audit is admitted for the attribute; the wire-rewritten octet was never covered by the session's own command HMAC, so verification fails as a bad-auth mismatch.");
    }
    ...
    private static byte[] PolicyCommandCodeDigest(TpmCcConstants commandCode)
    {
        ...
        Span<byte> zero = stackalloc byte[size];
        zero.Clear();
        _ = TpmPolicyDigest.ExtendForCommandCode(zero, commandCode, SessionAlg, digest, BaseMemoryPool.Shared);
