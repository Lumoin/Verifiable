...
internal sealed class TpmInHouseSimulatorAttestCompanionSessionTests
{
    ...
    /// rather than accepted and echoed back auditing nothing (TPM 2.0 Library Part 2, clause 8.4, Table 38).
    ...
    public async Task QuoteSignSlotNamingAPersistentObjectHandleIsRefusedWithHandleAtItsOwnIndex()
    {
        ...
        //The first persistent-object handle (TPM_HT_PERSISTENT, Part 2, clause 7.4, Table 33): a well-formed
    ...
    /// bits Table 38 names, so neither can admit a bit the other refuses.
    /// </remarks>
    [TestMethod]
    ...
    /// 16.6.4, Table 15), and this command's first response parameter IS encryptable, so without that rule the
