...
/// Specification</see>, Part 3, clause 31.15.1: "If successful, the authorization secret (authValue) of the
...
/// </para>
/// <para>
/// <b>ADMIN role has no authValue path on an NV Index.</b> Part 3, clause 31.15.1 states the requirement
...
/// TPM_CC_NV_ChangeAuth". Part 1, clause 16.2's ADMIN bullet offers an authValue alternative only for an
...
internal sealed class TpmInHouseSimulatorNvChangeAuthTests
{
    ...
    /// <summary>The SHA-256 digest width in octets - the <c>newAuth</c> ceiling for a SHA-256 nameAlg Index (Part 1, clause 16.6.4.2).</summary>
    ...
    /// <summary>The declared data area size of a <c>TPM_NT_PIN_FAIL</c> Index: the whole 8-octet <c>TPMS_NV_PIN_COUNTER_PARAMETERS</c> (Part 2, clause 13.3).</summary>
    ...
    /// <c>TPM_NT_PIN_FAIL</c> with its spec-mandated <c>TPMA_NV_NO_DA</c> (TPM 2.0 Library Part 3, clause
    /// 31.3.1), Index-authValue reads, owner-hierarchy provisioning and reporting, and the
    ...
    /// the <c>TPMA_NV_WRITTEN</c> the first write sets (TPM 2.0 Library Part 3, clause 31.7) - for the Indexes
    ...
    /// Library Specification</see>, Part 3, clause 31.15.1). <c>pinLimit</c>, <c>TPMA_NV_WRITTEN</c>, and the
    ...
    /// 2.0 Library Specification</see>, Part 1, clause 16.6.5's policy note on the authValue term of the HMAC
    /// key, and clause 34.2.6.6's pinCount rule, which is written in terms of the authorization outcome). The
    ...
    /// (<see href="https://trustedcomputinggroup.org/resource/tpm-library-specification/">TPM 2.0 Library
    /// Specification</see>, Part 1, clause 34.2.6.6) - so the code carries no session-index modifier at all.
    ...
    /// 2.0 Library Specification</see>, Part 3, clause 31.15.1's unconditional "requires that a policy session
    /// be used"; Part 1, clause 16.2 offers the authValue alternative only for an object with
    ...
    /// reason the password arm is: Part 3, clause 31.15.1 requires a POLICY session, and the ADMIN role's
    /// authValue alternative in Part 1, clause 16.2 is scoped to objects with an <c>adminWithPolicy</c>
    ...
    /// (<see href="https://trustedcomputinggroup.org/resource/tpm-library-specification/">TPM 2.0 Library
    /// Specification</see>, Part 3, clause 31.15.1; Part 1, clause 16.2's ADMIN note repeats it) - is
    ...
    public async Task NvChangeAuthUnderAPolicyThatNeverBoundACommandCodeIsRefusedWithPolicyFail()
    {
        ...
        Assert.AreEqual(HmacKeyHarness.SessionEncodedRc(TpmRcConstants.TPM_RC_POLICY_FAIL, 0), result.ResponseCode, "nvIndex's authorizing session is session 1 of Table 269 (TPM 2.0 Library Part 2, clause 6.6.2); a policy that binds no command code is session-encoded TPM_RC_POLICY_FAIL there.");
    }
    ...
    /// the situation Part 3, clause 31.15.1's note describes ("administrative actions on nvIndex require
    ...
    /// <c>TPM_RC_ATTRIBUTES</c> rather than honoured. Part 1, clause 18.1's note is the reason: "A policy
    ...
    public async Task NvChangeAuthWithADecryptAttributedAuthorizingPolicySessionIsRefusedWithAttributes()
    {
        ...
        using TpmDevice tamperingDevice = TpmDevice.Create(ClaimDecryptOnChangeAuthAsync, BaseMemoryPool.Shared, TestEntropy.NewCounterStream());
    ...
    /// (<see href="https://trustedcomputinggroup.org/resource/tpm-library-specification/">TPM 2.0 Library
    /// Specification</see>, Part 1, clause 16.6.4.3) and only the remainder is measured against "the size of
    /// the digest produced by the nameAlg of the NV Index" (Part 3, clause 31.15.1; Part 1, clause 16.6.4.2).
    ...
    public async Task NvChangeAuthStripsTrailingZerosBeforeMeasuringNewAuthAgainstTheNameAlgDigestSize()
    {
        ...
        Assert.AreEqual(HmacKeyHarness.ParameterEncodedRc(TpmRcConstants.TPM_RC_SIZE, 0), overlongResult.ResponseCode, "Table 269: newAuth is TPM2_NV_ChangeAuth()'s sole parameter (parameter 1); a value wider than the nameAlg digest size is parameter-encoded TPM_RC_SIZE at index 0.");
    ...
    /// <summary>
    /// The response-key changeover, pinned on both sides at once. Part 3, clause 31.15.1: "Since the NV Index
    ...
    /// captured exchange (Part 1, clause 16.6.5's equation 17 over clause 15.8's rpHash): the NEW-keyed one
    ...
    public async Task NvChangeAuthResponseIsKeyedOnTheNewAuthValueAndAnOldKeyedOneIsRejectedByTheHost()
    {
        ...
        using TpmDevice forgingDevice = TpmDevice.Create(ForgeOldKeyedResponseAsync, BaseMemoryPool.Shared, TestEntropy.NewCounterStream());
    ...
    /// either - it only ever enters as the authorizing session's HMAC key term (Part 1, clause 16.6.5's
    ...
    /// (<see href="https://trustedcomputinggroup.org/resource/tpm-library-specification/">TPM 2.0 Library
    /// Specification</see>, Part 1, clause 16.6.9), and a decrypt-only session's <c>sessionValue</c> is that
    /// session key alone (clause 18.1), so the XOR keystream over <c>newAuth</c> (clause 18.2) derives from
    ...
    /// honestly. The salted overload folds a salt only the TPM can recover (clause 16.6.12, equation 25), so
    ...
    public async Task TheUnsaltedRotationYieldsToThePublicNonceKeystreamButTheSaltedOneDoesNot()
    {
        ...
        using(TpmDevice enrollDevice = TpmDevice.Create(simulator.SubmitAsync, BaseMemoryPool.Shared, TestEntropy.NewCounterStream()))
        {
    ...
    /// TPMS_NV_PUBLIC)</c>
    /// (<see href="https://trustedcomputinggroup.org/resource/tpm-library-specification/">TPM 2.0 Library
    /// Specification</see>, Part 1, clause 13, Table 9; Part 2, clause 13.6, Table 251). This transcribes
    /// that recipe independently - from the marshaled field order, with the policy digest re-derived from Part
    /// 3, clause 23.17's and clause 23.11's own extend formulas - and requires the Name the TPM reports to
    /// equal it. An enrollment that installed an Empty Policy (which can never satisfy an ADMIN check, Part 1,
    /// clause 10.2) would produce a different Name and fail here.
    /// </summary>
    [TestMethod]
    ...
    /// <c>pinCount</c>-neutral in BOTH directions. Part 3, clause 31.15.1 asks for nothing beyond that assertion
    /// to satisfy ADMIN role, and Part 1, clause 16.6.5's policy note confines the authValue term of the session
    /// HMAC key to a policy that ALSO asserted <c>TPM2_PolicyAuthValue</c>
    /// (<see href="https://trustedcomputinggroup.org/resource/tpm-library-specification/">TPM 2.0 Library
    /// Specification</see>) - so this session proves nothing about the current PIN, and clause 34.2.6.6's
    /// counter rule, written entirely in terms of the authorization outcome, has no outcome to record. The
    /// session is unbound and unsalted, so its session key is the Empty Buffer (clause 16.6.9) and the whole
    ...
    /// either direction. Part 1, clause 34.2.6.6 ties the counter to the authorization of the Index - "If the
    /// authorization fails, pinCount is incremented for a PIN Fail Index"
    /// (<see href="https://trustedcomputinggroup.org/resource/tpm-library-specification/">TPM 2.0 Library
    /// Specification</see>) - and the companion session authorizes no entity at all (Part 1, clause 18.1: a
    ...
    /// admits the rotation to exactly the bound <c>newAuth</c> and refuses any other value with
    /// <c>TPM_RC_POLICY_FAIL</c>, session-encoded to the same index — a policy may pin the value an Index may be rotated TO.
    /// <see href="https://trustedcomputinggroup.org/resource/tpm-library-specification/">TPM 2.0 Library Part 3, clause 23.24; clause 31.15, Table 269; Part 4 CompareParametersHash</see>.
    /// </summary>
    [TestMethod]
    public async Task NvChangeAuthUnderAParametersBindingRotatesOnlyToTheBoundNewAuth()
    {
        ...
        Assert.AreEqual(HmacKeyHarness.SessionEncodedRc(TpmRcConstants.TPM_RC_POLICY_FAIL, 0), otherResult.ResponseCode, "The pHash binding mismatch is the policy's own TPM_RC_POLICY_FAIL, session-encoded to the sole authorizing policy session.");
    ...
    public async Task NvChangeAuthWithATamperedDecryptSessionHmacIsRefusedWithoutMovingThePinCounter()
    {
        ...
        using TpmDevice splicingDevice = TpmDevice.Create(SpliceDecryptSessionHmacAsync, BaseMemoryPool.Shared, TestEntropy.NewCounterStream());
    ...
    /// 1, clause 34.2.6.6 makes the counter a function of the Index's own authorization outcome
    /// (<see href="https://trustedcomputinggroup.org/resource/tpm-library-specification/">TPM 2.0 Library
    /// Specification</see>), and a structural refusal - wrong authorization KIND (Part 3, clause 31.15.1), a
    /// policy of the wrong shape or scope (Part 1, clause 16.2's ADMIN note), or an inadmissible session
    /// attribute (Part 1, clause 18.1's note, refused by Part 3, clause 5.5 before clause 5.6's check 9 ever
    ...
    public async Task TheStructuralNvChangeAuthRefusalsLeaveThePinCounterWhereTheyFoundIt()
    {
        ...
        using TpmDevice decryptClaimingDevice = TpmDevice.Create(ClaimDecryptOnChangeAuthAsync, BaseMemoryPool.Shared, TestEntropy.NewCounterStream());
        ...
        Assert.AreEqual(HmacKeyHarness.SessionEncodedRc(TpmRcConstants.TPM_RC_POLICY_FAIL, 0), policyFailResult.ResponseCode, "A session that bound no command code can never assert ADMIN role.");
        ...
        Assert.AreEqual(HmacKeyHarness.SessionEncodedRc(TpmRcConstants.TPM_RC_POLICY_CC, 0), policyCcResult.ResponseCode, "A policy scoped to another command is TPM_RC_POLICY_CC, session-encoded to the sole authorizing slot, the digest having matched.");
    ...
    /// unsalted default every session key is the Empty Buffer (TPM 2.0 Library Part 1, clause 16.6.9), so the
    /// authorizing session's HMAC key is the stored PIN form alone (clause 16.6.5, equation 17): a transcript
    ...
    /// (<see href="https://trustedcomputinggroup.org/resource/tpm-library-specification/">TPM 2.0 Library
    /// Specification</see>, Part 3, clause 31.15.1's response-key sentence). The salted overload folds a
    /// KDFa-derived session key only the TPM holding the salt key can reproduce (clause 16.6.12, equation 25) in
    ...
    /// asserted (clause 19.1's note), which is the whole reason the replacement value rides a companion.
    /// </summary>
    [TestMethod]
    public async Task TheSaltedRotationClosesTheCommandAndResponseHmacOraclesTheUnsaltedOneLeavesOpen()
    {
        ...
        Assert.AreEqual(TpmSeConstants.TPM_SE_POLICY, authorizing.SessionType, "The session that authorizes an ADMIN-role command must be a POLICY session (Part 3, clause 31.15.1).");
        Assert.AreEqual((uint)TpmRh.TPM_RH_NULL, authorizing.Bind, "The authorizing session is unbound: Part 1, clause 34.2.8.3 forbids binding a session to a PIN Index outright.");
    ...
    /// alone (TPM 2.0 Library Part 1, clause 34.2.6.1) - so a test can establish an exact counter state without
    ...
    private static byte[] ComputePolicyDigest(TpmPolicy policy)
    {
        ...
        int written = policy.ComputeDigest(SessionAlg, digest, BaseMemoryPool.Shared);
    ...
    /// (<see href="https://trustedcomputinggroup.org/resource/tpm-library-specification/">TPM 2.0 Library
    /// Specification</see>, Part 3, clause 23.17), then
    ...
    /// <summary>
    /// Transcribes the MINIMAL rotation <c>authPolicy</c> - the one assertion Part 3, clause 31.15.1 actually
    ...
    /// 2.0 Library Specification</see>, Part 3, clause 23.11) folded over the all-zero starting digest a fresh
    /// policy session carries (Part 1, clause 16.7). Written out with <see cref="BinaryPrimitives"/> and the
    ...
    /// (<see href="https://trustedcomputinggroup.org/resource/tpm-library-specification/">TPM 2.0 Library
    /// Specification</see>, Part 2, clause 13.6, Table 251) hashed per Part 1, clause 13, Table 9. Uses
    ...
    /// (<see href="https://trustedcomputinggroup.org/resource/tpm-library-specification/">TPM 2.0 Library
    /// Specification</see>, Part 1, clause 16.6.5, equation 17). Callers supply the whole concatenated key: for
    /// an unbound, unsalted session it reduces to the authorization value alone, since such a session's key is
    /// the Empty Buffer (clause 16.6.9); for a salted session it is the term a transcript observer cannot
    ...
    /// (<see href="https://trustedcomputinggroup.org/resource/tpm-library-specification/">TPM 2.0 Library
    /// Specification</see>, Part 1, clause 15.7, equation 15), over the parameter area exactly as it crossed the
    ...
    /// (<see href="https://trustedcomputinggroup.org/resource/tpm-library-specification/">TPM 2.0 Library
    /// Specification</see>, Part 1, clause 16.6.5, equation 17, with clause 16.6.3.4's
    ...
    /// Library Specification</see>, Part 1, clause 16.6.9): XOR-decrypts with the command-direction mask keyed
    /// on that value (clause 18.2, <c>nonceNewer</c> = nonceCaller, <c>nonceOlder</c> = the session's nonceTPM,
    ...
    /// over it (TPM 2.0 Library Part 1, clause 18.2).
    /// </summary>
    ...
    /// Part 3, clause 11.1). <c>TPMT_SYM_DEF</c>'s <c>keyBits</c>/<c>mode</c> unions collapse to nothing under
    /// <c>TPM_ALG_NULL</c> (Part 2, clause 11.1.6, Table 162), and <c>authHash</c> is the last parameter, so the
    ...
    {
        var simulator = new TpmSimulator("tpm-in-house-nv-changeauth", rsaSigningBackend: MicrosoftTpmRsaSigningBackend.Create(), rng: TestEntropy.NewCounterStream(), timeProvider: new FakeTimeProvider(TestClock.CanonicalEpoch));
    ...
    /// implementation under test reports (TPM 2.0 Library Part 3, clause 11.1).
    /// </summary>
