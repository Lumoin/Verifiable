...
/// </summary>
/// <remarks>
/// <c>TPM2_CreatePrimary()</c> always generates its own recipient key pair (DHKEM's <c>skR</c>/<c>pkR</c>),
/// so the KEM keys this file creates carry no fixed vector material; the RFC 9180 Appendix A.3.1 vector-exact
/// proofs — <c>TPM2_Decapsulate()</c> over the vector's <c>enc</c> against the vector's recipient key, loaded
/// with its private scalar through <c>TPM2_LoadExternal()</c> (TPM 2.0 Library Part 3, clause 12.3), and
/// <c>TPM2_Encapsulate()</c> reproducing the vector's <c>enc</c> under a fixed ephemeral — live in
/// <see cref="TpmInHouseSimulatorLoadExternalKemVectorTests"/>. The DHKEM core those effects call
/// (<see cref="Dhkem.ExtractAndExpandAsync"/>) is proven against the same vector in
/// <see cref="Verifiable.Tests.Cryptography.DhkemTests"/>, and the host-side encapsulation core's reproduction
/// of the vector's <c>enc</c> in <see cref="DhkemEncapsulationTests"/>. What THIS file proves is that the
/// simulator's command-level plumbing (parsing, gating, framing) is correct, and — independently of any vector
/// — that the simulator's own <c>kem_context</c> assembly matches an ephemeral this file generates and derives
/// against entirely outside the simulator
...
internal sealed class TpmInHouseSimulatorKemTests
{
    ...
    /// 2.0 Library Specification</see>, Part 2: Structures, clause 12.2.3.5, Table 229).
    /// </summary>
    [TestMethod]
    ...
    /// Specification</see>, Part 2: Structures, clause 12.2.3.5, Table 229).
    /// </summary>
    [TestMethod]
    public async Task CreatePrimaryEccKemKeyWithRestrictedAttributeReturnsKdf()
    {
        ...
        Assert.AreEqual(
            HmacKeyHarness.ParameterEncodedRc(TpmRcConstants.TPM_RC_KDF, parameterIndex: 1), result.ResponseCode,
            "Table 191: inPublic is TPM2_CreatePrimary()'s second parameter (index 1); a KEM kdf on a RESTRICTED decrypt template must be refused with TPM_RC_KDF there.");
    }
    ...
    /// (<see href="https://trustedcomputinggroup.org/resource/tpm-library-specification/">TPM 2.0 Library
    /// Specification</see>, Part 2: Structures, clause 12.2.3.5, Table 229). No shipped factory can express
    ...
    public async Task CreatePrimaryEccKemKeyWithMgf1KdfSchemeReturnsKdf()
    {
        ...
        Assert.AreEqual(
            HmacKeyHarness.ParameterEncodedRc(TpmRcConstants.TPM_RC_KDF, parameterIndex: 1), result.ResponseCode,
            "Table 191: inPublic is TPM2_CreatePrimary()'s second parameter (index 1); a non-HKDF kdf.scheme on an otherwise-KEM-shaped template must be refused with TPM_RC_KDF there.");
    }
    ...
    public async Task CreatePrimaryEccKemKeyWithHkdfSha384OnP256ReturnsKdf()
    {
        ...
        Assert.AreEqual(
            HmacKeyHarness.ParameterEncodedRc(TpmRcConstants.TPM_RC_KDF, parameterIndex: 1), result.ResponseCode,
            "Table 191: inPublic is TPM2_CreatePrimary()'s second parameter (index 1); HKDF-SHA384 on P-256 names no DHKEM suite this simulator wires, so it must be refused with TPM_RC_KDF there rather than minted.");
    }
    ...
    public async Task CreatePrimaryEccKemKeyWithNonNullSymmetricReturnsSymmetric()
    {
        ...
        Assert.AreEqual(
            HmacKeyHarness.ParameterEncodedRc(TpmRcConstants.TPM_RC_SYMMETRIC, parameterIndex: 1), result.ResponseCode,
            "Table 191: inPublic is TPM2_CreatePrimary()'s second parameter (index 1); a KEM template's non-NULL symmetric must be refused with TPM_RC_SYMMETRIC there.");
    }
    ...
    public async Task CreatePrimaryEccKemKeyWithSignEncryptSetReturnsScheme()
    {
        ...
        Assert.AreEqual(
            HmacKeyHarness.ParameterEncodedRc(TpmRcConstants.TPM_RC_SCHEME, parameterIndex: 1), result.ResponseCode,
            "Table 191: inPublic is TPM2_CreatePrimary()'s second parameter (index 1); a KEM template carrying SIGN_ENCRYPT must be refused with TPM_RC_SCHEME there — TPM_ALG_ECDH is not a signing scheme.");
    }
    ...
    public async Task CreatePrimaryEccTemplateWithNullSchemeAndHkdfKdfFallsThroughToKdf()
    {
        ...
        Assert.AreEqual(
            HmacKeyHarness.ParameterEncodedRc(TpmRcConstants.TPM_RC_KDF, parameterIndex: 1), result.ResponseCode,
            "Table 191: inPublic is TPM2_CreatePrimary()'s second parameter (index 1); a NULL-scheme ECC template carrying a non-NULL kdf must fall through to TPM_RC_KDF there, not the generic TPM_RC_SCHEME catch-all.");
    }
    ...
    /// (<see href="https://trustedcomputinggroup.org/resource/tpm-library-specification/">TPM 2.0 Library
    /// Specification</see>, Part 2: Structures, clause 12.2.3.5, Table 229; Part 3: Commands, clause 24.1.1).
    /// </summary>
    [TestMethod]
    ...
    public async Task EncapsulateAgainstAKdfNullEccSigningKeyReturnsKey()
    {
        ...
        Assert.AreEqual(HmacKeyHarness.HandleEncodedRc(TpmRcConstants.TPM_RC_KEY, 0), result.ResponseCode, "A kdf-NULL ECC signing key is not a KEM key and must be refused with TPM_RC_KEY at keyHandle, handle 1 of Table 60.");
    }
    ...
    public async Task EncapsulateAgainstAnRsaKeyReturnsKey()
    {
        ...
        Assert.AreEqual(HmacKeyHarness.HandleEncodedRc(TpmRcConstants.TPM_RC_KEY, 0), result.ResponseCode, "An RSA key is never a KEM key (Table 101 has no RSA arm) and must be refused with TPM_RC_KEY at keyHandle, handle 1 of Table 60.");
    }

    /// <summary>
    /// A loaded KEYEDHASH sealed-data object lives in the simulator's sealed-object table, not among its
    /// signing/KEM keys, so <c>TPM2_Encapsulate()</c> refuses it through its dedicated sealed-object arm — never
    ...
    public async Task EncapsulateAgainstASealedKeyedhashObjectReturnsKey()
    {
        ...
        Assert.AreEqual(HmacKeyHarness.HandleEncodedRc(TpmRcConstants.TPM_RC_KEY, 0), result.ResponseCode, "A sealed KEYEDHASH object is not a KEM key and must be refused with TPM_RC_KEY.");
    }
    ...
    public async Task DecapsulateAgainstAKdfNullEccSigningKeyReturnsKey()
    {
        ...
        Assert.AreEqual(HmacKeyHarness.HandleEncodedRc(TpmRcConstants.TPM_RC_KEY, 0), code, "A kdf-NULL ECC signing key is not a KEM key and must be refused with TPM_RC_KEY on Decapsulate.");
    }
    ...
    public async Task DecapsulateAgainstARealStorageParentReturnsKey()
    {
        ...
        Assert.AreEqual(HmacKeyHarness.HandleEncodedRc(TpmRcConstants.TPM_RC_KEY, 0), code, "A real storage parent has no kdf and must be refused with TPM_RC_KEY before the ATTRIBUTES gate is ever reached.");
    }
    ...
    /// (independently confirmed by reading <c>TPM2_Create()</c>'s and <c>TPM2_Import()</c>'s own parse-time
    /// type gates). The
    ...
    /// <c>TPM2_Decapsulate()</c>'s own parser would have produced from a real wire frame. No private member, reflection,
    ...
    /// level, proving the command's own exception-to-RC mapping
    ...
    public async Task HandFramedDecapsulateWithOverMaxCiphertextSizeReturnsSize()
    {
        ...
        Assert.AreEqual(
            ...
            "Table 62: ciphertext is TPM2_Decapsulate()'s sole parameter (index 0); a declared size past MaxSize must be refused with TPM_RC_SIZE there, checked before any rental.");
    }
    ...
    /// declared is refused <c>TPM_RC_INSUFFICIENT</c> at the command level, proving the command's own
    /// <c>ArgumentOutOfRangeException</c> → <c>TPM_RC_INSUFFICIENT</c> mapping
    /// (<see href="https://trustedcomputinggroup.org/resource/tpm-library-specification/">TPM 2.0 Library
    /// Specification</see>, Part 2: Structures, clause 10.3.14, Table 102).
    /// </summary>
    [TestMethod]
    public async Task HandFramedDecapsulateWithTruncatedCiphertextReturnsInsufficient()
    {
        ...
        Assert.AreEqual(
            ...
            "Table 62: ciphertext is TPM2_Decapsulate()'s sole parameter (index 0); a declared size exceeding the octets actually supplied must be refused with TPM_RC_INSUFFICIENT there.");
    }
    ...
    /// this command's own parser implements for a tag mismatch (a command with no authorization area at
    ...
    /// reaches the command's own parse-level RC mapping. Any well-formed auth area suffices
