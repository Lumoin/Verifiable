...
/// </para>
/// <para>
/// The over-sessions <c>TPM2_Create()</c> tail decode applies the identical probe-then-parse capture behind
/// a real HMAC session; its own over-bound-<c>creationPCR</c> refusal (a
...
internal sealed class TpmInHouseSimulatorCreationParameterTests
{
    ...
    public async Task CreatePrimaryWithCreationPcrCountAboveTheListBoundReturnsSize()
    {
        ...
        Assert.AreEqual(HmacKeyHarness.ParameterEncodedRc(TpmRcConstants.TPM_RC_SIZE, 3), code, "creationPCR is TPM2_CreatePrimary()'s fourth parameter (Table 191, index 3); a count above MaxSelections (Table 128) is parameter-encoded TPM_RC_SIZE.");
    }
    ...
    public async Task CreateWithCreationPcrCountAboveTheListBoundReturnsSize()
    {
        ...
        Assert.AreEqual(HmacKeyHarness.ParameterEncodedRc(TpmRcConstants.TPM_RC_SIZE, 3), code, "creationPCR is TPM2_Create()'s fourth parameter (Table 18, index 3); a count above MaxSelections (Table 128) is parameter-encoded TPM_RC_SIZE.");
    }
    ...
    public async Task CreatePrimaryWithOutsideInfoAboveTheDataBoundReturnsSize()
    {
        ...
        Assert.AreEqual(HmacKeyHarness.ParameterEncodedRc(TpmRcConstants.TPM_RC_SIZE, 2), code, "outsideInfo is TPM2_CreatePrimary()'s third parameter (Table 191, index 2); wider than Tpm2bData.MaxSize (Table 91) is parameter-encoded TPM_RC_SIZE.");
    }
    ...
    public async Task CreateWithOutsideInfoAboveTheDataBoundReturnsSize()
    {
        ...
        Assert.AreEqual(HmacKeyHarness.ParameterEncodedRc(TpmRcConstants.TPM_RC_SIZE, 2), code, "outsideInfo is TPM2_Create()'s third parameter (Table 18, index 2); wider than Tpm2bData.MaxSize (Table 91) is parameter-encoded TPM_RC_SIZE.");
    }
    ...
    public async Task CreatePrimaryWithTruncatedOutsideInfoReturnsInsufficient()
    {
        ...
        Assert.AreEqual(HmacKeyHarness.ParameterEncodedRc(TpmRcConstants.TPM_RC_INSUFFICIENT, 2), code, "outsideInfo is TPM2_CreatePrimary()'s third parameter (Table 191, index 2); a declared size past the frame's own end is parameter-encoded TPM_RC_INSUFFICIENT.");
    }
    ...
    public async Task CreateWithTruncatedOutsideInfoReturnsInsufficient()
    {
        ...
        Assert.AreEqual(HmacKeyHarness.ParameterEncodedRc(TpmRcConstants.TPM_RC_INSUFFICIENT, 2), code, "outsideInfo is TPM2_Create()'s third parameter (Table 18, index 2); a declared size past the frame's own end is parameter-encoded TPM_RC_INSUFFICIENT.");
    }
    ...
    public async Task CreatePrimaryWithTruncatedCreationPcrBitmapReturnsInsufficient()
    {
        ...
        Assert.AreEqual(HmacKeyHarness.ParameterEncodedRc(TpmRcConstants.TPM_RC_INSUFFICIENT, 3), code, "creationPCR is TPM2_CreatePrimary()'s fourth parameter (Table 191, index 3); a selection truncated mid-bitmap (Table 107) is parameter-encoded TPM_RC_INSUFFICIENT.");
    ...
    public async Task CreateWithTruncatedCreationPcrBitmapReturnsInsufficient()
    {
        ...
        Assert.AreEqual(HmacKeyHarness.ParameterEncodedRc(TpmRcConstants.TPM_RC_INSUFFICIENT, 3), code, "creationPCR is TPM2_Create()'s fourth parameter (Table 18, index 3); a selection truncated mid-bitmap (Table 107) is parameter-encoded TPM_RC_INSUFFICIENT.");
        Assert.AreEqual(baseline, trackingPool.OutstandingCount, "The already-rented, non-empty outsideInfo must be released when the trailing creationPCR then refuses.");
    }
    ...
    /// (<see href="https://trustedcomputinggroup.org/resource/tpm-library-specification/">TPM 2.0 Library
    /// Specification</see>, Part 2: Structures, clause 10.3.3, Table 91; Part 4's <c>TPM2B_DATA</c> unmarshalling
    /// routine, which checks the size bound ahead of the array read).
    /// </summary>
    [TestMethod]
    public async Task CreatePrimaryWithOverBoundAndTruncatedOutsideInfoReturnsSize()
    {
        ...
        Assert.AreEqual(HmacKeyHarness.ParameterEncodedRc(TpmRcConstants.TPM_RC_SIZE, 2), code, "outsideInfo is TPM2_CreatePrimary()'s third parameter (Table 191, index 2); declaring a size past Tpm2bData.MaxSize (Table 91) is parameter-encoded TPM_RC_SIZE even when the frame is also too short to supply it.");
    ...
    /// (<see href="https://trustedcomputinggroup.org/resource/tpm-library-specification/">TPM 2.0 Library
    /// Specification</see>, Part 2: Structures, clause 10.8.7, Table 128; Part 4's <c>TPML_PCR_SELECTION</c>
    /// unmarshalling routine, which checks the count bound ahead of the per-selection loop).
    /// </summary>
    [TestMethod]
    public async Task CreatePrimaryWithOverBoundAndTruncatedCreationPcrReturnsSize()
    {
        ...
        Assert.AreEqual(HmacKeyHarness.ParameterEncodedRc(TpmRcConstants.TPM_RC_SIZE, 3), code, "creationPCR is TPM2_CreatePrimary()'s fourth parameter (Table 191, index 3); a count past MaxSelections (Table 128) is parameter-encoded TPM_RC_SIZE even when the frame is also too short to supply every entry.");
        Assert.AreEqual(baseline, trackingPool.OutstandingCount, "The already-rented, non-empty outsideInfo must be released when the trailing creationPCR then refuses.");
    }
    ...
    public async Task CreatePrimaryCarrierPoolBalanceAcrossRefusalsAndSuccess()
    {
...

        {
            ...
            TpmRcConstants code = await SubmitCreateFamilyCommandAsync(
                simulator, trackingPool.Pool, TpmCcConstants.TPM_CC_CreatePrimary, (uint)TpmRh.TPM_RH_OWNER,
                inSensitive, inPublic, outsideInfoWire, pcrSelectionWire).ConfigureAwait(false);
            Assert.AreEqual(HmacKeyHarness.ParameterEncodedRc(TpmRcConstants.TPM_RC_SIZE, 3), code, "creationPCR is TPM2_CreatePrimary()'s fourth parameter (Table 191, index 3); an over-bound one must still answer parameter-encoded TPM_RC_SIZE under the metered pool.");
        }
...

        {
            ...
            TpmRcConstants code = await SubmitCreateFamilyCommandAsync(
                simulator, trackingPool.Pool, TpmCcConstants.TPM_CC_CreatePrimary, (uint)TpmRh.TPM_RH_OWNER,
                inSensitive, inPublic, outsideInfoWire, pcrSelectionWire).ConfigureAwait(false);
            Assert.AreEqual(HmacKeyHarness.ParameterEncodedRc(TpmRcConstants.TPM_RC_SIZE, 2), code, "outsideInfo is TPM2_CreatePrimary()'s third parameter (Table 191, index 2); an over-bound one must still answer parameter-encoded TPM_RC_SIZE under the metered pool.");
        }
    ...
    public async Task CreateCarrierPoolBalanceAcrossRefusalsAndSuccess()
    {
...

        {
            ...
            TpmRcConstants code = await SubmitCreateFamilyCommandAsync(
                ...
                inSensitive, inPublic, outsideInfoWire, pcrSelectionWire).ConfigureAwait(false);
            Assert.AreEqual(HmacKeyHarness.ParameterEncodedRc(TpmRcConstants.TPM_RC_SIZE, 3), code, "creationPCR is TPM2_Create()'s fourth parameter (Table 18, index 3); an over-bound one must still answer parameter-encoded TPM_RC_SIZE under the metered pool.");
        }
...

        {
            ...
            TpmRcConstants code = await SubmitCreateFamilyCommandAsync(
                simulator, trackingPool.Pool, TpmCcConstants.TPM_CC_Create, parentHandle,
                inSensitive, inPublic, outsideInfoWire, pcrSelectionWire).ConfigureAwait(false);
            Assert.AreEqual(HmacKeyHarness.ParameterEncodedRc(TpmRcConstants.TPM_RC_SIZE, 2), code, "outsideInfo is TPM2_Create()'s third parameter (Table 18, index 2); an over-bound one must still answer parameter-encoded TPM_RC_SIZE under the metered pool.");
        }
    ...
    /// so the parser's bound-against-remaining-octets probe answers <c>TPM_RC_INSUFFICIENT</c>.
    /// </summary>
    ...
    {
        var simulator = new TpmSimulator("tpm-in-house-creation-parameters", signingBackend: BouncyCastleTpmEccSigningBackend.Create(), rng: TestEntropy.NewCounterStream(), timeProvider: new FakeTimeProvider(TestClock.CanonicalEpoch));
