...
internal static class HmacKeyHarness
{
    ...
    /// <param name="includeRsaBackend">Whether to wire an RSA signing backend alongside the ECC one, for a test that drives an RSA-keyed command (<c>TPM2_RSA_Decrypt()</c>, an RSA <c>TPM2_Sign()</c>) rather than answering the faithful <c>TPM_RC_COMMAND_CODE</c> a backend-less simulator gives every such command.</param>
    /// <returns>The operational simulator.</returns>
    public static async Task<TpmSimulator> CreateOperationalAsync(string name, BaseMemoryPool pool, CancellationToken cancellationToken, ReadOnlyMemory<byte> seed = default, bool includeRsaBackend = false)
    {
        var simulator = new TpmSimulator(
            name, signingBackend: BouncyCastleTpmEccSigningBackend.Create(),
            rsaSigningBackend: includeRsaBackend ? MicrosoftTpmRsaSigningBackend.Create() : null,
    ...
    /// <summary>
    /// Creates an ECC P-256 ECDSA-SHA-256 signing primary under the requested hierarchy (the owner hierarchy by
    /// default), with an empty authValue, for a test that needs a resolvable signing-capable object at a real
    /// handle — <c>TPM2_VerifyDigestSignature()</c>'s companion-session wire form, for instance
    /// (<see cref="Verifiable.Tests.Tpm.TpmInHouseSimulatorNoAuthSessionKeyCommandTests.VerifyDigestSignatureOverAXorDecryptCompanionWithAnOverBoundDigestAnswersTheParameterEncodedSizeUnchanged"/>),
    /// resolves its <c>keyHandle</c> before any parameter is decoded, so a placeholder handle number is not
    /// enough there. The caller owns and flushes the response.
    /// </summary>
    /// <param name="tpm">The TPM device.</param>
    /// <param name="registry">The response codec registry.</param>
    /// <param name="pool">The memory pool.</param>
    /// <param name="cancellationToken">A token observed across the exchange.</param>
    /// <param name="hierarchy">The hierarchy the primary is created under.</param>
    /// <returns>The CreatePrimary response for the signing primary.</returns>
    public static async Task<CreatePrimaryResponse> CreateEccSigningPrimaryAsync(TpmDevice tpm, TpmResponseRegistry registry, BaseMemoryPool pool, CancellationToken cancellationToken, TpmRh hierarchy = TpmRh.TPM_RH_OWNER)
    {
        ...
        Assert.IsTrue(result.IsSuccess, $"CreatePrimary (ECC P-256 signing key) under '{hierarchy}' failed: '{result.ResponseCode}'.");
    ...
    public static Tpm2bDigest DuplicationPolicyDigest(BaseMemoryPool pool)
    {
        ...
        _ = TpmPolicyDigest.ExtendForCommandCode(current, TpmCcConstants.TPM_CC_Duplicate, NameAlg, destination, pool);
    ...
    public static async Task<DuplicateResponse> DuplicateAsync(
        ...
        CancellationToken cancellationToken)
    {
        TpmResult<DuplicateResponse> result = await DuplicateRawAsync(
            tpm, registry, pool, objectHandle, objectName, newParentHandle, newParentName, cancellationToken).ConfigureAwait(false);
    ...
    /// <summary>
    /// Runs the DUP-role export of a KEYEDHASH object to <paramref name="newParentHandle"/> under a fresh policy
    /// session latched to <c>TPM_CC_Duplicate</c>, returning the raw result so a caller can assert either success
    /// or a specific refusal — the same choreography <see cref="DuplicateAsync"/> asserts success over, extracted
    /// so a clause 5.4 handle-area probe can inspect the refusal <see cref="DuplicateAsync"/>'s assertion would
    /// otherwise hide.
    /// </summary>
    /// <param name="tpm">The TPM device.</param>
    /// <param name="registry">The response codec registry.</param>
    /// <param name="pool">The memory pool.</param>
    /// <param name="objectHandle">The loaded object handle placed at <c>objectHandle</c>.</param>
    /// <param name="objectName">The Name folded into cpHash for <c>objectHandle</c>.</param>
    /// <param name="newParentHandle">The value placed at <c>newParentHandle</c>, or <c>TPM_RH_NULL</c> for the bare form.</param>
    /// <param name="newParentName">The Name folded into cpHash for <c>newParentHandle</c>.</param>
    /// <param name="cancellationToken">A token observed across the exchange.</param>
    /// <returns>The raw duplicate result; the caller disposes the value on success.</returns>
    public static async Task<TpmResult<DuplicateResponse>> DuplicateRawAsync(
        TpmDevice tpm, TpmResponseRegistry registry, BaseMemoryPool pool, uint objectHandle, ReadOnlyMemory<byte> objectName, uint newParentHandle, ReadOnlyMemory<byte> newParentName,
        CancellationToken cancellationToken)
    {
    ...
    {
        StartAuthSessionInput startInput = StartAuthSessionInput.CreateBoundUnsaltedHmacSession(bindHandle, NameAlg, TestEntropy.NewCounterStream(), pool, symmetric);
        TpmResult<StartAuthSessionResponse> startResult = await TpmCommandExecutor.ExecuteAsync<StartAuthSessionResponse>(
            tpm, startInput, [], null, pool, registry, cancellationToken).ConfigureAwait(false);
        ...
        TpmSession session = await TpmSession.CreateBoundAsync(
            ...
            started.NonceTPM, NameAlg, TestEntropy.NewCounterStream(), pool, symmetric: symmetric, isBoundToAuthorizedEntity: isBoundToAuthorizedEntity, cancellationToken: cancellationToken).ConfigureAwait(false);
    ...
    /// <summary>
    /// Applies the handle-index response-code modifier a TPM adds when a format-one failure is attributed to a
    /// specific handle in the command's own handle area: <c>rc = baseRc + TPM_RC_n(0x100·(index+1))</c>, the N
    /// field's number being one based while <paramref name="handleIndex"/> is the handle's zero-based position
    /// (TPM 2.0 Library Part 2, clause 6.6.2, Table 16). <c>TPM_RC_H</c> is 0x000, so unlike
    /// <see cref="SessionEncodedRc"/> no session offset is added.
    /// </summary>
    /// <param name="baseRc">The unmodified format-1 response code.</param>
    /// <param name="handleIndex">The zero-based index of the handle the failure names.</param>
    ...
    /// <summary>
    /// Applies the parameter-index response-code modifier a TPM adds when a format-one failure is attributed to
    /// a specific parameter in the command's own parameter area: <c>rc = baseRc + TPM_RC_P + TPM_RC_n(0x100·(index+1))</c>,
    /// the N field's number being one based while <paramref name="parameterIndex"/> is the parameter's zero-based
    /// position (TPM 2.0 Library Part 2, clause 6.6.2, Table 15: "When an error is associated with a parameter,
    /// TPM_RC_P (0x040) is added and N is set to the parameter number.").
    /// </summary>
    /// <param name="baseRc">The unmodified format-1 response code.</param>
    /// <param name="parameterIndex">The zero-based index of the parameter the failure names.</param>
