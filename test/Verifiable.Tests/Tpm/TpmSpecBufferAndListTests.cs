...
internal sealed class TpmSpecBufferAndListTests
{
    ...
    /// matching the aliasing contract <see cref="Tpm2bName.AsReadOnlyMemory"/> establishes (Part 2, clause 10.3.3).
    /// </summary>
    [TestMethod]
    ...
    public void TpmlPcrSelectionParseBelowPcrSelectMinThrowsAndRentsNothing()
    {
        ...
        catch(ArgumentOutOfRangeException)
        {
            ...
            //preceding Assert.Fail fires only when Parse returns instead of throwing.
        }
    ...
    /// original wire bytes exactly. TPM 2.0 Library Part 2, clause 11.2.4.6, printed page 178 introduces the
    /// structure — "This Table 194 sized buffer holds the largest RSA public key supported by the TPM" — and
    /// Table 194, printed page 178 frames it as a <c>UINT16 size</c> ahead of <c>buffer[size]</c>.
    /// </summary>
    [TestMethod]
    ...
    /// bound (TPM 2.0 Library Part 2, printed page 178), 512 octets at the RSA-4096 this library supports — is
    ...
    /// <see cref="Tpm2bPublicKeyRsa.MaxRsaKeyBytes"/> (Table 194, printed page 178).
    /// </summary>
    [TestMethod]
    ...
    /// <see cref="Tpm2bPublicKeyRsa.Dispose"/>, balancing back to the pre-rental baseline. <c>modulus</c>
    /// is disposed explicitly, not via a <see langword="using"/> declaration, because the balance assertion
    /// right after it must see the rental already returned.
    /// </summary>
    [TestMethod]
    ...
    /// Library Part 2, printed page 178) says "The value of zero is only valid for create", so the empty form
    ...
    /// remaining buffer, rather than sizing the backing array from a hostile length (Part 2, clause 10.8.4;
    /// <c>TPM_RC_SIZE</c>).
    /// </summary>
    [TestMethod]
    ...
    /// remaining buffer (Part 2, clause 10.8.1; <c>TPM_RC_SIZE</c>).
    /// </summary>
    [TestMethod]
    ...
    /// exactly for a non-empty list. TPM 2.0 Library Part 2, clause 10.8.2, Table 123, printed page 150 frames
    ...
    /// <see cref="TpmlCca.Empty"/> — a shape Table 123, printed page 150 admits explicitly ("number of values
    ...
    /// (Part 2, clause 10.8.2, Table 123, printed page 150).
    /// </summary>
    [TestMethod]
    ...
    /// remaining buffer (Part 2, clause 10.8.3; <c>TPM_RC_SIZE</c>).
    /// </summary>
    [TestMethod]
    ...
    /// (<c>TPM_ST_CREATION</c>) on printed page 144 and Table 113 (<c>TPM_ST_VERIFIED</c>) on printed page 146,
    /// Table 114 (<c>TPM_ST_AUTH_SIGNED</c> or <c>TPM_ST_AUTH_SECRET</c>) on printed page 147 and Table 115
    /// (<c>TPM_ST_HASHCHECK</c>) on printed page 148. Every arm here supplies a hierarchy the ticket WOULD accept, so only the tag can
