...
/// </remarks>
[TestClass]
internal sealed class TpmInHouseSimulatorGetTimeCarrierTests
{
    ...
    /// </summary>
    private static byte[] CarrierProofNonce { get; } = "GetTime carrier ownership proof nonce for the TPM 2.0 library"u8.ToArray();
    ...
    /// <summary>A wrong guess at either slot's authValue, distinct from the signing key's real one and from the endorsement hierarchy's empty one.</summary>
    ...
    public async Task RefusedGetTimeAtTheEntryTransitionReturnsTheParseRentedCarriersToPool()
    {
        ...
        long baseline = trackingPool.OutstandingCount;
        {
            ...
            using GetTimeInput getTimeInput = GetTimeInput.ForEcdsa(
                TpmiDhObject.FromValue(TpmSimulatorState.TransientHandleBase + 0x00FFFF00), CarrierProofNonce,
    ...
    /// <summary>
    /// A session-authorized <c>TPM2_GetTime()</c> refused by the continuation — the scheme hash algorithm is one
    ...
    /// </summary>
    [TestMethod]
    public async Task RefusedGetTimeAtTheOverSessionContinuationReturnsTheParseRentedCarriersToPool()
    {
        using var trackingPool = new MeteredHousePool();
        using TpmSimulator simulator = await CreateOperationalAsync(trackingPool.Pool).ConfigureAwait(false);
        using TpmDevice tpm = TpmDevice.Create(simulator.SubmitAsync);
    ...
    /// <summary>
    /// A session-authorized <c>TPM2_GetTime()</c> whose FIRST slot — <c>@privacyAdminHandle</c> at session index
    /// 0 — presents a command HMAC computed over an authValue the endorsement hierarchy does not carry never
    /// reaches its continuation at all: the verification step terminates the command with an authorization
    /// failure (TPM 2.0 Library Part 2, clause 6.6.2). The queued request still owns every carrier the parser
    /// rented, and the mismatch path releases them before the rejection is framed.
    /// </summary>
    [TestMethod]
    public async Task GetTimeOverSessionWithAMismatchedCommandHmacAtThePrivacyAdminSlotReturnsTheParseRentedCarriersToPool()
    {
        using var trackingPool = new MeteredHousePool();
        using TpmSimulator simulator = await CreateOperationalAsync(trackingPool.Pool).ConfigureAwait(false);
        using TpmDevice tpm = TpmDevice.Create(simulator.SubmitAsync);
        ...
        TpmResult<GetTimeResponse> result = await GetTimeOverTwoRealSessionsAsync(
            tpm, registry, trackingPool, ak, TpmAlgIdConstants.TPM_ALG_SHA256, WrongPasswordBytes, SignKeyPasswordBytes).ConfigureAwait(false);
        Assert.AreEqual(
            SessionEncodedRc(TpmRcConstants.TPM_RC_BAD_AUTH, sessionIndex: 0), result.ResponseCode,
            "A wrong authValue folded into the privacy-administrator session's command HMAC is an uncharged authorization failure blamed on session index 0, or this path proves nothing about a first-slot mismatch.");
    ...
    /// <summary>
    /// A session-authorized <c>TPM2_GetTime()</c> whose SECOND slot — <c>@signHandle</c> at session index 1 —
    ...
    /// </summary>
    [TestMethod]
    public async Task GetTimeOverSessionWithAMismatchedCommandHmacAtTheSignSlotReturnsTheParseRentedCarriersToPool()
    {
        using var trackingPool = new MeteredHousePool();
        using TpmSimulator simulator = await CreateOperationalAsync(trackingPool.Pool).ConfigureAwait(false);
        using TpmDevice tpm = TpmDevice.Create(simulator.SubmitAsync);
        ...
        TpmResult<GetTimeResponse> result = await GetTimeOverTwoRealSessionsAsync(
            tpm, registry, trackingPool, ak, TpmAlgIdConstants.TPM_ALG_SHA256, ReadOnlyMemory<byte>.Empty, WrongPasswordBytes).ConfigureAwait(false);
        Assert.AreEqual(
    ...
    /// <summary>
    /// A successful session-authorized <c>TPM2_GetTime()</c> over two real HMAC sessions returns every carrier
    ...
    public async Task SuccessfulGetTimeOverSessionReturnsTheParseRentedCarriersToPool()
    {
        ...
        TpmResult<GetTimeResponse> result = await GetTimeOverTwoRealSessionsAsync(
            tpm, registry, trackingPool, ak, TpmAlgIdConstants.TPM_ALG_SHA256, ReadOnlyMemory<byte>.Empty, SignKeyPasswordBytes).ConfigureAwait(false);
        Assert.IsTrue(result.IsSuccess, $"The session-authorized time attestation must succeed with the correct authValues: '{result.ResponseCode}'.");
        ...
        Assert.AreEqual(
            baseline, trackingPool.OutstandingCount,
            "A successful time attestation must return both parse-rented HMAC carriers and the qualifying-data carrier to the pool once the response is disposed.");
    }

    /// <summary>
    /// A <c>TPM2_GetTime()</c> over a MIXED authorization area whose <c>TPM_RS_PW</c> privacy-administrator slot
    /// carries a non-empty <c>nonceCaller</c> on the wire is refused with <c>TPM_RC_NONCE</c> encoded to that
    /// slot while the slot is still being read, so the parse rents nothing at all and the pool balance does not
    /// move.
    /// </summary>
    /// <remarks>
    /// <para>
    ...
    /// </remarks>
    [TestMethod]
    public async Task GetTimeOverAMixedAreaWithAPasswordSlotNonceIsRefusedAtParseAndRentsNothing()
    {
        using var trackingPool = new MeteredHousePool();
        using TpmSimulator simulator = await CreateOperationalAsync(trackingPool.Pool).ConfigureAwait(false);
        using TpmDevice tpm = TpmDevice.Create(simulator.SubmitAsync);
        ...
        {
            if(ReadCommandCode(command.Span) != TpmCcConstants.TPM_CC_GetTime)
            {
        ...
        TpmResult<GetTimeResponse> result = await GetTimeOverAMixedAreaAsync(tpm, plantingTpm, registry, trackingPool, ak).ConfigureAwait(false);
        if(result.IsSuccess)
        {
            result.Value.Dispose();
        }
        ...
        Assert.AreEqual(
            ...
            "The refusal names the offending slot — the password privacy-administrator slot at index 0 — session-index-encoded (TPM 2.0 Library Part 2, clause 6.6.2).");
    ...
    /// </summary>
    [TestMethod]
    public async Task GetTimeWithQualifyingDataOverTheDataBoundIsRefusedAtParseAndRentsNothing()
    {
        using var trackingPool = new MeteredHousePool();
        using TpmSimulator simulator = await CreateOperationalAsync(trackingPool.Pool).ConfigureAwait(false);
        using TpmDevice tpm = TpmDevice.Create(simulator.SubmitAsync);
        ...
        long baseline = trackingPool.OutstandingCount;
        {
            using TpmPasswordSession atBoundPrivacyAdminAuth = TpmPasswordSession.CreateEmpty(trackingPool.Pool);
            using TpmPasswordSession atBoundSignAuth = TpmPasswordSession.CreateEmpty(trackingPool.Pool);
            using GetTimeInput atBoundInput = GetTimeInput.ForEcdsa(
                ak.ObjectHandle, atBound, TpmAlgIdConstants.TPM_ALG_SHA256, trackingPool.Pool);

            TpmResult<GetTimeResponse> atBoundResult = await TpmCommandExecutor.ExecuteAsync<GetTimeResponse>(
                tpm, atBoundInput, [atBoundPrivacyAdminAuth, atBoundSignAuth], null, trackingPool.Pool, registry, TestContext.CancellationToken).ConfigureAwait(false);
            Assert.IsTrue(
                atBoundResult.IsSuccess,
                $"A qualifyingData of exactly sizeof(TPMT_HA) octets is inside the TPM2B_DATA bound and must be attested: '{atBoundResult.ResponseCode}'.");
            atBoundResult.Value.Dispose();

            using TpmPasswordSession overBoundPrivacyAdminAuth = TpmPasswordSession.CreateEmpty(trackingPool.Pool);
            using TpmPasswordSession overBoundSignAuth = TpmPasswordSession.CreateEmpty(trackingPool.Pool);
            using GetTimeInput overBoundInput = GetTimeInput.ForEcdsa(
                ak.ObjectHandle, overBound, TpmAlgIdConstants.TPM_ALG_SHA256, trackingPool.Pool);
    ...
    /// <param name="privacyAdminSlotAuthValue">The authValue term folded into the privacy-administrator slot's session; empty is the endorsement hierarchy's own value.</param>
    /// <param name="signSlotAuthValue">The authValue term folded into the sign slot's session.</param>
    /// <returns>The time-attestation result, not asserted for success.</returns>
    private async Task<TpmResult<GetTimeResponse>> GetTimeOverTwoRealSessionsAsync(
        TpmDevice tpm, TpmResponseRegistry registry, MeteredHousePool trackingPool, CreatePrimaryResponse ak,
        TpmAlgIdConstants schemeHashAlg, ReadOnlyMemory<byte> privacyAdminSlotAuthValue, ReadOnlyMemory<byte> signSlotAuthValue)
    {
        StartAuthSessionResponse privacyAdminStarted = await StartHmacSessionAsync(tpm, registry, trackingPool).ConfigureAwait(false);
        uint privacyAdminSessionHandle = privacyAdminStarted.SessionHandle.Value;
        ...
        try
        {
            ...
            return await TpmCommandExecutor.ExecuteAsync<GetTimeResponse>(
                tpm, getTimeInput, [privacyAdminSession, signSession], handleNames, trackingPool.Pool, registry, TestContext.CancellationToken).ConfigureAwait(false);
        }
        finally
        {
            _ = await TpmCommandExecutor.ExecuteAsync<FlushContextResponse>(
                tpm, FlushContextInput.ForHandle(privacyAdminSessionHandle), [], null, trackingPool.Pool, registry, CancellationToken.None).ConfigureAwait(false);
            _ = await TpmCommandExecutor.ExecuteAsync<FlushContextResponse>(
    ...
    /// <summary>
    /// Attests the current time with <paramref name="ak"/> over a MIXED authorization area — a <c>TPM_RS_PW</c>
    ...
    /// <param name="ak">The attestation key's CreatePrimary response.</param>
    /// <returns>The time-attestation result, not asserted for success.</returns>
    private async Task<TpmResult<GetTimeResponse>> GetTimeOverAMixedAreaAsync(
        TpmDevice plainTpm, TpmDevice plantingTpm, TpmResponseRegistry registry, MeteredHousePool trackingPool, CreatePrimaryResponse ak)
    {
        ...
        try
        {
            ...
            return await TpmCommandExecutor.ExecuteAsync<GetTimeResponse>(
                plantingTpm, getTimeInput, [privacyAdminAuth, signSession], handleNames, trackingPool.Pool, registry, TestContext.CancellationToken).ConfigureAwait(false);
        }
        finally
        {
            _ = await TpmCommandExecutor.ExecuteAsync<FlushContextResponse>(
    ...
    /// <summary>
    /// Creates a primary ECC P-256 signing key under the endorsement hierarchy with an empty authValue, for the
    ...
    private async Task<CreatePrimaryResponse> CreateSigningPrimaryAsync(TpmDevice tpm, TpmResponseRegistry registry, BaseMemoryPool pool)
    {
        ...
        Assert.IsTrue(result.IsSuccess, $"CreatePrimary (ECC P-256, endorsement) failed: '{result.ResponseCode}'.");
