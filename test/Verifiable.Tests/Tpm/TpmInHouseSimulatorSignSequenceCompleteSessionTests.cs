...
internal sealed class TpmInHouseSimulatorSignSequenceCompleteSessionTests
{
    ...
    /// </summary>
    [TestMethod]
    public async Task SignSequenceCompleteOverSessionsWithADifferentSigningKeyReturnsBareSignContextKeyAndRollsNoNonce()
    {
        BaseMemoryPool pool = BaseMemoryPool.Shared;
        using TpmSimulator simulator = await CreateOperationalAsync(nameof(SignSequenceCompleteOverSessionsWithADifferentSigningKeyReturnsBareSignContextKeyAndRollsNoNonce), pool).ConfigureAwait(false);
        using TpmDevice tpm = TpmDevice.Create(simulator.SubmitAsync);
        TpmResponseRegistry registry = CreateRegistry();
        ...
        try
        {
            ...
            Assert.AreEqual(
                TpmRcConstants.TPM_RC_SIGN_CONTEXT_KEY, refused.ResponseCode,
                "Completing over sessions with a key different from the one that started the sequence must answer the BARE TPM_RC_SIGN_CONTEXT_KEY, exactly as the password form does.");
    ...
    /// TPM": over sessions the flush rides the same transition that installs the framed response, so a second
    /// completion on the same handle answers <c>TPM_RC_HANDLE</c> while BOTH sessions remain live and complete a
    /// freshly started sequence (<see href="https://trustedcomputinggroup.org/resource/tpm-library-specification/">TPM
    ...
    public async Task SignSequenceCompleteOverSessionsFlushesTheSequenceOnSuccessWhileBothSessionsStayUsable()
    {
        ...
        try
        {
            ...
            TpmResult<SignSequenceCompleteResponse> second = await CompleteOverSessionsAsync(
                tpm, registry, pool, firstSequence, key.ObjectHandle, key.Name.AsReadOnlyMemory(), "message"u8.ToArray(), [sequenceSession, keySession]).ConfigureAwait(false);
            Assert.AreEqual(
                TpmRcConstants.TPM_RC_HANDLE, second.ResponseCode,
                "A second completion on an already-flushed sequence must be refused with TPM_RC_HANDLE, proving the flush rode the successful response.");
    ...
    /// </summary>
    [TestMethod]
    public async Task SignSequenceCompleteOverSessionsOnAUserWithAuthClearKeyReturnsBarePolicyFail()
    {
        BaseMemoryPool pool = BaseMemoryPool.Shared;
        using TpmSimulator simulator = await CreateOperationalAsync(nameof(SignSequenceCompleteOverSessionsOnAUserWithAuthClearKeyReturnsBarePolicyFail), pool).ConfigureAwait(false);
        using TpmDevice tpm = TpmDevice.Create(simulator.SubmitAsync);
        TpmResponseRegistry registry = CreateRegistry();
        ...
        try
        {
            ...
            Assert.AreEqual(
                ...
                "A userWithAuth-CLEAR key must refuse the key slot's HMAC session with a BARE TPM_RC_POLICY_FAIL: it is the session's SHAPE, not its credential, that is inadmissible for the USER role.");
    ...
    /// </summary>
    [TestMethod]
    public async Task SignSequenceCompleteOverSessionsOnAVerificationSequenceReturnsBareMode()
    {
        BaseMemoryPool pool = BaseMemoryPool.Shared;
        using TpmSimulator simulator = await CreateOperationalAsync(nameof(SignSequenceCompleteOverSessionsOnAVerificationSequenceReturnsBareMode), pool).ConfigureAwait(false);
        using TpmDevice tpm = TpmDevice.Create(simulator.SubmitAsync);
        TpmResponseRegistry registry = CreateRegistry();
        ...
        try
        {
            ...
            Assert.AreEqual(
                TpmRcConstants.TPM_RC_MODE, result.ResponseCode,
                "TPM2_SignSequenceComplete() over a verification sequence must answer the BARE TPM_RC_MODE, the same command rule the password form answers.");
        }
        finally
        {
            sequenceSession.Dispose();
    ...
    /// </summary>
    [TestMethod]
    public async Task SignSequenceCompleteOverSessionsOnARestrictedKeyWithATpmGeneratedFirstBlockReturnsBareAttributes()
    {
        BaseMemoryPool pool = BaseMemoryPool.Shared;
        using TpmSimulator simulator = await CreateOperationalAsync(nameof(SignSequenceCompleteOverSessionsOnARestrictedKeyWithATpmGeneratedFirstBlockReturnsBareAttributes), pool).ConfigureAwait(false);
        using TpmDevice tpm = TpmDevice.Create(simulator.SubmitAsync);
        TpmResponseRegistry registry = CreateRegistry();
        ...
        try
        {
            ...
            Assert.AreEqual(
                ...
                "A restricted signing key completing a message whose first presented block begins with TPM_GENERATED_VALUE must be refused with the BARE TPM_RC_ATTRIBUTES.");
        }
        finally
        {
            sequenceSession.Dispose();
    ...
    private async Task<TpmSimulator> CreateRsaCapableOperationalAsync(string name, BaseMemoryPool pool)
    {
        var simulator = new TpmSimulator(
            name, signingBackend: BouncyCastleTpmEccSigningBackend.Create(), rsaSigningBackend: MicrosoftTpmRsaSigningBackend.Create());
