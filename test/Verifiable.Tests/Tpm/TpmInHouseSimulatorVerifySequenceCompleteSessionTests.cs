...
internal sealed class TpmInHouseSimulatorVerifySequenceCompleteSessionTests
{
    ...
    /// completes, flushes the sequence, and a further completion answers <c>TPM_RC_HANDLE</c>
    /// (<see href="https://trustedcomputinggroup.org/resource/tpm-library-specification/">TPM 2.0 Library
    ...
    public async Task VerifySequenceCompleteOverAnHmacSessionWithAWrongSignatureAnswersSignatureBareAndRollsNoNonce()
    {
        ...
        try
        {
            ...
            Assert.AreEqual(
                TpmRcConstants.TPM_RC_SIGNATURE, badResult.ResponseCode,
                "A failed signature check is one of the command's own rules, answered BARE — never session-encoded — because no slot is to blame.");
            Assert.IsTrue(
            ...
            Assert.AreEqual(
                TpmRcConstants.TPM_RC_HANDLE, replayResult.ResponseCode,
                "A successful completion flushes the sequence context, so a further completion finds no such sequence (Part 1, clause 29.4.6's {F}).");
        }
        finally
        {
            session.Dispose();
    ...
    /// <summary>
    /// A companion claiming <c>audit</c> is refused with <c>TPM_RC_ATTRIBUTES</c> at its own index: no command
    /// audit is modelled, so a session asserting the command is being audited is refused rather than admitted and
    /// echoed back auditing nothing
    /// (<see href="https://trustedcomputinggroup.org/resource/tpm-library-specification/">TPM 2.0 Library
    /// Specification</see>, Part 2: Structures, clause 8.4, Table 38; Part 1: Architecture, clause 15.6.1).
    /// </summary>
    [TestMethod]
    public async Task VerifySequenceCompleteWithACompanionClaimingAuditIsRefusedWithAttributesAtTheCompanionSlot()
    {
        BaseMemoryPool pool = BaseMemoryPool.Shared;
        (TpmRcConstants responseCode, TpmRcConstants baseError) = await VerifyCompleteWithPlantedCompanionAsync(
            pool, TpmtSymDef.Xor(HmacSessionAlg), TpmaSession.CONTINUE_SESSION | TpmaSession.AUDIT).ConfigureAwait(false);

        Assert.AreEqual(TpmRcConstants.TPM_RC_ATTRIBUTES, baseError, "The audit attribute is refused categorically, the same fail-closed posture the encryption attributes get.");
        Assert.AreEqual(
            SessionEncodedRc(TpmRcConstants.TPM_RC_ATTRIBUTES, sessionIndex: 1), responseCode,
            "The refusal names the companion slot at index 1, session-index-encoded (Part 2, clause 6.6.2).");
    }
    ...
    /// </summary>
    [TestMethod]
    public async Task VerifySequenceCompleteOverAnHmacSessionWithADifferentKeyAnswersSignContextKeyBare()
    {
        BaseMemoryPool pool = BaseMemoryPool.Shared;
        using TpmSimulator simulator = await CreateOperationalAsync(pool).ConfigureAwait(false);
        using TpmDevice tpm = TpmDevice.Create(simulator.SubmitAsync);
        ...
        try
        {
            ...
            Assert.AreEqual(
                ...
                "A key other than the one that started the sequence is a command-rule refusal, answered bare, never session-encoded.");
    ...
    /// </summary>
    [TestMethod]
    public async Task VerifySequenceCompleteOverAnHmacSessionAgainstASigningSequenceAnswersModeBare()
    {
        BaseMemoryPool pool = BaseMemoryPool.Shared;
        using TpmSimulator simulator = await CreateOperationalAsync(pool).ConfigureAwait(false);
        using TpmDevice tpm = TpmDevice.Create(simulator.SubmitAsync);
        ...
        try
        {
            ...
            Assert.AreEqual(
                ...
                "A SIGNING sequence presented to TPM2_VerifySequenceComplete() must be refused with the bare TPM_RC_MODE.");
        }
        finally
        {
            session.Dispose();
        ...
        BaseMemoryPool pool, TpmtSymDef companionSymmetric, TpmaSession companionAttributes)
    {
        ...
        StartAuthSessionInput companionStartInput = StartAuthSessionInput.CreateUnboundUnsaltedHmacSession(HmacSessionAlg, companionSymmetric);
        ...
        TpmDevice tpm, TpmResponseRegistry registry, BaseMemoryPool pool, TpmtSymDef symmetric, ReadOnlyMemory<byte> authValue)
    {
        ...
        var session = new TpmSession(new TpmHandle(started.SessionHandle.Value), started.NonceTPM, HmacSessionAlg, pool, symmetric);
