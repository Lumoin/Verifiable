...
internal sealed class TpmCommandExecutorSequenceNameTests
{
    ...
    public async Task SimulatorAcceptsTheEmptyBufferNameTheExecutorDerivesForASequenceUpdateOverAnHmacSession()
    {
        ...
        StartAuthSessionInput sessionInput = StartAuthSessionInput.CreateUnboundUnsaltedHmacSession(SessionAlg, TestEntropy.NewCounterStream(), pool);
        ...
        using var session = new TpmSession(new TpmHandle(started.SessionHandle.Value), started.NonceTPM, SessionAlg, TestEntropy.NewCounterStream(), pool);
    ...
    private static TpmSession CreateUnboundHmacSession(ReadOnlySpan<byte> authValue, BaseMemoryPool pool)
    {
        ...
        var session = new TpmSession(new TpmHandle(HmacSessionHandle), nonceTpm, SessionAlg, TestEntropy.NewCounterStream(), pool);
