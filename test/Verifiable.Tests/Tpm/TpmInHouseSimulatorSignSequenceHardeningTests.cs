...
internal sealed class TpmInHouseSimulatorSignSequenceHardeningTests
{
    ...
    /// <c>TPM2_SignSequenceComplete()</c>'s own parser would have produced from a real wire frame. No private member,
    ...
    public async Task SignSequenceCompleteAgainstAKeyForcedX509SignReturnsAttributes()
    {
        ...
        Assert.AreEqual(
            HmacKeyHarness.HandleEncodedRc(TpmRcConstants.TPM_RC_ATTRIBUTES, 1), rejection.ResponseCode,
    ...
    public async Task SignSequenceCompleteAgainstAKeyForcedSignEncryptClearReturnsKey()
    {
        ...
        Assert.AreEqual(
            HmacKeyHarness.HandleEncodedRc(TpmRcConstants.TPM_RC_KEY, 1), rejection.ResponseCode,
    ...
    public async Task SignSequenceStartWithAuthOverMaxSizeHandFramedReturnsSize()
    {
        ...
        Assert.AreEqual(HmacKeyHarness.ParameterEncodedRc(TpmRcConstants.TPM_RC_SIZE, 0), code, "auth is TPM2_SignSequenceStart()'s first parameter (Table 87, index 0); one octet over Tpm2bAuth.MaxSize must be refused with parameter-encoded TPM_RC_SIZE.");
    }
    ...
    public async Task SignSequenceCompleteWithABufferOverMaxSizeHandFramedReturnsSize()
    {
        ...
        Assert.AreEqual(HmacKeyHarness.ParameterEncodedRc(TpmRcConstants.TPM_RC_SIZE, 0), code, "buffer is TPM2_SignSequenceComplete()'s sole parameter (Table 124, index 0); one octet over Tpm2bMaxBuffer.MaxSize must be refused with parameter-encoded TPM_RC_SIZE.");
    }
