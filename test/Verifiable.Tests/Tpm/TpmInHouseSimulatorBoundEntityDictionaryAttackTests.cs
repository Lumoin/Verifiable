...
internal sealed class TpmInHouseSimulatorBoundEntityDictionaryAttackTests
{
    ...
    public async Task DecryptCompanionSessionBoundToADaProtectedEntityWithHmacMismatchCountsAuthFail()
    {
        ...
        try
        {
            ...
            StartAuthSessionInput primaryStart = StartAuthSessionInput.CreateUnboundUnsaltedHmacSession(SessionAlg);
            ...
            using TpmSession primarySession = new(new TpmHandle(primarySessionHandle), primaryStartResult.Value.NonceTPM, SessionAlg, pool);
    ...
    public async Task PolicySessionBoundToADaProtectedEntityWithAuthFailureCountsAuthFail()
    {
        ...
        int written = rotationPolicy.ComputeDigest(SessionAlg, rotationAuthPolicy);
    ...
    private async Task DriveIntoLockoutAsync(TpmDevice device, BaseMemoryPool pool, TpmResponseRegistry registry)
    {
        ...
        //Arrange machinery, not the normative case under proof: the password arm answers the BARE
        //TPM_RC_AUTH_FAIL (not session-index-encoded) for this rejection. Lockout engagement itself is proven
    ...
    {
        var simulator = new TpmSimulator("tpm-in-house-bound-entity-da", signingBackend: BouncyCastleTpmEccSigningBackend.Create());
