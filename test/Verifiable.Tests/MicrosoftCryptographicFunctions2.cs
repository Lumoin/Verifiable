...
{
    public static class MicrosoftCryptographicFunctions2
    {
            ...
            string keyType)
        {
            ...
            if(!driverSupportedFormats.Any(pref => pref.Item1 == outputFormat && pref.keyType == keyType))
            {
        ...
        private static ValueTask<ReadOnlyMemory<byte>> SignECDsaAsync(
            ...
            string keyType)
        {
            var driverSupportedFormats = new[] { ("Raw", keyType) };

            var (transformedKeyMaterial, outputFormat) = keyMaterialTransformer(privateKey, inputFormat, driverSupportedFormats);
