...
namespace Verifiable.Tests.Builders
{
    ...
    public sealed class WebDidBuilderTests
    {
        ...
        public async ValueTask CanBuildComplexWebDidWithServicesAndVerificationRelationships(DidWebTestData testData)
        {
            ...
            var webDidDocument = builder.Build(testData.KeyPair.PublicKey, testData.CryptoSuite, webDomain, DidRepresentationType.JsonLd);
            ...
            Assert.AreEqual(1, webDidDocument.Context.Contexes.Count);
            ...
            Assert.AreEqual(testData.CryptoSuite.VerificationMethodType, verificationMethod.Type);
            ...
            {
                Assert.IsTrue(service.ServiceEndpoint!.StartsWith(expectedPathBase),
                    $"Service endpoint {service.ServiceEndpoint} should start with {expectedPathBase}");
            }
            ...
            bool verified = await resolvedVerificationMethod.VerifySignatureAsync(contentToSign, signature, ExactSizeMemoryPool<byte>.Shared);
        ...
        public void CanBuildWebDidWithAllRepresentationTypes(DidWebTestData testData)
        {
            ...
            Assert.AreEqual(2, docJsonLd.Context.Contexes!.Count);
