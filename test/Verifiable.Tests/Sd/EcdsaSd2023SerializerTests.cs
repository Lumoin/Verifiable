...
/// For end-to-end workflow tests, see <see cref="Credential.CredentialSecuringMethodsTests"/>.
/// </para>
/// <para>
...
public sealed class EcdsaSd2023SerializerTests
{
    ...
    public void BaseProofSerializationRoundTrips()
    {
        ...
        var parsed = EcdsaSd2023CborSerializer.ParseBaseProofBytes(serialized);
        ...
        Assert.IsTrue(baseSignature.AsSpan().SequenceEqual(parsed.BaseSignature), "Base signature must round-trip.");
        Assert.IsTrue(publicKey.AsSpan().SequenceEqual(parsed.PublicKey), "Public key must round-trip.");
        ...
        Assert.AreEqual("/issuer", parsed.MandatoryPointers[0]);
        Assert.AreEqual("/issuanceDate", parsed.MandatoryPointers[1]);
        Assert.AreEqual("/credentialSubject/id", parsed.MandatoryPointers[2]);
    }
    ...
    public void DerivedProofSerializationRoundTrips()
    {
        ...
        var parsed = EcdsaSd2023CborSerializer.ParseDerivedProofBytes(serialized, TestSetup.Base64UrlEncoder);
