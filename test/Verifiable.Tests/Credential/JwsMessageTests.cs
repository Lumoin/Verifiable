...
public sealed class JwsMessageTests
{
    ...
    public async ValueTask SignJwsReturnsJwsMessageWithCorrectStructure()
    {
        ...
        Assert.IsTrue(protectedHeader.ContainsKey("alg"));
        Assert.IsTrue(protectedHeader.ContainsKey("typ"));
        Assert.IsTrue(protectedHeader.ContainsKey("kid"));
        Assert.AreEqual("EdDSA", protectedHeader["alg"]);
        Assert.AreEqual(Ed25519VerificationMethodId, protectedHeader["kid"]);
    }
    ...
    [TestMethod]
    public async ValueTask CompactJwsRoundTripsToJwsMessage()
    {
        var credential = JsonSerializer.Deserialize<VerifiableCredential>(UnsignedCredentialJson, JsonOptions)!;
        PrivateKeyMemory privateKeyMemory = CreateEd25519PrivateKey();

        JwsMessage originalMessage = await credential.SignJwsAsync(
            privateKeyMemory,
            Ed25519VerificationMethodId,
            CredentialSerializer,
            HeaderSerializer,
        ...
        JwsMessage parsedMessage = JwsParsing.ParseCompact(
            compact,
            TestSetup.Base64UrlDecoder,
        ...
        Assert.IsTrue(originalMessage.Signatures[0].Signature.Span.SequenceEqual(parsedMessage.Signatures[0].Signature.Span));
    }
    ...
    {
        var header = new Dictionary<string, object> { ["alg"] = "ES256" };
        byte[] payload = [1, 2, 3, 4];
        byte[] signature = [5, 6, 7, 8];

        var sig1 = new JwsSignatureComponent("encoded", header, signature);
        var sig2 = new JwsSignatureComponent("encoded", header, signature);

        var msg1 = new JwsMessage(payload, sig1);
        var msg2 = new JwsMessage(payload, sig2);
    ...
    public void CompactSerializationThrowsForMultipleSignatures()
    {
        ...
        var sig1 = new JwsSignatureComponent("encoded1", header, signature);
        var sig2 = new JwsSignatureComponent("encoded2", header, signature);

        var message = new JwsMessage(payload, [sig1, sig2]);
    ...
    public void CompactSerializationThrowsForDetachedPayload()
    {
        ...
        var sig = new JwsSignatureComponent("encoded", header, signature);
        var message = new JwsMessage(payload, sig, isDetachedPayload: true);
