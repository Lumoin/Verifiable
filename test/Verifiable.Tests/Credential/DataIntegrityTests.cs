...
public sealed class DataIntegrityTests
{
    ...
    /// </summary>
    /*lang=json,strict*/
    private const string UnsignedCredentialJson = """
        {
            "@context": [
    ...
    public async ValueTask CreateDataIntegrityProofMatchesTestVector()
    {
        ...
        [
            new DataIntegrityProof
            {
                ...
                Created = new DateTime(2023, 2, 24, 23, 36, 38, DateTimeKind.Utc),
    ...
    public async ValueTask SignCredentialAsJwtSucceeds()
    {
        ...
        var headerBase64Url = Base64Url.Encode(Encoding.UTF8.GetBytes(headerJson));
        var payloadBase64Url = Base64Url.Encode(Encoding.UTF8.GetBytes(credentialJson));
        ...
        var jwt = $"{signingInput}.{Base64Url.Encode(signature.AsReadOnlySpan())}";
        ...
        var signatureBytesFromJwt = Base64Url.Decode(parts[2]);
        var pooledSignatureBytes = SensitiveMemoryPool<byte>.Shared.Rent(signatureBytesFromJwt.Length);
        signatureBytesFromJwt.CopyTo(pooledSignatureBytes.Memory.Span);
        var signatureToVerify = new Signature(pooledSignatureBytes, Tag.Ed25519Signature);
        ...
        var decodedPayload = Encoding.UTF8.GetString(Base64Url.Decode(parts[1]));
