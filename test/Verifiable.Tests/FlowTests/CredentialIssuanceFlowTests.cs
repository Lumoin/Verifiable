...
public sealed class CredentialIssuanceFlowTests
{
    ...
    public async Task TamperedCredentialFailsVerificationWithRdfc(DidWebTestData testData)
    {
        ...
        var unsignedCredential = await CredentialBuilder.BuildAsync(issuer, subject, additionalTypes: [AlumniCredentialType]);
    ...
    /// Uses FakeTimeProvider to control the verification time.
    /// </summary>
    [TestMethod]
    [DynamicData(nameof(DidWebTheoryData.GetDidTheoryTestData), typeof(DidWebTheoryData))]
    public async Task ExpiredCredentialIsDetected(DidWebTestData testData)
    {
        ...
        //Fixed credential validity period: 2025-01-01 to 2025-06-30.
        var validFrom = new DateTime(2025, 1, 1, 0, 0, 0, DateTimeKind.Utc);
        var validUntil = new DateTime(2025, 6, 30, 23, 59, 59, DateTimeKind.Utc);

        //Sign at a time when credential is valid.
        var signingTime = new FakeTimeProvider(new DateTimeOffset(2025, 3, 15, 12, 0, 0, TimeSpan.Zero));
        ...
        var signedCredential = await unsignedCredential.SignAsync(
            ...
            SerializeProofOptions,
            SensitiveMemoryPool<byte>.Shared,
            timeProvider: signingTime,
            cancellationToken: TestContext.CancellationToken);

        //Verify at a time after credential has expired.
        var verificationTime = new FakeTimeProvider(new DateTimeOffset(2025, 7, 15, 12, 0, 0, TimeSpan.Zero));
    ...
    public async Task NotYetValidCredentialIsDetected(DidWebTestData testData)
    {
        ...
        //Fixed credential validity period: 2025-07-01 to 2025-12-31.
        var validFrom = new DateTime(2025, 7, 1, 0, 0, 0, DateTimeKind.Utc);
        var validUntil = new DateTime(2025, 12, 31, 23, 59, 59, DateTimeKind.Utc);

        //Sign at a time before credential becomes valid (pre-issuance).
        var signingTime = new FakeTimeProvider(new DateTimeOffset(2025, 6, 15, 12, 0, 0, TimeSpan.Zero));
        ...
        //Verify at a time before credential becomes valid.
        var verificationTime = new FakeTimeProvider(new DateTimeOffset(2025, 6, 20, 12, 0, 0, TimeSpan.Zero));
