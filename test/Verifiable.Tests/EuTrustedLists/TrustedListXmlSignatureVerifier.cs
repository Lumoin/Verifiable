...
/// <remarks>
/// <para>
/// Staged, promotable worked example: production namespace (<c>Verifiable.Cryptography.Pki.Xml</c>,
/// matching <see cref="TrustedListXmlParser"/>), no test-framework type, no dependency on
/// <c>TestInfrastructure</c>. Only <see cref="System.Security.Cryptography.Xml"/> is test-project-only — its
/// <c>PackageReference</c> is on the test project, never on <c>Verifiable.Cryptography</c>.
/// </para>
/// <para>
...
public static class TrustedListXmlSignatureVerifier
{
        ...
        XmlElement signatureElement, X509Certificate2 signerCertificate, BaseMemoryPool pool, CancellationToken cancellationToken)
    {
        ...
        PkiDigestAlgorithm? digestAlgorithm = XmlSignatureWellKnown.DigestAlgorithmFromUri(digestAlgorithmUri);
        ...
        using DigestValue computed = await CryptographicKeyEvents.ComputeDigestAsync(
            signerCertificate.RawData, digestAlgorithm.Value.OutputByteLength, digestAlgorithm.Value.DigestTag, pool, cancellationToken: cancellationToken).ConfigureAwait(false);
