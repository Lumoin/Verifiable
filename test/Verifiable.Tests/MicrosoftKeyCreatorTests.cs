...
namespace Verifiable.Tests
{
    ...
    public class MicrosoftKeyCreatorTests
    {
        ...
        {
            var rsaKeys = MicrosoftKeyCreator.CreateRsa2048Keys(ExactSizeMemoryPool<byte>.Shared);
            var signature = MicrosoftCryptographicFunctions3.SignRs256(rsaKeys.PrivateKey.AsReadOnlyMemory(), TestData);
            bool isVerified = await MicrosoftCryptographicFunctions3.VerifyRsa(TestData, new ReadOnlyMemory<byte>(signature.ToArray()), rsaKeys.PublicKey.AsReadOnlyMemory());
            Assert.IsTrue(isVerified);
        }
    }
    ...
    {
        public static ValueTask<bool> VerifyRsa(ReadOnlyMemory<byte> dataToVerify, ReadOnlyMemory<byte> signature, ReadOnlyMemory<byte> publicKeyMaterial)
        {
            try
            {
                using(RSA rsa = RSA.Create())
                {
                    // First decode using RsaUtilities to get the actual modulus
                    //byte[] modulus = publicKeyMaterial.ToArray();
                    byte[] modulus = RsaUtilities.Decode(publicKeyMaterial.Span);
                    ...
                    // Verify using the same algorithm and padding as SignRs256
                    var isVerified = rsa.VerifyData(dataToVerify.Span, signature.Span, HashAlgorithmName.SHA256, RSASignaturePadding.Pkcs1);
                    var isVerified2 = rsa.VerifyData(dataToVerify.Span, signature.Span, HashAlgorithmName.SHA256, RSASignaturePadding.Pss);
                    return ValueTask.FromResult(isVerified);
                }
            }
            catch(Exception ex)
            {
                Console.WriteLine($"RSA verification failed: {ex.Message}, {ex.StackTrace}");
        ...
        public static ReadOnlySpan<byte> SignRs256(ReadOnlyMemory<byte> privateKeyBytes, ReadOnlySpan<byte> dataToSign)
        {
            return SignRsa(privateKeyBytes.Span, dataToSign, HashAlgorithmName.SHA256, RSASignaturePadding.Pkcs1);
        }


        public static ReadOnlySpan<byte> SignRsa_2(ReadOnlyMemory<byte> publicKeyBytes, ReadOnlyMemory<byte> privateKeyBytes, ReadOnlySpan<byte> dataToSign)
        {
            using(RSA rsa = RSA.Create())
            {
                rsa.ImportParameters(new RSAParameters
                {
                    Modulus = publicKeyBytes.ToArray(),
                    Exponent = [0x01, 0x00, 0x01], //Default exponent 65537.
                    D = privateKeyBytes.ToArray()
                });
        ...
        private static ReadOnlySpan<byte> SignRsa(ReadOnlySpan<byte> privateKeyBytes, ReadOnlySpan<byte> dataToSign, HashAlgorithmName hashAlgorithmName, RSASignaturePadding rsaSignaturePadding)
        {
            ...
            {
                key.ImportRSAPrivateKey(privateKeyBytes, out _);
