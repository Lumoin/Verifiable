...
internal sealed class ResolveCapabilitiesAsyncTests
{
    ...
    public async Task ResolveCapabilitiesAsyncIsConsultedPerRequest()
    {
        ...
        //Per-request: read a flag the test sets on RequestContext to decide
        ...
        host.Server.Integration.ResolveCapabilitiesAsync = (registration, ctx, ct) =>
        {
            ...
            HashSet<ServerCapabilityName> active = vetoJwks
                ? [.. registration.AllowedCapabilities.Where(c => c != ServerCapabilityName.JwksEndpoint)]
        ...
        RequestContext contextWithoutVeto = new();
        ...
        RequestContext contextWithVeto = new();
