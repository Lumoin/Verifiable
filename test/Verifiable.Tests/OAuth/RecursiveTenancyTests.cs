...
internal sealed class RecursiveTenancyTests
{
    ...
    private static ImmutableHashSet<ServerCapabilityName> OperatorCapabilities { get; } =
    ...
    private static ImmutableHashSet<ServerCapabilityName> CustomerCapabilities { get; } =
        ImmutableHashSet.Create(
            ServerCapabilityName.AuthorizationCode,
            ServerCapabilityName.JwksEndpoint,
            ServerCapabilityName.DiscoveryEndpoint);
    ...
    public async Task PerRequestCapabilityAttenuationIsTenantScoped()
    {
        ...
        {
            HashSet<ServerCapabilityName> active = [.. registration.AllowedCapabilities];
            if(registration.TenantId.Value.StartsWith("cust-", StringComparison.Ordinal))
            {
                active.Remove(ServerCapabilityName.JwksEndpoint);
            }
            return ValueTask.FromResult<IReadOnlySet<ServerCapabilityName>>(active);
        };
