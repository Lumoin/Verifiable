...
/// <summary>
/// Shared fixtures for the CTAP enterprise attestation test suites: builds a
...
/// </summary>
internal static class CtapEnterpriseAttestationFixtures
{
    /// <summary>The key identifier every fixture-minted enterprise attestation key is registered under.</summary>
    private const string AttestationKeyIdentifier = "ctap-ep-fixture-attestation-key";
    ...
    private const string RootSubjectName = "CN=Ctap Enterprise Attestation Capstone Root";
    ...
    /// bytes to the state/getInfo surface, which never parses or validates certificate content.
    /// </summary>
    ...
    /// can independently verify a certified mint's signature was produced by THIS specific key, the one
    /// thing <see cref="BuildProvisioning"/> itself discards.
    /// </summary>
    /// <param name="pool">The memory pool the attestation key is minted from.</param>
    ...
    /// real-wire capstones that close the loop through the already-shipped RP-side
    /// <c>PackedAttestation.VerifyCertifiedAsync</c> — unlike <see cref="BuildProvisioning"/>'s opaque
    /// 32-byte placeholder (sufficient for state/getInfo/mint-shape tests, which never
    ...
    /// non-critical OCTET STRING — never the AAGUID extension's double wrap). The
    ...
    /// "wrapped in two OCTET STRINGS" AAGUID shape.
    /// </summary>
    ...
    /// <see cref="CtapMakeCredentialGetAssertionFixtures.CreateSimulator"/> — the one composition every
    /// enterprise-attestation test needing a capable authenticator shares, rather than each reimplementing the
    ...
    /// </summary>
    /// <param name="runId">The simulator's run identifier, threaded to <see cref="CtapMakeCredentialGetAssertionFixtures.CreateSimulator"/>.</param>
    /// <param name="pool">The memory pool the seeded attestation key and certificate chain are minted from.</param>
    /// <param name="preConfiguredRpIds">The vendor's pre-configured RP ID list — see <see cref="BuildProvisioning"/>.</param>
    /// <param name="timeProvider">The simulator's time source, threaded to <see cref="CtapMakeCredentialGetAssertionFixtures.CreateSimulator"/>.</param>
    /// <returns>The built, enterprise-attestation-capable simulator. The caller owns it and must dispose it.</returns>
    [SuppressMessage("Reliability", "CA2000:Dispose objects before losing scope",
        Justification = "Ownership of the built CtapEnterpriseAttestationProvisioning transfers to CtapMakeCredentialGetAssertionFixtures.CreateSimulator's own enterpriseAttestationProvisioning parameter, and from there to the returned CtapAuthenticatorSimulator, whose Dispose disposes it; the analyzer cannot see this transfer through the nested method calls.")]
        ...
        string runId, BaseMemoryPool pool, IReadOnlyList<string>? preConfiguredRpIds, CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            using PooledMemory response = await CtapConfigFixtures.SendAuthenticatorConfigAsync(simulator, request, pool, cancellationToken).ConfigureAwait(false);
