            ...
            CryptoFunctionRegistry<CryptoAlgorithm, Purpose>.Initialize(
                (CryptoAlgorithm algorithm, Purpose purpose, string? qualifier = null) =>
                {
                    return (algorithm, purpose, qualifier) switch
                    {
                        ...
                        var (alg, purp, q) when alg.Equals(CryptoAlgorithm.Rsa2048) && purp.Equals(Purpose.Signing) => MicrosoftCryptographicFunctions.SignRsaSha256Pkcs1Async,
                        var (alg, purp, q) when alg.Equals(CryptoAlgorithm.Rsa4096) && purp.Equals(Purpose.Signing) => MicrosoftCryptographicFunctions.SignRsaSha512Pkcs1Async,
                ...
                (CryptoAlgorithm algorithm, Purpose purpose, string? qualifier = null) =>
                {
                    return (algorithm, purpose, qualifier) switch
                    {
                        ...
                        var (alg, purp, q) when alg.Equals(CryptoAlgorithm.Rsa2048) && purp.Equals(Purpose.Signing) => MicrosoftCryptographicFunctions.VerifyRsaSha256Pkcs1Async,
                        var (alg, purp, q) when alg.Equals(CryptoAlgorithm.Rsa4096) && purp.Equals(Purpose.Signing) => MicrosoftCryptographicFunctions.VerifyRsaSha512Pkcs1Async,
