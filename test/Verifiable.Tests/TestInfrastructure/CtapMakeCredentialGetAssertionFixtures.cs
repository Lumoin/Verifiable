...
/// <summary>
/// Shared fixtures for the CTAP <c>authenticatorMakeCredential</c>/<c>authenticatorGetAssertion</c>
...
/// </summary>
internal static class CtapMakeCredentialGetAssertionFixtures
{
    ...
    /// provisioning fixture's leaf certificate (the <c>id-fido-gen-ce-aaguid</c> extension) can
    ...
    /// is ALWAYS wired, regardless of whether <paramref name="enterpriseAttestationProvisioning"/>
    ...
    /// <summary>Builds an <c>authenticatorMakeCredential</c> request model with sensible defaults.</summary>
    ...
    /// input), or <see langword="null"/> to omit the key.
    /// </param>
    ...
    /// <summary>Builds an <c>authenticatorGetAssertion</c> request model with sensible defaults.</summary>
    [SuppressMessage("Reliability", "CA2000:Dispose objects before losing scope",
    ...
    {
        byte[] envelope = CtapMakeCredentialGetAssertionRequestEnvelopes.BuildGetNextAssertionEnvelope();
    ...
    /// requested; <paramref name="excludeList"/>/<paramref name="pinUvAuthParam"/>/
    /// <paramref name="pinUvAuthProtocol"/> thread through to the underlying mc request for
    ...
    public static async Task<CtapRegisteredCredential> RegisterCredentialAsync(
    ...
    /// <summary>Convenience overload of <see cref="RegisterCredentialAsync"/> for tests that need only the minted credential ID bytes, disposing the intermediate carrier once its bytes are copied out. <paramref name="credProtect"/> mints the credential with that <c>credProtect</c> level requested.</summary>
...
/// <summary>
/// A credential minted by <see cref="CtapMakeCredentialGetAssertionFixtures.RegisterCredentialAsync"/>: the
...
internal sealed record CtapRegisteredCredential(CredentialId CredentialId, CoseKey PublicKey);
