...
/// <summary>
/// The real-wire capstones' platform-role fixtures for <c>authenticatorLargeBlobs</c> (<c>0x0C</c>)
...
/// <see cref="CtapNfcTransportHarness"/>, mirroring <see cref="CtapLargeBlobsFixtures"/>'s
/// authenticator-side role but confined to the platform's own
/// obligations (everything here except truncated SHA-256 is platform-actor, never
/// authenticator-side).
/// </summary>
/// <remarks>
/// <para>
...
/// EXISTING GENERIC FUNCTIONS satisfies the house "check the project crypto surface first" rule; only
...
/// array of maps followed by <c>LEFT(SHA-256(array bytes), 16)</c>) are platform-only shapes this library
...
/// <see cref="CtapMakeCredentialGetAssertionFixtures.BuildMakeCredentialExtensionsInput"/> hand-rolls extension
/// input CBOR with <see cref="CborWriter"/> directly.
/// </para>
/// </remarks>
internal static class CtapLargeBlobPlatformFixtures
{
    ...
    /// THIRTY-TWO-bit little-endian <c>offset</c>.
    /// </summary>
    ...
    /// gate's armed path) or omitting the auth pair entirely when it is <see langword="null"/> (the
        ...
        CtapPinUvAuthProtocolId protocolId, CancellationToken cancellationToken)
    {
        ...
        {
            byte[] param = await CtapLargeBlobsFixtures.ComputeSetSignatureAsync(token, protocolId, offset, fragment, pool, cancellationToken)
                .ConfigureAwait(false);
