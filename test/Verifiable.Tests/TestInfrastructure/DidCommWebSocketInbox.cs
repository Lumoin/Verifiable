...
/// </para>
/// <para>
/// SSRF: this loopback test channel connects the <c>ClientWebSocket</c> directly, with no policy check, because
/// the endpoint is a trusted loopback inbox. A PRODUCTION socket transport cannot reuse the HTTP path's
/// <c>OutboundFetch</c> (its URL + redirect loop is HTTP-only), so it MUST itself resolve the endpoint host and
/// call <c>OutboundFetchPolicy.EvaluateResolvedAddress</c> on each resolved address — connecting only to a
/// permitted, pinned address — BEFORE <c>ClientWebSocket.ConnectAsync</c>, mirroring the connection-time pinning
/// the HTTP path gets for free from <c>OutboundFetch</c>.
/// </para>
/// </remarks>
internal sealed class DidCommWebSocketInbox: IAsyncDisposable
{
    ...
    private DidCommWebSocketInbox(WebApplication app)
    {
        this.app = app;
    }


    /// <summary>The <c>ws://127.0.0.1:{port}/</c> endpoint a sender delivers to.</summary>
    ...
    public static async Task<DidCommWebSocketInbox> StartAsync(CancellationToken cancellationToken)
    {
        ...
        var inbox = new DidCommWebSocketInbox(app);
        ...
        inbox.Endpoint = new UriBuilder(boundAddress) { Scheme = "ws", Path = "/" }.Uri;
    ...
    public static DidCommSendDelegate CreateSendDelegate()
    {
