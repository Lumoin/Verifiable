...
/// <summary>
/// Builds a platform-side <c>authenticatorClientPIN</c> cryptographic session for the PIN-path
...
/// <c>authenticate</c>) rather than a test-only crypto reimplementation. ECDH is symmetric: calling
/// <see cref="CtapPinUvAuthProtocol.DecapsulateAsync"/> with the
...
/// </summary>
internal static class CtapPinCryptoFixtures
{
    ...
    public static async Task<CtapPlatformPinSession> EstablishSessionAsync(
        Ctap2TransceiveDelegate transceive, CtapPinUvAuthProtocolId protocolId, BaseMemoryPool pool, CancellationToken cancellationToken)
    {
        ...
        return new CtapPlatformPinSession(protocol, sharedSecret, platformKeys.PrivateKey, platformKeys.PublicKey, platformPublicKeyCose, pool);
    }
...
/// <see cref="CtapPinUvAuthProtocol"/> operations the PIN-path subcommand tests drive the
...
/// </summary>
internal sealed class CtapPlatformPinSession: IDisposable
{
    ...
    /// <summary>Initializes an already-established session. Use <see cref="CtapPinCryptoFixtures.EstablishSessionAsync"/>.</summary>
    internal CtapPlatformPinSession(
    ...
    {
        byte[] paddedPin = CtapPinCryptoFixtures.BuildPaddedPin(newPin);
    ...
    /// drive a pinHashEnc that fails to DECRYPT (<see cref="CtapPinCryptoFixtures.BuildMalformedPinHashEnc"/>)
    ...
    {
        byte[] paddedNewPin = CtapPinCryptoFixtures.BuildPaddedPin(newPin);
    ...
    /// 13228-13248): <c>saltEnc = encrypt(sharedSecret, salt1 [|| salt2])</c>,
    ...
    /// <see cref="CtapPinCryptoFixtures.BuildMalformedPinHashEnc"/>'s own shape: protocol two's
