...
/// <summary>
/// Shared fixtures for the <c>authenticatorLargeBlobs</c> (<c>0x0C</c>) write-machine test suite:
...
/// the same way <see cref="CtapConfigFixtures"/> computes <c>authenticatorConfig</c>'s own verify
...
/// bootstrap sequences (setPIN, fingerprint enrollment completion, <c>toggleAlwaysUv</c>) the gate's
/// three arming triggers each need.
/// </summary>
internal static class CtapLargeBlobsFixtures
{
    ...
    public static async Task<byte[]> EstablishPinAndIssueTokenAsync(
        ...
        string? rpId = null)
    {
        await CtapConfigFixtures.EstablishPinAsync(simulator, pool, protocolId, DefaultPin, cancellationToken).ConfigureAwait(false);

        return await CtapConfigFixtures.IssueTokenAsync(simulator, pool, protocolId, DefaultPin, permissions, rpId, cancellationToken).ConfigureAwait(false);
    }
    ...
    /// own token gate is UNCONDITIONAL — and <c>getPinUvAuthTokenUsingUvWithPermissions</c>
    ...
    /// alone, no PIN ever set" — the gate's own <c>IsProtectedByUserVerification</c> predicate is
    ...
    /// <summary>Computes a gated <c>authenticatorBioEnrollment</c> subcommand's own <c>pinUvAuthParam</c> (<c>uint8(modality) || uint8(subCommand) || subCommandParams</c>).</summary>
