...
/// mc/ga fixtures compute their own <c>pinUvAuthParam</c> — through
/// <see cref="CtapPinUvAuthProtocol.AuthenticateAsync"/> over the actual token bytes, never a test-only
/// crypto reimplementation. Mirrors <see cref="CtapMakeCredentialGetAssertionFixtures"/>'s envelope-building
/// role for the mc/ga surface.
/// </summary>
internal static class CtapConfigFixtures
{
    ...
    /// the platform sends none, eliding that segment entirely rather than encoding an
    /// empty CBOR map.
    /// </summary>
    ...
    /// <c>pinUvAuthParam</c> against, mirroring the binding tests' own helper for mc/ga.
    /// </summary>
