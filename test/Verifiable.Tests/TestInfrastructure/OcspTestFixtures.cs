...
/// <summary>
/// Mints RFC 6960/RFC 5280 fixtures for the OCSP test classes: certificates carrying Authority
...
internal static class OcspTestFixtures
{
    ...
    /// <summary>
    /// Builds a minimal, hand-assembled, well-formed <c>OCSPResponse</c> carrying only what
    /// <see cref="Verifiable.Cryptography.Pki.OcspResponseVerification.TryReadProducedAt"/> needs to read: the
    /// envelope's <c>responseStatus</c>/<c>responseBytes</c>, and <c>BasicOCSPResponse.tbsResponseData</c>'s
    /// <c>responderID</c> (a <c>byKey</c> stand-in, skipped by that method) and <c>producedAt</c>. No
    /// <c>SingleResponse</c> content, signature, or embedded certificate — this fixture serves the producedAt-only
    /// cross-check (XAdES's <c>OCSPIdentifier/ProducedAt</c> consistency check), never signature verification.
    /// </summary>
    /// <param name="producedAt">The <c>producedAt</c> instant to encode.</param>
    /// <returns>The pooled response carrier; the caller disposes it.</returns>
    internal static PkiCertificateMemory BuildMinimalOcspResponseWithProducedAt(DateTimeOffset producedAt)
    {
        var basicWriter = new AsnWriter(AsnEncodingRules.DER);
        using(basicWriter.PushSequence())                                              //BasicOCSPResponse.
        {
            using(basicWriter.PushSequence())                                          //tbsResponseData.
            {
                ...
                basicWriter.WriteGeneralizedTime(producedAt);                          //producedAt.
                using(basicWriter.PushSequence())                                      //responses -- empty.
                {
                }
            }
