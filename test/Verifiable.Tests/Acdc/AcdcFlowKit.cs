...
internal static class AcdcFlowKit
{
    ...
    /// <summary>
    /// Verifies a fetched Issuer KEL and returns its anchored seals: it reconstructs the KEL from the fetched bytes
    /// and replays it through the production <see cref="KeriIssuerAnchors.ReplayAsync"/> path, requiring every event
    /// to verify and the replay's own established AID to be the expected Issuer's. Returns <see langword="null"/>
    /// when the KEL does not verify or is not the expected Issuer's, so a caller can fail closed.
    /// </summary>
    /// <param name="kelBytes">The fetched KEL bytes.</param>
    /// <param name="expectedAid">The Issuer AID the replay must establish.</param>
    ...
    /// <returns>The anchored seals, AID-paired by the replay, or <see langword="null"/> when the KEL does not verify or is not the expected Issuer's.</returns>
    public static Task<IReadOnlyList<KeriAnchoredSeal>?> VerifyKelAndReadAnchorsAsync(ReadOnlyMemory<byte> kelBytes, string expectedAid, List<IDisposable> disposables, BaseMemoryPool pool, CancellationToken cancellationToken)
    {
    ...
    /// <param name="cancellationToken">A token to cancel the replay.</param>
    /// <returns>The anchored seals, AID-paired by the replay, or <see langword="null"/> when the KEL does not verify or is not the expected Issuer's.</returns>
    public static async Task<IReadOnlyList<KeriAnchoredSeal>?> VerifyKelAndReadAnchorsAsync(ReadOnlyMemory<byte> kelBytes, string expectedAid, DelegationSealResolver? resolveDelegationSeal, List<IDisposable> disposables, BaseMemoryPool pool, CancellationToken cancellationToken)
    {
        List<LogEntry<KeriKeyEvent, CryptoProof>> log = ReconstructKel(kelBytes, disposables, pool);
        var events = new List<KeriKelEvent>(log.Count);
        foreach(LogEntry<KeriKeyEvent, CryptoProof> entry in log)
        {
            events.Add(new KeriKelEvent(entry.CanonicalBytes, entry.Proofs));
        }

        KeriIssuerAnchorReplayResult replay = await KeriIssuerAnchors.ReplayAsync(
            events, JsonFieldMapDecoder, CesrSerializationKind.Json, AcdcTestSupport.AgileDigest, pool, new FakeTimeProvider(TestClock.CanonicalEpoch), resolveDelegationSeal, cancellationToken).ConfigureAwait(false);

        if(!replay.IsVerified || !string.Equals(replay.IssuerAid, expectedAid, StringComparison.Ordinal))
        {
            return null;
        }
