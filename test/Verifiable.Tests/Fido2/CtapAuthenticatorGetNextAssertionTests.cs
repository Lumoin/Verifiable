...
internal sealed class CtapAuthenticatorGetNextAssertionTests
{
    ...
    public async Task GetNextAssertionSignatureReusesOriginalClientDataHashAndVerifiesIndependently()
    {
        ...
        CtapRegisteredCredential olderCredential = await RegisterCredentialAsync(simulator, pool, olderUserId, TestContext.CancellationToken);
    ...
    /// <summary>
    /// A single-credential <c>authenticatorGetAssertion</c> (the ordinary case) remembers no
    ...
    private static byte[] ReadAndDisposeUserId(PooledMemory response, BaseMemoryPool pool)
    {
        using(response)
        {
            Assert.AreEqual(WellKnownCtapStatusCodes.Ok, response.AsReadOnlySpan()[0]);
