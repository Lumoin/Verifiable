...
/// <summary>
/// The real-wire capstones for enterprise attestation (CTAP 2.3 §7.1): three end-to-end
/// flows over the UNCHANGED <see cref="CtapNfcTransportHarness"/>, each reconstructing every assertion
...
/// verifier — the ONE test here that mints a REAL X.509 certificate chain via
/// <see cref="CtapEnterpriseAttestationFixtures.BuildRealCertificateProvisioning"/>, rather than the other tests' opaque
...
internal sealed class CtapAuthenticatorEnterpriseAttestationFlowTests
{
    ...
    private const string VendorListedRpId = "ep-capstone-vendor.example";

    /// <summary>An rp.id PROVABLY absent from the vendor's pre-configured list — grants only under <c>enterpriseAttestation: 2</c>, the platform-vetted path.</summary>
    private const string PlatformVettedRpId = "ep-capstone-platform.example";

    /// <summary>An rp.id used for the personal (non-enterprise, parameter-absent) mc request.</summary>
    private const string PersonalRpId = "ep-capstone-personal.example";

    /// <summary>An rp.id never on any fixture's pre-configured list, for the value-1 fallthrough leg.</summary>
    private const string UnlistedRpId = "ep-capstone-unlisted.example";

    /// <summary>
    /// The <c>clientDataHash</c> bytes every <see cref="CtapMakeCredentialGetAssertionFixtures.BuildMakeCredentialRequest"/>
    ...
    /// </summary>
    private static byte[] ExpectedClientDataHashBytes => CtapMakeCredentialGetAssertionFixtures.BuildFixedBytes(32, 0x10);


    /// <summary>
    /// The lifecycle capstone: ONE device, ONE wire session, all three enterprise-attestation
    ...
    /// no PIN is ever established in this capstone) &#8594; <c>getInfo</c> now shows
    /// <c>ep:true</c> &#8594; a PERSONAL mc (no <c>enterpriseAttestation</c> parameter) returns a regular
    /// self attestation with <c>epAtt</c> absent &#8594; <c>enterpriseAttestation: 1</c> for the
    /// VENDOR-pre-provisioned (seed-listed) rp.id grants <c>epAtt: true</c> &#8594; <c>enterpriseAttestation:
    /// 2</c> for a PLATFORM-vetted rp.id PROVABLY absent from the vendor list (asserted against the
    /// same list literal the fixture was seeded with) also grants <c>epAtt: true</c> — proving the two
    /// authorization sources are genuinely independent. The value-1 grant's wire <c>attStmt</c> is
    ...
    public async Task LifecycleAcrossAllThreeContextsClosesThroughRpVerifierOverRealApduTransport()
    {
        ...
        (CtapEnterpriseAttestationProvisioning provisioning, X509Certificate2 rootCertificateResult) =
            CtapEnterpriseAttestationFixtures.BuildRealCertificateProvisioning(pool, aaguid, serialNumber, vendorPreConfiguredRpIds);
        ...
        CtapMakeCredentialRequest personalRequest = CtapMakeCredentialGetAssertionFixtures.BuildMakeCredentialRequest(
            pool, rpId: PersonalRpId, userId: CtapMakeCredentialGetAssertionFixtures.BuildFixedBytes(16, 0x81));
        ...
        CtapMakeCredentialGetAssertionFixtures.DisposeMakeCredentialRequest(personalRequest);
        Assert.IsFalse(personalDecoded.EpAtt.HasValue, "the personal (parameter-absent) mc must never carry epAtt.");
        ...
        Assert.IsNull(personalStatement.X5c, "the personal (parameter-absent) mc must be a regular self attestation, no x5c.");

        CtapMakeCredentialRequest vendorRequest = CtapMakeCredentialGetAssertionFixtures.BuildMakeCredentialRequest(
            pool, rpId: VendorListedRpId, userId: CtapMakeCredentialGetAssertionFixtures.BuildFixedBytes(16, 0x82), enterpriseAttestation: 1);
        CtapMakeCredentialResponse vendorDecoded = await CtapAuthenticatorMakeCredentialClient.MakeCredentialAsync(
            ...
            .ConfigureAwait(false);
        CtapMakeCredentialGetAssertionFixtures.DisposeMakeCredentialRequest(vendorRequest);
        ...
        Assert.IsFalse(
            ...
            "the platform-vetted rp.id used below must be PROVABLY absent from the same vendor list literal the fixture was seeded with.");
        CtapMakeCredentialRequest platformRequest = CtapMakeCredentialGetAssertionFixtures.BuildMakeCredentialRequest(
            pool, rpId: PlatformVettedRpId, userId: CtapMakeCredentialGetAssertionFixtures.BuildFixedBytes(16, 0x83), enterpriseAttestation: 2);
        CtapMakeCredentialResponse platformDecoded = await CtapAuthenticatorMakeCredentialClient.MakeCredentialAsync(
            ...
            .ConfigureAwait(false);
        CtapMakeCredentialGetAssertionFixtures.DisposeMakeCredentialRequest(platformRequest);
        Assert.IsTrue(
    ...
    /// <summary>
    /// The vendor-facilitated wire proof (CTAP 2.3 line 3345/3350): on ONE capable+enabled
    ...
    /// than in-process (mirroring the equivalent in-process matrix).
    /// </summary>
    [TestMethod]
    public async Task VendorFacilitatedGrantIsGatedByPreConfiguredRpIdListOverRealApduTransport()
    {
        ...
        CtapMakeCredentialRequest listedRequest = CtapMakeCredentialGetAssertionFixtures.BuildMakeCredentialRequest(
            pool, rpId: VendorListedRpId, userId: CtapMakeCredentialGetAssertionFixtures.BuildFixedBytes(16, 0x91), enterpriseAttestation: 1);
        CtapMakeCredentialResponse listedDecoded = await CtapAuthenticatorMakeCredentialClient.MakeCredentialAsync(
            ...
            .ConfigureAwait(false);
        CtapMakeCredentialGetAssertionFixtures.DisposeMakeCredentialRequest(listedRequest);
        ...
        CtapMakeCredentialRequest unlistedRequest = CtapMakeCredentialGetAssertionFixtures.BuildMakeCredentialRequest(
            pool, rpId: UnlistedRpId, userId: CtapMakeCredentialGetAssertionFixtures.BuildFixedBytes(16, 0x92), enterpriseAttestation: 1);
        CtapMakeCredentialResponse unlistedDecoded = await CtapAuthenticatorMakeCredentialClient.MakeCredentialAsync(
            ...
            .ConfigureAwait(false);
        CtapMakeCredentialGetAssertionFixtures.DisposeMakeCredentialRequest(unlistedRequest);
    ...
    public async Task AuthenticatorResetDisablesEnterpriseAttestationOverRealApduTransport()
    {
        ...
        CtapMakeCredentialRequest postResetRequest = CtapMakeCredentialGetAssertionFixtures.BuildMakeCredentialRequest(
        ...
        CtapCommandException postResetException = await Assert.ThrowsExactlyAsync<CtapCommandException>(() =>
            CtapAuthenticatorMakeCredentialClient.MakeCredentialAsync(
                ...
                .AsTask());
        CtapMakeCredentialGetAssertionFixtures.DisposeMakeCredentialRequest(postResetRequest);
    ...
    /// <summary>Reports whether <paramref name="rpIds"/> contains <paramref name="rpId"/> — the provably-absent check, with no collection-type dependency beyond <see cref="IReadOnlyList{T}"/>.</summary>
