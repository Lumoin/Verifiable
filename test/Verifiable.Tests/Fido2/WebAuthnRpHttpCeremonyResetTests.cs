...
/// <summary>
/// Capstone C: the real-Kestrel/real-APDU RP-HTTP ceremony proof that <c>authenticatorReset</c>
/// has an RP-VISIBLE consequence -- composing <see cref="WebAuthnRelyingPartyCeremonySkin"/> exactly like
/// <see cref="WebAuthnRpHttpCeremonyCredentialManagementTests"/> does, registering a discoverable
/// credential through the real ceremony, proving one assertion succeeds, then issuing bare
/// <c>authenticatorReset</c> (<c>0x07</c>) over the wire and proving the subsequent assertion ceremony
/// fails because the authenticator itself refuses the client's <c>authenticatorGetAssertion</c> call with
/// <see cref="WellKnownCtapStatusCodes.NoCredentials"/> (<c>0x2E</c>) -- observed as
/// <see cref="CtapCommandException.StatusCode"/> on the exact call the ceremony's client-side leg makes --
/// before finally proving a FULL re-registration ceremony through the SAME relying party succeeds
/// factory-fresh. The existing ceremony capstones in <see cref="WebAuthnRpHttpCeremonyTests"/>,
/// <see cref="WebAuthnRpHttpCeremonyRequiredUserVerificationTests"/>,
/// <see cref="WebAuthnRpHttpCeremonyAlwaysUvTests"/>, and
/// <see cref="WebAuthnRpHttpCeremonyCredentialManagementTests"/> are untouched by this file and remain
/// green.
/// </summary>
[TestClass]
internal sealed class WebAuthnRpHttpCeremonyResetTests
{
    /// <summary>Gets or sets the test context, supplying the ambient cancellation token.</summary>
    public TestContext TestContext { get; set; } = null!;

    /// <summary>The relying party identifier this capstone's ceremonies are scoped to.</summary>
    private const string RpId = "webauthn-rp-http-reset.example";

    /// <summary>The relying party origin this capstone's ceremonies embed and expect.</summary>
    private const string Origin = "https://webauthn-rp-http-reset.example";

    /// <summary>The PIN established on the simulator this capstone drives, both before and (freshly) after the reset.</summary>
    ...
    /// against it, then issues bare <c>authenticatorReset</c> over the wire -- proving the subsequent
    /// assertion ceremony fails to complete: the authenticator itself refuses the client's
    /// <c>authenticatorGetAssertion</c> call with <see cref="WellKnownCtapStatusCodes.NoCredentials"/>, so
    /// no assertion result ever reaches the RP -- and that a FULL re-registration ceremony through the
    /// SAME relying party, driven by a freshly established PIN, succeeds factory-fresh afterward.
    /// </summary>
    [TestMethod]
    public async Task ResetOverTheWireMakesSubsequentAssertionCeremonyFailThenAFreshRegistrationCeremonySucceeds()
    {
        BaseMemoryPool pool = BaseMemoryPool.Shared;
        CancellationToken cancellationToken = TestContext.CancellationToken;

        var skin = new WebAuthnRelyingPartyCeremonySkin(
            RpId, Origin, CtapMakeCredentialGetAssertionFixtures.BuildFixedBytes(16, 0xF6), "reset-user", "Reset User", pool,
        ...
        byte[] preResetAssertionToken = await IssueTokenAsync(harness, pool, mcGa, RpId, cancellationToken).ConfigureAwait(false);
        await AssertOverRealTransportsWithTokenAsync(httpClient, harness, pool, preResetAssertionToken, cancellationToken).ConfigureAwait(false);
        Assert.AreEqual(1, skin.AssertionResultRequestCount, "the pre-reset assertion ceremony must succeed through the RP.");
        ...
        {
            Assert.AreEqual(WellKnownCtapStatusCodes.Ok, resetResponse.AsReadOnlySpan()[0], "authenticatorReset must succeed over the wire.");
        }

        //No token is issuable post-reset without a PIN, so the post-reset assertion leg is driven
        //tokenlessly: the authenticator has no PIN set again yet, and the ceremony's own client-side ga
        //call must fail regardless, purely because the credential store was erased.
        ...
        CtapCommandException resetAssertionException = await Assert.ThrowsExactlyAsync<CtapCommandException>(() =>
        ...
        Assert.AreEqual(
            WellKnownCtapStatusCodes.NoCredentials, resetAssertionException.StatusCode,
            "the authenticator must refuse the post-reset authenticatorGetAssertion call with NoCredentials, on the wire.");
        Assert.AreEqual(
        ...
        byte[] freshRegistrationToken = await IssueTokenAsync(harness, pool, mcGa, RpId, cancellationToken).ConfigureAwait(false);
        await RegisterOverRealTransportsWithTokenAsync(httpClient, harness, pool, freshRegistrationToken, cancellationToken).ConfigureAwait(false);
        Assert.AreEqual(2, skin.AttestationResultRequestCount, "a full re-registration ceremony through the same RP must succeed factory-fresh after the reset.");
        Assert.IsNotNull(skin.StoredCredential, "the post-reset re-registration must store a fresh credential record.");

        byte[] freshAssertionToken = await IssueTokenAsync(harness, pool, mcGa, RpId, cancellationToken).ConfigureAwait(false);
        await AssertOverRealTransportsWithTokenAsync(httpClient, harness, pool, freshAssertionToken, cancellationToken).ConfigureAwait(false);
        Assert.AreEqual(2, skin.AssertionResultRequestCount, "an assertion ceremony against the freshly re-registered credential must succeed.");
    }
    ...
    /// <paramref name="token"/>, mirroring <see cref="RegisterOverRealTransportsWithTokenAsync"/>'s shape
    /// for <c>authenticatorGetAssertion</c>, asserting the RP accepted the resulting assertion.
    /// </summary>
    ...
    /// carrying <paramref name="permissions"/>, optionally bound to <paramref name="rpId"/>, decrypting it
