...
/// <summary>
/// The real-wire capstones for <c>authenticatorLargeBlobs</c> (<c>0x0C</c>) and the
/// <c>largeBlobKey</c> extension (CTAP 2.3 §12.3): the same real, unmodified APDU transport stack
/// (<see cref="CtapNfcTransportHarness"/>) every other capstone uses, driving five flows end to
/// end — (A) the fresh-state <c>get</c> plus a tokenless multi-fragment write exercising both discard
/// disciplines, (B) the protection arc and the <c>lbw</c> permission carve-out, (C) the full §12.3
/// story with REAL DEFLATE compression and AES-256-GCM encryption in test code
/// (<see cref="CtapLargeBlobPlatformFixtures"/>), (D) <c>authenticatorReset</c>'s restoration of the
...
/// <see cref="CtapLargeBlobsFixtures"/>'s own verify-message helper.
/// </summary>
[TestClass]
internal sealed class CtapAuthenticatorLargeBlobsFlowTests
{
    ...
    /// <summary>
    /// Capstone A: the fresh-state <c>get</c> is byte-exact against the 17-byte initial constant,
    /// then a completely tokenless three-fragment write (960 + 960 + 80 = 2000 bytes, spanning the
    /// <see cref="CtapAuthenticatorState.MaxFragmentLength"/> boundary at least twice) is
    ...
    public async Task FreshStateGetAndTokenlessMultiFragmentWriteWithDiscardDisciplinesOverRealApduTransport()
    {
        ...
        byte[] fullArray = CtapLargeBlobPlatformFixtures.BuildValidSerializedArray(pool, payloadLength: (2 * fragmentLength) + 80);

        byte firstStatus = await CtapLargeBlobPlatformFixtures.SendFragmentAsync(
        ...
        byte wrongOffsetStatus = await CtapLargeBlobPlatformFixtures.SendFragmentAsync(
        ...
        byte secondStatus = await CtapLargeBlobPlatformFixtures.SendFragmentAsync(
        ...
        byte thirdStatus = await CtapLargeBlobPlatformFixtures.SendFragmentAsync(
        ...
        byte[] readBack = await CtapLargeBlobPlatformFixtures.ReadEntireSerializedArrayAsync(harness.Transceive, pool, cancellationToken).ConfigureAwait(false);
        Assert.AreSequenceEqual(fullArray, readBack, "the read-back array must byte-exactly match the committed write.");

        byte throwawayFirstStatus = await CtapLargeBlobPlatformFixtures.SendFragmentAsync(
        ...
        byte throwawayContinuationStatus = await CtapLargeBlobPlatformFixtures.SendFragmentAsync(
        ...
        byte[] readBackAfterDiscard = await CtapLargeBlobPlatformFixtures.ReadEntireSerializedArrayAsync(harness.Transceive, pool, cancellationToken)
            .ConfigureAwait(false);
        Assert.AreSequenceEqual(
    ...
    /// <summary>
    /// Capstone B: <c>setPIN</c> arms the conditional gate — a subsequent tokenless <c>set</c> now
    ...
    {
        const string RpId = "lb-capstone-b.example";
        BaseMemoryPool pool = BaseMemoryPool.Shared;
        CancellationToken cancellationToken = TestContext.CancellationToken;
        ...
        byte armedTokenlessStatus = await CtapLargeBlobPlatformFixtures.SendFragmentAsync(
            harness.Transceive, pool, new byte[] { 0x80 }, offset: 0, length: 17, token: null, ProtocolId, cancellationToken).ConfigureAwait(false);
        Assert.AreEqual(WellKnownCtapStatusCodes.PuatRequired, armedTokenlessStatus, "setPIN must arm the gate, observed on the wire.");
        ...
        byte[] firstEntry = CtapLargeBlobPlatformFixtures.BuildValidSerializedArray(pool, payloadLength: 20);
        await CtapLargeBlobPlatformFixtures.WriteSerializedArrayAsync(
        ...
        byte[] readBackAfterLbwWrite = await CtapLargeBlobPlatformFixtures.ReadEntireSerializedArrayAsync(harness.Transceive, pool, cancellationToken)
            .ConfigureAwait(false);
        ...
        byte[] mcClientDataHash = CtapMakeCredentialGetAssertionFixtures.BuildFixedBytes(32, 0x10);
        byte[] mcParam = await CtapConfigFixtures.ComputeSignatureAsync(token, ProtocolId, mcClientDataHash, pool, cancellationToken).ConfigureAwait(false);
        ...
        byte[] secondEntry = CtapLargeBlobPlatformFixtures.BuildValidSerializedArray(pool, payloadLength: 25);
        await CtapLargeBlobPlatformFixtures.WriteSerializedArrayAsync(
        ...
        byte[] readBackAfterCarveOut = await CtapLargeBlobPlatformFixtures.ReadEntireSerializedArrayAsync(harness.Transceive, pool, cancellationToken)
            .ConfigureAwait(false);
        Assert.AreSequenceEqual(
    ...
    /// (<see cref="CtapLargeBlobPlatformFixtures"/>), reads it back, decrypts, and decompresses —
    ...
    {
        const string RpId = "lb-capstone-c.example";
        BaseMemoryPool pool = BaseMemoryPool.Shared;
        CancellationToken cancellationToken = TestContext.CancellationToken;
        ...
        byte[] userId = CtapMakeCredentialGetAssertionFixtures.BuildFixedBytes(16, 0xF0);
        CtapMakeCredentialRequest mcRequest = CtapMakeCredentialGetAssertionFixtures.BuildMakeCredentialRequest(
        ...
        ReadOnlyMemory<byte> gaExtensions = CtapMakeCredentialGetAssertionFixtures.BuildGetAssertionExtensionsInput(largeBlobKey: true);
        ...
        byte[] opaqueData = Encoding.UTF8.GetBytes("lb capstone-c opaque payload, encrypted and compressed for real over the wire.");
        await CtapLargeBlobPlatformFixtures.WriteEncryptedEntryAsync(
        ...
        byte[] recovered = await CtapLargeBlobPlatformFixtures.ReadAndDecryptEntryAsync(harness.Transceive, pool, largeBlobKey, cancellationToken)
            .ConfigureAwait(false);
    ...
    /// <c>get</c> (line 7705's MUST). A fresh pending write begun after the reset is
    ...
    public async Task FactoryResetRestoresConstantAndPowerCycleDiscardsPendingWriteOverRealApduTransport()
    {
        ...
        byte[] readBackBeforeReset = await CtapLargeBlobPlatformFixtures.ReadEntireSerializedArrayAsync(harness.Transceive, pool, cancellationToken)
            .ConfigureAwait(false);
        ...
        byte pendingStatus = await CtapLargeBlobPlatformFixtures.SendFragmentAsync(
            harness.Transceive, pool, new byte[] { 0x80 }, offset: 0, length: 17, token: null, ProtocolId, cancellationToken).ConfigureAwait(false);
        ...
        byte continuationAfterCycleStatus = await CtapLargeBlobPlatformFixtures.SendFragmentAsync(
            harness.Transceive, pool, "v"u8.ToArray(), offset: 1, length: null, token: null, ProtocolId, cancellationToken).ConfigureAwait(false);
        ...
        byte[] readBackAfterCycle = await CtapLargeBlobPlatformFixtures.ReadEntireSerializedArrayAsync(harness.Transceive, pool, cancellationToken)
            .ConfigureAwait(false);
        Assert.AreSequenceEqual(
    ...
    /// exercised on the TOKENLESS path), and the FIRST commit's array is still readable,
    ...
    public async Task IntegrityFailureOnTheWireLeavesPreviousArrayReadableOverRealApduTransport()
    {
        ...
        byte[] corrupted = CtapLargeBlobPlatformFixtures.BuildValidSerializedArray(pool, payloadLength: 25);
        corrupted[^1] ^= 0xFF;

        byte corruptedStatus = await CtapLargeBlobPlatformFixtures.SendFragmentAsync(
        ...
        byte[] readBackAfterFailure = await CtapLargeBlobPlatformFixtures.ReadEntireSerializedArrayAsync(harness.Transceive, pool, cancellationToken)
            .ConfigureAwait(false);
        Assert.AreSequenceEqual(
        ...
        Ctap2TransceiveDelegate transceive, BaseMemoryPool pool, int get, int offset, CancellationToken cancellationToken)
    {
        ...
        byte[] envelope = CtapLargeBlobsFixtures.BuildEnvelope(request);
    ...
    {
        byte[] subCommandParams = CtapCredentialManagementFixtures.BuildSubCommandParams(rpIdHash: rpIdHash);
        byte[] message = CtapCredentialManagementFixtures.BuildMessage(WellKnownCtapCredentialManagementSubCommands.EnumerateCredentialsBegin, subCommandParams);
