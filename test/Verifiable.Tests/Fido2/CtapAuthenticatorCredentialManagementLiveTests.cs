...
/// observes the deletion through the ordinary path), and the global-discard rule's cross-FAMILY
...
/// <c>CtapAuthenticatorAlwaysUvLiveTests</c>'s role for <c>authenticatorConfig</c>.
/// </summary>
[TestClass]
internal sealed class CtapAuthenticatorCredentialManagementLiveTests
{
    ...
    /// <summary>
    /// The global-discard rule reaches ACROSS the two credMgmt enumeration families: beginning a
    ...
    public async Task EnumerateCredentialsBeginDiscardsLiveRpEnumerationSequence()
    {
        ...
        using CtapAuthenticatorSimulator simulator = CreateSimulator("cm-live-cross-family", BaseMemoryPool.Shared, residentCredentialCapacity: 3);
        BaseMemoryPool pool = BaseMemoryPool.Shared;
        ...
        byte[] rpsBeginParam = await CtapConfigFixtures.ComputeSignatureAsync(token, protocolId, rpsBeginMessage, pool, TestContext.CancellationToken);
        ...
        byte[] credsBeginParam = await CtapConfigFixtures.ComputeSignatureAsync(token, protocolId, credsBeginMessage, pool, TestContext.CancellationToken);
