...
/// </summary>
[TestClass]
internal sealed class XmlCanonicalizationC14N11FixtureTests
{
    ...
    /// <summary>
    /// Proves <see href="https://www.w3.org/TR/2008/REC-xml-c14n11-20080502/">Canonical XML 1.1</see>
    ...
    public void PisCommentsAndOutsideOfDocumentElementCanonicalizeUncommented()
    {
        ...
        AssertCanonicalFormEquals(expected, Canonicalize(Section31Input, XmlCanonicalizationAlgorithm.CanonicalXml11));
    }
    ...
    public void PisCommentsAndOutsideOfDocumentElementCanonicalizeCommented()
    {
        ...
        AssertCanonicalFormEquals(expected, Canonicalize(Section31Input, XmlCanonicalizationAlgorithm.CanonicalXml11WithComments));
    }
    ...
    public void WhitespaceInDocumentContentIsRetained()
    {
        ...
        AssertCanonicalFormEquals(document, Canonicalize(document, XmlCanonicalizationAlgorithm.CanonicalXml11));
    }
    ...
    public void StartAndEndTagsNormalizeSortAndSuppressSuperfluousDeclarations()
    {
        ...
        AssertCanonicalFormEquals(expected, Canonicalize(document, XmlCanonicalizationAlgorithm.CanonicalXml11));
    }
    ...
    /// section 3.6, whose example transcodes an ISO-8859-1 document to UTF-8. Delta: this surface accepts
    /// only UTF-8 and UTF-16 and refuses every other declared encoding — conformant, since section 2.1
    /// makes UTF-8 and UTF-16 required and ISO-8859-1 merely RECOMMENDED — so the example's canonical
    /// form, the two octets C2 A9, is unreachable and the input is refused as
    ...
    public void DocumentSubsetRendersAncestorContextAndPropagatedDeclarations()
    {
        ...
        using(table)
        {
            XmlNodeSet nodeSet = XmlNodeSet.ElementSubtree(table!, XmlCanonicalizationC14N10FixtureTests.FindElement(table!, "e3"))
                .IncludingAncestor(XmlCanonicalizationC14N10FixtureTests.FindElement(table!, "e1"));
            string expected = "<e1 xmlns=\"http://www.ietf.org\" xmlns:w3c=\"http://www.w3.org\"><e3 xmlns=\"\" id=\"E3\"></e3></e1>";

            AssertCanonicalFormEquals(expected, Canonicalize(table!, nodeSet, XmlCanonicalizationAlgorithm.CanonicalXml11));
        }
    }
    ...
    /// <summary>
    /// Proves <see href="https://www.w3.org/TR/2008/REC-xml-c14n11-20080502/">Canonical XML 1.1</see>
    /// section 3.8: "xml:id not inherited" and "xml:base fixup performed" over the same subset shape as
    /// section 3.7. The fixup on <c>e1</c> joins the omitted <c>doc</c>'s <c>xml:base="something/else"</c>
    /// into a rendered <c>xml:base</c> on <c>e1</c>; the fixup on <c>e3</c> joins only the contiguously
    /// omitted <c>e2</c>'s <c>bar/</c> with <c>e3</c>'s own <c>foo</c> — per the section 2.4 note that
    /// "En ... E1 is for contiguously omitted elements, for example only e2 in the example in
    /// Section 3.8" — giving <c>xml:base="bar/foo"</c> per erratum E11-01 of the
    /// <see href="https://www.w3.org/2008/05/xml-c14n11-errata">Canonical XML 1.1 errata</see>, which
    /// corrects the value printed in the Recommendation. Adapted per the DOCTYPE prohibition of this
    /// surface: the three DOCTYPE lines are dropped and with them the DTD-defaulted
    /// <c>xml:space="preserve"</c> on <c>e3</c> in the expectation; the rest is verbatim.
    /// </summary>
    [TestMethod]
    public void DocumentSubsetDropsXmlIdAndFixesUpXmlBase()
    {
