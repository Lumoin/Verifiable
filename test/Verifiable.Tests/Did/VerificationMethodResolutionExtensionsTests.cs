...
namespace Verifiable.Tests.Did
{
    ...
    public sealed class VerificationMethodResolutionExtensionsTests
    {
        ...
        public async Task SelectFromAuthenticationAsyncWithLocalResolutionReturnsResolvedResults()
        {
            ...
            Assert.AreEqual(2, resultsList.Count, "Should have exactly two authentication methods.");
        ...
        public async Task SelectFromAssertionMethodAsyncWithLocalResolutionReturnsResolvedResults()
        {
            ...
            Assert.AreEqual(2, resultsList.Count, "Should have exactly two assertion methods.");
            ...
            Assert.IsTrue(methodIds.Contains("#key-1"), "Should contain reference to #key-1");
            Assert.IsTrue(methodIds.Contains("#key-2"), "Should contain reference to #key-2");
        }
        ...
        public async Task SelectFromAllVerificationRelationshipsAsyncReturnsAllMethods()
        {
            ...
            Assert.AreEqual(8, resultsList.Count, "Should have methods from all verification relationships.");
            ...
            Assert.IsTrue(distinctIds.Contains("#key-1"), "Should contain #key-1");
            Assert.IsTrue(distinctIds.Contains("#key-2"), "Should contain #key-2");
            Assert.IsTrue(distinctIds.Contains("#embedded-key"), "Should contain #embedded-key");
        }
        ...
        public async Task SelectFromAuthenticationAsyncWithExternalResolverReturnsExternalMethod()
        {
            ...
            Assert.AreEqual(1, resultsList.Count, "Should have one authentication method.");
        ...
        public void ExtractKeyMaterialFromVerificationMethodReturnsValidData()
        {
            ...
            var keyMaterial = method.ExtractKeyMaterial(ExactSizeMemoryPool<byte>.Shared);

            using(keyMaterial.keyMaterial)
            {
                Assert.IsTrue(keyMaterial.keyMaterial.Memory.Length > 0, "Key material should not be empty.");
            }
        }
        ...
        public void ToPublicKeyMemoryFromVerificationMethodCreatesValidInstance()
        {
            ...
            using var publicKeyMemory = method.ToPublicKeyMemory(ExactSizeMemoryPool<byte>.Shared);

            Assert.IsTrue(publicKeyMemory.AsReadOnlyMemory().Length > 0, "Public key memory should not be empty.");
